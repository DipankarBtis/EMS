...
function doSubmit(appTaxesNo)
{
	...
	//if(contract_type=="E" || contract_type=="F" || contract_type=="W")
	{
		if(parseInt(appTaxesNo))
		{
			cform_flag = document.forms[0].cform_flag;
		}
	...
	//if(contract_type=="E" || contract_type=="F" || contract_type=="W")
	/*{
		if(cform_flag.length !=undefined)
		{
			if(cform_flag.length>0)
			{
				for(var x=0;x<cform_flag.length;x++)
				{
					var index = x+1;
					
					if(trim(cform_flag[x].value) == "0" || trim(cform_flag[x].value) == "")
					{
						msg+="Select C-Form Applicable or not for Row-"+index+" in Applicable Taxes!\n";
						flag=false;
					}
				}
			}
		...
		{
			if(trim(cform_flag.value) == "0" || trim(cform_flag.value) == "")
			{
				msg+="Select C-Form Applicable or not in Applicable Taxes!\n";
				flag=false;
			}
		}
	}*/
...
function setApplicableTax(index)
{
	//var cform_flag = document.getElementById("cform_flag_"+index);
	var app_tax_dwn = document.getElementById("app_tax_dwn_"+index).value;
	
	 document.getElementById("cform_flag_"+index).value = app_tax_dwn;
}
...
contract.setCallFlag("CONTRACT_BILLING_DTL_DLNG");
...
Vector VTAX_PLANT_NAME = contract.getVTAX_PLANT_NAME();
Vector VTAX_BU_PLANT_NM = contract.getVTAX_BU_PLANT_NM();
Vector VTAX_PLANT_SEQ = contract.getVTAX_PLANT_SEQ();
Vector VTAX_BU_PLANT_SEQ = contract.getVTAX_BU_PLANT_SEQ();
...
Vector VCFORM_FLAG = contract.getVCFORM_FLAG();
			...
			<div class="card cardmain">
				<div class="card-header cdheader">
					<div class="d-flex justify-content-between">
					    <div class="topheader">
					    <%=couterparty_abbr%> [<%=display_map_id%>] Billing Details
						    			...
						    			<div class="col">
						    				<select class="form-select form-select-sm" name="exch_calc_base" onchange="showExchngCriteria(this);">
									    		<option value="D">On Particular Day Base</option>
									     		<!-- <option value="A">On Daily Basis of Billing Period</option> --><!-- HM20250915 : Removed as per Vijay's feedback -->
									     	</select>
										...
										<div class="col">
						    				<select class="form-select form-select-sm" name="inv_criteria" id="exchngCri" <%if(!exchng_cal.equals("D")){ %>style="display:none;"<%} %>>
									    		<option value="">--Select--</option>
									     		...
									     		<option value="LPF">Last Day of Previous Fortnight</option>
									     		<!-- <option value="LST">Last Day of Billing Cycle</option>
									     		<option value="PRE">Previous Day of Billing Cycle</option> <!-- HM20250915 : Removed as per Vijay's feedback-->
									     	</select> 
				...
				<div class="card-body cdbody">
					<div class="row m-b-5">
						<label class="form-label subheader"><i class="fa fa-snowflake-o"></i> Applicable Taxes</label>
					</div>
					<div class="row">
							...
							<table class="table table-bordered table-hover" id="example">
								<thead>
									<tr>
										...
										<th>Hold C-Form Amount</th>
										<%-- <%} %> --%>
									</tr>
								</thead>
								<tbody>
								<%if(VTAX_PLANT_NAME.size()>0){ %>
									<%for(int i=0; i<VTAX_PLANT_NAME.size(); i++){ %>
										<tr><!-- style="background:#99ffcc;" -->
											<td align="center"><%=VTAX_BU_PLANT_NM.elementAt(i)%></td>
											<td align="center"><%=VTAX_PLANT_NAME.elementAt(i)%></td>
											<td align="center"><%=VINVOICE_CATEGORY.elementAt(i) %></td>
											<td align="center"><%=VINVOICE_TYPE.elementAt(i) %></td>
											<td align="center"
											<%if(VTAX_STRUCT_CD.elementAt(i).equals("-")){ %>style="background-color: #ffe6e6" title="Tax Details Not Configured!"<%} %>>
											<%if(!VTAX_STRUCT_CD.elementAt(i).equals("-")){ %>
											[<%=VTAX_STRUCT_CD.elementAt(i)%>]- <%=VTAX_STRUCT_NM.elementAt(i)%>
											<%}else {%>-<%} %></td>
											<td align="center" <%if(VTAX_STRUCT_CD.elementAt(i).equals("-")){ %>style="background-color: #ffe6e6" title="Tax Details Not Configured!"<%} %>>
											<%=VTAX_SAP_CODE.elementAt(i)%></td>
											<%-- <%if(contract_type.equals("E") || contract_type.equals("F") || contract_type.equals("W")) { %> --%>
											<td align="center"  style="width:100px;" >
												<div align="center">
											    	<select class="form-select form-select-sm" name="app_tax_dwn" id="app_tax_dwn_<%=i%>" onchange="setApplicableTax('<%=i%>')">
											    		<option value="0">--Select--</option>
											    		<option value="Y">Yes</option>
											    		<option value="N">No</option>
											    	</select>
										    	</div>
										    	<script>document.getElementById('app_tax_dwn_<%=i%>').value="<%=VCFORM_FLAG.elementAt(i)%>"</script>
										    	<input type="hidden" name="cform_flag" id="cform_flag_<%=i%>" value="<%=VCFORM_FLAG.elementAt(i)%>">
										    	<input type="hidden" name="cform_plant_seq" value="<%=VTAX_PLANT_SEQ.elementAt(i)%>">
										    	<input type="hidden" name="cform_bu_seq" value="<%=VTAX_BU_PLANT_SEQ.elementAt(i)%>">
											</td>
											<%-- <%} %> --%>
										</tr>
									<%} %>
								<%}else{ %>
									<tr>
										<td colspan="7" align="center"><%=utilmsg.infoMessage("<b>No Taxes Configured!</b>") %></td>
									</tr>
								<%} %>
								</tbody>
							</table>
				...
				<div class="card-footer cdfooter text-center">
					<div class="d-flex justify-content-between">
						...
						<input type="button" class="btn btn-warning com-btn" value="Submit" onclick="doSubmit('<%=VTAX_PLANT_NAME.size()%>');">
						<%}else{ %>
