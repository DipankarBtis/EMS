...
function enabled_TransPlantDtl(obj,id1,id2,nom_transptr)
{
	if(obj.checked)
	{
	...
	else
	{
		...
		{
			obj.checked=true;
			alert("Can't un-checked, nomination is done for this transporter plant!");
		}
	}
}
...
function doSubmit()
{
	...
	if(trim(adv_adjust) == "" && contract_type!=="X" && opration=="MODIFY")
	{
...
function setValues(strTransCd,strTrans,strPlant,strBuPlant,strPlantChrg,strGxBuPlant,strChargehistdesc)
{
	var chk_plant = document.forms[0].chk_plant;
	var chk_trans = document.forms[0].chk_trans;
	var chk_bu_plant = document.forms[0].chk_bu_plant;
	...
	if(chk_plant!=null && chk_plant.length!=undefined)
 	{
  		for(var i=0;i<chk_plant.length;i++)
  		{
   			for(var j=0;j<sepPlant.length;j++)
   			{
     			...
     			{
     				document.getElementById("history_btn_"+sepPlant[j]).setAttribute("title", sepstrChargehistdesc[j]);
...
function variableDCQ(cont_type,agmt,agmt_rev,cont,cont_rev,cont_status_flg)
{
	...
	if(trim(cont_type)!="" && (trim(cont)!="" && cont!="0") && (trim(counterparty_cd)!="" && counterparty_cd!="0"))
	{
		...
		if(cont_status_flg=='X' || cont_status_flg=='T' || cont_status_flg=='C')
		{
			url = "../contract_master/frm_contract_dcq_dtl_fcc.jsp?counterparty_cd="+counterparty_cd+"&cont_no="+cont+"&cont_rev_no="+cont_rev+
			"&agmt_no="+agmt+"&agmt_rev_no="+agmt_rev+"&contract_type="+cont_type+"&start_dt="+temp_start_dt+"&end_dt="+temp_end_dt+
			"&u="+u;
		}
		else
...
function openLiabilityClause(cont_status_flg)
{
	...
	{
		url = "frm_cont_liability_clause_fcc.jsp?counterparty_cd="+counterparty_cd+"&agreement_type="+agmt_type+
...
function doClosureRequest(type)
{
	...
	if(flag)
	{
		...
		if(a)
		{
			...
			//document.forms[0].cont_status_flg.value="E";
			document.getElementById("loading").style.visibility = "visible";
			document.forms[0].submit();
		}
	}
...
function doReopenRequest()
{
	...
	if(a)
	{
		...
		//document.forms[0].cont_status_flg.value="R";
		document.getElementById("loading").style.visibility = "visible";
		document.forms[0].submit();
	}
}
...
function checkNom_Status(clearance)
{
	var chk_bu_plant = document.forms[0].chk_bu_plant;
...
Vector VSEL_CHARGE_DESC = contract.getVSEL_CHARGE_DESC();
...
//Pratham Bhatt 20250317 : Added for closure effective date.
...
<body onload="setValues('<%=strTransCd%>','<%=strTrans%>','<%=strPlant%>','<%=strBuPlant%>','<%=strPlantChrg%>','<%=strGxBuPlant%>','<%=strChargehistdesc %>');liabilityShows();billingShows();messurmentShows();off_spec_gasShows();terminatorShows();">
			...
			<div class="card cardmain">
				<div class="card-header cdheader">
					<div class="d-flex justify-content-between">
					    <div class="topheader">
					    	Gas Sales Contract (SN/LOA/IGX)<!-- //HM : Removed default X as per Vijay's UAT Obs. Gas Supply Contract -->
					    </div>
					    <div class="btn-group">
						...
						<div class="col-sm-4 col-xs-4 col-md-4">  
							<div class="form-group row">
				    			<div class="col-sm-4 col-xs-4 col-md-4">
				    				...
				    				<%if(!cont_no.equals("")){ %> value="<%=dealMapping%>" <%} %>
							...
							<div class="form-group row">
				    			...
				    			<div class="col">
				  				...
				  				<div class="col" <%if(clearance.equals("IGX")){ %>style="display:none"<%} %>>  
									<div class="form-group row">
										<div class="col">
						    				<label class="form-label"><b>Auto-adjust ADV<span class="s-red">*</span></b></label>
						  				</div>				    			
						  			</div>
								</div>
								<div class="col" <%if(clearance.equals("IGX")){ %>style="display:none"<%} %>>  
						...
						<div class="col-sm-4 col-xs-4 col-md-4">  
							<div class="form-group row">
				    			...
				    			<div class="col-auto">
									...
									<%}else{ %>
									<input type="button" class="btn btn-sm request_btn" value="TCQ Change Request" disabled>
									<%} %>
				    			</div>
				  			</div>
						</div>	
				    			...
				    			<div class="col-sm-12 col-xs-12 col-md-12">
				    				<%if(VBU_CD.size() > 0) {%>
					    				<%for(int i=0; i<VBU_CD.size(); i++){ %>
					    					<input <%if(clearance.equals("IGX")){ %>type="radio"<%}else{ %>type="checkbox"<%} %> class="form-check-input" name="chk_bu_plant" value="<%=VBU_PLANT_SEQ_NO.elementAt(i)%>" onchange="checkNom_Status('<%=clearance%>');">&nbsp;<%=VBU_PLANT_ABBR.elementAt(i)%>&nbsp;&nbsp;
						...
						<div class="col-sm-4 col-xs-4 col-md-4">  
							<div class="form-group row">
				    			<div id="billing_flag_div1" class="col-sm-12 col-xs-12 col-md-12">
						      		<input type="button" class="btn btn-sm config_btn" value="Configure Billing" <%if(opration.equals("MODIFY")){ %>onclick="openBillingDtl();"<%} else {%>style="background:#e6e6e6;color:black;" disabled <%} %>>
				    			</div>
				  			</div>
						</div>
					</div>
					...
					<div class="row m-b-5">
						<div class="col-sm-12 col-xs-12 col-md-12">  
							<div class="form-group row">
								...
								<div class="col-auto">
									<%if(write_access.equals("Y")){ %>
									<input type="button" class="btn btn-sm request_btn" value="Price Change Request" onclick="doPriceChangeRequest();" <%if(!fcc_flg.equals("Y") || cont_status_flg.equals("X")||cont_status_flg.equals("C")||cont_status_flg.equals("T")){%>style="background:#e6e6e6;color:black;" disabled<%}else if(!is_allocated.equals("Y")){%> style="background:#e6e6e6;color:black;" disabled<%}%>>
									<%}else{ %>
									<input type="button" class="btn btn-sm request_btn" value="Price Change Request" disabled>
									<%} %>
								</div>				    			
				    			<div class="col-auto" title="<%if(cont_status_flg.equals("R")){%>If contract Re-Opened then, closure request button will be enabled once fcc is completed!<%}else if(closure_request_flag.equals("Y")){%>Closure request generated<%}else if(cont_status_flg.equals("C")){%>Contract Closed<%}else if(!is_allocated.equals("Y")){%>Closure can be done once qty is allocated to the contract!<%}else if(!is_expired.equals("Y") && !cont_status_flg.equals("T")){%>Closure can be applied on expired contract only, use Terminate Request!<%}%>">
									...
									<%}else{ %>
									<input type="button" class="btn btn-sm request_btn" value="Closure Request" disabled>
									<%} %>
				    			</div>
				    			...
				    			<div class="col-auto" title="<% if(!closure_request_flag.equals("O")){%>Activates when contract is closed or terminated!<%}else if(closure_request_flag.equals("O")){%>Contract Reopen Request generated!<%}%>">
									...
									<%}else{ %>
									<input type="button" class="btn btn-sm request_btn" value="Re-Open Request" disabled>
									<%} %>
				    			</div>
				    			<div class="col-auto" title="<%if(cont_status_flg.equals("R")){%>If contract Re-Opened then, terminate request button will be enabled once fcc is completed!<%}if(is_expired.equals("Y")){%>Contract is expired, so close contract!<%}else if(!termination_forced.equals("Y")){%>Configure Termination/Suspention (Force Majeure) to enable!<%}else if(closure_request_flag.equals("R")){%>Termination request is generated for the contract!<%}else if(cont_status_flg.equals("T")){%>Contract Terminated!<%}%>">
									...
									<%}else{ %>
									<input type="button" class="btn btn-sm request_btn" value="Terminate Request" disabled>
									<%} %>
				    			</div>
				  			</div>
						</div>
		      					...
		      					<%if(VTRANS_CD.elementAt(j).equals(VTEMP_TRANS_CD.elementAt(i))){ 
			      					...
			      					<%} %>
										<div class="col-sm-2 col-xs-2 col-md-2">  
											<div class="form-group row">
												<div class="col-sm-12 col-xs-12 col-md-12">
								    				<input type="checkbox" class="form-check-input" name="chk_trans" onclick="enabled_TransPlantDtl(this,'trans_cd<%=VTRANS_CD.elementAt(j)%><%=VTRANS_PLANT_SEQ_NO.elementAt(j)%>','trans_plant_seq_no<%=VTRANS_CD.elementAt(j)%><%=VTRANS_PLANT_SEQ_NO.elementAt(j)%>','<%=VNOM_SEL_TRANS_CHK.elementAt(j)%>')">&nbsp;<%=VTRANS_PLANT_ABBR.elementAt(j)%>&nbsp;&nbsp;
							    	...
							    	<%for(int i=0; i<VPLANT_SEQ_NO.size(); i++){ %>	
							    		<tr>
											<td align="center">	
				      							...
				      							onclick="setTransChrg(this,'<%=i%>','<%=VNOM_SEL_CUST_CHK.elementAt(i)%>');" >
