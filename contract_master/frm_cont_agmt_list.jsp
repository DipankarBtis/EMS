...
function refresh(active_status)
{
	...
	var url = "../contract_master/frm_cont_agmt_list.jsp?active_status="+active_status+"&counterparty_cd="+counterparty_cd+"&agreement_type="+agreement_type;
...
function refresh_date()
{
	...
	if(flag && flg)
	{
		if(trim(from_dt)!="" && trim(to_dt)!="")
		{
			...
			{
			var url = "../contract_master/frm_cont_agmt_list.jsp?active_status="+active_status+"&counterparty_cd="+counterparty_cd+
...
Vector VEXPIRED = contract.getVEXPIRED();
%>
<body>
<form method="post">
<div class="box-body">
					...
					<div class="row mb-2">
					    <div class="col-12">
					        ...
					        <% if(active_status.equals("N")) { %>
					            ...
					            <div class="col-12 col-sm-auto">
					                <button type="button" class="btn btn-warning com-btn" onclick="refresh_date();">Apply Filters</button>
					            </div>
					        <% } %>
							...
							<table class="table table-bordered ems_sorttbl" id="ems_table0">
								<thead id="ems_tbsort0">
									<tr valign="top">
										...
										<th class="ems_thsort0">Agreement Base</th>
								...
								<%if(VBUYER_CD.size() > 0){ %>
									...
									<tr>
										<td align="center"><input type="radio" onclick="setValue('<%=VBUYER_CD.elementAt(i) %>','<%=VAGMT_NO.elementAt(i)%>','<%=VAGMT_REV_NO.elementAt(i)%>','<%=agreement_type%>','<%=VCONT_NAME.elementAt(i) %>')" <%if(VEXPIRED.elementAt(i).equals("Y")){%>disabled<%}%>></td>
										...
										<td align="center">
										<%if(VEXPIRED.elementAt(i).equals("Y")){%>
