...
function refresh(opration,clearance)
{
	...
	var url = "frm_contract_mst_fcc.jsp?counterparty_cd="+counterparty_cd+"&opration="+opration+"&u="+u+
...
function openContList()
{
	...
	{
		newWindow = window.open("frm_contract_list.jsp?counterparty_cd="+counterparty_cd+"&clearance="+clearance+"&contract_type="+contract_type+"&fcc_flag=Y","Supply Contract List","top=10,left=10,width=1100,height=600,scrollbars=1");
	}
	else
	{
		newWindow.close();
...
function openBillingDtl()
{
	...
	var url = "frm_contract_billing_dtl_fcc.jsp?counterparty_cd="+counterparty_cd+"&contract_type="+contract_type+"&cont_no="+cont_no+
...
function setContDetail(countpty_cd,agmt_no,agmt_rev_no,cont_no,cont_rev_no,cont_type)
{
	...
	var url = "frm_contract_mst_fcc.jsp?counterparty_cd="+countpty_cd+"&opration="+opration+"&cont_no="+cont_no+"&cont_rev_no="+cont_rev_no+
...
function setAgmtDetail(countpty_cd,agmt_no,agmt_rev_no,agmt_type,agmt_nm)
{
	...
	var url = "frm_contract_mst_fcc.jsp?counterparty_cd="+countpty_cd+"&opration="+opration+
...
function fcc(fcc_flg)
{
	...
	var tcq_request_flag = document.forms[0].tcq_request_flag.value;
	var price_change_request_flag = document.forms[0].price_change_request_flag.value;
	...
	if(price_change_request_flag == "Y")
	{
		msg += "Price Change Request Initiated for the Contract!\n";
		flag=false;
	}
	else if(tcq_request_flag == "Y")
	{
		msg += "TCQ Modification Initiated for the Contract!\n";
		flag=false;
	}
	...
	if(flag)
	{
		...
		if(fcc_flg=="Y")
		{
			a = confirm("Do you want to Approve Customer Contract?");
		}
		else
		{
			a = confirm("Do you want to Disapprove Customer Contract?");
		}
		if(a)
		{
			document.forms[0].fcc_flg.value=fcc_flg
			document.getElementById("loading").style.visibility = "visible";
			document.forms[0].option.value="SUPPLY_CONT_FCC"
...
function creditExceedDaysConfig(cont_type,agmt,agmt_rev,cont,cont_rev)
{
	...
	{
		creditExceedWindow = window.open(url,"Credit Exceed Days Configuration","top=10,left=100,width=1200,height=300,scrollbars=1");
	}
	else
	{
...
function variableDCQ(cont_type,agmt,agmt_rev,cont,cont_rev)
{
	...
	{
		var url = "../contract_master/frm_contract_dcq_dtl_fcc.jsp?counterparty_cd="+counterparty_cd+"&cont_no="+cont+"&cont_rev_no="+cont_rev+
			"&agmt_no="+agmt+"&agmt_rev_no="+agmt_rev+"&contract_type="+cont_type+"&start_dt="+temp_start_dt+"&end_dt="+temp_end_dt+
			"&u="+u;
		
		if(!dcqWindow || dcqWindow.closed)
		{
			dcqWindow = window.open(url,"Variable DCQ","top=10,left=100,width=1200,height=300,scrollbars=1");
		}
		else
		{
			dcqWindow.close();
...
function setTransChrg(obj,index)
{
	if(obj.checked)
	{
		document.getElementById("transportation_charges"+index).disabled=false;
	}
	else
	{
		document.getElementById("transportation_charges"+index).disabled=true;
		document.getElementById("transportation_charges"+index).value="";
	}
}
...
function openLiabilityClause()
{
	...
	var url = "frm_cont_liability_clause_fcc.jsp?counterparty_cd="+counterparty_cd+"&agreement_type="+agmt_type+
...
%>
<%-- <body style="pointer-events: none;" onload="setValues('<%=strTransCd%>','<%=strTrans%>','<%=strPlant%>','<%=strBuPlant%>','<%=strPlantChrg%>','<%=strGxBuPlant%>');liabilityShows();billingShows();messurmentShows();off_spec_gasShows();terminatorShows();"> --%>
			...
			<div class="card cardmain">
				<div class="card-header cdheader">
					<div class="d-flex justify-content-between">
					    <div class="topheader">
					    	Gas Sales Contract (SN/LOA/IGX) FCC
					    </div>
					</div>
				</div>
				<div class="card-body cdbody">
					...
					<div class="row m-b-5">
						...
						<div class="col-sm-4 col-xs-4 col-md-4"> 
				    		...
				    		<%}else{ %>
				    			<div class="col-sm-12 col-xs-12 col-md-12">
				    		<%} %>
				    				<select class="form-select form-select-sm" name="contract_type" style="pointer-events: auto; "onchange="refresh('<%=opration%>','<%=clearance%>');">
					...
					<div class="row m-b-5" <%if(!opration.equals("MODIFY")){ %>style="display:none;"<%} %>>
						...
						<div class="col-sm-4 col-xs-4 col-md-4">
							<div class="form-group row">
				    			...
				    			<div class="col">
				    				<input type="button" name="security_btn" class="btn btn-sm config_btn" value="Security Config" style="pointer-events: auto;" onclick="securityPreReceipt('<%=contract_type%>','<%=agmt_no%>','<%=agmt_rev_no%>','<%=cont_no%>','<%=cont_rev_no%>');">
					...
					<div class="row m-b-5">
						...
						<%-- <div class="col-sm-2 col-xs-2 col-md-2">  
							<div class="form-group row">
				    			<div class="col-sm-12 col-xs-12 col-md-12">
				    				<input type="button" class="btn btn-sm config_btn" value="Variable Price Config" data-bs-toggle="modal" data-bs-target="">
				  				</div>
				  			</div>
						</div>
						<div class="col-sm-4 col-xs-4 col-md-4">  
						...
						<div class="col-sm-2 col-xs-2 col-md-2">  
							<div class="form-group row">
								<div class="col-sm-12 col-xs-12 col-md-12">
				    				...
				    				data-bs-toggle="modal" data-bs-target="#TransModal"  style="pointer-events: none;">
				  				</div>
				  			</div>
						</div>
						<div class="col-sm-10 col-xs-10 col-md-10">  
						...
						<div class="col-sm-4 col-xs-4 col-md-4">  
							<div class="form-group row">
				    			<div id="liability_div2" class="col-sm-12 col-xs-12 col-md-12">
				    				<input type="button" class="btn btn-sm config_btn" value="Configure Liability" style="pointer-events: auto;" <%if(opration.equals("MODIFY")){ %>onclick="openLiabilityClause();"<%} %>>&nbsp;
				    				<%if(opration.equals("MODIFY")){ %>
						...
						<div class="col-sm-4 col-xs-4 col-md-4">  
							<div class="form-group row">
				    			<div id="billing_flag_div1" class="col-sm-12 col-xs-12 col-md-12">
						      		<input type="button" class="btn btn-sm config_btn" value="Configure Billing" style="pointer-events: auto;" <%if(opration.equals("MODIFY")){ %>onclick="openBillingDtl();"<%} %>>
				    			</div>
				  			</div>
						</div>
					</div>
						...
						<div class="row m-b-5">
							...
							<div class="col-sm-4 col-xs-4 col-md-4">  
								<div class="form-group row">
					    			...
					    			<%if(approve_access.equals("Y")&&(!cont_status_flg.equals("X")&&!cont_status_flg.equals("T")&&!cont_status_flg.equals("C"))){ %>
							    	...
							    	<%for(int i=0; i<VPLANT_SEQ_NO.size(); i++){ %>	
							    		<tr>
											<td align="center">	
				      							...
				      							onclick="setTransChrg(this,'<%=i%>');" style="pointer-events:none;">
				      						</td>
				      						<td>
