...
function refresh(opration,clearance)
{
	...
	var url = "frm_dlng_contract_mst.jsp?counterparty_cd="+counterparty_cd+"&opration="+opration+"&u="+u+
...
function doSubmit()
{
	...
	var adv_adjust = document.forms[0].adv_adjust.value;
	...
	if(trim(adv_adjust) == "" && contract_type!=="W" && opration=="MODIFY")
	{
		msg+="Select Adjust Advance!\n";
		flag=false;
	}
		...
		if(a)
		{
			...
			if(clearance=="KYC")
			{
				if(trim(no_of_security_dtl) != "")
				{
					...
					{
						temp_msg += "Please Enter Security Detail and Adjust Advance after Submitting Contract Detail!\n";
					}
				}
				else
				{
...
function openContList()
{
	...
	//HM : Removed Counterpaty and Contract Type restriction as per Vijay's UAT Obs.
	/* if(counterparty_cd == "" || counterparty_cd == "0")
	{
		msg+="Select Counterparty!\n";
		flag=false;
	...
	if(flag)
	{
		...
		{
			newWindow = window.open("frm_dlng_contract_list.jsp?counterparty_cd="+counterparty_cd+"&clearance="+clearance+"&contract_type="+contract_type,"Supply Contract List","top=10,left=10,width=1100,height=600,scrollbars=1");
		}
		else
		{
			newWindow.close();
...
function openBillingDtl()
{
	...
	var url = "frm_dlng_contract_billing_dtl.jsp?counterparty_cd="+counterparty_cd+"&contract_type="+contract_type+"&cont_no="+cont_no+
...
function setContDetail(countpty_cd,agmt_no,agmt_rev_no,cont_no,cont_rev_no,cont_type)
{
	...
	var url = "frm_dlng_contract_mst.jsp?counterparty_cd="+countpty_cd+"&opration="+opration+"&cont_no="+cont_no+"&cont_rev_no="+cont_rev_no+
...
function setAgmtDetail(countpty_cd,agmt_no,agmt_rev_no,agmt_type,agmt_nm)
{
	...
	var url = "frm_dlng_contract_mst.jsp?counterparty_cd="+countpty_cd+"&opration="+opration+
...
function securityPreReceipt(cont_type,agmt,agmt_rev,cont,cont_rev,cont_status_flg)
{
	...
	{
		var url="";
		if(cont_status_flg=='X' || cont_status_flg=='T' || cont_status_flg=='C')
		{
...
function creditExceedDaysConfig(cont_type,agmt,agmt_rev,cont,cont_rev,cont_status_flg)
{
	var counterparty_cd = document.forms[0].counterparty_cd.value;
	var temp_start_dt = document.forms[0].temp_start_dt.value;
	var temp_end_dt = document.forms[0].temp_end_dt.value;
	...
	if(trim(cont_type)!="" && (trim(cont)!="" && cont!="0") && (trim(counterparty_cd)!="" && counterparty_cd!="0"))
	{
		...
		if(cont_status_flg=='X' || cont_status_flg=='T' || cont_status_flg=='C')
		{
			url = "../credit_risk/frm_credit_exceed_days_fcc.jsp?counterparty_cd="+counterparty_cd+"&cont_no="+cont+"&cont_rev_no="+cont_rev+
			"&agmt_no="+agmt+"&agmt_rev_no="+agmt_rev+"&contract_type="+cont_type+"&start_dt="+temp_start_dt+"&end_dt="+temp_end_dt+
			"&u="+u;
		}
		else
		{
			url = "../credit_risk/frm_credit_exceed_days.jsp?counterparty_cd="+counterparty_cd+"&cont_no="+cont+"&cont_rev_no="+cont_rev+
...
var dcqWindow;
function variableDCQ(cont_type,agmt,agmt_rev,cont,cont_rev,cont_status_flg)
{
	var counterparty_cd = document.forms[0].counterparty_cd.value;
	var temp_start_dt = document.forms[0].temp_start_dt.value;
	var temp_end_dt = document.forms[0].temp_end_dt.value;
	...
	if(trim(cont_type)!="" && (trim(cont)!="" && cont!="0") && (trim(counterparty_cd)!="" && counterparty_cd!="0"))
	{
		...
		if(cont_status_flg=='X' || cont_status_flg=='T' || cont_status_flg=='C')
		{
			url = "../contract_master/frm_dlng_fcc_dcq_dtl.jsp?counterparty_cd="+counterparty_cd+"&cont_no="+cont+"&cont_rev_no="+cont_rev+
			"&agmt_no="+agmt+"&agmt_rev_no="+agmt_rev+"&contract_type="+cont_type+"&start_dt="+temp_start_dt+"&end_dt="+temp_end_dt+
			"&u="+u;
		}
		else
		{
			url = "../contract_master/frm_contract_dcq_dtl.jsp?counterparty_cd="+counterparty_cd+"&cont_no="+cont+"&cont_rev_no="+cont_rev+
				"&agmt_no="+agmt+"&agmt_rev_no="+agmt_rev+"&contract_type="+cont_type+"&start_dt="+temp_start_dt+"&end_dt="+temp_end_dt+
				"&u="+u;
		}
...
function doTcqRequest()
{
	...
	var delta_tcq_sign = document.forms[0].delta_tcq_sign.value;		
	var closure_mmbtu = document.forms[0].closure_mmbtu.value;		
	...
	if(closure_mmbtu !='')
	{
		diff = parseFloat(var_tcq)-parseFloat(closure_mmbtu);
		if(diff<=0)
		{
			diff_flag=true;
		}
	}
	if((delta_tcq_sign=='-' && tcq_sign=='-'))
	{
		msg+="TCQ is already short than supplied, so TCQ reduction is not allowed! \n";
		flag=false;
	}
	else if((delta_tcq_sign=='-' && tcq_sign=='+') && diff_flag)
	{
		msg+="Enter value greater than Released MMBTU ("+ parseFloat(closure_mmbtu).toFixed(2)+" MMBTU)! \n";
		flag=false;
	}
	...
	if(flag)
	{
		...
		if(delta_tcq_sign=='-')
		{
			tcq_mod_msg="\n\nThe TCQ modification will be for delta TCQ =\n"+parseFloat(var_tcq).toFixed(2)+"(Quantity Modification) - "+parseFloat(closure_mmbtu).toFixed(2)+"(Released MMBTU) = "+parseFloat(diff).toFixed(2)+" MMBTU";
		}
		var a = confirm("New Contract Revision will be Submitted Along With TCQ Modification Request!"+tcq_mod_msg+"\n\nDo You Want To Revise The Contract Details, With TCQ Modification Request?");
		if(a)
		{
...
function doContractDurationChangeRequest()
{
	...
	var new_start_dt = document.forms[0].new_start_dt;
	var new_end_dt = document.forms[0].new_end_dt;
	var max_dt = document.forms[0].max_date.value;
	var min_nom_dt = document.forms[0].min_nom_date.value;
	...
	if(new_start_dt.value == "" || new_start_dt.value == " " || new_end_dt.value == "" || new_end_dt.value == " ")
	{
...
function doPriceChangeRequest()
{
	...
	//PB 20250911: for allowing to change the price with in one month after expiry
	var split_end_dt = temp_end_dt.split("/");
	var day=parseInt(split_end_dt[0]);
	var month=parseInt(split_end_dt[1])-1;
	var year=parseInt(split_end_dt[2]);
	var tempDate = new Date(year, month, day);
	var newEndDate = new Date(tempDate);
	newEndDate.setMonth(newEndDate.getMonth() + 1);
	var temp_dt = String(newEndDate.getDate()).padStart(2,'0') + "/"+String(newEndDate.getMonth()+1).padStart(2,'0')+"/"+String(newEndDate.getFullYear()).padStart(2,'0')
	var val_dt = compareDate(sysdate,temp_dt);
	//var val_dt = compareDate(sysdate,temp_end_dt);
...
var priceWindow;
function variablePriceConfig(cont_type,agmt,agmt_rev,cont,cont_rev,cont_ref,cont_status,cont_status_flg)
{
	var counterparty_cd = document.forms[0].counterparty_cd.value;
	...
	if(trim(cont_type)!="" && (trim(cont)!="" && cont!="0") && (trim(counterparty_cd)!="" && counterparty_cd!="0"))
	{
		...
		if(cont_status_flg=='X' || cont_status_flg=='T' || cont_status_flg=='C')
		{
			url = "../market_risk/frm_config_price_mst_fcc.jsp?counterparty_cd="+counterparty_cd+"&cont_no="+cont+"&cont_rev_no="+cont_rev+"&cont_ref="+cont_ref+
		...
		{
			url = "../market_risk/frm_config_price_mst.jsp?counterparty_cd="+counterparty_cd+"&cont_no="+cont+"&cont_rev_no="+cont_rev+"&cont_ref="+cont_ref+
				"&agmt_no="+agmt+"&agmt_rev_no="+agmt_rev+"&contract_type="+cont_type+"&start_dt="+temp_start_dt+"&end_dt="+temp_end_dt+"&cont_status="+cont_status+
				"&u="+u;
		}
...
function openLiabilityClause(cont_status_flg)
{
	...
	{
		url = "frm_dlng_fcc_liability_clause.jsp?counterparty_cd="+counterparty_cd+"&agreement_type="+agmt_type+
	...
	{
		url = "frm_cont_liability_dtl.jsp?counterparty_cd="+counterparty_cd+"&agreement_type="+agmt_type+
				"&agmt_no="+agmt_no+"&agmt_rev_no="+agmt_rev_no+"&start_dt="+start_dt+"&contract_type="+contract_type+"&tcq="+tcq+"&dcq="+dcq+
				"&end_dt="+end_dt+"&cont_no="+cont_no+"&cont_rev_no="+cont_rev_no+"&rate="+rate+"&rate_unit="+rate_unit+"&u="+u;
	}
...
function doClosureRequest(type)
{
	var counterparty_cd = document.forms[0].counterparty_cd.value;
	var opration = document.forms[0].opration.value;
	var contract_type = document.forms[0].contract_type.value;
	...
	/*if(temp_start_dt != "" & temp_end_dt != "")
	{
		if(parseInt(val_dt) == 1)
		{
			msg+="Contract Expired! - Closure request cann't be Generated!";
			flag=false;
		}
	}*/
	if(flag)
	{
		...
		//var a = confirm("Do you want to submit closure request for the contract?");
		if(a)
		{
			if(type=='C')
			{
				document.forms[0].change_request.value="CLOSURE";
			}
			else if(type=='T')
			{
...
function checkNom_Status(clearance,type)
{
	var chk_bu_plant = document.forms[0].chk_bu_plant;
	var chk_truck_trans = document.forms[0].chk_truck_trans;
	var bu_nom_flg = document.forms[0].bu_nom_flg;
	var truck_trans_nom = document.forms[0].truck_trans_nom;
	var cust_plant_nom = document.forms[0].cust_plant_nom;
	var chk_plant =document.forms[0].chk_plant;
	var fill_nom=document.forms[0].fill_nom;
	var chk_fill_station=document.forms[0].chk_fill_station;
	  		...
	  		for(var i=0;i<chk_bu_plant.length;i++)
	  		{
	   			//for(var j=0;j<bu_nom_flg.length;j++)
	   			{
	     			if(bu_nom_flg[i].value == 'Y' && !chk_bu_plant[i].checked)
	     			{
	     				if(clearance=='KYC')
	     				{
		     				alert("Can't un-checked, nomination is done for this Businness Unit!");
	     				}
	     				else
	     				{
	...
	//for truck transporter
	else if(type=='truck_trans')
	{
		if(chk_truck_trans!=null && chk_truck_trans.length!=undefined)
		{
			for(var i=0;i<chk_truck_trans.length;i++)
	  		{
	   			//for(var j=0;j<truck_trans_nom.length;j++)
	   			{
	     			if(truck_trans_nom[i].value == 'Y' && !chk_truck_trans[i].checked)
	     			{
	     				//if(clearance=='KYC')
	     				{
		     				alert("Can't un-checked, nomination is done for this Truck Transporter!");
	     				}
	     				//else
	     				{
		     				//alert("Nomination is already done, can't select other business unit!");
		...
		else 
	 	{
	   		//for(var j=0;j<truck_trans_nom.length;j++)
	   		{
	   			if(truck_trans_nom.value == 'Y' && !chk_truck_trans.checked)
	     		{
	   				//if(clearance=='KYC')
	 				{
	     				alert("Can't un-checked, nomination is done for this Truck Transporter!");
	...
	//for Customer plant
	else if(type=='cust_plant')
	{
		if(chk_plant!=null && chk_plant.length!=undefined)
		{
			for(var i=0;i<chk_plant.length;i++)
	  		{
	   			//for(var j=0;j<cust_plant_nom.length;j++)
	   			{
	     			if(cust_plant_nom[i].value == 'Y' && !chk_plant[i].checked)
	     			{
	     				//if(clearance=='KYC')
	     				{
		...
		else
	 	{
	   		//for(var j=0;j<cust_plant_nom.length;j++)
	   		{
	   			if(cust_plant_nom.value == 'Y' && !chk_plant.checked)
	     		{
	   				//if(clearance=='KYC')
	 				{
	     				alert("Can't un-checked, nomination is done for this Customer Plant!");
	...
	//for filling station 
	else if(type=='fill_station')
	{
		if(chk_fill_station!=null && chk_fill_station.length!=undefined)
		{
			for(var i=0;i<chk_fill_station.length;i++)
	  		{
	   			//for(var j=0;j<fill_nom.length;j++)
	   			{
	     			if(fill_nom[i].value == 'Y' && !chk_fill_station[i].checked)
	     			{
	     				//if(clearance=='KYC')
	     				{
		     				alert("Can't un-checked, nomination is done for this Filling Station!");
	     				}
	     				//else
	     				{
		     				//alert("Nomination is already done, can't select other business unit!");
		...
		else
	 	{
	   		//for(var j=0;j<cust_plant_nom.length;j++)
	   		{
	   			if(fill_nom.value == 'Y' && !chk_fill_station.checked)
	     		{
	   				//if(clearance=='KYC')
	 				{
	     				alert("Can't un-checked, nomination is done for this Filling Station!");
...
var varPriceWindow;
function openVariablePriceInfo(cont_type,agmt,agmt_rev,cont,cont_rev,cont_ref,cont_status,cont_status_flg)
{
	var counterparty_cd = document.forms[0].counterparty_cd.value;
...
Vector VTRUCK_TRANS_NM = contract.getVTRUCK_TRANS_NM();

Vector VFILL_STATION_ABBR = contract.getVFILL_STATION_ABBR();
Vector VFILL_STATION_CD = contract.getVFILL_STATION_CD();

Vector VNOM_SEL_BU_CHK = contract.getVNOM_SEL_BU_CHK();
Vector VNOM_SEL_TRUCK_TRANS_CHK = contract.getVNOM_SEL_TRUCK_TRANS_CHK();
Vector VNOM_SEL_CUST_CHK = contract.getVNOM_SEL_CUST_CHK();
Vector VNOM_SEL_FILL_CHK = contract.getVNOM_SEL_FILL_CHK();
...
if(agmt_type.equals(""))
{
	if(clearance.equals("IGX"))//HM : Added default Spot type for IGX as per Vijay's UAT Obs.
	{
		agmt_type="1";
	}
	else
...
<body onload="setValues('<%=strTransCd%>','<%=strTrans%>','<%=strPlant%>','<%=strBuPlant%>','<%=strPlantChrg%>','<%=strGxBuPlant%>','<%=strFillStaCd%>','<%=strFillStaAbbr%>');liabilityShows();billingShows();messurmentShows();off_spec_gasShows();terminatorShows();">
			...
			<div class="card cardmain">
				<div class="card-header cdheader">
					<div class="d-flex justify-content-between">
					    <div class="topheader">
					    	DLNG Sales Contract (SN/LOA/IGX)
					    </div>
					    <div class="btn-group">
				...
				<div class="card-body cdbody">
					...
					<div class="row m-b-5">
						...
						<div class="col-sm-4 col-xs-4 col-md-4">  
							<div class="form-group row">
				    			<div class="col-sm-4 col-xs-4 col-md-4"">
				    				<input type="text" class="form-control form-control-sm" name="" <%if(!cont_no.equals("")){ %>value="<%=dealMapping%>"<%} %> maxLength="50" readOnly>
				    			</div>
					...
					<div class="row m-b-5">
						...
						<div class="col-sm-4 col-xs-4 col-md-4">  
							<div class="form-group row">
				    			<div class="col-sm-12 col-xs-12 col-md-12">
				      				<input type="text" class="form-control form-control-sm" name="trade_ref_no" value="<%=trade_ref_no%>" maxLength="25" oninput="handleComma(this,'Trade Ref#')">
				    			</div>
				  			</div>
						</div>
					</div>
							...
							<div class="form-group row">
				    			<div class="col">
				      				<div class="input-group input-group-sm" >
			      						...
			      						onblur="validateDate(this);<%if(contract_type.equals("F")){ %>checkSigningDdaDate();<%} %>" onchange="validateDate(this);<%if(contract_type.equals("F")){ %>checkSigningDdaDate();<%} %>" autocomplete="off">
							...
							<div class="form-group row">
				    			<div class="col">
				      				<div class="input-group input-group-sm" >
			      						...
			      						onblur="validateDate(this);checkSigningStartDt(document.forms[0].start_dt, this.value);<%if(contract_type.equals("F")){ %>checkSigningDdaDate();<%} %>" 
			      						onchange="validateDate(this);checkSigningStartDt(document.forms[0].start_dt,this.value);<%if(contract_type.equals("F")){ %>checkSigningDdaDate();<%} %>" autocomplete="off">
			      						<span class="input-group-text"><i class="fa fa-calendar fa-lg"></i></span>
		      						</div>
					...
					<div class="row m-b-5">
						<div class="col-sm-2 col-xs-2 col-md-2">
							<%if(clearance.equals("IGX")){ %><label class="form-label"><b>Trade Type<span class="s-red">*</span></b></label><%}else{ %>
					...
					<div class="row m-b-5">
						<div class="col-sm-2 col-xs-2 col-md-2">  
							<div class="form-group row">
				    			...
				    			<%if(clearance.equals("IGX")){ %>
				    			title="Trade Delivery Mode : <%=agmt_agmt_base%>"<%}else{ %>
				    			title="Agreement AGMT BASE : <%=agmt_agmt_base%>"<%} %>><b>
								<%if(clearance.equals("IGX")){ %>
				    			Trade Delivery Mode<%}else{ %>
				    			Agreement Base<%} %>
				    			<span class="s-red">*</span></b></label>
				  			</div>
						</div>
						<div class="col-sm-4 col-xs-4 col-md-4">  
							<div class="form-group row">
				    			...
				    			<div class="col">
				      				<div class="input-group input-group-sm" >
										<input type="text"
											...
											onchange="validateDate(this);checkEffDtWithContStDt(this,'<%=min_counterparty_eff_dt%>');checkStartEndDate(this,document.forms[0].end_dt,'F');checkSigningStartDt(this,document.forms[0].signing_dt.value);<%if(contract_type.equals("F")){ %>checkAgmtStartdate();<%} %>"
											autocomplete="off"
											<%if(!cont_status_flg.equals("F") || !is_inv_submitted.equals("0")) {%>
							...
							<div class="form-group row">
				    			<div class="col-sm-12 col-xs-12 col-md-12">
				      				<div class="input-group input-group-sm" >
			      						...
			      						onchange="validateDate(this);checkStartEndDate(document.forms[0].start_dt,this,'T');<%if(contract_type.equals("F")){ %>checkAgmtEnddate();<%} %>" autocomplete="off"
			      						<%if(!cont_status_flg.equals("F")) { %>readOnly style="pointer-events: none;"<%} %>>
			      						<span class="input-group-text"><i class="fa fa-calendar fa-lg"></i></span>
		      						</div>
					...
					<div class="row m-b-5" <%if(!opration.equals("MODIFY")){ %>style="display:none;"<%} %>>
						...
						<div class="col-sm-4 col-xs-4 col-md-4">
							<div class="form-group row">
				    			...
				    			<div class="col">
				  				...
				  				<%} %>
				  				<div class="col" <%if(clearance.equals("IGX")){ %>style="display:none"<%} %>>  
									<div class="form-group row">
										<div class="col">
								...
								<div class="col-sm-4 col-xs-4 col-md-4" <%if(clearance.equals("IGX")){ %>style="display:none"<%} %>>  
									<div class="form-group row">
						    			<div class="col">
						...
						<div class="col-sm-4 col-xs-4 col-md-4">
							<div class="form-group row">								
				    			<div class="col">
				    				<input type="button" name="credit_exceed_btn" class="btn btn-sm config_btn" value="Credit Exceed Config" onclick="creditExceedDaysConfig('<%=contract_type%>','<%=agmt_no%>','<%=agmt_rev_no%>','<%=cont_no%>','<%=cont_rev_no%>','<%=cont_status_flg%>');">
				  				</div>				  				
				  			</div>
						</div>
					</div>
					<div class="row m-b-5">
						...
						<div class="col-sm-4 col-xs-4 col-md-4"> 
							<div class="form-group row">
				    			...
				    			<div class="col-auto">
		      						<i name="price_hist" id="price_hist" class="fa fa-info-circle fa-lg" aria-hidden="true" style="pointer-events: auto; color:#800040;" title="<%=price_change_history%>"></i>
				    			</div>
				    			<%if(opration.equals("MODIFY")){ %>
				    			<div class="col-auto">
				    				<input type="button" class="btn btn-sm config_btn" value="Variable Price Config" onclick="variablePriceConfig('<%=contract_type%>','<%=agmt_no%>','<%=agmt_rev_no%>','<%=cont_no%>','<%=cont_rev_no%>','<%=cont_ref_no%>','<%=cont_status%>','<%=cont_status_flg%>');">
				  				</div>
				  				<div class="col-auto">
		      						<i name="variable_price" id="var_price" class="fa fa-info-circle fa-2x" aria-hidden="true" style="pointer-events: auto; color:#0000FF; vertical-align: middle;" onclick="openVariablePriceInfo('<%=contract_type%>','<%=agmt_no%>','<%=agmt_rev_no%>','<%=cont_no%>','<%=cont_rev_no%>','<%=cont_ref_no%>','<%=cont_status%>','<%=cont_status_flg%>')"></i>
				    			</div>
				  				<%} %>
				  			</div> 
						</div>
					...
					<div class="row m-b-5" <%if((opration.equals("INSERT") || (cont_no.equals("0") || cont_no.equals(""))) || !is_allocated.equals("Y")){%>style="display:none;"<%} %>>
						...
						<div class="col-sm-4 col-xs-4 col-md-4">  
							<div class="form-group row">
				    			...
				    			<div class="col-auto">
				    				<input type="button" class="btn btn-sm request_btn" value="TCQ Change Request" onclick="doTcqRequest();" <%if(!fcc_flg.equals("Y") || cont_status_flg.equals("X")||cont_status_flg.equals("C")||cont_status_flg.equals("T")){%>style="background:#e6e6e6;color:black;" disabled<%} %>>
				    			</div>
				  			</div>
						</div>						
					</div>
					...
					<div class="row m-b-5">
						<div class="col-sm-2 col-xs-2 col-md-2">  
							<div class="form-group row">
				    			<label class="form-label"><b>Released MMBTU</b></label>
				  			</div>
						</div>	
						<div class="col-sm-4 col-xs-4 col-md-4">  
							<div class="form-group row">
								<div class="col-auto">
		      						<select class="form-select form-select-sm" name="delta_tcq_sign" disabled>
		      							<option value="+">+</option>
		      							<option value="-">-</option>
		      						</select>
		      						<%if(!delta_tcq_sign.equals("")){%>
		      							<script>document.forms[0].delta_tcq_sign.value="<%=delta_tcq_sign%>"</script>
		      						<%}%>
		      						<input type="hidden" name="delta_tcq_sign1" value="<%=delta_tcq_sign%>">
		      					</div>
					  			<div class="col-auto">
					      			<input type="text" class="form-control form-control-sm" name="closure_mmbtu" value="<%=balance_qty %>" readOnly>
					    		</div>
					    		<div class="col-auto">
					    			<label class="form-label"><b>Eff. Date</b></label>
					    		</div>
					    		<div class="col-auto">
					    			<div class="input-group input-group-sm" >
			      						<input type="text" class="form-control form-control-sm date" name="closure_eff_dt" value="<%=closure_eff_dt %>" maxLength="10" onblur="validateDate(this);" onchange="validateDate(this);" autocomplete="off" readOnly>
			      						<span class="input-group-text"><i class="fa fa-calendar fa-lg"></i></span>
		      						</div>
					    		</div>
							</div>
					...
					<div class="row m-b-5">
						...
						<div class="col-sm-4 col-xs-4 col-md-4">
							<div class="form-group row">
				    			...
				    			<%if(opration.equals("MODIFY")){ %>
				    			<div class="col">
				    				<input type="button" class="btn btn-sm config_btn" value="Variable DCQ Config" onclick="variableDCQ('<%=contract_type%>','<%=agmt_no%>','<%=agmt_rev_no%>','<%=cont_no%>','<%=cont_rev_no%>','<%=cont_status_flg%>');">
				    			</div>			    		
				    			<%} %>	
				  			</div>
						</div>
				    			...
				    			<div class="col-sm-12 col-xs-12 col-md-12">
				    				<%if(VBU_CD.size() > 0) {%>
					    				<%for(int i=0; i<VBU_CD.size(); i++){ %>
					    					<input <%if(clearance.equals("IGX")){ %>type="radio"<%}else{ %>type="checkbox"<%} %> class="form-check-input" name="chk_bu_plant" value="<%=VBU_PLANT_SEQ_NO.elementAt(i)%>" onchange="checkNom_Status('<%=clearance%>','bu_plant');">&nbsp;<%=VBU_PLANT_ABBR.elementAt(i)%>&nbsp;&nbsp;
				    			...
				    			<div class="col-sm-12 col-xs-12 col-md-12">
				    				<%if(VTRUCK_TRANS_CD.size() > 0) {%>
					    				<%for(int i=0; i<VTRUCK_TRANS_CD.size(); i++){ %>
					    					<input type="checkbox" class="form-check-input" name="chk_truck_trans" value="<%=VTRUCK_TRANS_CD.elementAt(i)%>" title="<%=VTRUCK_TRANS_NM.elementAt(i)%>" onchange="checkNom_Status('<%=clearance%>','truck_trans');" >&nbsp;<%=VTRUCK_TRANS_ABBR.elementAt(i)%>&nbsp;&nbsp;
				    			...
				    			<div class="col-sm-12 col-xs-12 col-md-12">
				    				<%if(VPLANT_SEQ_NO.size() > 0) {%>
					    				<%for(int i=0; i<VPLANT_SEQ_NO.size(); i++){ %>
					    					<input type="checkbox" class="form-check-input" name="chk_plant" value="<%=VPLANT_SEQ_NO.elementAt(i)%>" onchange="checkNom_Status('<%=clearance%>','cust_plant')">&nbsp;<%=VPLANT_ABBR.elementAt(i)%>&nbsp;&nbsp;
					    					<input type="hidden" name="cust_plant_nom" value="<%=VNOM_SEL_CUST_CHK.elementAt(i)%>">
					    				<%}%>
				    			...
				    			<div class="col-sm-12 col-xs-12 col-md-12">
				    				<%if(VFILL_STATION_CD.size() > 0) {%>
					    				<%for(int i=0; i<VFILL_STATION_CD.size(); i++){ %>
					    					<input type="checkbox" class="form-check-input" name="chk_fill_station" value="<%=VFILL_STATION_CD.elementAt(i)%>" onchange="checkNom_Status('<%=clearance%>','fill_station')">&nbsp;<%=VFILL_STATION_ABBR.elementAt(i)%>&nbsp;&nbsp;
					...
					<div class="row m-b-5" id="linkTrans">
						...
						<div class="col-sm-4 col-xs-4 col-md-4">  
							<div class="form-group row">
				    			<div class="col-sm-12 col-xs-12 col-md-12">
				    				<textarea class="form-control form-control-sm" readOnly style="background:#99ffcc;" rows="<%=gt_contract_dtl.split(", ").length<=0?1:gt_contract_dtl.split(", ").length%>"><%=gt_contract_dtl.replace(", ","\n")%></textarea>
					...
					<div class="row m-b-5">
						...
						<div class="col-sm-10 col-xs-10 col-md-10">  
							<div class="form-group row">
				    			<div class="col-sm-12 col-xs-12 col-md-12">
				    				<input type="checkbox" name="chk_seller_nom" class="form-check-input" value="M" <%if(sell_month_nom.equals("Y")||sell_month_nom.equals("")){ %>checked<%} %>>&nbsp;Monthly&nbsp;&nbsp;
						...
						<div class="col-sm-4 col-xs-4 col-md-4">  
							<div class="form-group row">
				    			<div id="liability_div2" class="col-sm-12 col-xs-12 col-md-12">
				    				<input type="button" class="btn btn-sm config_btn" value="Configure Liability" <%if(opration.equals("MODIFY")){ %>onclick="openLiabilityClause('<%=cont_status_flg%>');"<%} else {%> disabled <%} %>>&nbsp;
				    				<%if(opration.equals("MODIFY")){ %>
				...
				<div class="card-footer cdfooter text-center">
					<div class="d-flex justify-content-between">
						...
						&& !price_change_request_flag.equals("Y") && !cont_status_flg.equals("X") && !cont_status_flg.equals("C") &&!cont_status_flg.equals("T") && !closure_request_flag.equals("Y") && !closure_request_flag.equals("R")){ %>
				...
				<div class="card-body cdbody">
					<div class="row m-b-5">
						<div class="col-sm-12 col-xs-12 col-md-12">  
							<div class="form-group row">
								<div class="col-auto">
				    				<input type="button" class="btn btn-sm request_btn" value="Modify Contract Duration" onclick="" data-bs-toggle="modal" data-bs-target="#ChangeDurationModal" <%if(!fcc_flg.equals("Y") ||cont_status_flg.equals("X")||cont_status_flg.equals("C")||cont_status_flg.equals("T")){%>style="background:#e6e6e6;color:black;" disabled<%}%>>
				    			</div>
								<div class="col-auto"> 
									<input type="button" class="btn btn-sm request_btn" value="Price Change Request" onclick="doPriceChangeRequest();" <%if(!fcc_flg.equals("Y") || !is_allocated.equals("Y")||cont_status_flg.equals("X")||cont_status_flg.equals("C")||cont_status_flg.equals("T")){%>style="background:#e6e6e6;color:black;" disabled<%}%>>
								</div>				    			
				    			<div class="col-auto" title="<%if(cont_status_flg.equals("R")){%>If contract Re-Opened then, closure request button will be enabled once fcc is completed!<%}else if(closure_request_flag.equals("Y")){%>Closure request generated<%}else if(cont_status_flg.equals("C")){%>Contract Closed<%}else if(!is_allocated.equals("Y")){%>Closure can be done once qty is allocated to the contract!<%}else if(!is_expired.equals("Y") && !cont_status_flg.equals("T")){%>Closure can be applied on expired contract only, use Terminate Request!<%}%>">
				    				<!-- <input type="button" class="btn btn-sm request_btn" value="Closure Request" style="background:#e6e6e6;color:black;"> -->
				    				<input type="button" class="btn btn-sm request_btn" value="Closure Request" <%if((opration.equals("MODIFY")&&is_allocated.equals("Y")&&!closure_request_flag.equals("Y")&&!cont_status_flg.equals("C") &&!cont_status_flg.equals("T") &&!cont_status_flg.equals("R"))&&(is_expired.equals("Y"))){%>onclick="doClosureRequest('C');"<%}else{%>style="background:#e6e6e6;color:black;" disabled<%} %>>
				    			</div>
				    			<div class="col-auto" title="<%if(is_allocated.equals("Y") && !cancel_flag && !closure_request_flag.equals("A") && !closure_request_flag.equals("O")){%>Can't cancel the contract if qty is allocate, close the contract!<%}%>">
				    				<input type="button" class="btn btn-sm request_btn" value="Cancel Contract" data-bs-toggle="modal" data-bs-target="#CancelModal" <%if(opration.equals("MODIFY") && !cont_status_flg.equals("X")){if((is_allocated.equals("Y") || cont_no.equals("0") || cont_no.equals(""))&&!cancel_flag){ %>style="background:#e6e6e6;color:black;" disabled<%}}else{%>style="background:#e6e6e6;color:black;" disabled<%} %>>
				    			</div>
				    			<div class="col-auto" title="<% if(!closure_request_flag.equals("O")){%>Activates when contract is closed or terminated!<%}else if(closure_request_flag.equals("O")){%>Contract Reopen Request generated!<%}%>">
				    				<input type="button" class="btn btn-sm request_btn" value="Re-Open Request" onclick = "doReopenRequest();"  <%if((opration.equals("INSERT") ||(!cont_status_flg.equals("C")&&!cont_status_flg.equals("T")))||closure_request_flag.equals("O")){%> style="background:#e6e6e6;color:black;" disabled <%} %>>
				    			</div>
								<div class="col-auto" title="<%if(cont_status_flg.equals("R")){%>If contract Re-Opened then, terminate request button will be enabled once fcc is completed!<%}if(is_expired.equals("Y")){%>Contract is expired, so close contract!<%}else if(!termination_forced.equals("Y")){%>Configure Termination/Suspention (Force Majeure) to enable!<%}else if(closure_request_flag.equals("R")){%>Termination request is generated for the contract!<%}else if(cont_status_flg.equals("T")){%>Contract Terminated!<%}%>">
				    				<input type="button" class="btn btn-sm request_btn" value="Terminate Request" onclick="doClosureRequest('T');"  <%if(opration.equals("INSERT")||!termination_forced.equals("Y")|| cont_status_flg.equals("C") ||cont_status_flg.equals("T")||closure_request_flag.equals("X")||cont_status_flg.equals("R") || is_expired.equals("Y") || !is_allocated.equals("Y") || closure_request_flag.equals("R")){%> style="background:#e6e6e6;color:black;" disabled <%} %>>
				    			</div>
				  			</div>
						</div>
					</div>
					...
					<%if(!cont_no.equals("0") && !cont_no.equals("")){ %>
						...
						<%} %>
						<%if((no_of_security_dtl.equals("0") || no_of_security_dtl.equals("") || adv_adjust.equals("")) && clearance.equals("KYC")){ %>
							<div class="row m-b-5">
								<div class="col-sm-12 col-xs-12 col-md-12">
									<%=utilmsg.errorMessage("<b>Security Detail/Adjust Advance not Configured. The Contract will not appear for FCC!</b>")%>
								</div>
							</div>
						<%} %>
					...
					<%if(closure_request_flag.equals("Y")) {%>
						<div class="row m-b-5">
							<div class="col-sm-12 col-xs-12 col-md-12">
								<%=utilmsg.infoMessage("<b>Contract Closure Request has been Generated for the Contract!</b>")%>
							</div>
						</div>		
					<%}else if(closure_request_flag.equals("O")){%>
						<div class="row m-b-5">
							<div class="col-sm-12 col-xs-12 col-md-12">
								<%=utilmsg.infoMessage("<b>Contract Re-Open Request has been Generated for the Contract!</b>")%>
							</div>
						</div>		
					<%}else if(closure_request_flag.equals("R")){%>
						<div class="row m-b-5">
							<div class="col-sm-12 col-xs-12 col-md-12">
								<%=utilmsg.infoMessage("<b>Contract Termination Request has been Generated for the Contract!</b>")%>
							</div>
						</div>		
					<%}%>
				</div>
...
<!-- Contract Duration Modification Modal -->
<div class="modal fade" id="ChangeDurationModal" data-bs-backdrop="static" data-bs-keyboard="false">
	<div class="modal-dialog modal-dialog-scrollable modal-xl">
		<div class="modal-content">
			<div class="modal-header cdheader">
        		<div class="topheader">
					Contract [<%=dealMapping%>] Duration Modification
				</div>
        		<input type="button" class="btn-close" data-bs-dismiss="modal" onclick="(function() {document.forms[0].new_start_dt.value='';document.forms[0].new_end_dt.value='';})();">
      		</div>
      		<div class="modal-body mdbody">
      			<div class="cdbody">
      				<div class="row m-b-5">
						<div class="col-sm-2 col-xs-2 col-md-2">  
							<div class="form-group row">
				    			<label class="form-label"><b>New Start Date<span class="s-red">*</span></b></label>
				  			</div>
						</div>
						<div class="col-sm-4 col-xs-4 col-md-4"> 
							<div class="form-group row">
				    			<div class="col">
				    				<div class="input-group input-group-sm" >
			      						<input type="text" class="form-control form-control-sm date fmsdtpick" name="new_start_dt" value="<%=start_dt%>" maxLength="10" onblur="validateDate(this);checkStartEndDate(this,document.forms[0].new_end_dt,'F');" onchange="validateDate(this);checkStartEndDate(this,document.forms[0].new_end_dt,'F');" autocomplete="off">
			      						<span class="input-group-text"><i class="fa fa-calendar fa-lg"></i></span>
		      						</div>
		      					</div>
				  			</div> 
						...
						<div class="col-sm-2 col-xs-2 col-md-2">  
							<div class="form-group row">
				    			<label class="form-label"><b>New End Date<span class="s-red">*</span></b></label>
				  			</div>
						</div>
						<div class="col-sm-4 col-xs-4 col-md-4"> 
							<div class="form-group row">
				    			<div class="col">
				    				<div class="input-group input-group-sm" >
			      						<input type="text" class="form-control form-control-sm date fmsdtpick" name="new_end_dt" value="<%=end_dt%>" maxLength="10" onblur="validateDate(this);checkStartEndDate(document.forms[0].new_start_dt,this,'T');" onchange="validateDate(this);checkStartEndDate(document.forms[0].new_start_dt,this,'T');" autocomplete="off">
			      						<span class="input-group-text"><i class="fa fa-calendar fa-lg"></i></span>
		      						</div>
		      					</div>
				  			</div> 
			...
			<div class="modal-footer cdfooter">
				<div class="" align="right">
					<input type="button" class="btn btn-warning com-btn" value="Submit" onclick="doContractDurationChangeRequest();">
				</div>
			</div>
		</div>
	</div>
...
<div class="modal fade" id="CancelModal" data-bs-backdrop="static" data-bs-keyboard="false">
	<div class="modal-dialog modal-dialog-scrollable modal-xl">
		<div class="modal-content">
			<div class="modal-header cdheader">
        		<div class="topheader">
					Contract [<%=dealMapping%>] Cancellation
				</div>
...
<input type="hidden" name="option" value="DLNG_CONT_MST">
