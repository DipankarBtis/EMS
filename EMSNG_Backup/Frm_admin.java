...
public class Frm_admin extends HttpServlet
{
	...
	private void InsertUpdatePasswordPolicy(HttpServletRequest request) throws SQLException 
	{
		...
		try
		{
			...
			if(!eff_dt.equals("") && !uid_min_length.equals("") && !uid_max_length.equals(""))
			{
				...
				{
					query = "INSERT INTO FMS_ADMIN_POLICY(COMPANY_CD,USR_MIN,USR_MAX,PWD_LEN_MIN,"
	...
	private void InsertUpdateUserDetailsStatus(HttpServletRequest request) throws SQLException 
	{
		...
		try
		{
			...
			{
				if(!user_cd.equals(null) && !user_cd.trim().equals(""))
				{
					if(!user_cd.equals("0") && change_status.equals("N"))
					{
						query = "UPDATE FMS_EMP_MST SET LOCK_STATUS=?, MODIFY_DT=SYSDATE, MODIFY_BY=? "
								+ "WHERE EMP_CD=? ";
						stmt=dbcon.prepareStatement(query);
						stmt.setString(1, "Y");
					...
					{
						query = "UPDATE FMS_EMP_MST SET LOCK_STATUS = NULL, MODIFY_DT=SYSDATE, MODIFY_BY=?, INVALID_LOGIN=? "
								+ "WHERE EMP_CD=? ";
						stmt=dbcon.prepareStatement(query);
			...
			else if(mod_type.equals("user_status")) 
			{
				if(!user_cd.equals(null) && !user_cd.trim().equals(""))
				{
					if(!user_cd.equals("0") && change_status.equals("N"))
					{
						query = "UPDATE FMS_EMP_MST SET EMP_STATUS=?, MODIFY_DT=SYSDATE, MODIFY_BY=? "
								+ "WHERE EMP_CD=? ";
						stmt=dbcon.prepareStatement(query);
						stmt.setString(1, "Y");
						stmt.setString(2, emp_cd);
					...
					{
						query = "UPDATE FMS_EMP_MST SET EMP_STATUS=?, MODIFY_DT=SYSDATE, MODIFY_BY=?,IS_RECOVERED=?,RECOVERED_ON=SYSDATE "
								+ "WHERE EMP_CD=? ";
						stmt=dbcon.prepareStatement(query);
						stmt.setString(1, "Y");
						stmt.setString(2, emp_cd);
			...
			if(group_cd != null)
			{
				for(int i=0; i<group_cd.length; i++)
				{
					if(!group_cd[i].equals(""))
					{
						...
						if(count > 0)
						{
							query1 ="SELECT GROUP_NAME,ACTIVE "
									+ "FROM FMS_GROUP_MST "
									+ "WHERE GROUP_CD=? ";
							stmt1=dbcon.prepareStatement(query1);
							...
							if(!prev_group_nm.equals(group_nm[i]) || !prev_active_flag.equals(active_flag[i]))
							{
						...
						{
							query = "INSERT INTO FMS_GROUP_MST(GROUP_CD,GROUP_NAME,ACTIVE,ENT_BY,ENT_DT) "
									+ "VALUES (?,?,?,?,?,sysdate)";
							stmt=dbcon.prepareStatement(query);
							stmt.setString(1, group_cd[i]);
	...
	private void InsertUpdateMenuDetail(HttpServletRequest request) throws SQLException 
	{
		...
		try
		{
			...
			if(group_cd.equals("other"))
			{
				...
				query="SELECT DISTINCT(SUB_MENU_CD), SUB_MENU_NM FROM FMS_FORM_MST WHERE MODULE_CD=? AND UPPER(TRIM(SUB_MENU_NM)) =?";
				stmt=dbcon.prepareStatement(query);
				...
				if(gcd.equals("0"))
				{
					query = "SELECT MAX(SUB_MENU_CD) FROM FMS_FORM_MST WHERE MODULE_CD=?";
					stmt=dbcon.prepareStatement(query);
					stmt.setString(1, module_cd);
					rset=stmt.executeQuery();
					if(rset.next())
			...
			if(opration.equals("MODIFY"))
			{
				if(!menu_cd.equals("") && !menu_cd.equals("0"))
				{
					...
					query = "UPDATE FMS_FORM_MST SET FORM_NAME=?,MODULE_CD=?,CLASSPATH=?,FORM_TYPE=?,"
							+ "SUB_MENU_CD=?,SUB_MENU_NM=?,MODIFY_BY=?,MODIFY_DT=SYSDATE,SUB_MENU_SEQ=?,FLAG=? "
							+ "WHERE FORM_CD=?";
					stmt=dbcon.prepareStatement(query);
			...
			{
				int formCd=0;
				query = "SELECT MAX(FORM_CD) FROM FMS_FORM_MST";
				stmt=dbcon.prepareStatement(query);
				rset=stmt.executeQuery();
				if(rset.next())
				{
				...
				query="INSERT INTO FMS_FORM_MST(FORM_CD,FORM_NAME,MODULE_CD,CLASSPATH,FORM_TYPE,FLAG,SUB_MENU_CD,SUB_MENU_NM,SUB_MENU_SEQ,ENT_BY,ENT_DT) "
					+ "VALUES(?,?,?,?,?,?,?,?,?,?,sysdate)";
				stmt=dbcon.prepareStatement(query);
			...
			if(!user_nm.equals("") && !user_id.equals(""))
			{
					...
					if(!user_cd.equals("0") && !user_cd.equals(""))
					{
						...
						if(count==0) 
						{
							query = "UPDATE FMS_EMP_MST SET EMP_NM=?, EMAIL_ID=?, EMP_STATUS=?, REMARK=?,"
									+ "MODIFY_DT=SYSDATE, MODIFY_BY=?, PH_NO=?, MOB_NO=? "
									+ "WHERE EMP_CD=? ";
							stmt=dbcon.prepareStatement(query);
				...
				else // INSERT
				{
					...
					else
					{
						...
						query = "INSERT INTO FMS_EMP_MST(EMP_CD,EFF_DT,EMP_NM,EMP_UID,EMAIL_ID,EMP_STATUS,REMARK,ENT_DT,ENT_BY,COMPANY_CD,"
								+ "PH_NO,MOB_NO) "
						...
						query = "INSERT INTO FMS_EMP_PASSWD_MST(EMP_CD,COMPANY_CD,PASSWORD,LOGIN_STATUS,LAST_CHANGE,"
						...
						query = "INSERT INTO FMS_EMP_PASSWD_DTL(COMPANY_CD,EMP_CD,SEQ_NO,PASSWORD,LOGIN_STATUS,LAST_CHANGE,LOCKED_FLAG,RESET_FLAG,"
								+ "EXPIRY_DATE,ENT_DT,ENT_BY) "
								+ "SELECT COMPANY_CD,EMP_CD,?,PASSWORD,LOGIN_STATUS,LAST_CHANGE,LOCKED_FLAG,RESET_FLAG,"
								+ "EXPIRY_DATE,ENT_DT,ENT_BY "
				...
				if(chflg)
				{
					...
					query="INSERT INTO FMS_EMP_MST_LOG(EMP_CD,SEQ_NO,EFF_DT,EMP_NM,EMP_UID,EMAIL_ID,EMP_STATUS,PH_NO,MOB_NO,COMPANY_CD,UNIT_CD,"
							+ "REMARK,ENT_DT,ENT_BY,MODIFY_DT,MODIFY_BY) SELECT EMP_CD,?,EFF_DT,EMP_NM,EMP_UID,EMAIL_ID,EMP_STATUS,PH_NO,"
							+ "MOB_NO,COMPANY_CD,UNIT_CD,REMARK,ENT_DT,ENT_BY,MODIFY_DT,MODIFY_BY "
							+ "FROM FMS_EMP_MST "
							+ "WHERE EMP_CD=? ";
					stmt=dbcon.prepareStatement(query);
					stmt.setString(1, seq_no);
				...
				if(count > 0)
				{
					...
					if(current_pwd.equals(existing_pwd))
					{
						...
						if(count == 0)
						{
							...
							query = "INSERT INTO FMS_EMP_PASSWD_DTL(EMP_CD,COMPANY_CD,SEQ_NO,PASSWORD,LOGIN_STATUS,LAST_CHANGE,LOCKED_FLAG,RESET_FLAG,"
									+ "EXPIRY_DATE,ENT_DT,ENT_BY) "
									+ "SELECT EMP_CD,COMPANY_CD,?,PASSWORD,LOGIN_STATUS,LAST_CHANGE,LOCKED_FLAG,RESET_FLAG,"
									+ "EXPIRY_DATE,ENT_DT,ENT_BY "
									+ "FROM FMS_EMP_PASSWD_MST "
									+ "WHERE EMP_CD=? ";
							stmt=dbcon.prepareStatement(query);
			...
			if((!comp_cd.equals("0") && !comp_cd.equals("")) && (!user_cd.equals("0") && !user_cd.equals("")) && (!group_cd.equals("0") && !group_cd.equals("")) && !from_dt.equals("") && !to_dt.equals(""))
			{
				...
				else
				{
					...
					else 
					{
						...
						{
							query="INSERT INTO FMS_EMP_GROUP_DTL(COMPANY_CD,EMP_CD,GROUP_CD,FROM_DT,TO_DT,ENT_DT,ENT_BY) "
	...
	private void InsertUserMailBody(HttpServletRequest request, String comp_cd,String user_nm,String user_cd,String user_id,String email_id, String password) throws Exception
	{
		...
		try
		{
			SUPPORT_EMAIL_1=utilBean.getKeyValueFromSrvSettingUsingKeyNm("SUPPORT_EMAIL_1");
			SUPPORT_EMAIL_2=utilBean.getKeyValueFromSrvSettingUsingKeyNm("SUPPORT_EMAIL_2");
			...
			String to_mail_new_user = email_id+","+utilBean.getToMailReceipentList("0","User Activity Notification","Admin","NA","On-Event");
			String cc_mail_lock_user = utilBean.getCcMailReceipentList("0","User Activity Notification","Admin","NA","On-Event");
