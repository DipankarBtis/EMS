										...
										<tr <%if(!activity_type.equals("PREPARE") && !activity_type.equals("APPROVE")) {%>style="pointer-events: auto;"<%} %>>
											...
											<th>
												<span class="badge rounded-pill" style="background:orange;color:black;font-size:12px;">
													<input type="radio" name="gx_invoice_type" value="S" <%if(!activity_type.equals("PREPARE") && default_chk.equals("S")){ %>checked<%}else if(default_chk.equals("S")){%>checked<%} %> onclick="doEnabled(this.value);">&nbsp;System Generated
												</span>
											</th>
											<th>
												<span class="badge rounded-pill" style="background:skyblue;color:black;font-size:12px;">
													<input type="radio" name="gx_invoice_type" value="P" onclick="doEnabled(this.value);" <%if(activity_type.equals("PREPARE") && sg_submission_chk){%>checked<%}else if(default_chk.equals("P")){ %>checked<%} %> <%if(!sg_submission_chk){ %>disabled<%} %>>&nbsp;Party Generated
												</span>
											</th>
											<th>
												Difference
													...
													<div class="d-flex justify-content-between">
														...
														<i class="fa fa-file-code-o fa-2x" 
															onclick="doGenXML('<%=counterparty_cd%>','<%=invoice_no%>','<%=financial_year%>','<%=invoice_seq%>','<%=contract_type%>',
														 		'S','<%=invoice_type%>','N','<%=gx_counterparty_cd%>','<%=bu_unit%>','<%=agmt_no%>','<%=cont_no%>','N','VIEW');"
															style="color: orange;">													
														</i>
														<%} %>
													...
													<div class="d-flex justify-content-between">
														...
														<i class="fa fa-file-code-o fa-2x" 
															onclick="doGenXML('<%=counterparty_cd%>','<%=pg_invoice_no%>','<%=financial_year%>','<%=invoice_seq%>','<%=contract_type%>',
														 		'P','<%=invoice_type%>','N','<%=gx_counterparty_cd%>','<%=bu_unit%>','<%=agmt_no%>','<%=cont_no%>','N','VIEW');"
															style="color: orange;">													
														</i>
														<%} %>
														<%if(write_access.equals("Y")){ %>
