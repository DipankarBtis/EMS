...
function refresh(opration)
{
	...
	if(flag)
	{
		var url = "frm_purchase_other_invoice.jsp?form_cd="+formCd+"&form_nm="+formNm+"&counterparty_cd="+counterparty_cd+
				"&mod_cd="+mod_cd+"&mod_nm="+mod_nm+"&month="+month+"&year="+year+"&billing_cycle="+billing_cycle+
				"&period_start_dt="+period_start_dt+"&period_end_dt="+period_end_dt+"&mapping_id="+mapping_id+
...
function openInvoiceList()
{
	...
	var url="rpt_pur_other_invoice_list.jsp?counterparty_cd="+counterparty_cd+"&month="+month+"&year="+year;
...
function setInvoiceDetail(financial,inv_typ,inv_seq,inv_no,mapping_id,add_type,bu,freq,p_start_dt,p_end_dt)
{
	...
	var url = "frm_purchase_other_invoice.jsp?form_cd="+formCd+"&form_nm="+formNm+"&counterparty_cd="+counterparty_cd+
			"&mod_cd="+mod_cd+"&mod_nm="+mod_nm+"&month="+month+"&year="+year+"&invoice_type="+inv_typ+"&opration="+opration+
...
function printPDF()
{
	...
	var url = "../pdf_reports/purchase/other_invoice/pdf_other_invoice.jsp?counterparty_cd="+counterparty_cd+
	...
	if(!newWindow || newWindow.closed)
	{
		newWindow= window.open(url,"Other Invoice","top=10,left=70,width=900,height=700,scrollbars=1");
	}
	else
	{
		newWindow.close();
...
function openTaxStructMst()
{
	...
	if(!newWindow || newWindow.closed)
	{
		newWindow = window.open("frm_tax_structure_list.jsp","Tax Structure List","top=10,left=10,width=1100,height=600,scrollbars=1");
	}
	else
	{
		newWindow.close();
...
remittance.setCallFlag("OTHER_INVOICE");
			...
			<div class="card cardmain">
				<div class="card-header cdheader">
					<div class="d-flex justify-content-between">
					    <div class="topheader">
					    	Trader Free Flow Invoice Generation
					    </div>
					</div>
				</div>
				    ...
				    <div class="row m-b-5">
						...
						<div class="col-sm-4 col-xs-4 col-md-4">  
							<div class="form-group row">
				    			...
				    			<div class="col">
				    				<input type="text" class="form-control form-control-sm" name="invoice_ref" value="<%=invoice_ref%>">
				    			</div>
				    		</div>
				    	</div>
				    	<div class="col-sm-2 col-xs-2 col-md-2">  
				   ...
				   <div class="row m-b-5">
				    ...
				    <div class="row m-b-5">
				    	<div class="col-sm-6 col-xs-6 col-md-6" align="right"> 
				    		<div class="form-group row">
				    			<div class="col-sm-12 col-xs-12 col-md-12">
				    				<input type="button" class="btn btn-warning com-btn" value="Submit" onclick="doSubmit();" <%if(invoice_check_flag.equals("Y")){ %>disabled<%} %>>
				    				<input type="button" class="btn btn-warning com-btn" value="View" onclick="printPDF();">
				    			</div>
				    		</div>
				    	</div>
				    </div>
...
<input type="hidden" name="option" value="OTHER_INVOICE">
