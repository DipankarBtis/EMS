...
%>
<body onload="<%if(activity_type.equals("PREPARE") && sg_submission_chk) {%>doEnabled('P');<%}else{%>doEnabled('<%=default_chk %>');<%}%>difference();copyInvoiceNo();<%if(!msg.equals("")){%>refreshParent('<%=accroid%>');<%}%>"
			...
			<div class="card cardmain">
				<div class="card-header cdheader">
					<div class="d-flex justify-content-between">
					    <div class="topheader">
					      	<%=entity_nm%> <%if(inv_flag.equals("P")){ %>Provisional <%} %>Service Remittance Detail
					    </div>
					</div>
				</div>
						...
						<div class="col-md-12 col-sm-12 col-xs-12">
							<div class="table-responsive">
								<table class="table table-bordered table-hover">
									<thead>
										<tr>
											<th colspan="3"><%=entity_nm%> Detail</th>
								...
								<table class="table table-bordered table-hover" id="inv_difference">
										...
										<tr <%if(!activity_type.equals("PREPARE") && !activity_type.equals("APPROVE")) {%>style="pointer-events: auto;"<%} %>>
											...
											<th>
												<span class="badge rounded-pill" style="background:orange;color:black;font-size:12px;">
													<input type="radio" name="invoice_type" value="S" <%if(!activity_type.equals("PREPARE") && default_chk.equals("S")){ %>checked<%}else if(default_chk.equals("S")){%>checked<%} %> onclick="doEnabled(this.value);">&nbsp;System Generated
												</span>
											</th>
											<th>
												<span class="badge rounded-pill" style="background:skyblue;color:black;font-size:12px;">
													<input type="radio" name="invoice_type" value="P" onclick="doEnabled(this.value);" <%if(activity_type.equals("PREPARE") && sg_submission_chk){%>checked<%}else if(default_chk.equals("P")){ %>checked<%} %> <%if(!sg_submission_chk){ %>disabled<%} %>>&nbsp;Party Generated
												</span>
											</th>
											<th>
												Difference
									...
									<tbody>
											...
											<td align="center">
												<div style="width:250px;">
													<div class="input-group input-group-sm">
														...
														onkeyup=" partyGenCalc();difference();checkRateFormate(this,'<%=energy_unit%>','<%=price_cd%>')">
														<span class="input-group-text"><%=price_cd_nm%>/<%=energy_unit_nm%></span>
													</div>
												...
												<div style="width:250px;">
													<div class="d-flex justify-content-between">
														...
														onclick="doGenXML('<%=counterparty_cd%>','<%=invoice_no%>','<%=financial_year%>','<%=invoice_seq%>','<%=contract_type%>',
														 'S','','N','<%=bu_unit%>','<%=cont_no%>','N','<%=accroid%>','<%=inv_flag%>','VIEW');"
														style="color: orange;">													
														</i>
														<%} %>
												...
												<div style="width:250px;">
													<div class="d-flex justify-content-between">
														...
														onclick="doGenXML('<%=counterparty_cd%>','<%=pg_invoice_no%>','<%=financial_year%>','<%=invoice_seq%>','<%=contract_type%>',
														 'P','','N','<%=bu_unit%>','<%=cont_no%>','N','<%=accroid%>','<%=inv_flag%>','VIEW');"
														style="color: orange;">													
														</i>
														<%} %>
														<%if(write_access.equals("Y")){ %>
										...
										<tr style="<%if(activity_type.equals("PREPARE")){%>display:none;<%}else{ %>pointer-events: auto;<%}%>">
											<td><b>View <%=inv_lineitem%> / Status</b></td>
											<td align="center">
      			...
      			<div class="cdbody">
					...
					<div class="row">
						<div class="col-md-12 col-sm-12 col-xs-12">
							...
							<%=utilmsg.infoMessage("Financial Year: <b>"+financial_year+"</b> Transaction > 1Lakh |  This invoice Gross> 30K, <b>TDS: "+tdsYesNo+"</b>") %>
						</div>
					</div>
      			</div>
      		</div>
...
<input type="hidden" name="energy_unit_nm" value="<%=energy_unit_nm%>">
