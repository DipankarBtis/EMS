...
function refresh(accroid)
{
	...
	if(flag)
	{
		var url = "../sales_invoice/frm_sales_crdr_preparation.jsp?&u="+u+"&pdf_type="+pdf_type+
...
function openCreditDebitInv(accroid,genType)
{
	...
	var url = "frm_generate_sales_crdr.jsp?u="+u+"&month="+month+"&year="+year+"&operation=PREPARE&accroid="+accroid+"&crdr_gen_type="+genType;
...
function openActivity1(counterparty_cd,inv_seq,fin_yr,bu_st_cd,operation,accroid,inv_flag,ref_no,genType)
{
	var u = document.forms[0].u.value;
	
	var url = "frm_generate_sales_crdr.jsp?counterparty_cd="+counterparty_cd+"&invoice_type="+inv_flag+"&invoice_seq="+inv_seq+"&operation="+operation+
			"&financial_year="+fin_yr+"&bu_state_tin="+bu_st_cd+"&accroid="+accroid+"&inv_flag="+inv_flag+"&sel_inv_no="+ref_no+"&crdr_gen_type="+genType+"&u="+u;
		...
		bu_state_tin,sap_approval_flag,agmt_no,cont_no, inv_pdf_flag,accroid)
{
	...
	var url = "../accounting/rpt_view_crdr_sales_inv_sap_approval.jsp?financial_year="+financial_year+"&invoice_seq="+invoice_seq+
...
function printPDF(counterparty_cd,agmt_no,agmt_rev,cont_no,cont_rev,cont_type,plant_seq,
		period_st_dt,period_end_dt,bu,fin_yr,bu_st_cd,inv_seq,index,cargo_no,accroid,inv_flag)
{
	...
	{
		var url = "pdf_sales_crdr_invoice.jsp?counterparty_cd="+counterparty_cd+"&contract_type="+cont_type+"&cont_no="+cont_no+
			"&cont_rev="+cont_rev+"&agmt_no="+agmt_no+"&agmt_rev="+agmt_rev+"&plant_seq="+plant_seq+"&period_start_dt="+period_st_dt+
...
Vector VCRDR_GEN_TYPE = sales_inv.getVCRDR_GEN_TYPE();
									...
									<!-- <div class="d-flex justify-content-center"> -->
										<div class="form-group row">
											<div class="col">
												<!-- <label class="form-label"><b>Invoice Type</b></label> -->
												<div class="btn-group">
													<label class="btn btn-outline-secondary subbtngrp" onclick="openCreditDebitInv('<%//=accro2%>','CRDR_IMB');"><i class="fa fa-plus-circle"></i>&nbsp;Create New Credit/Debit With Imbalance/Ship or Pay</label>
												</div>
								  			</div>
										</div>
									<!-- </div> -->
													...
													<div class="table-responsive">
														<table class="table table-bordered table-hover serchtbl" id="example<%=j%>">
															...
															<tbody>
																	...
																	<tr>
																		...
																		<td align="center">
																			...
																			onclick="openActivity1('<%=VCOUNTERPTY_CD.elementAt(i)%>','<%=VINVOICE_SEQ.elementAt(i)%>','<%=VFINANCIAL_YEAR.elementAt(i) %>',
																								'<%=VBU_STATE_TIN.elementAt(i)%>','INSERT','<%=heading%>','<%=VINV_FLAG.elementAt(i) %>','<%=VREF_NO.elementAt(i)%>','<%=VCRDR_GEN_TYPE.elementAt(i)%>');"
																			style="color:black;">																		
																			</i>					
																		</td>
																		<td align="center">
																			...
																			onclick="openActivity1('<%=VCOUNTERPTY_CD.elementAt(i)%>','<%=VINVOICE_SEQ.elementAt(i)%>','<%=VFINANCIAL_YEAR.elementAt(i) %>',
																								'<%=VBU_STATE_TIN.elementAt(i)%>','MODIFY','<%=heading%>','<%=VINV_FLAG.elementAt(i) %>','<%=VREF_NO.elementAt(i)%>','<%=VCRDR_GEN_TYPE.elementAt(i)%>');"
																		...
																		<%if(VRE_PRINT_PDF.elementAt(i).equals("Y")){%>
																			<!-- <span class="fa-stack fa-lg">
																				<i class="fa fa-flag fa-stack-2x" style="color:grey;"></i>
																		...
																		<td align="center">
																			<i class="fa fa-print fa-2x" title="<%=VPDF_TYPE.elementAt(i)%>"
																				...
																				color:#800000;
																				<%}else if(!VINV_APPROVED_FLAG.elementAt(i).equals("Y") 
																							|| !VIS_IRN_GENERATED.elementAt(i).equals("Y") 
																							|| VPDF_TYPE.elementAt(i).equals(pdf_type)){ %>
																				pointer-events: none; opacity: .65; color: gray;
																				<%} else{%>
																				color:#800000;
																				<%}%>">
																...
																<%}else{ %>
																	<tr>
																		<td align="center" colspan="<%if(heading.equals("LTCORA_INV_HEAD")) {%>18<%}else{%>17<%}%>"><%=utilmsg.infoMessage("<b>No Credit/Debit Note is Generated!</b>") %></td>
																	</tr>
																<%} %>
															</tbody>
														</table>
