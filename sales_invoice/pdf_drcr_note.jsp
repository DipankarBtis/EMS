...
<jsp:useBean class="com.etrm.fms.util.DataBean_Pdf_DRCR_Generation" id="pdfFile" scope="request"/>
<%
...
for(int i=0; i<pdf_type.length; i++)
{
	...
	pdfFile.setDrcr_flag(drcr_flag);
	pdfFile.setDrcr_seq(drcr_seq);
	pdfFile.setCrtieria(criteria);
	pdfFile.setdrcr_fin_yr(drcr_fin_yr);
	pdfFile.setCallFlag("PDF_DRCR_NOTE");
	pdfFile.init();
	
	String file_url = pdfFile.getFile_url();
	file_nm = pdfFile.getFile_nm();
	pdfpath = file_url+"/work_data"+owner_cd+""+CommonVariable.drcr_inv_path+""+file_nm;
}
