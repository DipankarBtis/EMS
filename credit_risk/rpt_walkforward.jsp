...
function refresh(clearance)
{
	...
	if(report_days >= 21 && report_days <=365)
	{
			...
			var url = "rpt_walkforward.jsp?u="+u+"&report_days="+report_days+"&clearance="+clearance+"&counterparty_cd="+counterparty_cd;
...
var newWindow;
function showDetails(exposure_dt,clearance,from_dt,report_days,counterparty_cd,type,cust_nm,deal_id,total,prev_total)
{
	url="rpt_walkforward_expo_details.jsp?exposure_dt="+exposure_dt+"&clearance="+clearance+"&from_dt="+from_dt+"&report_days="+report_days+"&counterparty_cd="+counterparty_cd+"&type="+type+"&cust_nm="+cust_nm+"&deal_id="+deal_id+"&total="+total+"&prev_total="+prev_total;
...
Vector V_EXPOSURE_DATE = db_walkforward.getV_EXPOSURE_DATE();
Vector VSEC_EXPOSURE_DATE = db_walkforward.getVSEC_EXPOSURE_DATE();
%>
<body>
					...
					<div class="row m-b-5">
						...
						<div class="col-sm-3 col-xs-3 col-md-3">
							<div class="form-group row">
				    			<div class="col">
				      				<select class="form-select form-select-sm" name="counterparty_cd" onchange="refresh('<%=clearance%>')" disabled>
				    	...
				    	<div class="col-sm-2 col-xs-2 col-md-2">
							<div class="form-group row">
				    			<div class="col">
				      				<span style="color:red;">*(MIN 21 DAYS & MAX 365 DAYS)</span>
				    			</div>
				  			</div>
						</div>
					</div>
						...
						<table class="table table-bordered">
							...
							<tbody>
								<tr>
									...
									<td colspan="3" align="center"><b>Limit (INR)</b></td>
									<%for(int i=0; i < VLIMIT_CALCULATION.size(); i++){ %>
									<td><div align="right"><b><%=VLIMIT_CALCULATION.elementAt(i)%></b></div></td>
									<%} %>
								</tr>
								...
								<tr>
									...
									<td colspan="3" align="center"><b>Settlement Sell Open Credit (INR) <font color='blue'>(Cumulative)</font></b></td>
									<%for(int i=0; i < VSETTLEMENTSELLCREDIT.size(); i++){ %>
									<td><div align="right"><b><%=VSETTLEMENTSELLCREDIT.elementAt(i)%></b></div></td>
									<%}%>
								</tr>
								<tbody class="body1" id="body1" style="display:none;">
									...
									<tr>
										...
										<%for(j=j; j < VCUSTWISESETTLEMENTSELLCREDIT.size(); j++){ 
											...
											{%>
												<td title="Click to view detail" <%if(!VCUSTWISESETTLEMENTSELLCREDIT.elementAt(j).equals("0.00")){ %>onclick="showDetails('<%=V_EXPOSURE_DATE.elementAt(j)%>','<%=clearance%>','<%=from_dt%>','<%=report_days%>','<%=counterparty_cd%>','OPEN_CREDIT',&quot;<%=VCOUNTERPARTY_NM.elementAt(i)%>&quot;,'<%=originalDealId%>','<%=VCUSTWISESETTLEMENTSELLCREDIT.elementAt(j)%>','<%=VCUSTWISESETTLEMENTSELLCREDITFORPREVDT.elementAt(j)%>')"<%} %>>
								...
								<tr>
									...
									<td colspan="3" align="center"><b>Settlement Sell Exposures (INR)</b></td>
									<%for(int i=0; i < VSETTLEMENTSELLEXPOSUR.size(); i++){ %>
									<td><div align="right"><b><%=VSETTLEMENTSELLEXPOSUR.elementAt(i)%></b></div></td>
									<%}%>
								</tr>
								<tbody class="body2" id="body2" style="display:none;">
									...
									<tr>
										...
										<%for(z=z; z < VCUSTWISESETTLEMENTSELLEXPOSURE.size(); z++){ 
											...
											{
											%>
												<td title="Click to view detail" <%if(!VCUSTWISESETTLEMENTSELLEXPOSURE.elementAt(z).equals("0.00")){ %>onclick="showDetails('<%=VSEC_EXPOSURE_DATE.elementAt(z) %>','<%=clearance%>','<%=from_dt%>','<%=report_days%>','<%=counterparty_cd%>','EXPOSURE',&quot;<%=VSEC_COUNTERPARTY_NM.elementAt(i)%>&quot;,'<%=originalDealId%>','<%=VCUSTWISESETTLEMENTSELLEXPOSURE.elementAt(z)%>','<%=VCUSTWISESETTLEMENTSELLEXPOSUREFORPREVDT.elementAt(z)%>')"<%} %>>
								...
								<tr>
									<td></td>
									<td colspan="3" align="center"><b>Available Credit (INR)</b></td>
									<%for(int i=0; i < VAVAILABLECREDIT.size(); i++){ %>
									<td><div align="right"><b><%=VAVAILABLECREDIT.elementAt(i)%></b></div></td>
									<%} %>
								<tr>
							</tbody>
						</table>
