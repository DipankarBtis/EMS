			...
			<div class="card cardmain">
				<div class="card-header cdheader">
					<div class="d-flex justify-content-between">
					    <div class="topheader">
					    	<%=sec_category_nm%> Security Details For <%=counterparty_abbr%> (<%=display_map_id%>) [<%if(contract_type.equals("V")){ %>Trade Confirm Date<%}else{ %>Contract Period<%} %> : <%=cont_start_dt %> <%if(!contract_type.equals("V")){ %>- <%=cont_end_dt %><%} %>]
					    </div>
					</div>
				</div>
							...
							<table class="table table-bordered table-hover" id="example">
								...
								<%if(VCOUNTERPARTY_CD.size()>0) {%>
									...
									<tr>
										...
										<td align="center"><%=owner_cd%>-<%=VSECURITY_REF.elementAt(i)%></td>
