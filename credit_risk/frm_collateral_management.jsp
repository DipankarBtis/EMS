					...
					<div class="row">
						<div class="col-sm-3 col-xs-3 col-md-12">
							<div class="d-flex justify-content-between"> 
								...
								<%-- <%if(counterparty_status.equals("Y")){ %> --%>
								<div class="btn-group" align="right">
									<label class="btn btn-outline-secondary subbtngrp1" <%if(counterparty_cd.equals("0")){ %>onclick="selectCounterparty()"<%}else{ %>data-bs-toggle="modal" data-bs-target="#AddNewSecurity"<%} %>>Add New Security</label>
								</div>
								<%-- <%} %> --%>
							</div>
						</div>
								...
								<table class="table table-bordered" id="filterbysearch">
									...
									for(int i=0; i<VCOUNTERPARTY_CD.size(); i++){
										...
										<tr>
											...
											<%if(write_access.equals("Y") && !security_type.equals("OA") && !security_type.equals("ADV")){ %>
												<font title="Click to Edit" <%if(VEXP_VAL.elementAt(i).equals("Y") || VSTATUS.elementAt(i).equals("R") || VSTATUS.elementAt(i).equals("C")){%><%}else{ %>style="color:var(--header_color)"<%} %>>
													...
													<i <%if(VEXP_VAL.elementAt(i).equals("Y")|| VSTATUS.elementAt(i).equals("R") || VSTATUS.elementAt(i).equals("C")){%>class="fa fa-eye fa-lg" <%}else{ %>	class="fa fa-edit fa-lg"<%} %>
														...
														setDealNoDtl('<%=VSEL_DEAL_DTL.elementAt(i) %>','<%=VSHARE_PERCENT.elementAt(i) %>'); showDealNo();">
													</i>
											...
											<td align="center"><%=owner_cd+"-"+VSEC_REF_NO.elementAt(i) %></td>
											<td align="center">
											...
											<td align="center"><%=VDIS_CONTRACT_TYPE.elementAt(i) %></td>
				    			...
				    			<div class="col-sm-12 col-xs-12 col-md-12">
				      				<select class="form-select form-select-sm" name="issuing_bankName">
				      					...
				      					<%for(int i=0; i<VMST_BANK_CD.size(); i++){ %>
				      						<option value="<%=VMST_BANK_CD.elementAt(i)%>"><%=VMST_BANK_NM.elementAt(i)%></option>
				      					<%} %>
				      				</select>
				    			</div>
				  			</div>
      		...
      		<div class="modal-body mdbody">
      			<div class="cdbody">
      				...
      				<div class="row m-b-5">
						...
						<div class="col-sm-2 col-xs-2 col-md-2">  
							<div class="form-group row">
				    			<label class="form-label"><b> Guarantor Name<%if(VPO_GUARANTOR_CD.size()>0){ %><span class="s-red">*</span><%} %></b></label>
				  			</div>
						</div>
						<div class="col-sm-4 col-xs-4 col-md-4">  
							<div class="form-group row">
        			...
        			<div>
	        			<table class="table table-bordered" >
	        				...
	        				<tbody>
	        					...
	        					<tr id="table_tr_<%=i%>">
		        					<td valign="top" align="center" style="display:none;" id="new_column_deal_<%=VCONTRACT_DTL.elementAt(i)%>">
		        						<input type="checkbox" class="form-check-input" name="new_chk_deal" value="<%=VCONTRACT_DTL.elementAt(i)%>" onclick="enableSplit(this,'new_split_value_<%=VCONTRACT_DTL.elementAt(i)%>');">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		        					...
		        					<td align="center">
	        							<span
	    									...
	    									<%}else if(V_BUY_SELL.elementAt(i).equals("Sell")){ %>
	    										class="alert alert-primary"
	    									<%}%>><b><%=V_BUY_SELL.elementAt(i)%></b>
    									</span>
	        						</td>
		        					<td align="center">
	        							<%=VCONTRACT_TYPE_NAME.elementAt(i) %>
	        						</td>
		        					<td align="center">
		        						<%=VNEW_COUNTPTY_ABBR.elementAt(i)%>-<%=V_DEAL_NO.elementAt(i)%> <label style="color:blue;"> (<%=VCONT_REF.elementAt(i) %>)</label>
		        					</td>
		        					<td align="center">
		        						<%=V_DURATION.elementAt(i) %>
		        					</td>
		        					<td align="center">
        				...
        				<tbody>
		        			...
		        			{ 
			        			...
			        			String[] cont_typ_nm = temp_cont_type_name.split(", ");
			     				...
			     				{
			     				%>
		        				<tr id="table_tr_lc_<%=i%>">
		        					<td style="display:none;" align="center" id="column_deal_<%=dealDtl[j]%>">
		       							<input type="checkbox" class="form-check-input"  name="chk_deal" value="<%=dealDtl[j]%>" onclick="enableSplit(this,'split_value_<%=dealDtl[j]%>');">
		        					...
		        					<td align="center">
		        						<%=cpAbbr[j]%>-<%=dealNo[j]%> <label style="color:blue;"> (<%=cont_ref[j] %>)</label>
		        					</td>
		        					<td align="center">
		        						<%=duration[j] %>
		        					</td>
        			...
        			<table class="table table-bordered" >
        				...
        				<tbody>
		        			...
		        			{ 
		        				...
		        				<% for(int j=0; j<dealDtl.length; j++)
			        			{%>
		        				<tr id="table_tr_pcg_<%=i%>">
		        					<td style="display:none;" align="center" id="pcg_column_deal_<%=dealDtl[j]%>">
		        						<input type="checkbox" class="form-check-input" name="pcg_chk_deal" value="<%=dealDtl[j]%>" onclick="enableSplit(this,'pcg_split_value_<%=dealDtl[j]%>');">
		        						<%--HP20230914 &nbsp;<%=VCOUNTERPARTY_ABBR.elementAt(j)%>-<%=dealNo[j]%>&nbsp;&nbsp;&nbsp;&nbsp; --%>
...
<input type="hidden" name="pcg_count" value="<%=VPO_GUARANTOR_CD.size()%>">
