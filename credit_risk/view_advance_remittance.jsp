...
<jsp:useBean class="com.etrm.fms.credit_risk.DataBean_Advance" id="cr_inv" scope="request"></jsp:useBean>
<%
...
%>
<body <%if(contract_type.equals("O") || contract_type.equals("Q")) {%>onload="setInvoiceNo();"<%} %>>
			...
			<div align="center">
				...
				<font face="Arial" style="font-size: 14px;font-weight: bold;">	
					<%if(contract_type.equals("O") || contract_type.equals("Q")) {%>
						...
						<%} %>
					<%}else{ %>
						<%-- <%if(isReversal.equals("N")){ %>ADVANCE <%} else {%>ADVANCE REVERSAL<%}%>  --%>
						<%if(cr_dr.equals("CR")){ %>ADVANCE <%} else {%>ADVANCE REVERSAL<%}%> 
					<%} %>
				</font>
		...
		<td colspan="7">
			<div align="center">
				...
				<%}else if(contract_type.equals("D") || contract_type.equals("T") || contract_type.equals("G") || contract_type.equals("P")){ %>
					...
					<br>
					between <%=counterparty_nm%> and <%=owner_nm%>
				<%}else{ %>
					In respect of 
					<%if(contract_type.equals("S")){ %>Supply Notice (<%=contRef%>) executed on <%=signing_dt%> pursuant to Framework Gas Sales Agreement executed on <%=agmtSigningDt%>
					<%}else if(contract_type.equals("O")){ %><%--  pursuant to Framework --%> LTCORA Agreement executed on <%=agmtSigningDt%> & CN (<%=contRef%>) executed on <%=signing_dt%>
					...
					<%}else if(contract_type.equals("L")){ %>Letter of Agreement (<%=contRef%>) executed on <%=signing_dt%> 
					<%}else if(contract_type.equals("X")){ %>Exchange Transaction (<%=contRef%>) executed on <%=signing_dt%>
					<%} %> 
					<br>
					between <%=owner_nm%> and <%=counterparty_nm%>
				<%} %>	
				</font>
			</div>
		</td>
		...
		<td colspan="3" width="40%">
			<div align="left">
				<font size="1.5px" face="Arial">
					...
					<b>To:</b>
					<br><%if(gx.equals("I")){%><%=gx_counterparty_nm%><%}else{%><%=counterparty_nm%><%} %>
...
<table width="100%"  border="0" align="center" cellpadding="2" cellspacing="0">	
	<tr valign="middle">
		...
		<td colspan="2" width="25%">
			<div align="center">
				<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0">
					<tr><td><div align="right"><font size="1.5px" face="Arial"><b><%if(cr_dr.equals("DR")) {%>Refund<%}else{%>Receipet<%} %> Voucher Date :&nbsp;</b></font></div></td></tr>
					<tr><td><div align="right"><font size="1.5px" face="Arial"><b><%=owner_abbr%> <%if(cr_dr.equals("DR")) {%>Refund<%}else{%>Receipet<%} %> Voucher Seq No.:&nbsp;</b></font></div></td></tr>
				</table>
			</div>
		</td>
		<td colspan="1" width="15%">
			<div align="center">
				<table width="100%"  border="1" align="center" cellpadding="0" cellspacing="0">
					<tr><td><div align="right"><font size="1.5px" face="Arial"><b>&nbsp;<%=received_dt%></b></font></div></td></tr>
					<tr>
						<td>
						<%if(status.equals("O")){ %>
		...
		<td colspan="7">
			<div align="center">
				<%if(status.equals("O")){ %>
					...
					onclick="printPDF(this,'<%=counterparty_cd%>','<%=seq_no%>', '<%=seq_rev_no%>', '<%=gx%>','<%=isReversal%>','<%=contract_type%>');">&nbsp;<b>Yes</b>&nbsp;&nbsp;&nbsp;&nbsp;
				<%}else {%>
...
<input type="hidden" name="crdr" value="<%=cr_dr%>">
</form>
</body>
</html>