...
{ 	
	...
	else
	{
		...
		document.forms[0].cancel_flg.value="";
	}
...
%>
<body onload="<%if(!msg.equals("")){%>refreshParent();<%}%>">
							...
							<table class="table table-bordered table-hover" id="example">
								...
								<%if(VFROM_DT.size()>0) {%>
									...
									<tr>
										<td align="center">
											<input type="radio" name="rd" 
												...
												onclick="doModify('<%=VSEQ_NO.elementAt(i)%>','<%=VFROM_DT.elementAt(i)%>','<%=VTO_DT.elementAt(i)%>',
													'<%=VREMARK.elementAt(i)%>','<%=VSTATUS.elementAt(i)%>','<%=VVALUE.elementAt(i)%>','<%=VCURRENCY.elementAt(i)%>');">&nbsp;&nbsp;
											<%=i+1%>
										</td>
										<td align="center">
										...
										<td align="right"><%=VCURRENCY_NM.elementAt(i)%></td>																	
										<td align="center">
											<input type="hidden" name="line_status" id="line_status<%=i%>" value="<%=VSTATUS.elementAt(i)%>" >
												<span class="
								...
								<%}else{ %>
									<tr>
										<td colspan="7" align="center"><%=utilmsg.infoMessage("<b>No Credit Exceed Days Configured for the Contract!</b>")%></td>
									</tr>
								<%} %>
								</tbody>
							</table>
