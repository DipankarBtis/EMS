...
function refresh(index)
{
	...
	if(path!="")
	{
		var url = "frm_sf_file_download.jsp?&u="+u+"&path="+path+"&dwn_folder="+dwn_folder+"&from_dt="+from_dt+"&to_dt="+to_dt;
...
function refresh1()
{
	...
	var dwn_folder = document.forms[0].dwn_folder.value;
	var prev_dwn_folder = document.forms[0].prev_dwn_folder.value;
	...
	if(dwn_folder != prev_dwn_folder)
	{
		var url = "frm_sf_file_download.jsp?&u="+u+"&dwn_folder="+dwn_folder+"&from_dt="+from_dt+"&to_dt="+to_dt;
	}
	else
	{
		var url = "frm_sf_file_download.jsp?&u="+u+"&dwn_folder="+dwn_folder+"&from_dt="+from_dt+"&to_dt="+to_dt;
...
function goBack()
{
	...
	var url = "frm_sf_file_download.jsp?path="+path_sub_sub+"&u="+u+"&dwn_folder="+dwn_folder;
...
String dwn_folder = request.getParameter("dwn_folder")==null?"SF_INB_XML":request.getParameter("dwn_folder");
//String path_sub_sub = request.getParameter("path_sub_sub")==null?"":request.getParameter("path_sub_sub");
//String path_sub = "";
/* if(dwn_folder.equals("SF_INB_XML"))
{
	path_sub = ""+CommonVariable.work_dir+"/"+"SF_XML"+"/"+"INBOUND"+"/"+path;
	
	dwn_path = CommonVariable.work_dir+"/"+"SF_XML"+"/"+"INBOUND"+"/"+path;
}
else if(dwn_folder.equals("SF_OUB_XML"))
{
	path_sub = ""+CommonVariable.work_dir+owner_cd+"/"+"SF_XML"+"/"+"OUTBOUND/"+path;
	
	dwn_path = CommonVariable.work_dir+owner_cd+"/"+"SF_XML"+"/"+"OUTBOUND/"+path;
}
 */
...
String forRealPath[] = path_sub.split("work_data");

String dwn_path="work_data"+forRealPath[1];
String file_path = request.getRealPath("work_data"+forRealPath[1]);
%>
<body>
							...
							<div class="form-group row">
								<div class="col-sm-12 col-xs-12 col-md-12" align="left">
									<div class="btn-group">
										...
										<%if(!inner_path.equals("")) {%>style="pointer-events: none;"<%} %>>
											<option value="SF_INB_XML">SF Inbound XML</option>
											<option value="SF_OUB_XML">SF Outbound XML</option>
										</select>
										<%if(!inner_path.equals("")) {%>
										<span class="btn btn-outline-secondary subbtngrp" title="Back" onclick="refresh1(document.forms[0].inner_path.value='');">
						...
						<div class="col">
							<div class="form-group row">
								<div class="col-sm-12 col-xs-12 col-md-12" align="right">
									<span class="btn rounded-circle" title="Back" onclick="refresh1(document.forms[0].inner_path.value='');">
									  &nbsp;<i class="fa fa-step-backward fa-2x"></i>&nbsp;
									</span>
									 <!-- style="background:var(--header_color);color:var(--header_font_color);" -->
								</div>
							</div>
						</div>
					<%} %> --%>
...
<input type="hidden" name="prev_dwn_folder" value="<%=dwn_folder%>">
