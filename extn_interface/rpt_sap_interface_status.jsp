		...
		<div class="col-md-12 col-sm-12 col-xs-12">
			<div class="card cardmain">
				...
				for(int i=0; i<VMTM_SAP_INTERFACE_DISPLAY.size();i++){%>
					...
					%>
						<div class="accordion-item accor_item">
							<h2 class="accordion-header" id="heading_mtm">
 								<button name="mtm_sub_module_cd" class="accordion-button collapsed accor-btn" type="button" data-bs-toggle="collapse" data-bs-target="#collapse_mtm<%=ctn+1 %>" aria-expanded="false" aria-controls="collapse_mtm<%=ctn+1%>"><%=VMTM_TITLE.elementAt(ctn) %>&nbsp;<font color="blue">(<%=VMTM_INDEX.elementAt(ctn++) %>)</font></button>	
					    	</h2>
					    	<div id="collapse_mtm<%=ctn%>" class="accordion-collapse collapse" aria-labelledby="heading_mtm">
					    		<div class="accordion-body accor-body">
									<div class="row">
										<div class="table-responsive">
											<table class="table table-bordered" id="mtm_filter<%=ctn%>">
												<thead id="mtm_tbsearch<%=ctn%>">
													<tr>
														<th align="center">Sr#</th>
														<th align="center">Doc Num</th><!-- FMSngRefNum -->
														<th align="center">Deal Num</th><!-- FMS Contract# -->
...
$(document).ready(function() {
	...
	$('#filter'+j+' #tbsearch'+j+' th').each(function(i){
		...
		{
			$(this).html(title+'<div align="center"><input type="text" class="form-control form-control-sm" id="table_'+title+j+'" onkeyup="SearchAct(this,'+i+','+j+');" placeholder="Search '+title+'" style="width:100px"/></div>');
		}
	});
...
$(document).ready(function() {
	...
	$('#acc_filter'+j+' #acc_tbsearch'+j+' th').each(function(i){
		...
		{
			$(this).html(title+'<div align="center"><input type="text" class="form-control form-control-sm" id="acc_table_'+title+j+'" onkeyup="SearchAcc(this,'+i+','+j+');" placeholder="Search '+title+'" style="width:100px"/></div>');
		}
	});
...
$(document).ready(function() {
	...
	for(j=1;j<=size-1;j++)
	{
	$('#mtm_filter'+j+' #mtm_tbsearch'+j+' th').each(function(i){
		//alert(i)
		var title = $(this).text();
		if(title == "")
		{
		...
		{
			//$(this).html(title+'<div align="center"><input type="text" class="form-control form-control-sm" id="mtm_table_'+title+'" onkeyup="Search(this,'+i+');" placeholder="Search '+title+'" style="width:40px"/></div>');
		}
		else
		{
			$(this).html(title+'<div align="center"><input type="text" class="form-control form-control-sm" id="mtm_table_'+title+j+'" onkeyup="SearchMtm(this,'+i+','+j+');" placeholder="Search '+title+'" style="width:100px"/></div>');
		}
	});
