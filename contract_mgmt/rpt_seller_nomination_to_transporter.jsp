...
function refresh_rpt(msg,msg_type)
{
	...
	var url = "rpt_seller_nomination_to_transporter.jsp?gas_dt="+gas_dt+"&msg="+msg+"&msg_type="+msg_type+"&u="+u;
...
var newWindow;
function viewSellerNomiToTrans(index,COUNTERPARTY_CD,PLANT_SEQ,file,PDF_NAME,contact_person)
{
...
function printAllPDF()
{
	...
	if(selCnt>0)
	{
		url ="rpt_view_seller_nomination_to_transporter.jsp?gas_dt="+gas_dt+"&counterparty_cd="+all_counterparty_cd+"&plant_seq="+all_plant_seq+	
...
function sendAllMail()
{
	...
	if(selCnt>0)
	{
		...
		if(a)
		{
			url ="frm_seller_nom_to_trans_send_mail.jsp?gas_dt="+gas_dt+"&counterparty_cd="+all_counterparty_cd+"&plant_seq="+all_plant_seq+	
			"&contact_person_cd="+all_contact_person_cd+"&file="+file+"&u="+u;
														...
														if(index > 0){ %>
															...
															%>
																<tr>
																	...
																	<td><%=VTRANSPORTER_PLANT_ABBR.elementAt(j)%>
																	...
																	<td align="center">
																		...
																		onclick="viewSellerNomiToTrans('<%=j%>','<%=trans_cd%>','<%=VTRANSPORTER_PLANT_SEQ.elementAt(j)%>','HTML','<%=VPDF_NAME.elementAt(j)%>',document.getElementById('contact_person_cd<%=i%><%=j%>'))"></i>
																	</td>
																	<td align="center">
																	<%if(print_access.equals("Y")){ %>
																		<i class="fa fa-file-excel-o fa-2x excel_icon" 
																		onclick="viewSellerNomiToTrans('<%=j%>','<%=trans_cd%>','<%=VTRANSPORTER_PLANT_SEQ.elementAt(j)%>','XLS','<%=VPDF_NAME.elementAt(j)%>',document.getElementById('contact_person_cd<%=i%><%=j%>'))"></i>
																	<%}else{ %>
																	...
																	<td align="center">
																		<%if(print_access.equals("Y")){ %>
																			...
																			onclick="viewSellerNomiToTrans('<%=j%>','<%=trans_cd%>','<%=VTRANSPORTER_PLANT_SEQ.elementAt(j)%>','PDF','<%=VPDF_NAME.elementAt(j)%>',document.getElementById('contact_person_cd<%=i%><%=j%>'))"></i>
																		<%}else{ %>
																	...
																	<td align="center">
																		...
																		<i class="fa fa-envelope-o fa-2x mail_icon" style="opacity: .65; color: green;" title="Mail already Sent!" onclick="viewSellerNomiToTrans('<%=j%>','<%=trans_cd%>','<%=VTRANSPORTER_PLANT_SEQ.elementAt(j)%>','MAIL','<%=VPDF_NAME.elementAt(j)%>',document.getElementById('contact_person_cd<%=i%><%=j%>'));"></i>
																		<%}else if(VPDF_EXISTS.elementAt(j).equals("Y") && execute_access.equals("Y")){ %>
																		<i class="fa fa-envelope-o fa-2x mail_icon" onclick="viewSellerNomiToTrans('<%=j%>','<%=trans_cd%>','<%=VTRANSPORTER_PLANT_SEQ.elementAt(j)%>','MAIL','<%=VPDF_NAME.elementAt(j)%>',document.getElementById('contact_person_cd<%=i%><%=j%>'))"></i>
																		<%}else{ %>
