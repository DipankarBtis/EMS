...
function refresh(active_status)
{
	...
	var url = "../contract_mgmt/frm_nomination_contract_list.jsp?active_status="+active_status+"&gas_dt="+gas_dt+"&nomination_freq="+nomination_freq+"&nomination_type="+nomination_type;
...
function refresh_date()
{
	...
	if(flag && flg)
	{
		if(trim(from_dt)!="" && trim(to_dt)!="")
		{
			...
			{
			var url = "../contract_mgmt/frm_nomination_contract_list.jsp?active_status="+active_status+"&gas_dt="+gas_dt+"&nomination_freq="+nomination_freq+
...
Vector VBOE_NO = contract.getVBOE_NO();
%>
<body>
<form method="post">
<div class="box-body">
								...
								<%if(VBUYER_CD.size() > 0){ %>
									...
									<tr>
										<td align="center">
											...
											else if(VBOE_NO.elementAt(i).equals("")){%> disabled title="BOE# not Updated For the Cargo!" <%} %>
											
											onclick="setValue('<%=VBUYER_CD.elementAt(i) %>','<%=VAGMT_NO.elementAt(i)%>',
												'<%=VAGMT_REV_NO.elementAt(i)%>','<%=VCONT_NO.elementAt(i)%>',
												'<%=VCONT_REV_NO.elementAt(i)%>','<%=VSTART_DT.elementAt(i)%>','<%=VEND_DT.elementAt(i)%>','<%=VCONTRACT_TYPE.elementAt(i)%>','<%=VCARGO_NO.elementAt(i)%>')">
										</td>
