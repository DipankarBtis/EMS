		...
		if(trim(from_dt)!="" && trim(to_dt)!="")
	   	{
			...
			if(value!=1)
			{
				var url = "rpt_allocation_to_customer.jsp?counterparty_cd="+counterparty_cd+
						"&u="+u+"&from_dt="+from_dt+"&to_dt="+to_dt+"&sel_bu_plant="+sel_bu; //RG20250922
...
function exportToXls()
{
	...
	var url = "xls_allocation_to_customer.jsp?counterparty_cd="+counterparty_cd+"&from_dt="+from_dt+"&to_dt="+to_dt+"&sel_bu_plant="+sel_bu+"&sel_bu_plant_abbr="+sel_bu_plant_abbr; //RG20250922
...
Vector VBU_PLANT_ABBR=cont_mgmt.getVBU_PLANT_ABBR();//RG20250922
Vector VBU_PLANT_SEQ_NO=cont_mgmt.getVBU_PLANT_SEQ();//RG20250922
%>
<body>
...
<!-- 						<div class="col-sm-3 col-xs-3 col-md-3"></div> --><!-- RG20250924 commented due to alignment issues  -->
						...
						<div class="col-sm-3 col-xs-3 col-md-3">  
							<div class="form-group row">
								...
								<div class="col">
									<select class="form-select form-select-sm" name="sel_bu" onchange="">
									<option value="0">--All--</option>
				...
				<%if(VCOUNTERPARTY_CD.size()>0){ %>
					...
					%>
						<div class="row m-b-5">
							<label class="form-label subheader"><i class="fa fa-snowflake-o"></i> <%=VSEGMENT.elementAt(a) %></label>
						</div>
							...
							%>
								<div class="accordion">
									<div class="accordion-item accor_item">
										<h2 class="accordion-header" id="heading<%=a %>_<%=i%>">
											<button class="accordion-button collapsed accor-btn" type="button" data-bs-toggle="collapse" data-bs-target="#collapse<%=a %>_<%=i%>" aria-expanded="false" aria-controls="collapse<%=a %>_<%=i%>">
								    			<%=VCOUNTERPARTY_ABBR.elementAt(i)%>&nbsp;-&nbsp;<%=VCOUNTERPARTY_NM.elementAt(i)%>
								      		</button>
								    	</h2>
								    	<div id="collapse<%=a %>_<%=i%>" class="accordion-collapse collapse" aria-labelledby="heading<%=a %>_<%=i%>">
