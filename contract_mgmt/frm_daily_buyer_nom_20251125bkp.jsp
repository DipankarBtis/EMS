...
function refresh()
{
	...
	var url = "frm_daily_buyer_nom.jsp?gas_dt="+gas_dt+"&u="+u;
	...
	<div class="row">
		<div class="col-md-12 col-sm-12 col-xs-12">
			...
			<div class="card cardmain">
							...
							<div class="form-group row">
								<div class="col-auto">
									<div class="form-check form-switch">
										<input class="form-check-input" name="active" type="checkbox" role="switch" id="flexSwitchCheckChecked" checked onclick="setNomCutoffTime(this);">
									  	<label class="form-check-label" for="flexSwitchCheckChecked" id="lb">
									  		<b>Nomination Cut-off Time</b>
									  	</label>
									  	<input type="hidden" name="cutoff_time_flag" id="cutoff_time_flag" value="Y">
									</div>
								</div>
							</div>
						</div> -->
										...
										<h2 class="accordion-header" id="heading<%=l%>">
    										<button class="accordion-button collapsed accor-btn" type="button" data-bs-toggle="collapse" data-bs-target="#collapse<%=l%>" aria-expanded="false" aria-controls="collapse<%=l%>">
								    			...
								    			<%if(Double.parseDouble(""+VTRANS_PLANT_WISE_TOTAL_MMBTU.elementAt(j)) > 0){%>
								    				<font color="green" style="background: white;padding: 2px 5px 4px 5px;border-radius: 30px;">
																	...
																	%>
																		<tr>
																			<td align="center" valign="middle" <%if(VNOM_BLOCK.elementAt(l).equals("Y")) {%>style="background: #df9fbf;" title="Invoice Generated!"<%} %>>
																				...
																				onclick="setEnableDisabled(this,'<%=l%>');calculateSCM('<%=j%>','<%=l%>');totalQty();"
																				...
																				<tr>
																						...
																						<div style="width:150px;">
																							<div class="row m-b-5">
																								<div class="col-auto">
																									...
																									onclick="setSubEnableDisabled(this,'<%=l%>','<%=q%>');calculateSCM('<%=j%>','<%=l%>');totalSubQty('<%=l%>');checkQty('<%=l%>');checkTCQ('<%=l%>');totalQty();" disabled>
																					...
																					<td align="center">
																						<div style="width:100px;">
																							...
																							onblur="negNumber(this);checkNumber1(this,9,2);calculateSCM('<%=j%>','<%=l%>');totalSubQty('<%=l%>');checkQty('<%=l%>');checkTCQ('<%=l%>');totalQty();" disabled>
																						</div>
																					</td>
																					<td align="center">
																						<div style="width:100px;">
				...
				<div class="card-body cdbody">
					<div class="row m-b-5">
						<div align="center"><%=utilmsg.infoMessage("<b>No Contract is Active or Approved for the Selected Gas Day!</b>") %></div>
					</div>
				</div>
				<%} %>
			</div>
