...
contract_mgmt.setCallFlag("ALLOCATION_TO_CUSTOMER_PLANT");
contract_mgmt.setComp_cd(owner_cd);
contract_mgmt.setFrom_dt(from_dt);
contract_mgmt.setTo_dt(to_dt);
contract_mgmt.setCounterparty_cd(counterparty_cd);
contract_mgmt.setBu(bu_seq);
contract_mgmt.setBu_plant(bu_plant);
contract_mgmt.setAgmt_no(agmt_no);
contract_mgmt.setCont_no(cont_no);
contract_mgmt.setCont_rev(cont_rev);
contract_mgmt.setCont_type(cont_type);
contract_mgmt.setDeal_no(deal_no);
contract_mgmt.setCargo_no(cargo_no);
contract_mgmt.init();

Vector VMST_COUNTERPARTY_CD = contract_mgmt.getVMST_COUNTERPARTY_CD();
Vector VMST_COUNTERPARTY_NM = contract_mgmt.getVMST_COUNTERPARTY_NM();
Vector VMST_COUNTERPARTY_ABBR = contract_mgmt.getVMST_COUNTERPARTY_ABBR();
Vector VBU_PLANT_ABBR=contract_mgmt.getVBU_PLANT_ABBR();
Vector VBU_PLANT_SEQ=contract_mgmt.getVBU_PLANT_SEQ();
...
Vector VCOUNTERPARTY_PLANT_SEQ = contract_mgmt.getVCOUNTERPARTY_PLANT_SEQ();
Vector VCOUNTERPARTY_PLANT_NM = contract_mgmt.getVCOUNTERPARTY_PLANT_NM();

Vector VINDEX = contract_mgmt.getVINDEX();
Vector VCONT_NO = contract_mgmt.getVCONT_NO();
Vector VGAS_DT = contract_mgmt.getVGAS_DT();
Vector VQTY_MMBTU = contract_mgmt.getVQTY_MMBTU();
Vector VQTY_SCM = contract_mgmt.getVQTY_SCM();
Vector VTOTAL_QTY_MMBTU = contract_mgmt.getVTOTAL_QTY_MMBTU();
Vector VTOTAL_QTY_SCM = contract_mgmt.getVTOTAL_QTY_SCM();
Vector VPLANT_NM = contract_mgmt.getVPLANT_NM();
Vector VPLANT_SEQ_NO = contract_mgmt.getVPLANT_SEQ_NO();
Vector VCONTRACT_TYPE = contract_mgmt.getVCONTRACT_TYPE();
Vector VCONT_REV = contract_mgmt.getVCONT_REV();
Vector VAGMT_NO = contract_mgmt.getVAGMT_NO();
Vector VAGMT_REV = contract_mgmt.getVAGMT_REV();
Vector VCARGO_NO = contract_mgmt.getVCARGO_NO();
Vector VCOLOR = contract_mgmt.getVCOLOR();
Vector VAGMT_BASE = contract_mgmt.getVAGMT_BASE();

String deal_map = contract_mgmt.getDeal_map();
String cont_ref_no = contract_mgmt.getCont_ref_no();
String agmt_base = contract_mgmt.getAgmt_base();
int temp_count = contract_mgmt.getTemp_count();
...
if(VMST_COUNTERPARTY_CD.size()>0)
{
	...
	{ 
		...
		if(deal_no.equals("0"))
		{
			...
			sheetName = VCOUNTERPARTY_ABBR.elementAt(i)+"-"+"("+VDIS_CONT_MAPPING.elementAt(i)+"-["+VCONT_REF.elementAt(i)+"])"+base_agmt;
		}
		else
		{
			...
			sheetName = VCOUNTERPARTY_ABBR.elementAt(i)+"-"+"("+deal_map+"-["+cont_ref_no+"])"+agmt_base;
		}
	    ...
	    cell0.setCellValue("Customer Allocation (By Plant And Contract) From "+from_dt+" To "+to_dt);
	    ...
	    if(deal_no.equals("0"))
		{
	    	...
	    	cont_map = VCOUNTERPARTY_ABBR.elementAt(i)+"-"+VCOUNTERPARTY_NM.elementAt(i)+"-"+"("+VDIS_CONT_MAPPING.elementAt(i)+"-["+VCONT_REF.elementAt(i)+"])"+base_agmt;
		}
		else
		{
			if(agmt_base.equals("D"))
			...
			cont_map = VCOUNTERPARTY_ABBR.elementAt(i)+"-"+VCOUNTERPARTY_NM.elementAt(i)+"-"+"("+deal_map+"-["+cont_ref_no+"])"+agmt_base;
		}
