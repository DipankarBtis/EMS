...
function addrow()
{
	//Converting VEMP_CD Vector into JS Array
	var vemp_cd = document.forms[0].vemp_cd.value;
	vemp_cd = vemp_cd.substring(1,vemp_cd.length-1);
	vemp_cd = vemp_cd.split(",");
	//Converting VEMP_NM into JS Array
	var vemp_nm = document.forms[0].vemp_nm.value;
	vemp_nm = vemp_nm.substring(1,vemp_nm.length-1);
	vemp_nm = vemp_nm.split(",");
	...
	//Select element for selecting group owner.
	var td05 = document.createElement("td");
	td05.className="text-center"
	var div05 = document.createElement("DIV");
	div05.align='center';
	var input05 = document.createElement("select")
	input05.setAttribute("class","form-select form-select-sm");
	input05.setAttribute("name","group_owner");
	input05.setAttribute("id","group_owner"+new_seq_no);
	var option1 = document.createElement("option");
	option1.setAttribute("value",0);
	option1.innerHTML = "--Select--";
	input05.appendChild(option1);
	for(let k=0;k<vemp_cd.length;k++)
	{
		var option2 = document.createElement("option");
		option2.setAttribute("value",vemp_cd[k]);
		option2.innerHTML = vemp_nm[k];
		input05.appendChild(option2);
	}
...
function doSubmit()
{
	...
	var group_owner = document.forms[0].group_owner;
...
Vector VGROUP_OWNER = dbadmin.getVGROUP_OWNER();
//NOTE: The Group owner list shows Avtive user only. 
%>
							...
							<table class="table table-bordered">
								...
								<%for(int i=0; i<VGROUP_CD.size(); i++){ %>
									<tr>
										...
										<td>
											<div align="center">
												<select class="form-select form-select-sm" name="group_owner" id="group_owner<%=VGROUP_CD.elementAt(i)%>">
						      						<option value="0"> -- Select -- </option>
						      						<%for(int e=0;e<VEMP_CD.size();e++){%>
						      							<option value="<%=VEMP_CD.elementAt(e)%>"><%=VEMP_NM.elementAt(e) %></option>
						      						<%}%>
<%-- 													<script>document.forms[0].group_owner[<%=i%>].value = '<%=VGROUP_OWNER.elementAt(i)%>';</script> --%>
													<script>document.getElementById("group_owner"+"<%=(i+2)%>").value = '<%=VGROUP_OWNER.elementAt(i)%>';</script>
												</select>
											</div>
										</td>
...
<input type="hidden" name="module_size" id="group_size" value="<%=VGROUP_CD.size()+1%>">
<input type="hidden" name="vemp_cd" id="vemp_cd" value="<%=VEMP_CD%>">
<input type="hidden" name="vemp_nm" id="vemp_nm" value="<%=VEMP_NM%>">
