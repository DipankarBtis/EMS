...
function refresh()
{
	...
	var url = "../dlng/frm_dlng_inv_chng_req_aprv.jsp?&u="+u+"&month="+month+"&year="+year;
	
	document.getElementById("loading").style.visibility = "visible";
	location.replace(url);
}

function doSubmit(action_flag,financial_year,bu_state_tin,invoice_seq,invoice_no,seq_no,invtype,accroid,invoice_type)
{
	var msg="";
	var flag=true;
...
dlng.setCallFlag("DLNG_INVOICE_CHANGE_REQUEST_APPROVAL");
dlng.setComp_cd(owner_cd);
dlng.setMonth(month);
dlng.setYear(year);
dlng.init();

Vector VINVOICE_LIST_ABBR = dlng.getVINVOICE_LIST_ABBR();
Vector VINVOICE_LIST_NAME = dlng.getVINVOICE_LIST_NAME();
Vector VINDEX = dlng.getVINDEX();

Vector VCOUNTERPTY_CD = dlng.getVCOUNTERPTY_CD();
Vector VCOUNTERPTY_ABBR = dlng.getVCOUNTERPTY_ABBR();
Vector VCOUNTERPTY_NM = dlng.getVCOUNTERPTY_NM();
Vector VDEAL_NO = dlng.getVDEAL_NO();
...
Vector VINVOICE_TYPE_NM = dlng.getVINVOICE_TYPE_NM();
...
Vector VACTION_FLAG = dlng.getVACTION_FLAG();
Vector VSEQ_NO = dlng.getVSEQ_NO();
Vector VREMAINING_DAYS = dlng.getVREMAINING_DAYS();
Vector VCOUNT = dlng.getVCOUNT();
Vector VOTH_INVOICE_TYPE = dlng.getVOTH_INVOICE_TYPE();
%>
<body>
			...
			<div class="card cardmain">
				<div class="card-header cdheader">
					<div class="d-flex justify-content-between">
					    <div class="topheader">
					    	DLNG Sales Invoice Change Request/Approval
					    </div>
					</div>
				</div>
														...
														<div class="table-responsive">
															...
															<thead id="tbsearch<%=j%>">
																<tr>
																	...
																	<th class="tbser<%=j%>" rowspan="2"><%if(heading.equals("LP_HEAD")) {%>Contract Period<%}else if(heading.equals("FFLOW_HEAD")){%>Billing Period<%}else{%>Gas Day<%} %></th>
															...
															<tbody>
																...
																if(index>0)
																{
																	...
																	<tr>
																		...
																		<td align="center">
																		<%if(heading.equals("FFLOW_HEAD") || heading.equals("LP_HEAD")) {%>
																		...
																		<td align="center">
																			...
																			onclick="doSubmit('R','<%=VFINANCIAL_YEAR.elementAt(i)%>','<%=VBU_STATE_TIN.elementAt(i)%>','<%=VINVOICE_SEQ.elementAt(i)%>','<%=VINVOICE_NO.elementAt(i)%>',
																			'<%=VSEQ_NO.elementAt(i)%>','<%=VINVOICE_TYPE.elementAt(i)%>','<%=heading%>','<%=VOTH_INVOICE_TYPE.elementAt(i)%>');">
																		</td>
																		<td align="center">
															   				...
															   				onclick="doSubmit('A','<%=VFINANCIAL_YEAR.elementAt(i)%>','<%=VBU_STATE_TIN.elementAt(i)%>','<%=VINVOICE_SEQ.elementAt(i)%>','<%=VINVOICE_NO.elementAt(i)%>',
															   				'<%=VSEQ_NO.elementAt(i)%>','<%=VINVOICE_TYPE.elementAt(i)%>','<%=heading%>','<%=VOTH_INVOICE_TYPE.elementAt(i)%>');">
															   			</td>
															   			<td align="center">	
															   				...
															   				onclick="doSubmit('X','<%=VFINANCIAL_YEAR.elementAt(i)%>','<%=VBU_STATE_TIN.elementAt(i)%>','<%=VINVOICE_SEQ.elementAt(i)%>','<%=VINVOICE_NO.elementAt(i)%>',
															   				'<%=VSEQ_NO.elementAt(i)%>','<%=VINVOICE_TYPE.elementAt(i)%>','<%=heading%>','<%=VOTH_INVOICE_TYPE.elementAt(i)%>');">
															   			</td>
																	</tr>
																	<%if(k==index){
...
<input type="hidden" name="option" value="DLNG_INVOICE_CHANGE_ACTION">
...
<input type="hidden" name="segment" value="DLNG">
