...
function doSubmit()
{
	...
	var crdr_type_nm = document.forms[0].crdr_type_nm.value;
	...
	if(flag)
	{
		...
		if(aaaa)
		{
			if(activityFlag=="CHECK")
			{
				...
				{
					activity_nm="Check"
					invNo+="Do you want to "+activity_nm+" "+crdr_type_nm+"?"
				}
		...
		<td colspan="7">
			<div align="center">
				<font size="1px" face="Arial">
					...
					In respect of 
					<%if(contract_type.equals("E")){ %>Supply Notice (<%=contRef%>) executed on <%=signingDt%> pursuant to Framework Gas Sales Agreement executed on <%=agmtSigningDt%>
					<%}else if(contract_type.equals("F")){ %>Letter of Agreement (<%=contRef%>) executed on <%=signingDt%> 
...
<input type="hidden" name="option" value="CRDR_INVOICE_APPROVAL">
...
<input type="hidden" name="crdr_type_nm" value="<%=NormalFont%>">
