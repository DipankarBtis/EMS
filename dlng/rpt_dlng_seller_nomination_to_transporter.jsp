...
function doSubmit(index,COUNTERPARTY_CD)
{
...
var newWindow;
function viewSellerNomiToTrans(index,COUNTERPARTY_CD,TRANS_CD,file,PDF_NAME,contact_person)
{
	...
	if(file=="XLS")
	{
		...
		url = "rpt_dlng_view_seller_nomination_to_transporter.jsp?gas_dt="+gas_dt+"&counterparty_cd="+COUNTERPARTY_CD+"&trans_cd="+TRANS_CD+	
	...
	else
	{
		if(trim(contact_person_cd)!="")
		{
			...
			if(file=="MAIL")
			{
				url = "frm_dlng_seller_nom_to_trans_send_mail.jsp?gas_dt="+gas_dt+"&counterparty_cd="+COUNTERPARTY_CD+"&trans_cd="+TRANS_CD+	
				"&contact_person_cd="+contact_person_cd+"&file="+file+"&u="+u;
...
function chkContactPerson()
{
	...
	//for(var k=0; k<parseInt(trans_cnt); k++)
	{
		...
		var contact_person_cd=document.getElementsByName('contact_person_cd');
		...
		if(chk!=undefined)
		{
			if(chk.length!=undefined)
			{
				for(var m=0;m<chk.length;m++)
				{
					...
					{ 
						if(trim(document.getElementById('contact_person_cd'+i).value) == "")
						{
...
function printAllPDF()
{
	...
	//for(var k=0; k<parseInt(trans_cnt); k++)
	{
		...
		var counterparty_cd=document.getElementsByName('counterpartyCd');
		//var truck_cd=document.getElementsByName('transCd'+k);
		var contact_person_cd=document.getElementsByName('contact_person_cd');
			...
			else
			{
				for(var m=0;m<chk.length;m++)
				{
					...
					{ 
						if(all_counterparty_cd=="")
				        {
							all_counterparty_cd=document.getElementById('counterpartyCd'+i).value;
							//all_truck_cd=document.getElementById('transCd'+k+i).value;
							all_contact_person_cd=document.getElementById('contact_person_cd'+i).value;
				        }
			          	else
			          	{
							all_counterparty_cd=all_counterparty_cd+"@@"+document.getElementById('counterpartyCd'+i).value;
							//all_truck_cd=all_truck_cd+"@@"+document.getElementById('transCd'+k+i).value;
							all_contact_person_cd=all_contact_person_cd+"@@"+document.getElementById('contact_person_cd'+i).value;
		          		}
						flag=true;
						selCnt = selCnt+1;
					}
	...
	if(selCnt>0)
	{
		url ="rpt_dlng_view_seller_nomination_to_transporter.jsp?gas_dt="+gas_dt+"&counterparty_cd="+all_counterparty_cd+	
...
function sendAllMail()
{
				...
				for(var m=0;m<chk.length;m++)
				{
					...
					{ 
						...
						if(all_counterparty_cd=="")
				        {
			          	...
			          	{
							all_counterparty_cd=all_counterparty_cd+"@@"+document.getElementById('counterpartyCd'+i).value;
							//all_truck_cd=all_plant_seq+"@@"+document.getElementById('transCd'+k+i).value;
							all_contact_person_cd=all_contact_person_cd+"@@"+document.getElementById('contact_person_cd'+i).value;
		          		}
						flag=true;
						selCnt = selCnt+1;
	...
	if(selCnt>0)
	{
		...
		if(a)
		{
			url ="frm_dlng_seller_nom_to_trans_send_mail.jsp?gas_dt="+gas_dt+"&counterparty_cd="+all_counterparty_cd+	
			"&contact_person_cd="+all_contact_person_cd+"&file="+file+"&u="+u;
								...
								<div class="row">
									<div class="table-responsive">
										<table class="table table-bordered table-hover">
											<thead>
												<tr>
													<th rowspan="2" valign="middle"><input type="checkbox" class="form-check-input" id="chk_all" onclick="checkAll(this,document.forms[0].chk,document.forms[0].contact_person_cd);"></th>
											...
											if(VTRANSPORTER_CD.size() > 0){ %>
												...
												%>
													<tr>
														<td align="center"><input type="checkbox" class="form-check-input" name="chk" id="chk<%=j%>" onclick="chkContactPerson(this,<%=j%>,document.forms[0].contact_person_cd)">
															<input type="hidden" name="counterpartyCd" id="counterpartyCd<%=j%>" value="<%=trans_cd%>">
														</td>
														<td align="center" title="<%=VTRANSPORTER_ABBR.elementAt(j)%>"><%=VTRANSPORTER_NM.elementAt(j)%>
															<input type="hidden" name="transCd" id="transCd<%=j%>" value="<%=VTRANSPORTER_CD.elementAt(j)%>">
														</td>
														<td align="center">
														...
														<td align="center">
															...
															onclick="viewSellerNomiToTrans('<%=j%>','<%=trans_cd%>','<%=VTRANSPORTER_CD.elementAt(j)%>','HTML','<%=VPDF_NAME.elementAt(j)%>',document.getElementById('contact_person_cd<%=j%>'))"></i>
														</td>
														<td align="center">
														<%if(print_access.equals("Y")){ %>
															<i class="fa fa-file-excel-o fa-2x excel_icon" 
															onclick="viewSellerNomiToTrans('<%=j%>','<%=trans_cd%>','<%=VTRANSPORTER_CD.elementAt(j)%>','XLS','<%=VPDF_NAME.elementAt(j)%>',document.getElementById('contact_person_cd<%=j%>'))"></i>
														<%}else{ %>
														...
														<td align="center">
															<%if(print_access.equals("Y")){ %>
																...
																onclick="viewSellerNomiToTrans('<%=j%>','<%=trans_cd%>','<%=VTRANSPORTER_CD.elementAt(j)%>','PDF','<%=VPDF_NAME.elementAt(j)%>',document.getElementById('contact_person_cd<%=j%>'))"></i>
															<%}else{ %>
														...
														<td align="center">
															...
															<i class="fa fa-envelope-o fa-2x mail_icon" style="opacity: .65; color: green;" title="Mail already Sent!" onclick="viewSellerNomiToTrans('<%=j%>','<%=trans_cd%>','<%=VTRANSPORTER_CD.elementAt(j)%>','MAIL','<%=VPDF_NAME.elementAt(j)%>',document.getElementById('contact_person_cd<%=j%>'));"></i>
															<%}else if(VPDF_EXISTS.elementAt(j).equals("Y") && execute_access.equals("Y")){ %>
															<i class="fa fa-envelope-o fa-2x mail_icon" onclick="viewSellerNomiToTrans('<%=j%>','<%=trans_cd%>','<%=VTRANSPORTER_CD.elementAt(j)%>','MAIL','<%=VPDF_NAME.elementAt(j)%>',document.getElementById('contact_person_cd<%=j%>'))"></i>
															<%}else{ %>
