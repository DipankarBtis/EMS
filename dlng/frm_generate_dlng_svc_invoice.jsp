...
function refresh()
{
	...
	var sell_cont_map = document.forms[0].sell_cont_map.value;
	var sell_cont_no = document.forms[0].sell_cont_no.value;
	var sell_cont_rev_no = document.forms[0].sell_cont_rev_no.value;
	var sell_contract_type = document.forms[0].sell_contract_type.value;
	var sell_agmt_no = document.forms[0].sell_agmt_no.value;
	var sell_agmt_rev_no = document.forms[0].sell_agmt_rev_no.value;
	var qtyMmbtu = document.forms[0].qtyMmbtu.value;
	var stateMapping = document.forms[0].stateMapping.value;
	
	var u = document.forms[0].u.value;
	
	var url = "frm_generate_dlng_svc_invoice.jsp?counterparty_cd="+counterparty_cd+"&contract_type="+contract_type+"&cont_no="+cont_no+
...
function doSubmit()
{
	...
	if(flag)
	{
		var a=confirm("Do you want to Submit Service Invoice?");
		if(a)
		{
			document.getElementById("loading").style.visibility = "visible";
...
function enableDisableTruckInv(obj,i_index)
{
	...
	var sell_invoice_seq = document.getElementById("sell_invoice_seq_"+i_index);
	var sell_invoice_no = document.getElementById("sell_invoice_no_"+i_index);
	var truck_trans_cd = document.getElementById("truck_trans_cd_"+i_index);
	var truck_cd = document.getElementById("truck_cd_"+i_index);
	var sell_mapping_id = document.getElementById("sell_mapping_id_"+i_index);
	var sell_period_start_dt = document.getElementById("sell_period_start_dt_"+i_index);
	var sell_period_end_dt = document.getElementById("sell_period_end_dt_"+i_index);
	var sales_inv_qty = document.getElementById("sales_inv_qty_"+i_index);
	
	if(obj.checked)
	{
		sel_checkbox_flag.disabled=false;
		sell_invoice_seq.disabled=false;
		sell_invoice_no.disabled=false;
		truck_trans_cd.disabled=false;
		truck_cd.disabled=false;
		sell_mapping_id.disabled=false;
		sell_period_start_dt.disabled=false;
		sell_period_end_dt.disabled=false;
		sales_inv_qty.disabled=false;
	}
	else
	{
		sel_checkbox_flag.disabled=true;
		sell_invoice_seq.disabled=true;
		sell_invoice_no.disabled=true;
		truck_trans_cd.disabled=true;
		truck_cd.disabled=true;
		sell_mapping_id.disabled=true;
		sell_period_end_dt.disabled=true;
...
{
	var sel_checkbox = document.forms[0].sel_checkbox;
	var salesInv_qty = document.forms[0].sales_inv_qty;
	...
	if(sel_checkbox.length != undefined)
	{
		for(var i=0; i<sel_checkbox.length;i++)
		{
			if(sel_checkbox[i].checked)
			{
				totalQty = totalQty + parseFloat(salesInv_qty[i].value);
			}
		}
	}
	else
	{
		if(sel_checkbox.checked)
		{
			totalQty = parseFloat(salesInv_qty.value);
		}
	}
...
dlng_inv.setCallFlag("DLNG_SVC_INVOICE_GENERATION");
...
dlng_inv.setSell_contract_type(sell_contract_type);
dlng_inv.setSell_agmt_rev_no(sell_agmt_rev_no);
dlng_inv.setSell_agmt_no(sell_agmt_no);
dlng_inv.setSell_cont_rev_no(sell_cont_rev_no);
dlng_inv.setSell_cont_no(sell_cont_no);
dlng_inv.init();
...
Vector VSALES_INV_LINKED = dlng_inv.getVSALES_INV_LINKED();
Vector VSALES_INVOICE_SEQ = dlng_inv.getVSALES_INVOICE_SEQ();
Vector VSALES_INVOICE_NO = dlng_inv.getVSALES_INVOICE_NO();
Vector VSALES_INV_DT = dlng_inv.getVSALES_INV_DT();
...
Vector VSALES_CONT_MAPP = dlng_inv.getVSALES_CONT_MAPP();
Vector VSELL_CONT_PERIOD_START_DT = dlng_inv.getVSELL_CONT_PERIOD_START_DT();
Vector VSELL_CONT_PERIOD_END_DT = dlng_inv.getVSELL_CONT_PERIOD_END_DT();
...
if(transport_mgmt_unit.equals("3"))//INR/Truck
{
	if(stateMapping.equals("") && operation.equals("INSERT"))
	{
		double totalQtyELInv=0;
		for(int w=0;w<VSALES_INVOICE_NO.size();w++)
		{
			if(VSALES_INV_LINKED.elementAt(w).equals("EL"))
			{
				totalQtyELInv++;
			}
		}
		
		qty_mmbtu = ""+(VSALES_INVOICE_NO.size() - totalQtyELInv);
	}
	else if(stateMapping.equals(""))
	{
		int linkedTruckInv=0;
		for(int w=0;w<VSALES_INVOICE_NO.size();w++)
		{
			if(stateMapping.equals("") && VSALES_INV_LINKED.elementAt(w).equals("Y") && operation.equals("MODIFY"))
			{
				linkedTruckInv++;
			}
		}
		qty_mmbtu = ""+linkedTruckInv;
	}
	else if(!stateMapping.equals(""))
	{
		String[] parts = stateMapping.split("-");
...
else if(transport_mgmt_unit.equals("4"))//INR/MT
{
	if(stateMapping.equals("") && operation.equals("INSERT"))
	{
		double totalQtyELInv=0;
		for(int w=0;w<VSALES_INVOICE_NO.size();w++)
		{
			if(VSALES_INV_LINKED.elementAt(w).equals("EL"))
			{
				totalQtyELInv = totalQtyELInv + Double.parseDouble(""+VSALES_INV_QTY.elementAt(w));
			}
		}
		
		qty_mmbtu = nf.format((Double.parseDouble(qtyMmbtu) - totalQtyELInv) / 51.5);
	}
	else if(stateMapping.equals(""))
	{
		double totalQtyTruckInv=0;
		for(int w=0;w<VSALES_INVOICE_NO.size();w++)
		{
			if(stateMapping.equals("") && VSALES_INV_LINKED.elementAt(w).equals("Y") && operation.equals("MODIFY"))
			{
				totalQtyTruckInv = totalQtyTruckInv + Double.parseDouble(""+VSALES_INV_QTY.elementAt(w));
			}
		}
		qty_mmbtu = nf.format(totalQtyTruckInv / 51.5);
	}
	else if(!stateMapping.equals(""))
	{
		String[] parts = stateMapping.split("-");
        ...
        for (String part : parts) 
        {
            if (part.contains("true")) 
            {
                trueCount = Integer.parseInt(part.replace("true", "")); 
                
                totalQtyTruckInv = totalQtyTruckInv + Double.parseDouble(""+VSALES_INV_QTY.elementAt(trueCount));
            }
        }
...
else
{
	if(stateMapping.equals("") && operation.equals("INSERT"))
	{
		...
		qty_mmbtu = ""+(Double.parseDouble(qtyMmbtu) - totalQtyELInv);
	}
	else if(stateMapping.equals(""))
	{
		double totalQtyTruckInv=0;
...
%>
<body onload="setCheckboxesState();totalInvQty();<%if(!msg.equals("")){ %>Do_close('<%=msg%>','<%=msg_type%>','<%=accroid%>');<%} %>"
						...
						<div class="accordion-item accor_item">
							<h2 class="accordion-header" id="heading">
 								<button class="accordion-button <%if(stateMapping.equals("") || (submission_chk && operation.equals("INSERT"))){%>collapsed<%}%> accor-btn" type="button" data-bs-toggle="collapse" data-bs-target="#collapse" aria-expanded="false" aria-controls="collapse">
					    			<font>DLNG Sales Invoice&nbsp;<font color="blue">(<%=total_truck_invoice%>)</font></font>
					      		</button>
					    	</h2>
											...
											<table class="table table-bordered table-hover" id="filterbysearch">
												<thead id="tbsearch0">
													<tr>
														...
														<th>Truck No</th>
														<th>Sales Contract</th><!--  No<br>[Contract Ref#] -->
														<!-- <th>Contract Period</th> -->
														<th>Invoice Date</th>
														<th>Invoice#</th>
														<th>Invoice Quanity (MMBTU)</th>
														<%if(transport_mgmt_unit.equals("5")){ %><th style="display:none">Distance</th><!-- INR/KM --><%} %>
													</tr>
												</thead>
												<tbody>
												<%if(VSALES_INVOICE_NO.size()>0){ %>
													<%for(int i=0; i<VSALES_INVOICE_NO.size(); i++){ %>
													<tr>
														<td align="center"
														...
														>
															<div align="center">
																...
																<input type="hidden" name="sell_invoice_seq" id="sell_invoice_seq_<%=i%>" value="<%=VSALES_INVOICE_SEQ.elementAt(i)%>">
																<input type="hidden" name="sell_invoice_no" id="sell_invoice_no_<%=i%>" value="<%=VSALES_INVOICE_NO.elementAt(i)%>">
																...
																<input type="hidden" name="sell_mapping_id" id="sell_mapping_id_<%=i%>" value="<%=VSALES_MAPPING_ID.elementAt(i)%>">
																<input type="hidden" name="sell_period_start_dt" id="sell_period_start_dt_<%=i%>" value="<%=VSELL_PERIOD_START_DT.elementAt(i)%>">
																<input type="hidden" name="sell_period_end_dt" id="sell_period_end_dt_<%=i%>" value="<%=VSELL_PERIOD_END_DT.elementAt(i)%>">
														...
														<td align="center"><%=VSALES_CONT_MAPP.elementAt(i)%></td>
														<%-- <td align="center"><%=VSELL_CONT_PERIOD_START_DT.elementAt(i)%> - <%=VSELL_CONT_PERIOD_END_DT.elementAt(i)%></td> --%>
														<td align="center"><%=VSALES_INV_DT.elementAt(i)%></td>
														<td align="center"><%=VSALES_INVOICE_NO.elementAt(i) %></td>
														<td align="center" align="right"><%=VSALES_INV_QTY.elementAt(i)%>
														</td>
														<%if(transport_mgmt_unit.equals("5")){ %>
														<td align="center" style="display:none">
															<div style="width:200px;">
																<div class="input-group input-group-sm" >
																	<input type="text" class="form-control form-control-sm" name="distance" value="" disabled>
																</div>
															</div>
														</td>
														<%} %>
												...
												<%}else{ %>
													<tr>
														<td align="center" colspan="8"><%=utilmsg.infoMessage("<b>No Invoice Generated for selected Service Contract!</b>") %></td>
													</tr>
												<%} %>
											...
											<td align="center">
												<div style="width:200px;">
													<div class="input-group input-group-sm" >
														...
														<span class="input-group-text"><%=transport_mgmt_unit_nm%></span>
													</div>
...
<input type="hidden" name="option" value="DLNG_SVC_INVOICE">
...
<input type="hidden" name="sell_cont_map" value="<%=sell_cont_map%>">
<input type="hidden" name="sell_cont_no" value="<%=sell_cont_no%>">
<input type="hidden" name="sell_cont_rev_no" value="<%=sell_cont_rev_no%>">
<input type="hidden" name="sell_contract_type" value="<%=sell_contract_type%>">
<input type="hidden" name="sell_agmt_no" value="<%=sell_agmt_no%>">
<input type="hidden" name="sell_agmt_rev_no" value="<%=sell_agmt_rev_no%>">
