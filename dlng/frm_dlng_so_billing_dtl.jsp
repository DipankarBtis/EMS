...
function showExchangerate()
{
	...
	if(inv_currency==2)
	{
		document.getElementById('exchnage_block').style.display="block";
		setExchgVal(document.forms[0].exchng_rate);
	}
	else
	{
		document.getElementById('exchnage_block').style.display="none";
	}
}
...
dlng.setCallFlag("SVC_CONTRACT_BILLING_DTL");
dlng.setComp_cd(owner_cd);
dlng.setCont_start_dt(start_dt);
...
dlng.setAgreement_type(agmt_type);
dlng.init();

Vector VEXCHNG_RATE_CD = dlng.getVEXCHNG_RATE_CD();
Vector VEXCHNG_RATE_NM = dlng.getVEXCHNG_RATE_NM();
Vector VINT_RATE_CD = dlng.getVINT_RATE_CD();
Vector VINT_RATE_NM = dlng.getVINT_RATE_NM();

Vector VPLANT_NAME = new Vector();//dlng.getVPLANT_NAME();
Vector VTAX_STRUCT_CD = new Vector();//dlng.getVTAX_STRUCT_CD();
Vector VTAX_STRUCT_NM = new Vector();//dlng.getVTAX_STRUCT_NM();
Vector VBU_PLANT_NM = new Vector();//dlng.getVBU_PLANT_NM();

Vector VINVOICE_TYPE = new Vector();//dlng.getVINVOICE_TYPE(); 
Vector VINVOICE_CATEGORY = new Vector();//dlng.getVINVOICE_CATEGORY();

Vector VSELECTED_PLANT_SEQ = dlng.getVSELECTED_PLANT_SEQ();
Vector VSELECTED_PLANT_ABBR = dlng.getVSELECTED_PLANT_ABBR();
Vector VSTATE_NM = dlng.getVSTATE_NM();
Vector VSTATE_CODE = dlng.getVSTATE_CODE();
Vector VPLANT_SEQ = dlng.getVPLANT_SEQ();

String contpty_abbr = dlng.getContpty_abbr();
String mapped_cont_no = dlng.getMapped_cont_no();
String billing_freq=dlng.getBilling_freq();
String billing_flag=dlng.getBilling_flag();
String due_date=dlng.getDue_date();
String sec_due_date=dlng.getSec_due_date();
String inv_currency=dlng.getInv_currency();
String payment_currency=dlng.getPayment_currency();
String interest_rate_cd=dlng.getInterest_rate_cd();
String interest_cal_sign=dlng.getInterest_cal_sign();
String interest_cal_per=dlng.getInterest_cal_per();
String exchng_note=dlng.getExchng_note();
String due_dt_in=dlng.getDue_dt_in();
String exclude_sat=dlng.getExclude_sat();
String exchng_rate_cd = dlng.getExchng_rate_cd();
String exchng_cal=dlng.getExchng_cal();
String exchng_criteria=dlng.getExchng_criteria();
String exchg_val=dlng.getExchg_val();
String billing_days = dlng.getBilling_days();
String eff_dt=dlng.getEff_dt();
String old_eff_dt = dlng.getOld_eff_dt();

String sat_days = dlng.getSat_days();
String holiday_state = dlng.getHoliday_state();
String plant_seq = dlng.getPlant_seq();
String disp_holiday_state = dlng.getDisp_holiday_state();
...
%>
<body onload="<%if(rate_unit.equals("2")){ %>showExchangerate();<%}%>setBillingPeriod(document.forms[0].billing_flag);setBillingDays(document.forms[0].freq);enablechk();setValues('<%=sat_days%>');showDispState();setPlantState('<%=strPlant%>','<%=holiday_state%>');">
			...
			<div class="card cardmain">
				<div class="card-header cdheader">
					<div class="d-flex justify-content-between">
					    <div class="topheader">
						Service Order <%=contpty_abbr %> [<%=mapped_cont_no %>] Billing Details
					...
					<div>
						...
						<div class="row m-b-5">
							...
							<div class="col-sm-4 col-xs-4 col-md-4">
								<div class="form-group row">
					    			<div class="col-sm-12 col-xs-12 col-md-12">
					    				<select class="form-select form-select-sm" name="inv_currency" onchange="showExchangerate();">
						...
						<%if(rate_unit.equals("2")){ %>
						<div style="display: none;" id="exchnage_block">
							<div class="row m-b-5" >
								<div class="col-sm-2 col-xs-2 col-md-2">
							    			...
							    			<div class="col">
							    				<select class="form-select form-select-sm" name="exch_calc_base" onchange="showExchngCriteria(this);">
										    		<option value="D">On Particular Day Base</option>
										     		<!-- <option value="A">On Daily Basis of Billing Period</option> --><!-- HM20251015 : Removed as per JD madam's feedback -->
										     	</select>
											...
											<div class="col">
							    				<select class="form-select form-select-sm" name="inv_criteria" id="exchngCri" <%if(!exchng_cal.equals("D")){ %>style="display:none;"<%} %>>
										    		<option value="">--Select--</option>
										     		...
										     		<!-- <option value="LST">Last Day of Billing Cycle</option>
									     		<option value="PRE">Previous Day of Billing Cycle</option> <!-- HM20251015 : Removed as per JD madam's feedback-->	     		
										     	</select> 
...
<input type="hidden" name="option" value="SVC_CONTRACT_BILLING_DTL">
