...
function refresh()
{
	...
	var url = "frm_dlng_daily_buyer_nom.jsp?gas_dt="+gas_dt+"&u="+u;
...
function setEnableDisabled(obj, selectedIndex,isInvoiced)
{
    ...
    for (var index = 0; index < totalRows; index++)
    {
        ...
        else
        {
            ...
            qty_mmbtu.value = document.getElementById("tmp_qty_mmbtu" + index).value;
...
function doSubmit()
{
	...
	var total_qunt_mmbtu = document.forms[0].total_qunt_mmbtu;
	...
	if(gen_time!=null && gen_time.length!=undefined)
	{
		for(var i=0; i<gen_time.length; i++)
		{
			if(chk[i].checked)
			{
				...
				if(trim(total_qunt_mmbtu[i].value)!="" && parseFloat(total_qunt_mmbtu[i].value)!=0.0)
				{
					if(parseFloat(total_qunt_mmbtu[i].value)!=parseFloat(qty_mmbtu[i].value))
					{
						msg+="Energy (MMBTU) for ROW - "+parseInt(i+1)+" should match with Total Truck Quantity !\n";
						flag=false;
					}
				}
	...
	else
	{
		...
		{ 	chk_count++
			...
			if(trim(total_qunt_mmbtu.value)!="" && parseFloat(total_qunt_mmbtu.value)!=0.0)
			{
				if(parseFloat(total_qunt_mmbtu.value)!=parseFloat(qty_mmbtu.value))
				{
					msg+="Energy (MMBTU) should match with Total Truck Quantity !\n";
					flag=false;
				}
			}
...
function doTruckSubmit(i_index)
{
	...
	document.forms[0].submit_flag.value = "NOM_DTL";
				...
				if(cont_internal_mapp_id.length!=undefined)
				{
					for(var i=0; i<cont_internal_mapp_id.length; i++)
					{
						if(int_map_id.value==cont_internal_mapp_id[i].value)
						{
							...
							{
								//alert("Contract TCQ : "+parseFloat(cont_tcq[i].value)+"\nTotal Buyer Nom : "+parseFloat(tot_mmbtu)+"\n\nThe Buyer Nomination Qty should not be > Contract TCQ!");
								alert("Contract TCQ : "+parseFloat(cont_tcq[i].value)+"\nEffective Buyer Nom (Best value of Allocation,Buyer Nomination) : "+parseFloat(tot_mmbtu)+"\nAllowed Nomination Quantity: "+allowedNom+"\n\nThe Buyer Nomination Qty should not be > Contract TCQ!");
								
								qty_mmbtu.value=tmp_qty_mmbtu;
...
cont_mgmt.setCallFlag("DAILY_BUYER_NOM");
										...
										%>
											<tr>
												...
												>
													...
													onclick="setEnableDisabled(this,'<%=l%>',<%if(VTEMP_TOTAL_NOM_BLOCK.contains("Y")&& (VCONTRACT_TYPE.elementAt(l).equals("O") || VCONTRACT_TYPE.elementAt(l).equals("Q"))) {%>'Y'<%}else{ %>'N'<%} %>);calculateSCM('<%=l%>');calculateMT('<%=l%>');totalQty();">
												...
												<td align="center">
													<div style="width:100px;">
														...
														<%-- onpaste="doPaste('<%=l%>');"  --%>
														onblur="negNumber(this);checkNumber1(this,9,2);checkQty('<%=l%>');calculateMT('<%=l%>');calculateSCM('<%=l%>');totalQty();" disabled><%-- checkTCQ('<%=l%>'); --%>
									...
									<tr>
										<td align="center">
											<div style="width:200px;">
												...
												'<%=VCONT_REV_NO.elementAt(i)%>','<%=gas_dt%>');">
									...
									<tr <%if(VTEMP_NOM_BLOCK.elementAt(j).equals("Y") || VTEMP_NOM_BLOCK.elementAt(j).equals("A")) {%>style="pointer-events: none;"<%}%>>
										...
										<td align="center">
											<div  style="width:100px;">
												...
												onchange="negNumber(this);checkNumber1(this,9,2);calcRemainBlncQty('<%=i%>');document.getElementById('nom_qunt_mt_<%=j%>_<%=i%>').value=getMtValueOFMmbtu(this,'<%=j%>','<%=i%>');checkTruckCapacity(this,'TruckCapInMmbtu_<%=j%>_<%=i%>');" 
...
function fetchBayDeatils(j_index,fillStCd,comp_cd,i_index)
{
	if((comp_cd !="" && comp_cd!="0"))
	{
		$.post("../servlet/DB_Dlng_ContractMgmt_Ajax"+ "?comp_cd="+comp_cd+"&fillStCd="+fillStCd+
			...
			console.log("In fetchBayDeatils j_index---"+j_index);
			
			if(j_index=="")
			{
				$('#add_sel_bay'+"_"+i_index).empty().append('<option value="">--Select--</option>');
				$('#add_sel_slot'+"_"+i_index).empty().append('<option value="">--Select--</option>');
			}
			else
			{
				$('#sel_bay'+j_index+"_"+i_index).empty().append('<option value="">--Select--</option>');
			...
			$.each(responseJson, function (index, json) {
				$.each(json.BAY_DTL, function (i, baySt) {
			        ...
			        if(j_index=="")
			        {
			        	$('#add_sel_bay'+'_'+i_index).append(option);
			        }
			        else
			        {
			        	$('#sel_bay'+j_index+'_'+i_index).append(option);
        ...
        $.post("../servlet/DB_Dlng_ContractMgmt_Ajax" + "?comp_cd=" + comp_cd + "&bayCd=" + bayCd + "&gasDt=" + gasDateStr +
            ...
            $.each(responseJson, function (index, json) {
                $.each(json.BAY_DTL, function (i, baySt) {
                	...
                	if(j_index=="")
                	{
                		var $slotSelect = $('#add_sel_slot'+ "_" + i_index);

                        $slotSelect.append($('<option>', {
                            value: baySt.BAYSLOTVALUE,
                            text: baySt.BAYSLOTTEXT,
                            //disabled: baySt.SLOTAVAILABLE === "N",
                            style: baySt.SLOTAVAILABLE === "N" ? 'color: red;pointer-events: none;' : ''
                        }));
                	}
                	else
                	{
...
<input type="hidden" name="option" value="BUYER_NOM">
