...
function refresh(active_status)
{
	...
	var url = "../dlng/frm_dlng_cont_agmt_svc_list.jsp?active_status="+active_status+"&counterparty_cd="+counterparty_cd+"&agreement_type="+agreement_type;
...
function refresh_date()
{
	...
	if(flag && flg)
	{
		if(trim(from_dt)!="" && trim(to_dt)!="")
		{
			...
			{
			var url = "../dlng/frm_dlng_cont_agmt_svc_list.jsp?active_status="+active_status+"&counterparty_cd="+counterparty_cd+
...
function setValue(countpty_cd,agreement_type,agmt_no,agmt_rev_no,agmt_nm)
{
	window.opener.setAgmtDetail(countpty_cd,agmt_no,agmt_rev_no,agreement_type,agmt_nm);
	window.close();	
}
</script>
</head>
...
dlng.setCallFlag("TMSA_AGREEMENT_LIST");
...
Vector VAGMT_REF_NO = dlng.getVAGMT_REF_NO(); 
Vector VAGMT_NAME = dlng.getVAGMT_NAME(); 
Vector VSTART_DT = dlng.getVSTART_DT();
Vector VEND_DT = dlng.getVEND_DT();
Vector VAGMT_TYP = dlng.getVAGMT_TYPE();
Vector VAGMT_STATUS = dlng.getVAGMT_STATUS(); 
Vector VDISP_AGMT_NO = dlng.getVDISP_AGMT_NO();
							...
							<table class="table table-bordered ems_sorttbl" id="ems_table0">
								<thead id="ems_tbsort0">
									<tr valign="top">
										...
										<th class="ems_thsort0">Agreement Type</th>
								...
								<%if(VCOUNTERPARTY_CD.size() > 0){ %>
									...
									<tr>
										<td align="center"><input type="radio" onclick="setValue('<%=VCOUNTERPARTY_CD.elementAt(i) %>','<%=agreement_type%>','<%=VAGMT_NO.elementAt(i)%>','<%=VAGMT_REV_NO.elementAt(i)%>','<%=VAGMT_NAME.elementAt(i) %>')"></td>
