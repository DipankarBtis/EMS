...
function setEnableDisabled(obj, index)
{
	 ...
	 for (var i = 0; i < radios.length; i++) 
	 {
		 ...
		 if (radio !== obj){
            ...
            var tmp_qty_mmbtu = document.getElementById("tmp_qty_mmbtu" + otherIndex);
				...
				<div class="alert alert-secondary alert-dismissible fade show" role="alert">
				  	<%for(int i=0 ;i<MSG_TRUCK_REG_NO.size(); i++) {%>
				  		<%if(MSG_SUBMITTED.elementAt(i).toString().equals("E")) {%>
				  			<%=utilmsg.errorMessage("Gas Dt : "+MSG_RPT_DT.elementAt(i)+", Contract# : "+MSG_DEAL_NO.elementAt(i)+", Truck : "+MSG_TRUCK_REG_NO.elementAt(i)+", Qty : "+MSG_QTY.elementAt(i)+", Allowable Credit : "+MSG_ALLOW_CREDIT.elementAt(i)+", Required Amount : "+MSG_CONSUMED_AMT.elementAt(i)+", Deficit Amount "+MSG_BALANCE.elementAt(i))%>
				  		<%}else{ %>
							<%=utilmsg.successMessage("Gas Dt : "+MSG_RPT_DT.elementAt(i)+", Contract# : "+MSG_DEAL_NO.elementAt(i)+", Truck : "+MSG_TRUCK_REG_NO.elementAt(i)+", Qty : "+MSG_QTY.elementAt(i)+", Allowable Credit : "+MSG_ALLOW_CREDIT.elementAt(i)+", Consumed Amount : "+MSG_CONSUMED_AMT.elementAt(i)+", Balance Amount "+MSG_BALANCE.elementAt(i))%>
						<%} %>
					<%} %>
							...
							<table class="table table-bordered table-hover">
								<thead>
									...
									<tr>
										...
										<th rowspan="2"><input style="display: none" class="form-check-input" type="checkbox" name="chkAll" onclick="checkAll(this,'<%=VENTRY_RANGE.elementAt(j)%>','<%=j%>');"></th>
												...
												<tr>
													...
													<td align="center">
														<div style="width:100px;">
															...
															<%-- onpaste="doPaste('<%=j%>','<%=l%>');"  --%>
															onblur="negNumber(this);checkNumber1(this,9,2);calculateSCM('<%=j%>','<%=l%>');calculateMT('<%=l%>');" disabled>
															<%-- checkQty('<%=l%>');checkTCQ('<%=l%>'); --%>
															...
															<input type="hidden" name="cont_buyer_nom" id="cont_buyer_nom_<%=l%>" value="<%=VCONT_BUYER_NOM.elementAt(l)%>">
														</div>
													</td>
													<td align="center">
														<div style="width:100px;">
													...
													'<%=cont_rev_no%>','<%=cont_ref%>','<%=counterparty_nm%>','<%=VCOUNTERPARTY_PLANT_SEQ.elementAt(l)%>','<%=VBU_PLANT_SEQ.elementAt(l)%>'
													);"></i>
													</td>
									...
									<tr <%if(VTEMP_NOM_BLOCK.elementAt(j).equals("Y") || VTEMP_NOM_BLOCK.elementAt(j).equals("A")) {%>style="pointer-events: none;"<%}%>>
										...
										<td align="center">
											<div  style="width:100px;">
												...
												onchange="negNumber(this);checkNumber1(this,9,2);calcRemainBlncQty('<%=i%>');document.getElementById('nom_qunt_mt_<%=j%>_<%=i%>').value=getMtValueOFMmbtu(this,'<%=j%>','<%=i%>');checkTCQ('<%=i %>');" 
