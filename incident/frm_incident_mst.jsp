						...
						<div class="col-sm-12 col-xs-12 col-md-12">
							<div class="table-responsive">
								<table class="table table-bordered" id="search_by_filter">
									    	...
									    	<div align="center">
										    	<select class="form-select form-select-sm" name="filter_scope" onchange="refresh();" style="width:75px;" >
											    	...
											    	<%for(int i=0; i<VFILTER_SCOPE.size(); i++){ %>
											    		<option value="<%=VFILTER_SCOPE.elementAt(i)%>">
											    		   <%if(VFILTER_SCOPE.elementAt(i).equals("")){%>
										    				...
										    				<%}else if(VFILTER_SCOPE.elementAt(i).equals("NEW")){%>
										    					New
										    				<%}%>&nbsp;(<%=VFILTER_COUNT_SCOPE.elementAt(i)%>)</option>
											    	  <%} %>
										    	</select>
									    	</div>
									    	<script>document.forms[0].filter_scope.value="<%=filter_scope%>"</script>
									    	</th>
									    	<th rowspan="2">
									    	Incident Type
									    	<br>
									...
									if(VINCIDENT_ID.size()>0){%>
										...
										%>
											<tr>
												...
												<%if(!VSTATUS.elementAt(i).equals("Close") && !VSTATUS.elementAt(i).equals("Defer")){ %>
													<font title="Click to Edit" style="color:var(--header_color)">
														...
														onclick="doModify('<%=VINCIDENT_ID.elementAt(i)%>','<%=VINCIDENT_TYPE.elementAt(i)%>',
																		...
																		'<%=VROOT_CAUSE.elementAt(i)%>','<%=VPROD_ROLLOUT_DT.elementAt(i)%>','<%=VPROD_APP_DT.elementAt(i)%>',
																		'<%=VROLLOUT_SCHEDULE.elementAt(i) %>','<%=VSCOPE.elementAt(i) %>','<%=VENT_CD.elementAt(i) %>');fetchEventDtl();">
														</i>
													</font>
												<%} %>	
												</td>
		    									...
		    									<td class="incident-detail-col"><%=VINCIDENT_DTL.elementAt(i).toString().replace("\n", "<br/>")%></td>
		    									...
		    									<% } %>
			 									...
			 									<%-- <td align="center"><%=VENT_PROFILE.elementAt(i)%></td> --%>
											...
											<tbody id="tbody<%=i%>" style="display:none;">
												<tr style="text-align:center;font-weight:bold;background:#bce6ff;color:#0c63e4;">
													...
													<td>Status</td>
													<td>Resolution</td>
													<td colspan="8" rowspan="<%=size+1%>" style="background:white;"></td>
												</tr>
												...
												%>
													<tr>
														...
														<%-- <td align="center"><%=VDTL_SCOPE.elementAt(j)%></td> --%>
														<td valign="middle" align="center">
														...
														<td align="center"><%=VDTL_ENT_PROFILE.elementAt(j)%></td>
									...
									<%}else{ %>
										<tr>
											<td colspan="18" align="center"><%=utilmsg.infoMessage("<b>No Inciednt Configured!</b>") %></td>
										</tr>
									<%} %>
									</tbody>
								</table>
      			...
      			<div class="cdbody">
      			  <div class="row m-b-5 d-flex justify-content-end">
					 ...
					 <div class="col-sm-4 col-xs-4 col-md-4">  
						<div class="form-group row">
				    		<div class="col-sm-12 col-xs-12 col-md-12">
				      			<select class="form-select form-select-sm" name="initiatd_by" <%if (!authorize_access.equals("Y")){ %>style="pointer-events:none; background-color: #e9ecef"<%} %>>
				      				<option value="0">--Select--</option>
				      				<%for(int i=0; i<VEMP_CD.size(); i++){ %>
				      				<option value="<%=VEMP_CD.elementAt(i)%>"><%=VEMP_NM.elementAt(i)%></option>
				      				<%} %>
				      			</select>
				      			<script>document.forms[0].initiatd_by.value="<%=emp_cd%>"</script>
				    		</div>
				  		</div>
				    </div>
				    </div>
