		...
		try
		{
	    	...
	    	if(ds != null) 
	    	{
	    		...
	    		if(conn != null)  
	    		{
	    			...
	    			else if(callFlag.equalsIgnoreCase("GENERATE_CRDR_DLNG_SALES_SAP_XML"))
	    			{
	    				generateSalesCRDRInvoiceXML();
	    			}
	    			...
	    			else if(callFlag.equalsIgnoreCase("AUTO_GENERATE_DLNG_SALES_SAP_XML"))
	    			{
	    				updateSAPapprovalFlag();
	    			}
	    			else if(callFlag.equalsIgnoreCase("CRDR_SALES_SAP_XML"))
	    			{
	    				generateSalesCRDRInvoiceXML();
	    				parseSAP_XMLfile();
	    			}
	    			...
	    			else if(callFlag.equalsIgnoreCase("DLNG_SERVICE_INVOICE_PREPARATION_LIST"))
	    			{
						VINVOICE_LIST_ABBR.add("TMS_INV");
						VINVOICE_LIST_ABBR.add("LTCORA_DLNG_INV");
						
						VINVOICE_LIST_NAME.add("DLNG Service Generation");
						VINVOICE_LIST_NAME.add("LTCORA DLNG Generation");
						...
						{
							getBillingCyclePeriod();
							getDlngServiceInvoicePreparationList();
						}
    					...
    					if(billing_cycle.equals("0"))
						{
							forAllLtcoraBillingOption();
						}
						else
						{
							getBillingCyclePeriod();
							getLtcoraDlngInvoicePreparationList();
						}
    					VINDEX.add(inv_index);
	    			}
	    			else if(callFlag.equalsIgnoreCase("DLNG_SVC_INVOICE_GENERATION"))
	    			{
	    				getSvcContractDetail();
	    				getSvcContractBillingDetail();
	    				getContactPerson();
	    				getBuContactPerson();
	    				getExistingServiceInvoiceDtl();
	    				getInvoiceDateCalc();
	    				getSvcInvoiceCalculation();
	    				getDlngSalesInvoiceList();
	    				if(!operation.equals("MODIFY") && !submission_chk)
	    				{
	    					getRemarks();
	    				}
	    				...
	    				if((operation.equals("INSERT") && !submission_chk) || operation.equals("MODIFY"))
	    				{
	    					checkSVCTCS_TDSApplicable();
	    				}
	    				getTcsTdsSvcInvDtl();
	    			}
	    			else if(callFlag.equalsIgnoreCase("DLNG_SERVICE_INVOICE_REPORT"))
	    			{
	    				getSvcInvoiceDetail();
	    				if(activityFlag.equals("APPROVE"))
	    				{
	    			...
	    			else if(callFlag.equalsIgnoreCase("EXISTING_CRDR_LIST"))
	    			{
	    				...
	    				VINVOICE_LIST_NAME.add("DLNG Sales Credit/Debit Generation");
	    				
	    				for(int i=0; i<VINVOICE_LIST_ABBR.size(); i++)
	    				{
	    					inv_index=0;
	    					String contType="'E','F','W'";
	    			...
	    			else if(callFlag.equalsIgnoreCase("LP_INVOICE_LIST"))
	    			{
	    				VINVOICE_LIST_ABBR.add("INV_HEAD");
	    				//VINVOICE_LIST_ABBR.add("SVC_INV_HEAD");
	    				//VINVOICE_LIST_ABBR.add("FFLOW_INV_HEAD");
	    				
	    				VINVOICE_LIST_NAME.add("DLNG (SN | LOA | IGX)");
	    				//VINVOICE_LIST_NAME.add("DLNG Service Order");
	    				//VINVOICE_LIST_NAME.add("Sales (FFlow) Invoice Generation");
	    				
	    				inv_index=0;
	    				getLpDlngInvoicePreparationList();
    					VINDEX.add(inv_index);
    					
						//inv_index=0;
						//getLpDlngSvcInvoicePreparationList();
						//VINDEX.add(inv_index);
...
//		    			}
	    			...
	    			else if(callFlag.equalsIgnoreCase("DLNG_LP_INVOICE_REPORT"))
	    			{
	    				getRefrenceInvoiceDtl();
	    				getLpInvoiceDetail();
	    				if(activityFlag.equals("APPROVE"))
	    			...
	    			else if(callFlag.equalsIgnoreCase("DLNG_INVOICE_CHANGE_REQUEST_APPROVAL"))
	    			{
    					VINVOICE_LIST_ABBR.add("INV_HEAD");
    					VINVOICE_LIST_ABBR.add("CRDR_HEAD");
    					VINVOICE_LIST_ABBR.add("LP_HEAD");
	    				
	    				VINVOICE_LIST_NAME.add("DLNG Sales Invoice");
	    				VINVOICE_LIST_NAME.add("DLNG Sales Credit/Debit Invoice");
	    				VINVOICE_LIST_NAME.add("DLNG Late Payment Invoice");
	    				
	    				getDLNGInvoiceChangeList();
    					
    					VINVOICE_LIST_ABBR.add("FFLOW_HEAD");
    					VINVOICE_LIST_NAME.add("DLNG FreeFlow Invoice");
    					getDLNGfflowInvoiceChangeList();
    					
	    			}
	    			else if(callFlag.equalsIgnoreCase("DLNG_LTCORA_INVOICE_GENERATION"))
	    			{
	    				getContractDetail();
	    				getContractBillingDetail();
	    				getContactPerson();
	    				...
	    				getExistingServiceInvoiceDtl();
	    				getInvoiceDateCalc();
	    				getLtcoraDlngInvoiceCalculation();
	    				getDlngSalesAllocatedTruckList();
	    				if(!operation.equals("MODIFY") && !submission_chk)
	    				{
	    					getRemarks();
	    				}
		...
		try
		{
			...
			String queryString="SELECT A.COMPANY_CD,A.COUNTERPARTY_CD,A.AGMT_NO,A.AGMT_REV,A.CONT_NO,A.CONT_REV,"
					...
					+ "AND F.NOM_REV_NO=(SELECT MAX(NOM_REV_NO) FROM FMS_DLNG_ALLOC_MST C "
						...
						+ "AND F.TRUCK_TRANS_CD=C.TRUCK_TRANS_CD AND F.TRUCK_CD=C.TRUCK_CD AND F.FILL_STATION_CD=C.FILL_STATION_CD "
						+ "AND F.BAY_CD=C.BAY_CD AND F.SLOT_START_TIME=C.SLOT_START_TIME AND F.SLOT_END_TIME=C.SLOT_END_TIME AND F.CARGO_NO=C.CARGO_NO) ";
	...
	public void getDlngFFlowInvoiceList()
	{
		...
		try
		{
			...
			String queryString="SELECT COMPANY_CD,COUNTERPARTY_CD,AGMT_NO,AGMT_REV,CONT_NO,CONT_REV,CONTRACT_TYPE, "
					...
					+ "PRINT_BY_ORI,PRINT_BY_TRI,PRINT_BY_DUP,SAP_APPROVAL,CARGO_NO,INVOICE_CATEGORY,TAX_STRUCT_CD,FIN_SYS,TO_CHAR(INVOICE_DT,'DD/MM/YYYY') "
			...
			while(rset.next())
			{
				...
				String queryString1="SELECT TO_CHAR(START_DT,'DD/MM/YYYY'),TO_CHAR(END_DT,'DD/MM/YYYY'),CONT_NAME,CONT_REF_NO,TRADE_REF_NO,CONT_REV "
						+ "FROM FMS_SUPPLY_CONT_MST A "
				...
				String fin_sys=rset.getString(27)==null?"":rset.getString(27);
				String inv_dt=rset.getString(28)==null?"":rset.getString(28);
				...
				else
				{
					...
					if(rset3.next())
					{
						...
						{
							VOTH_PDF_FILE_PATH.add(CommonVariable.signed_dlng_freeflow_inv_path);
						}
						else
						{
							VOTH_PDF_FILE_PATH.add(CommonVariable.dlng_freeflow_inv_path);
						}
					}
					else
					{
				...
				int re_print_count=isRePrintPDFRequestedFflow(comp_cd,bu_state_tin,inv_seq,fin_yr,"DLNG","REPRINT_PDF","A",inv_type);
			...
			String bu_plant_seq, String truckCd, String track_trans_cd, String period_start_dt, String period_end_dt,String state_code, String inv_flag, String mapping_id)
	{
		...
		try
		{
			...
			queryString5="SELECT INVOICE_NO,CHECKED_FLAG,APPROVED_FLAG,INVOICE_SEQ,PDF_INV_DTL,"
					+ "PRINT_BY_ORI,PRINT_BY_TRI,PRINT_BY_DUP,SAP_APPROVAL,FINANCIAL_YEAR,FIN_SYS,TO_CHAR(INVOICE_DT,'DD/MM/YYYY') "
			...
			if(rset5.next())
			{
				...
				fin_sys=rset5.getString(11)==null?"":rset5.getString(11);
				inv_dt=rset5.getString(12)==null?"":rset5.getString(12);
			...
			int re_print_count=isRePrintPDFRequested(comp_cd,state_code,inv_seq,fiscal_yr,"DLNG","REPRINT_PDF","A");
			VRE_PRINT_PDF.add(re_print_count>0?"Y":"N");
		}
		catch(Exception e)
		{
		...
		try
		{
			...
			if(contract_type.equals("O") || contract_type.equals("Q"))
			{
				...
				if(rset.next())
				{
					...
					stmt1.setString(8, "TLU");
					rset1=stmt1.executeQuery();
					if(rset1.next())
					{
						price=rset1.getString(2)==null?"":nf2.format(rset1.getDouble(2));
					}
					rset1.close();
					stmt1.close();
					...
					arrival_date=rset.getString(24)==null?"":rset.getString(24);
					...
					remark_2="Cargo "+ship_nm+" Dated "+arrival_date+"";
	...
	public void getContractBillingDetail()
	{
		...
		try
		{
			if(contract_type.equals("O") || contract_type.equals("Q"))
			{
				String queryString="SELECT INVOICE_CUR_CD,PAYMENT_CUR_CD,DUE_DATE,EXCHNG_RATE_CD,EXCHNG_CRITERIA,EXCHNG_RATE_CAL,DUE_DT_IN,"
			...
			{
				String queryString="SELECT INVOICE_CUR_CD,PAYMENT_CUR_CD,DUE_DATE,EXCHNG_RATE_CD,EXCHNG_CRITERIA,EXCHNG_RATE_CAL,DUE_DT_IN,"
						+ "EXCL_SAT_MAP,EXCHG_VAL,HOLIDAY_STATE,BILLING_FREQ,BILLING_DAYS,INT_CAL_RATE_CD,INT_CAL_SIGN,INT_CAL_PERCENTAGE "
				...
				if(rset.next())
				{
					...
					int_cal_rate_cd=rset.getString(13)==null?"":rset.getString(13);
					int_cal_sign=rset.getString(14)==null?"":rset.getString(14);
					int_cal_percentage=rset.getString(15)==null?"":rset.getString(15);
	...
	public void getExistingInvoiceDtl()
	{
		...
		try
		{
			String queryString="SELECT BU_CONTACT_PERSON_CD,CONTACT_PERSON_CD,INVOICE_SEQ,INVOICE_NO,TO_CHAR(INVOICE_DT,'DD/MM/YYYY'),"
			...
			if(rset.next())
			{
				...
				if(operation.equals("INSERT"))
				{
					...
					String queryString1="SELECT TAX_CODE,TAX_DESCR,TAX_AMT,TAX_BASE_AMT "
	...
	public void getInvoiceDateCalc()
	{
		...
		try
		{
			if((operation.equals("INSERT") && !submission_chk) || operation.equals("MODIFY"))
			{
				...
				if(contract_type.equals("O") || contract_type.equals("Q")) 
				{
					invoice_dt = systemDt;//HM20260131: As per Vijay's feedback Invoice date <= System Date for LTCORA DLNG Invoice 
					freq_date = systemDt;
				}
	...
	public void getInvoiceCalculation()
	{
		...
		try
		{
			...
			if((operation.equals("INSERT") && !submission_chk) || operation.equals("MODIFY"))
			{
				//GET SALES PRICE
				String new_price="";
				String queryString = "SELECT DISTINCT NEW_SALE_PRICE "
			...
			if(inv_flag.equals("LP"))
			{
				...
				else 
				{
					if((operation.equals("INSERT") && !submission_chk) || operation.equals("MODIFY"))
					{
						//float lateDays = utilDate.getDays(ori_inv_payrecv_dt, ori_inv_due_dt)-1 ;
						
						//HM20260203:Changed to int from float as giving vulnerability of "Cast one of the operands of this subtraction operation to a 'float"."
						int lateDays = utilDate.getDays(ori_inv_payrecv_dt, ori_inv_due_dt) - 1;

						//HM20260203:Changed to double from float as giving vulnerability of "Use a "double" or 'BigDecimar instead."
						double  calcInvAmt = ((Double .parseDouble(ori_inv_net_amt) * (lateDays * Double .parseDouble (int_total_percentage)))/(100*365));
					...
					if((contract_type.equals("B") || contract_type.equals("M")) && (operation.equals("INSERT") && !submission_chk) || operation.equals("MODIFY"))
					{
					...
					gross_amt1 = nf.format(Double.parseDouble(gross_include_transport_tariff));
	...
	public void getChekPostandDriverMst()
	{
		...
		try
		{
			if((operation.equals("INSERT") && !submission_chk) || operation.equals("MODIFY"))
			{
				...
				//CHECK POST MASTER
				String queryString = "SELECT CHKPOST_CD,CHKPOST_NAME "
	...
	public void getExchangeRateMaster()
	{
		...
		try
		{
			String queryString = "SELECT EXC_RATE_CD,EXC_RATE_NM,FLAG "
		...
		try
		{
			...
			else
			{
				...
				else if(exchng_rate_cal.equals("D"))
				{
					...
					String lt4nig=utilDate.getLastDateofFortnight(invoice_dt);
					if(exchang_criteria.equals("LPF")) //GO LIVE FEEDBACK FROM VIJAY ON 20250915
					{
						getExchangeRateEntry("B", lt4nig);
						lable_inv_criteria="Last Day of Previous Fortnight";
						lable_inv_date=lt4nig;
					}
					else if(exchang_criteria.equals("INV")) 
					{
					...
					String queryString="SELECT TO_CHAR(EFF_DT,'DD/MM/YYYY') "
							+ "FROM FMS_EXCHG_RATE_ENTRY A "
							+ "WHERE EXCHG_RATE_CD=? "
							+ "AND EFF_DT=(SELECT MAX(EFF_DT) FROM FMS_EXCHG_RATE_ENTRY B "
							//+ "WHERE A.EXCHG_RATE_CD=B.EXCHG_RATE_CD AND B.EFF_DT<TO_DATE(?,'DD/MM/YYYY')) ";
					...
					stmt=conn.prepareStatement(queryString); 
					stmt.setString(1, exchng_rate_cd);
					stmt.setString(2, lable_inv_date);
					rset=stmt.executeQuery();
					if(rset.next())
					{
					...
					/*else if(exchang_criteria.equals("LST")) //AS DISCUSSED WITH VIJAY ON 20250702
					{
					...
					if(!exchange_rate_mapping.equals(""))
					{
						...
						String queryString1="SELECT NVL(EXCHG_VAL,0),TO_CHAR(EFF_DT,'DD/MM/YYYY') "
								+ "FROM FMS_EXCHG_RATE_ENTRY A "
								+ "WHERE EXCHG_RATE_CD=? "
								+ "AND EFF_DT = TO_DATE(?,'DD/MM/YYYY')";
						stmt1=conn.prepareStatement(queryString1);
					...
					else
					{
						if(!submission_chk)
						{
							...
							stmt1.setString(2, lable_inv_date);
							rset1=stmt1.executeQuery();
							if(rset1.next())
							{
								exchng_rate=rset1.getDouble(1);
							...
							if(isExchngAvl==0) //AS PER INC#2600039 AND THIS IS NOT A BUG, IT'S ENH
							{
								...
								stmt1=conn.prepareStatement(queryString1);
								stmt1.setString(1, exchng_rate_cd);
								stmt1.setString(2, last_avlb_exchng_dt);
								rset1=stmt1.executeQuery();
								if(rset1.next())
								{
									exchng_rate=rset1.getDouble(1);
									exchang_rate_dt=rset1.getString(2)==null?"":rset1.getString(2);
									exchange_rate_mapping="P-"+exchng_rate_cd;
								}
								rset1.close();
								stmt1.close();
							}
	...
	public void getRemarks()
	{
		...
		try
		{
			...
			*/
			...
			String bank_formula=utilBean.getEntityBankFormula(conn, comp_cd, counterparty_cd, "C", "DLNG", invoice_dt);
	...
	public void getTurnoverDetail()
	{
		...
		try
		{
			...
			String queryString="SELECT TURNOVER_FLAG "
			...
			String queryString1="SELECT TURNOVER_FLAG "
					+ "FROM FMS_ENTITY_TURNOVER_DTL "
					+ "WHERE COMPANY_CD=? AND COUNTERPARTY_CD=? "
					+ "AND ENTITY=? AND FINANCIAL_YEAR=? AND TURNOVER_CD=? ";
			stmt1=conn.prepareStatement(queryString1);
	...
	public void checkSVCTCS_TDSApplicable()
	{
		String function_nm="checkSVCTCS_TDSApplicable()";
		try
		{
			//Trunover_prev_finantial_yr(Seller<10cr, Buyer<10cr) && Trans_current_finantial_yr > 50L ==> TCS/TDS Applicable : NA
			//Trunover_prev_finantial_yr(S>10cr, B<10cr) && Trans_current_finantial_yr > 50L ==> TCS/TDS Applicable : TCS
			...
			if(contract_type.equals("O") || contract_type.equals("Q"))
			{
				...
				Vector temp_tcs = new Vector();
				temp_tcs=TaxCalc.TCS_TDSAmountCalculationWithInfo(conn, comp_cd, counterparty_cd, "C", "TDS", "TLU","S",period_end_dt, gross_include_transport_tariff);
				tds_amt = nf.format(Double.parseDouble(""+temp_tcs.elementAt(0)));
				tds_struct_cd = ""+temp_tcs.elementAt(1);
				tds_struct_dt = ""+temp_tcs.elementAt(2);
				//tax_info = ""+temp_tcs.elementAt(3);
			...
			else
			{
				...
				if(applicable_abbr.equals("TCS"))
				{
					...
					if(total_difference>0)
					{
						...
						if(contract_type.equals("O") || contract_type.equals("Q")) 
						{
							temp_tcs=TaxCalc.TCS_TDSAmountCalculationWithInfo(conn, comp_cd, counterparty_cd, "C", "TCS", "TLU","S",period_end_dt, nf.format(total_difference));
						}
						else
						{
							temp_tcs=TaxCalc.TCS_TDSAmountCalculationWithInfo(conn, comp_cd, counterparty_cd, "C", "TCS", "DTM","S",period_end_dt, nf.format(total_difference));
						}
				...
				else if(applicable_abbr.equals("TDS"))
				{
					...
					if(total_difference>0)
					{
						...
						if(contract_type.equals("O") || contract_type.equals("Q")) 
						{
							temp_tcs=TaxCalc.TCS_TDSAmountCalculationWithInfo(conn, comp_cd, counterparty_cd, "C", "TDS", "TLU","S",period_end_dt, nf.format(total_difference));
						}
						else
						{
							temp_tcs=TaxCalc.TCS_TDSAmountCalculationWithInfo(conn, comp_cd, counterparty_cd, "C", "TDS", "DTM","S",period_end_dt, nf.format(total_difference));
						}
	...
	public void getTcsTdsInvDtl()
	{
		...
		{
			double total=0;
			String queryString="SELECT AGMT_NO,CONT_NO,CONTRACT_TYPE,INVOICE_AMT,INVOICE_NO,TO_CHAR(INVOICE_DT,'DD/MM/YYYY') "
			...
			while(rset.next())
			{
				...
				String contRef="";
				String queryString1="SELECT CONT_REF_NO  "
						+ "FROM FMS_SUPPLY_CONT_MST A "
	...
	public Double getTransactionAmount(String comp_cd,String counterparty_cd, String financial_year,String invoice_dt)
	{
		...
		{
			//RLNG INVOICE
			String queryString="SELECT NVL(SUM(INVOICE_AMT),0) "
					+ "FROM FMS_INVOICE_MST "
					+ "WHERE COMPANY_CD=? AND COUNTERPARTY_CD=? AND CONTRACT_TYPE IN (?,?) "
					+ "AND FINANCIAL_YEAR=? AND APPROVED_FLAG=? AND INVOICE_DT <=TO_DATE(?,'DD/MM/YYYY') "
			...
			String queryString1="SELECT NVL(SUM(INVOICE_AMT),0) "
	...
	public void getInvoiceDetailForViewBeforeSub()
	{
		...
		try
		{
			...
			if(contract_type.equals("O") || contract_type.equals("Q"))
			{
				String queryString="SELECT A.CONT_REF_NO,TO_CHAR(A.DDA_DT,'DD-MON-YY'),A.CONT_REF_NO,B.SHIP_CD,B.BOE_NO,TO_CHAR(B.BOE_DT,'DD-MON-YY'),TO_CHAR(SIGNING_DT,'DD-MON-YY'),"
			...
			{
				String queryString="SELECT CONT_REF_NO,TO_CHAR(DDA_DT,'DD-MON-YY'),TRADE_REF_NO,TO_CHAR(SIGNING_DT,'DD-MON-YY')  "
						+ "FROM FMS_SUPPLY_CONT_MST A "
						+ "WHERE COMPANY_CD=? AND CONTRACT_TYPE=? "
						+ "AND CONT_NO=? AND AGMT_NO=? AND COUNTERPARTY_CD=? "
						+ "AND CONT_REV = (SELECT MAX(CONT_REV) FROM FMS_SUPPLY_CONT_MST B WHERE A.COMPANY_CD=B.COMPANY_CD "
			...
			String queryString1="SELECT CONTACT_PERSON "
					+ "FROM FMS_ENTITY_CONTACT_MST A "
					+ "WHERE COMPANY_CD=? AND COUNTERPARTY_CD=? "
			...
			if(contract_type.equals("O") || contract_type.equals("Q"))
			{
				...
				String queryString = "SELECT A.STAT_NO, TO_CHAR(A.EFF_DT,'DD/MM/YYYY'), B.STAT_NM "
						+ "FROM FMS_COUNTERPARTY_PLANT_TAX A, FMS_GOVT_STAT_TAX B "
						+ "WHERE A.COUNTERPARTY_CD=? AND A.ENTITY=? "
				...
				while(rset.next())
				{
					...
					bu_tax_info+="<br>SAC No.: "+no
							+ "<br>Description of Service : "+sac_desc
	...
	public void getInvoiceDetail()
	{
		...
		try
		{
			String invDt="";
			String periodEndDt="";
			String queryString="SELECT BU_CONTACT_PERSON_CD,CONTACT_PERSON_CD,INVOICE_NO,TO_CHAR(INVOICE_DT,'DD-MON-YY'),"
					...
					+ "INVOICE_RAISED_IN,TDS_GROSS_AMT,TDS_GROSS_PERCENT,TDS_STRUCT_CD,TO_CHAR(INVOICE_DT,'DD/MM/YYYY'),TO_CHAR(PERIOD_END_DT,'DD/MM/YYYY') "
					+ "FROM FMS_DLNG_INVOICE_MST "
			...
			if(rset.next())
			{
				...
				tds_amt=rset.getString(33)==null?"":nf.format(rset.getDouble(33));
				tds_factor=rset.getString(34)==null?"":nf.format(rset.getDouble(34));
				tds_struct_cd=rset.getString(35)==null?"":rset.getString(35);
				
				invDt=rset.getString(36)==null?"":rset.getString(36);
				periodEndDt=rset.getString(37)==null?"":rset.getString(37);
			...
			String queryString1="SELECT CONT_REF_NO,TO_CHAR(DDA_DT,'DD-MON-YY'),TRADE_REF_NO,TO_CHAR(SIGNING_DT,'DD-MON-YY'),AGMT_BASE,DCQ,MDCQ_PERCENTAGE  "
					+ "FROM FMS_SUPPLY_CONT_MST A "
					+ "WHERE COMPANY_CD=? AND CONTRACT_TYPE=? "
					+ "AND CONT_NO=? AND AGMT_NO=? AND COUNTERPARTY_CD=? "
					+ "AND CONT_REV = (SELECT MAX(CONT_REV) FROM FMS_SUPPLY_CONT_MST B WHERE A.COMPANY_CD=B.COMPANY_CD "
	...
	public void generateSalesInvoiceXML()
	{
		...
		try
		{
			...
			{
		    	String queryString="SELECT TO_CHAR(PERIOD_END_DT,'DD/MM/YYYY'),BU_UNIT,INVOICE_NO,EXCHG_RATE_VALUE,NET_PAYABLE_AMT,"
			...
			{
				String queryString="SELECT TO_CHAR(PERIOD_END_DT,'DD/MM/YYYY'),BU_UNIT,INVOICE_NO,EXCHG_RATE_VALUE,NET_PAYABLE_AMT,"
						+ "TAX_STRUCT_CD,NULL,CONT_NO,GROSS_AMT,QTY,TCS_TDS,"
						+ "TCS_STRUCT_CD,NULL,TDS_STRUCT_CD,NULL,"
						...
						+ "TO_CHAR(INVOICE_DT,'DD/MM/YYYY'),TO_CHAR(APPROVED_DT,'DD/MM/YYYY'),INV_FLAG,QTY_UNIT "
				...
				if(rset.next())
				{
					...
					qty_unit=rset.getString(32)==null?"":rset.getString(32);
					
					 //AS Per Vijay's Feedback mail on 19/11/2025 : For Lump sum qty should be zero
					qty=rset.getString(10)==null?"":qty_unit.equals("0")?"0":nf.format(rset.getDouble(10));
					tcs_tds=rset.getString(11)==null?"":rset.getString(11);
					...
					//cash_flow="Commodity";
					cash_flow="DLNG Transport Service";		//AS PER VIJAY'S FEEDBACK 20251124
					if(contract_type.equals("O") || contract_type.equals("Q")) 
					{
						cash_flow="DLNG TLU Service";
					}
			...
			if(type_flag.equals("SVC")) //AS Per Vijay's Feedback mail on 19/11/2025
			{
				material_code="3344036";//HM20260129: "material_code" material code to be aligned as per LTCORA invoices as per Vijay's feedback
			}
		    ...
		    if(!gross_amt.equals(""))
		    {
		    	...
		    	TaxCode.appendChild(doc.createTextNode(taxCode));
		    	BusinessArea.appendChild(doc.createTextNode(businessAreaCode));
		    	if(type_flag.equals("SVC"))	
				{
		    		String prodPeriodYear=productionPeriodYear.substring(2);
		    		ItemText.appendChild(doc.createTextNode(itemText+" "+monthNm+"-"+prodPeriodYear));		//AS PER VIJAY'S FEEDBACK DLNG Transport Service MMM-YY
				}
		    	else
		    	{
			    ...
			    if(sap_approval_flag.equals("Y"))
		        {
			    	...
			    	if(fileExi.exists()) 
			        {
			        	...
			        	else if(type_flag.equals("SVC"))
			        	{
			        		...
			        		if(count > 0)
			        		{
			        			queryString="UPDATE FMS_DLNG_SVC_INV_FILE_DTL SET FILE_NAME=?,MODIFY_BY=?,MODIFY_DT=SYSDATE "
			        					+"WHERE COMPANY_CD=? AND INVOICE_SEQ=? AND BU_STATE_TIN=? "
			        					+ "AND FINANCIAL_YEAR=? "
			        					+ "AND PDF_TYPE=?";
			        			stmt3 = conn.prepareStatement(queryString);
			        		...
			        		{
			        			queryString="INSERT INTO FMS_DLNG_SVC_INV_FILE_DTL(COMPANY_CD,BU_STATE_TIN,INVOICE_SEQ,FINANCIAL_YEAR,PDF_TYPE,"
	...
	public void generateSalesCRDRInvoiceXML()
	{
		String function_nm="generateSalesInvoiceXML()";
		...
		try
		{
		    ...
		    if(!netPayable.equals(""))
		    {
			   ...
			   // Element Account = doc.createElement("Account");
		    	...
		    	PostingKey.appendChild(doc.createTextNode(pk));
		    	//Account.appendChild(doc.createTextNode(account)); // Will use VendorId to Send SAP Code
		    	CurrencyAmount.appendChild(doc.createTextNode(""+netPayable));
		    	TaxCode.appendChild(doc.createTextNode(taxCode));
		    	BusinessArea.appendChild(doc.createTextNode(businessAreaCode));
		    	Volume.appendChild(doc.createTextNode(qty));
		    	VolumeUnit.appendChild(doc.createTextNode("MMB"));
	...
	public void getSapInvoiceApprovalDetail()
	{
		...
		try
		{
			...
			{
				queryString ="SELECT SAP_APPROVAL,SAP_APPROVED_BY,TO_CHAR(SAP_APPROVED_DT,'DD/MM/YYYY'),FIN_SYS "
						+ "FROM FMS_DLNG_INVOICE_MST "
						+ "WHERE COMPANY_CD=? AND FINANCIAL_YEAR=? "
						+ "AND INVOICE_SEQ=? AND BU_STATE_TIN=? ";
			}
			...
			if(rset.next())
			{
				...
				gen_type=rset.getString(4)==null?"":rset.getString(4);
			}
			rset.close();
			stmt.close();
		}
	...
	public void getSendInvoiceMailDetail()
	{
		...
		try
		{
			...
			{
				String queryString1="SELECT BU_CONTACT_PERSON_CD,CONTACT_PERSON_CD,INVOICE_NO,TO_CHAR(DUE_DT,'DD/MM/YYYY'),"
			...
			{
				if(contract_type.equals("B") || contract_type.equals("M") || contract_type.equals("O") || contract_type.equals("Q"))
				{
					String queryString1="SELECT BU_CONTACT_PERSON_CD,CONTACT_PERSON_CD,INVOICE_NO,TO_CHAR(DUE_DT,'DD/MM/YYYY'),TO_CHAR(INVOICE_DT,'DD/MM/YYYY'),INV_FLAG "
							+ "FROM FMS_DLNG_SVC_INVOICE_MST "
							+ "WHERE COMPANY_CD=? AND COUNTERPARTY_CD=? "
							+ "AND BU_STATE_TIN=? AND FINANCIAL_YEAR=? "
							+ "AND INVOICE_SEQ=?";
				...
				{
					String queryString1="SELECT BU_CONTACT_PERSON_CD,CONTACT_PERSON_CD,INVOICE_NO,TO_CHAR(DUE_DT,'DD/MM/YYYY'),TO_CHAR(INVOICE_DT,'DD/MM/YYYY'),TRUCK_CD,INV_FLAG "
							+ "FROM FMS_DLNG_INVOICE_MST "
							+ "WHERE COMPANY_CD=? AND COUNTERPARTY_CD=? "
							+ "AND BU_STATE_TIN=? AND FINANCIAL_YEAR=? "
							+ "AND INVOICE_SEQ=?";
					...
					if(rset1.next())
					{
						...
						invoiceDt=rset1.getString(5)==null?"":rset1.getString(5);
						truckNo=dlng_util.getTruckRegNo(conn, (rset1.getString(6)==null?"":rset1.getString(6)));
						invFlag=rset1.getString(7)==null?"":rset1.getString(7);
					}
					rset1.close();
					stmt1.close();
				}
...
//					type=other_inv_str;
				...
				else if(invFlag.equals("TLU"))
				{
					type="DLNG TLU Service";
				}
				else if(contract_type.equals("B") || contract_type.equals("M"))
				{
					type="DLNG Service";
				...
				else
				{
					...
					else if(mail_inv_type.equals("SVC"))
					{
						queryString3="SELECT FILE_NAME "
								+ "FROM FMS_DLNG_SVC_INV_FILE_DTL "
								+ "WHERE COMPANY_CD=? AND BU_STATE_TIN=? "
					...
					if(mail_inv_type.equals("F"))
					{
						VMAIL_ATTACHMENT_PATH.add(CommonVariable.signed_dlng_freeflow_inv_path);
					}
					else
					{
		...
		try
		{
			if(flag.equals("FF"))
			{
				...
				while(rset3.next())
				{
					...
					if(pdf_signed.equals("Y"))
					{
						VPDF_FILE_PATH.add(CommonVariable.signed_dlng_freeflow_inv_path);
					}
					else
					{
						VPDF_FILE_PATH.add(CommonVariable.dlng_freeflow_inv_path);
					}
				}
				rset3.close();
				stmt3.close();
			}
			else if(flag.equals("SVC"))
			{
				queryString3="SELECT FILE_NAME,PDF_SIGNED "
						+ "FROM FMS_DLNG_SVC_INV_FILE_DTL "
						+ "WHERE COMPANY_CD=? AND BU_STATE_TIN=? "
				...
				while(rset3.next())
				{
					...
					if(pdf_signed.equals("Y"))
					{
						VPDF_FILE_PATH.add(CommonVariable.signed_dlng_service_inv_path);
					}
					else
					{
						VPDF_FILE_PATH.add(CommonVariable.dlng_service_inv_path);
					}
				}
				rset3.close();
				stmt3.close();
	...
	public void getDriverDetailForForm402()
	{
		...
		try
		{
			String queryString="SELECT DRIVER_NAME,DRIVER_ADDR,LICENCE_NO,LICENCE_ISSUE_STATE "
	...
	public void getTransporterDetail()
	{
		...
		try
		{
			...
			String queryString = "SELECT A.STAT_NO, TO_CHAR(A.EFF_DT,'DD/MM/YYYY'), B.STAT_CD "
					+ "FROM FMS_COUNTERPARTY_PLANT_TAX A, FMS_GOVT_STAT_TAX B "
					+ "WHERE A.COUNTERPARTY_CD=? AND A.ENTITY=? "
					+ "AND A.PLANT_SEQ_NO=? AND A.COMPANY_CD=? "
	...
	public void GenerateExcelForForm402() 
	{
		...
		try
		{
			String queryString="SELECT INVOICE_NO,TO_CHAR(INVOICE_DT,'DD/MM/YYYY'),ALLOC_QTY,GROSS_AMT,"
	...
	public void getCFormMst() 
	{
		...
		try
		{
			String queryString="SELECT COMPANY_CD,CFORM_NO,TO_CHAR(CFORM_DT,'DD/MM/YYYY'),COUNTERPARTY_CD,FINANCIAL_YEAR,ISSUING_STATE,"
		...
		try
		{
			...
			while(rset.next())
			{
				...
				if(plantState.equals(stateName))
				{
					...
					String queryString1="SELECT INVOICE_AMOUNT,TO_CHAR(INVOICE_DT,'DD/MM/YYYY') "
							+ "FROM FMS_CFORM_DTL "
							+ "WHERE COMPANY_CD=? AND CFORM_CD=? "
	...
	public void getReceivableTracking()
	{
		...
		try
		{
			...
			String queryString="SELECT COMPANY_CD,COUNTERPARTY_CD,AGMT_NO,AGMT_REV,CONT_NO,CONT_REV,CONTRACT_TYPE,FINANCIAL_YEAR, "
					+ "BU_STATE_TIN,INVOICE_SEQ,INVOICE_NO,TO_CHAR(INVOICE_DT,'DD/MM/YYYY') AS INVOICE_DT,TO_CHAR(PERIOD_START_DT,'DD/MM/YYYY'),"
					...
					+ "PAY_RECV_AMT,TO_CHAR(PAY_RECV_DT,'DD/MM/YYYY'),PLANT_SEQ,INVOICE_RAISED_IN,BU_UNIT,HOLD_AMT,'S',0 "
					+ "FROM FMS_DLNG_INVOICE_MST "
					+ "WHERE COMPANY_CD=? "
			...
			queryString+=" UNION ALL "
					+ "SELECT COMPANY_CD,COUNTERPARTY_CD,AGMT_NO,AGMT_REV,CONT_NO,CONT_REV,CONTRACT_TYPE,FINANCIAL_YEAR, "
					+ "BU_STATE_TIN,INVOICE_SEQ,INVOICE_NO,TO_CHAR(INVOICE_DT,'DD/MM/YYYY') AS INVOICE_DT,TO_CHAR(PERIOD_START_DT,'DD/MM/YYYY'),"
					+ "TO_CHAR(PERIOD_END_DT,'DD/MM/YYYY'),TO_CHAR(DUE_DT,'DD/MM/YYYY'),SALE_PRICE,SALE_PRICE_UNIT,"
					+ "SALE_AMT,GROSS_AMT,TAX_AMT,TAX_STRUCT_CD,NULL,INVOICE_AMT,NET_PAYABLE_AMT,"
					+ "TCS_TDS,TCS_AMT,TCS_FACTOR,TDS_GROSS_AMT,TDS_GROSS_PERCENT,TDS_TAX_AMT,TDS_TAX_PERCENT,"
					+ "PAY_RECV_AMT,TO_CHAR(PAY_RECV_DT,'DD/MM/YYYY'),PLANT_SEQ,INVOICE_RAISED_IN,BU_UNIT,HOLD_AMT,'SVC',CARGO_NO "
					+ "FROM FMS_DLNG_SVC_INVOICE_MST "
					+ "WHERE COMPANY_CD=? "
					+ "AND INVOICE_DT>=TO_DATE(?,'DD/MM/YYYY') AND INVOICE_DT<=TO_DATE(?,'DD/MM/YYYY') "
					+ "AND APPROVED_FLAG=? AND INVOICE_ID_SEQ IS NOT NULL AND INVOICE_AMT IS NOT NULL "
			...
			while(rset.next())
			{
				...
				String type_flag=rset.getString(38)==null?"":rset.getString(38);
				String cargo_no=rset.getString(39)==null?"":rset.getString(39);
				...
				if(cont_type.equals("B") || cont_type.equals("M"))
				{
					String queryString1="SELECT CONT_REF_NO "
				...
				String queryString1="SELECT TO_CHAR(PAY_RECV_DT,'DD/MM/YYYY'),PAY_RECV_AMT,ENT_BY,PAY_REMARK "
				...
				while(rset3.next())
				{
					...
					String remark=rset3.getString(4)==null?"":rset3.getString(4);
				...
				VRECV_DT.add(tempPaydt);
				VRECV_REMARK.add(tempRemark);
				VRECV_AMT.add(tempPayAmt);
				rset3.close();
				stmt3.close();
	...
	public void getAddressType()
	{
		...
		try
		{
			String queryString="SELECT DISTINCT ADDRESS_TYPE "
					+ "FROM FMS_COUNTERPARTY_ADDR_MST A "
	...
	public void getSacCodeDtl()
	{
		...
		try
		{
			String queryString = "SELECT SAC_CD,SAC_CODE,SAC_DESC,REMARKS,SAC_FLAG "
					+ "FROM FMS_SAC_MST "
					+ "WHERE SAC_FLAG=? ";
			stmt = conn.prepareStatement(queryString);
			stmt.setString(1, "Y");
			...
			while(rset.next())
			{
				 ...
				 String sac_desc = rset.getString(3)==null?"":rset.getString(3);
				 
				 VMST_SAC_CD.add(sac_cd);
				 VMST_SAC_CODE.add(sac_code);
				 VMST_SAC_DESC.add(sac_desc);
			}
			rset.close();
			stmt.close();
		}
	...
	public void getF_FlowInvoice()
	{
		...
		try
		{
			...
			String bank_formula=utilBean.getEntityBankFormula(conn, comp_cd, counterparty_cd, "C", "DLNG", sysdate);
			/*queryString2="SELECT COUNTERPARTY_CD,ENTITY,TO_CHAR(EFF_DT,'DD/MM/YYYY'),BANK_NAME,ACCOUNT_ID,IFSC_CODE,BRANCH_NAME,"
					+ "STATE_NM  "
					+ "FROM FMS_ENTITY_BANK_MST A "
					+ "WHERE ENTITY=? AND COUNTERPARTY_CD=? AND COMPANY_CD=? AND CATEGORY=? "
	...
	public void getExixtingF_FLowInvoice()
	{
		...
		try
		{
			String queryString="SELECT INVOICE_SEQ, INVOICE_NO, INVOICE_NO, TO_CHAR(INVOICE_DT,'DD/MM/YYYY'), TO_CHAR(DUE_DT,'DD/MM/YYYY'),"
			...
			String queryString1="SELECT LINE_NO,LINE_DESC,UNIT,QTY,RATE,AMOUNT "
	...
	public void getF_FlowAnnexureDetail()
	{
		...
		try
		{
			String queryString="SELECT ANNEXURE_SEQ,FILE_NAME "
					+ "FROM FMS_DLNG_FFLOW_INV_ANXRE "
					+ "WHERE COMPANY_CD=? AND INVOICE_SEQ=? AND BU_STATE_TIN=? "
					+ "AND FINANCIAL_YEAR=? AND INVOICE_TYPE=?";
			stmt=conn.prepareStatement(queryString);
	...
	public void getF_FlowInvoiceDetail()
	{
		...
		try
		{
			String queryString="SELECT INVOICE_SEQ, INVOICE_NO, INVOICE_NO, TO_CHAR(INVOICE_DT,'DD-MON-YY'), TO_CHAR(DUE_DT,'DD-MON-YY'),"
					+ "INVOICE_CATEGORY, INVOICE_TYPE, "
					+ "FINANCIAL_YEAR, NUM_LINE, LINKED_INVOICE, NOTE, "
					+ "CONTACT_PERSON_CD,BU_CONTACT_PERSON_CD, "
					+ "CHECKED_FLAG, CHECKED_BY, TO_CHAR(CHECKED_DT,'DD/MM/YYYY HH24:MI:SS'), "
			...
			{
				String queryString1="SELECT CONT_REF_NO,TO_CHAR(DDA_DT,'DD-MON-YY'),TRADE_REF_NO,TO_CHAR(SIGNING_DT,'DD-MON-YY')  "
						+ "FROM FMS_SUPPLY_CONT_MST A "
						+ "WHERE COMPANY_CD=? AND CONTRACT_TYPE=? "
						+ "AND CONT_NO=? AND AGMT_NO=? AND COUNTERPARTY_CD=? "
						+ "AND CONT_REV = (SELECT MAX(CONT_REV) FROM FMS_SUPPLY_CONT_MST B WHERE A.COMPANY_CD=B.COMPANY_CD "
		...
		try
		{
			...
			if(!invoice_prefix.equals(""))
			{
				...
				for(int i=1;i<=no_inv_no;i++)
				{
					...
					if(comp_cd.equals("2") && (contract_type.equals("F") || contract_type.equals("E") || contract_type.equals("W")))
					{
						if(invoice_type.equals("CR"))
						{
							if(bu_state_tin.equals("24")) //GJ
							{
								if(i <= 1)
								{
									count=count+1;
					...
					if(invoice_type.equals("CCR") || invoice_type.equals("CDR")) 
					{
						queryString+= "AND CONTRACT_TYPE IN ("+contType+") ";
					}
					...
					if(invoice_type.equals("TLU"))
					{
						...
						queryString+="SELECT COUNT(*) "
								+ "FROM FMS_FFLOW_INV_MST "
								+ "WHERE COMPANY_CD=? AND BU_STATE_TIN=? "
								+ "AND FINANCIAL_YEAR=? AND INVOICE_TYPE IN ('SI','UG','ST','DI') "
					...
					if(invoice_type.equals("TLU") || invoice_type.equals("CCR") || invoice_type.equals("CDR"))
					{
						stmt.setString(++selCount, comp_cd);
						stmt.setString(++selCount, bu_state_tin);
						stmt.setString(++selCount, financial_year);
					...
					if(invoice_type.equals("CR") || invoice_type.equals("DR"))
					{
						String queryString1="SELECT COUNT(*) "
								+ "FROM FMS_DLNG_INVOICE_MST "
								+ "WHERE COMPANY_CD=? AND FINANCIAL_YEAR=? "
								+ "AND BU_STATE_TIN=? AND INVOICE_ID_SEQ=? AND INV_FLAG=? ";
						stmt1=conn.prepareStatement(queryString1);
						stmt1.setString(1, comp_cd);
					...
					if(count==0)
					{	
						String queryString1="SELECT STATE_CODE "
								+ "FROM FMS_STATE_MST "
								+ "WHERE TIN=?";
						stmt1=conn.prepareStatement(queryString1);
	...
	public void getSegment()
	{
		...
		try
		{
			VSEGMENT.add(utilBean.getContractTypeName("F"));
			VSEGMENT.add(utilBean.getContractTypeName("E"));
			VSEGMENT.add(utilBean.getContractTypeName("W"));

			VSEGMENT.add(utilBean.getContractTypeName("B"));
			VSEGMENT.add(utilBean.getContractTypeName("M"));
	...
	public void getDlngInvoiceActualRptDtl()
	{
		...
		try
		{
			...
			String queryString="SELECT COMPANY_CD,COUNTERPARTY_CD,AGMT_NO,AGMT_REV,CONT_NO,CONT_REV,CONTRACT_TYPE,FINANCIAL_YEAR, "
					...
					+ "'','','',INV_FLAG,FIN_SYS,TRUCK_CD,TRUCK_TRANS_CD,'SG',0 "
					+ "FROM FMS_DLNG_INVOICE_MST "
					+ "WHERE COMPANY_CD=? "
					+ "AND INVOICE_DT>=TO_DATE(?,'DD/MM/YYYY') AND INVOICE_DT<=TO_DATE(?,'DD/MM/YYYY') "
					+ "AND APPROVED_FLAG=? AND INVOICE_ID_SEQ IS NOT NULL AND INVOICE_AMT IS NOT NULL ";
			...
			// SagarB20251006 Added below if-condition for DLNG Sales Actual Report
			if (chk.equals("F"))
			{	
			...
			queryString+= "UNION ALL "
					...
					+ "TCS_AMT,TCS_FACTOR,'',SAP_APPROVAL,"
					+ "'','','',INV_FLAG,FIN_SYS,NULL,NULL,'SVC',CARGO_NO "
					+ "FROM FMS_DLNG_SVC_INVOICE_MST "
					+ "WHERE COMPANY_CD=? "
					+ "AND INVOICE_DT>=TO_DATE(?,'DD/MM/YYYY') AND INVOICE_DT<=TO_DATE(?,'DD/MM/YYYY') "
					+ "AND APPROVED_FLAG=? AND INVOICE_ID_SEQ IS NOT NULL AND INVOICE_AMT IS NOT NULL "
					+ "AND INV_FLAG NOT IN ('CR')";
					//+ "AND SAP_APPROVAL=? "
			// SagarB20251006 Added below if-condition for DLNG Sales Actual Report
			if (chk.equals("F"))
			{	
			...
			while(rset.next())
			{
				...
				String type_flag = rset.getString(50)==null?"":rset.getString(50);
				String cargo_no = rset.getString(51)==null?"":rset.getString(51);
				...
				if(!temp_tds_tax_amt.equals("")){
					VTDS_TAX_AMT.add(nf.format(Math.abs(tds_tax_amt)));
				}else{
					VTDS_TAX_AMT.add("");
				}
				...
				if(!temp_pay_recv_amt.equals("")){
					VPAY_RECV_AMT.add(nf.format(Math.abs(pay_recv_amt)));
				}else{
					VPAY_RECV_AMT.add("");
				}
				VPAY_RECV_DT.add(temp_pay_recv_dt);
				...
				VPERIOD_END_DT.add(period_end_dt);
				VINVOICE_DUE_DT.add(inv_due_dt);
				VSALES_PRICE.add(""+utilBean.RateNumberFormat(Math.abs(sales_price), sales_price_cd));
				VSALES_PRICE_CD.add(sales_price_cd);
				VSALES_PRICE_NM.add(""+utilBean.getRateUnitNm(conn,sales_price_cd));
				...
				// SagarB20251006 Added below block to show QTY_MT values in DLNG Sales Actual Report
				String query1 = "SELECT QTY_MT "
						+ "FROM FMS_DLNG_ALLOC_MST "
						+ "WHERE COMPANY_CD = ? AND COUNTERPARTY_CD = ? AND AGMT_NO = ? "
						+ "AND AGMT_REV = ? AND CONT_NO = ? AND CONT_REV = ? AND CONTRACT_TYPE = ? "
						+ "AND PLANT_SEQ = ? AND BU_SEQ = ? AND GAS_DT >= TO_DATE(?, 'DD/MM/YYYY') "
						+ "AND GAS_DT <= TO_DATE(?, 'DD/MM/YYYY') AND TRUCK_CD = ? AND TRUCK_TRANS_CD = ? ";
				...
				stmt2.setString(9, rset.getString(35));
				stmt2.setString(10, period_st_dt);
				stmt2.setString(11, period_st_dt);
				stmt2.setString(12, truckCd);
				stmt2.setString(13, transCd);
				rset2 = stmt2.executeQuery();
				
				if (rset2.next()) 
				{
					VALLOC_MT.add(rset2.getString(1) == null ? "" : rset2.getString(1));
				}
				else 
				{
					VALLOC_MT.add("");
				...
				VPLANT_SEQ.add(plant_seq);
				VPLANT_ABBR.add(""+utilBean.getCounterpartyPlantABBR(conn,countpty_cd, comp_cd, plant_seq, "C"));
				...
				if(tcs_tds.equals("TCS"))
				{
					VTDS_GROSS_AMT.add(rset.getString(39)==null?"":nf.format(Math.abs(rset.getDouble(39))));
				...
				if(cont_type.equals("M") || cont_type.equals("B") )
				{
					String queryString1="SELECT CONT_REF_NO "
							+ "FROM FMS_SVC_CONT_MST A "
							+ "WHERE COMPANY_CD=? AND CONTRACT_TYPE=? "
			...
			stmt1 = conn.prepareStatement(queryString);
			stmt1.setString(++selCnt, comp_cd);
			stmt1.setString(++selCnt, from_dt);
			stmt1.setString(++selCnt, to_dt);
			stmt1.setString(++selCnt, "Y");
			// SagarB20251006 Added below if-condition for DLNG Sales Actual Report
			if (chk.equals("F")) 
			{	
				stmt1.setString(++selCnt, "Y");
			...
			if(!segment.equals("0") && !segment.equals(""))
			{
				stmt1.setString(++selCnt, segment);
			}
			rset1=stmt1.executeQuery();
			while(rset1.next())
			{
				...
				VBU_NM.add(""+utilBean.getCounterpartyPlantABBR(conn,comp_cd, comp_cd, bu_plant_seq, "B"));
				VPLANT_SEQ.add(plant_seq);
				if (!plant_seq.isEmpty() && Character.isLetter(plant_seq.charAt(0)))
				{
				    plant_seq = plant_seq.substring(1);
				}
				VPLANT_ABBR.add(""+utilBean.getCounterpartyPlantABBR(conn,countpty_cd, comp_cd, plant_seq, "C"));
				...
				{
					VTDS_GROSS_AMT.add(rset1.getString(26)==null?"":nf.format(Math.abs(rset1.getDouble(26))));
	...
	public void getDlngSalesInvoiceList()
	{
		String function_nm="getDlngSalesInvoiceList()";
		
		try
		{
			int count_truck_invoice =0;
			
			queryString5="SELECT TO_CHAR(INVOICE_DT,'DD/MM/YYYY'),INVOICE_SEQ,INVOICE_NO,TRUCK_CD,ALLOC_QTY,"
					+ "TRUCK_TRANS_CD,MAPPING_ID,TO_CHAR(PERIOD_START_DT,'DD/MM/YYYY'),TO_CHAR(PERIOD_END_DT,'DD/MM/YYYY'),"
					+ "AGMT_NO,AGMT_REV,CONT_NO,CONT_REV,CONTRACT_TYPE "
					+ "FROM FMS_DLNG_INVOICE_MST "
					+ "WHERE COMPANY_CD=? AND COUNTERPARTY_CD=? AND CONT_NO=? AND AGMT_NO=? AND CONTRACT_TYPE=? "
					+ "AND PLANT_SEQ=? AND BU_UNIT=? "
					+ "AND INVOICE_DT>=TO_DATE(?,'DD/MM/YYYY') AND INVOICE_DT<=TO_DATE(?,'DD/MM/YYYY') "
					+ "AND BU_STATE_TIN=? "
					+ "ORDER BY INVOICE_DT ASC";
			int stcount=0;
			stmt5=conn.prepareStatement(queryString5);
			stmt5.setString(++stcount, comp_cd);
			stmt5.setString(++stcount, counterparty_cd);
			stmt5.setString(++stcount, sell_cont_no);
			stmt5.setString(++stcount, sell_agmt_no);
			stmt5.setString(++stcount, sell_contract_type);
			stmt5.setString(++stcount, plant_seq);
			stmt5.setString(++stcount, bu_unit);
			stmt5.setString(++stcount, period_start_dt);
			stmt5.setString(++stcount, period_end_dt);
			stmt5.setString(++stcount, bu_state_tin);
			rset5=stmt5.executeQuery();
			while(rset5.next())
			{
				count_truck_invoice++;

				String invoice_dt=rset5.getString(1)==null?"":rset5.getString(1);
				String invoice_seq=rset5.getString(2)==null?"":rset5.getString(2);
				String invoice_no=rset5.getString(3)==null?"":rset5.getString(3);
				String truck_cd=rset5.getString(4)==null?"":rset5.getString(4);
				double alloc_qty=rset5.getDouble(5);
				String truck_trans_cd =rset5.getString(6)==null?"":rset5.getString(6);
				String mapping_id=rset5.getString(7)==null?"":rset5.getString(7);
				String period_start_dt=rset5.getString(8)==null?"":rset5.getString(8);
				String period_end_dt=rset5.getString(9)==null?"":rset5.getString(9);

				String agmtno=rset5.getString(10)==null?"":rset5.getString(10);
				String agmtrev=rset5.getString(11)==null?"":rset5.getString(11);
				String contno=rset5.getString(12)==null?"":rset5.getString(12);
				String contrev=rset5.getString(13)==null?"":rset5.getString(13);
				String cont_type=rset5.getString(14)==null?"":rset5.getString(14);
			
				String contRef="";
				String contStDt="";
				String contEndDt="";
				String queryString1="SELECT CONT_REF_NO,TO_CHAR(START_DT,'DD/MM/YYYY'),TO_CHAR(END_DT,'DD/MM/YYYY') "
						+ "FROM FMS_SUPPLY_CONT_MST A "
						+ "WHERE COMPANY_CD=? AND CONTRACT_TYPE=? "
						+ "AND CONT_NO=? AND AGMT_NO=? AND COUNTERPARTY_CD=? "
						+ "AND CONT_REV = (SELECT MAX(CONT_REV) FROM FMS_SUPPLY_CONT_MST B WHERE A.COMPANY_CD=B.COMPANY_CD "
				...
				{
					contRef=rset1.getString(1)==null?"":rset1.getString(1);
					contStDt=rset1.getString(2)==null?"":rset1.getString(2);
					contEndDt=rset1.getString(3)==null?"":rset1.getString(3);
				}
				rset1.close();
				stmt1.close();
				
				String deal_no=utilBean.NewDealMappingId(comp_cd, counterparty_cd, agmtno, agmtrev, contno, contrev, cont_type, "");//+" <br>["+contRef+"]";
				
				VSALES_INV_DT.add(invoice_dt);
				VSALES_INVOICE_SEQ.add(invoice_seq);
				VSALES_INVOICE_NO.add(invoice_no);
				VSALES_TRUCK_CD.add(truck_cd);
				VSALES_TRUCK_REG_NO.add(dlng_util.getTruckRegNo(conn, truck_cd));
				VSALES_INV_QTY.add(nf2.format(alloc_qty));
				VSALES_TRUCK_TRANS_CD.add(truck_trans_cd);
				VSALES_MAPPING_ID.add(mapping_id);
				VSELL_CONT_PERIOD_START_DT.add(contStDt);
				VSELL_CONT_PERIOD_END_DT.add(contEndDt);
				VSELL_PERIOD_START_DT.add(period_start_dt);
				VSELL_PERIOD_END_DT.add(period_end_dt);
				VSALES_CONT_MAPP.add(deal_no);
				
				int checkSalesInvLinked = isSalesInvLinked(invoice_seq, invoice_no);
				
				if(checkSalesInvLinked>0)
				{
					VSALES_INV_LINKED.add("Y");
				}
				else
				{
					int checkSalesInvLinkedElseWhere = isSalesInvLinkedElseWhere(invoice_seq, invoice_no);
					
					if(checkSalesInvLinkedElseWhere>0)
					{
						VSALES_INV_LINKED.add("EL");
					}
					else
					{
						VSALES_INV_LINKED.add("N");
					}
				}
			}
			rset5.close();
			...
			total_truck_invoice = ""+count_truck_invoice;
		}
		catch(Exception e)
		{
			new SystemErrorLogger().InsertErrorLogger(db_src_file_name, function_nm, e);
	...
	private int isSalesInvLinked(String sell_invoice_seq, String sell_invoice_no) 
	{
		String function_nm="isSalesInvLinked()";
		int isSalesInvLinked = 0;
		...
		try 
		{
			...
			String queryString="SELECT COUNT(*) "
					...
					+ "AND CONTRACT_TYPE=? "
					+ "AND FINANCIAL_YEAR=? "
					+ "AND BU_UNIT=? AND BU_STATE_TIN=? "
					+ "AND SVC_INVOICE_SEQ=? "
					+ "AND INVOICE_SEQ=? "
					+ "AND INVOICE_NO=? ";
			stmt_temp=conn.prepareStatement(queryString);
			stmt_temp.setString(++selCnt, comp_cd);
			stmt_temp.setString(++selCnt, counterparty_cd);
			stmt_temp.setString(++selCnt, agmt_no);
			stmt_temp.setString(++selCnt, cont_no);
			stmt_temp.setString(++selCnt, contract_type);
			stmt_temp.setString(++selCnt, exist_financial_year);
			stmt_temp.setString(++selCnt, bu_unit);
			stmt_temp.setString(++selCnt, bu_state_tin);
			stmt_temp.setString(++selCnt, invoice_seq);
			stmt_temp.setString(++selCnt, sell_invoice_seq);
			stmt_temp.setString(++selCnt, sell_invoice_no);
			rset_temp=stmt_temp.executeQuery();
			if(rset_temp.next())
			{
				isSalesInvLinked = rset_temp.getInt(1);
			}
			rset_temp.close();
			stmt_temp.close();
		} 
		...
		return isSalesInvLinked;
	}

	private int isSalesInvLinkedElseWhere(String sell_invoice_seq, String sell_invoice_no) 
	{
		String function_nm="isSalesInvLinkedElseWhere()";
		int isSalesInvLinkedElseWhere = 0;
		...
		try 
		{
			String queryString="SELECT COUNT(*) "
					...
					+ "AND CONT_NO !=? "
					+ "AND BU_UNIT=? AND BU_STATE_TIN=? "
					+ "AND (SVC_INVOICE_SEQ != ? OR SVC_INVOICE_SEQ IS NOT NULL)"
					+ "AND INVOICE_SEQ=? "
					+ "AND INVOICE_NO=? ";
			stmt_temp=conn.prepareStatement(queryString);
			...
			stmt_temp.setString(4, bu_unit);
			stmt_temp.setString(5, bu_state_tin);
			stmt_temp.setString(6, invoice_seq);
			stmt_temp.setString(7, sell_invoice_seq);
			stmt_temp.setString(8, sell_invoice_no);
			rset_temp=stmt_temp.executeQuery();
			if(rset_temp.next())
			{
				isSalesInvLinkedElseWhere = rset_temp.getInt(1);
			}
			rset_temp.close();
			stmt_temp.close();
		} 
		...
		return isSalesInvLinkedElseWhere;
	}

	public void getExistingServiceInvoiceDtl()
	{
		String function_nm="getExistingServiceInvoiceDtl()";
		try
		{
			String queryString="SELECT BU_CONTACT_PERSON_CD,CONTACT_PERSON_CD,INVOICE_SEQ,INVOICE_NO,TO_CHAR(INVOICE_DT,'DD/MM/YYYY'),"
					+ "TO_CHAR(DUE_DT,'DD/MM/YYYY'),QTY,SALE_PRICE,SALE_PRICE_UNIT,SALE_AMT,EXCHG_RATE_CD,TO_CHAR(EXCHG_RATE_DT,'DD/MM/YYYY'),"
					+ "EXCHG_RATE_VALUE,INVOICE_RAISED_IN,GROSS_AMT,TAX_AMT,TAX_STRUCT_CD,NULL,INVOICE_AMT,NET_PAYABLE_AMT,"
					+ "REMARK_1,REMARK_2,TCS_TDS,TCS_AMT,TCS_FACTOR,NULL,NULL,TCS_STRUCT_CD,NULL,"
					+ "TDS_GROSS_AMT,TDS_GROSS_PERCENT,TDS_STRUCT_CD,NULL,NULL,NULL,NULL,NULL,"
					+ "TO_CHAR(ENT_DT,'DD/MM/YYYY HH24:MI:SS'),TO_CHAR(APPROVED_DT,'DD/MM/YYYY HH24:MI:SS'),FINANCIAL_YEAR,QTY_UNIT,DIST_UNIT "
					+ "FROM FMS_DLNG_SVC_INVOICE_MST "
					+ "WHERE COMPANY_CD=? AND COUNTERPARTY_CD=? AND CONT_NO=? "
					+ "AND AGMT_NO=? AND PLANT_SEQ=? AND BU_UNIT=? AND CONTRACT_TYPE=? AND BU_STATE_TIN=? "
					+ "AND PERIOD_START_DT=TO_DATE(?,'DD/MM/YYYY') AND PERIOD_END_DT=TO_DATE(?,'DD/MM/YYYY') AND FINANCIAL_YEAR=? "
			...
			if(rset.next())
			{
				...
				if(operation.equals("INSERT"))
				{
					...
					String fiscal_yr=rset.getString(40)==null?"":rset.getString(40);
					qty_unit=rset.getString(41)==null?"":rset.getString(41);
					qty_unit_nm=utilBean.getEnergyUnitNm(conn, qty_unit);
					dist_unit=rset.getString(42)==null?"":rset.getString(42);
	...
	public void getTcsTdsSvcInvDtl()
	{
		String function_nm="getTcsTdsSvcInvDtl()";
		try
		{
			double total=0;
			int cnt=0;
			...
			if(contract_type.equals("B") || contract_type.equals("M"))
			{
				stmt.setString(++cnt, "B");
				stmt.setString(++cnt, "M");
				
			}
			else
			{
				stmt.setString(++cnt, "O");
				stmt.setString(++cnt, "Q");
			}
			stmt.setString(++cnt, financial_year);
			stmt.setString(++cnt, "Y");
			stmt.setString(++cnt, invoice_dt);
			rset=stmt.executeQuery();
			while(rset.next())
			{
				String agmtno=rset.getString(1)==null?"":rset.getString(1);
	...
	public void getSvcContractDetail()
	{
		String function_nm="getSvcContractDetail()";
		try
		{
			couterpty_abbr=""+utilBean.getCounterpartyABBR(conn,counterparty_cd);
			couterpty_nm=""+utilBean.getCounterpartyName(conn,counterparty_cd);
			...
			String queryString="SELECT COMPANY_CD,COUNTERPARTY_CD,AGMT_NO,AGMT_REV,CONT_NO,CONT_REV,"
					+ "TO_CHAR(START_DT,'DD/MM/YYYY'),TO_CHAR(END_DT,'DD/MM/YYYY'),CONT_NAME,CONT_REF_NO,CONTRACT_TYPE,"
					+ "TRANSPORT_MGMT_CHARGE,TRANSPORT_MGMT_UNIT ";
			queryString+= "FROM FMS_SVC_CONT_MST ";
			queryString+= "WHERE COMPANY_CD=? AND COUNTERPARTY_CD=? "
					+ "AND AGMT_NO=? AND AGMT_REV=? "
					+ "AND CONT_NO=? AND CONT_REV=? "
					+ "AND CONTRACT_TYPE=?";
			stmt=conn.prepareStatement(queryString);
			...
			if(rset.next())
			{
				...
				transport_mgmt_charge=rset.getString(12)==null?"":rset.getString(12);
				transport_mgmt_unit=rset.getString(13)==null?"":rset.getString(13);
				
				if(transport_mgmt_unit.equals("1"))
				{
					price_cd = "1";
					transport_mgmt_unit_nm = "INR/MMBTU";
					qty_unit = "1";
					qty_unit_nm = "MMBTU";
				}
				else if(transport_mgmt_unit.equals("2"))
				{
					price_cd = "2";
					transport_mgmt_unit_nm = "USD/MMBTU";
					qty_unit = "1";
					qty_unit_nm = "MMBTU";
				}
				else if(transport_mgmt_unit.equals("3"))
				{
					price_cd = "1";
					transport_mgmt_unit_nm = "INR/Truck";
					qty_unit = "0";
					qty_unit_nm = "TRUCK";
				}
				else if(transport_mgmt_unit.equals("4"))
				{
					price_cd = "1";
					transport_mgmt_unit_nm = "INR/MT";
					qty_unit = "5";
					qty_unit_nm = "MT";
				}
				else if(transport_mgmt_unit.equals("5"))
				{
					price_cd = "1";
					transport_mgmt_unit_nm = "INR/KM";
					qty_unit = "0";
					qty_unit_nm = "KM";
				}
				else
				...
				//for bu unit
				String queryString1 = "SELECT A.STAT_NO, TO_CHAR(A.EFF_DT,'DD/MM/YYYY'), B.STAT_NM "
						+ "FROM FMS_COUNTERPARTY_PLANT_TAX A, FMS_GOVT_STAT_TAX B "
						+ "WHERE A.COUNTERPARTY_CD=? AND A.ENTITY=? "
						+ "AND A.PLANT_SEQ_NO=? AND A.COMPANY_CD=? AND B.STAT_CD='1003' " //GSTIN Number cd
						+ "AND A.STAT_CD=B.STAT_CD AND A.STAT_NO IS NOT NULL";
	...
	public void getDlngServiceInvoicePreparationList()
	{
		String function_nm="getDlngServiceInvoicePreparationList()";
		...
		try
		{
			...
			if(billing_cycle.equals("8"))
			{
				...
				String queryString="SELECT A.COMPANY_CD,A.COUNTERPARTY_CD,A.AGMT_NO,A.AGMT_REV,A.CONT_NO,A.CONT_REV,"
						+ "TO_CHAR(A.START_DT,'DD/MM/YYYY'),TO_CHAR(A.END_DT,'DD/MM/YYYY'),A.CONT_NAME,A.CONT_REF_NO,A.CONTRACT_TYPE,"
						+ "TO_CHAR(C.EFF_DT,'DD/MM/YYYY'),A.FCC_FLAG,A.FILL_STATION_CD,A.PLANT_SEQ_NO,C.BILLING_DAYS "
						+ "FROM FMS_SVC_CONT_MST A, "
							+ "FMS_SVC_CONT_BILLING_DTL C "
						+ "WHERE A.COMPANY_CD = ? "
						+ "AND A.START_DT<=TO_DATE(?,'DD/MM/YYYY') AND A.END_DT>=TO_DATE(?,'DD/MM/YYYY') AND A.CONTRACT_TYPE IN ('B','M') "
						+ "AND A.CONT_REV = (SELECT MAX(B.CONT_REV) FROM FMS_SVC_CONT_MST B WHERE A.COMPANY_CD=B.COMPANY_CD "
						+ "AND A.COUNTERPARTY_CD=B.COUNTERPARTY_CD AND A.CONT_NO=B.CONT_NO AND A.AGMT_NO=B.AGMT_NO AND A.AGMT_REV=B.AGMT_REV "
						+ "AND A.CONTRACT_TYPE=B.CONTRACT_TYPE) "
						+ ""
						+ "AND A.COMPANY_CD=C.COMPANY_CD AND A.COUNTERPARTY_CD=C.COUNTERPARTY_CD AND A.CONT_NO=C.CONT_NO "
						+ "AND A.AGMT_NO=C.AGMT_NO AND A.CONTRACT_TYPE=C.CONTRACT_TYPE AND A.PLANT_SEQ_NO=C.PLANT_SEQ_NO AND C.BILLING_FREQ=? AND C.BILLING_FLAG=? "
						+ ""
						+ "AND ((C.EFF_DT>=TO_DATE(?,'DD/MM/YYYY') AND C.EFF_DT<=TO_DATE(?,'DD/MM/YYYY')) "
						+ ""
						+ "OR C.EFF_DT=(SELECT MAX(E.EFF_DT) FROM FMS_SVC_CONT_BILLING_DTL E WHERE C.COMPANY_CD=E.COMPANY_CD "
							+ "AND C.COUNTERPARTY_CD=E.COUNTERPARTY_CD AND C.AGMT_NO=E.AGMT_NO AND C.CONT_NO=E.CONT_NO "
							+ "AND C.CONTRACT_TYPE=E.CONTRACT_TYPE AND E.PLANT_SEQ_NO=C.PLANT_SEQ_NO AND E.EFF_DT<=TO_DATE(?,'DD/MM/YYYY'))) "
						+ "AND CONT_STATUS= ? ";
				stmt=conn.prepareStatement(queryString);
				stmt.setString(count++, comp_cd);
				stmt.setString(count++, temp_period_end_dt);
				stmt.setString(count++, temp_period_start_dt);
				stmt.setString(count++, billing_freq);
				stmt.setString(count++, billing_flag);
				stmt.setString(count++, temp_period_start_dt);
				stmt.setString(count++, temp_period_end_dt);
				stmt.setString(count++, temp_period_start_dt);
				...
				while(rset.next())
				{
					...
					String billing_eff_dt=rset.getString(12)==null?"":rset.getString(12);
					String fcc=rset.getString(13)==null?"":rset.getString(13);
					String filling_station_cd=rset.getString(14)==null?"":rset.getString(14);
					...
					for(int i=0;i<tot_row;i++)
					{
						...
						queryString4="SELECT DISTINCT TO_CHAR(EFF_DT,'DD/MM/YYYY'),BILLING_DAYS,BILLING_FREQ "
								+ "FROM FMS_SVC_CONT_BILLING_DTL A "
								+ "WHERE COMPANY_CD=? AND COUNTERPARTY_CD=? "
						...
						if(rset4.next())
						{
							innerBillingEffDt=rset4.getString(1)==null?"":rset4.getString(1);
							innerBillingDays=rset4.getString(2)==null?"1":rset4.getString(2);
							innerBillingFreq=rset4.getString(3)==null?"":rset4.getString(3);
						}
						rset4.close();
						stmt4.close();
					...
					String sell_cont_map = "", sell_cont_type="", sell_agmt_no="", sell_agmt_rev="", sell_cont_no="", sell_cont_rev="";
					
					queryString3="SELECT SELL_CONT_MAP "
					...
					stmt3.setString(5, agmtno);
					stmt3.setString(6, agmtrev);
					stmt3.setString(7, cont_type);
					rset3=stmt3.executeQuery();
					if(rset3.next())
					{
						sell_cont_map = rset3.getString(1)==null?"":rset3.getString(1);
						
						sell_cont_type = sell_cont_map.split("-")[0];
						sell_agmt_no = sell_cont_map.split("-")[1];
						sell_agmt_rev = sell_cont_map.split("-")[2];
						sell_cont_no = sell_cont_map.split("-")[3];
						sell_cont_rev = sell_cont_map.split("-")[4];
					}
					rset3.close();
					stmt3.close();
					...
					while(rset2.next())
					{
						...
						queryString0 = "SELECT INVOICE_NO "
								+ "FROM FMS_DLNG_INVOICE_MST "
								+ "WHERE COMPANY_CD = ? AND COUNTERPARTY_CD=? "
								+ "AND CONTRACT_TYPE = ? AND AGMT_NO = ? AND AGMT_REV = ? AND CONT_NO = ? "
								//+ "AND CONT_REV = ? "
								+ "AND INVOICE_DT <= TO_DATE(?,'DD/MM/YYYY') AND INVOICE_DT >= TO_DATE(?,'DD/MM/YYYY') "
								+ "AND PLANT_SEQ = ? AND BU_UNIT = ? "
								+ "AND PDF_INV_DTL IS NOT NULL ";
						...
						stmt0.setString(3, sell_cont_type);
						stmt0.setString(4, sell_agmt_no);
						stmt0.setString(5, sell_agmt_rev);
						stmt0.setString(6, sell_cont_no);
						//stmt0.setString(7, sell_cont_rev);
						stmt0.setString(7, temp_end_dt);
						stmt0.setString(8, temp_st_dt);
						stmt0.setString(9, plant_seq);
						stmt0.setString(10, bu_plant_seq);
						rset0 = stmt0.executeQuery();
						if(rset0.next()) 
						{
							...
							VSELL_AGMT_NO.add(sell_agmt_no);
							VSELL_AGMT_REV_NO.add(sell_agmt_rev);
							VSELL_CONT_NO.add(sell_cont_no);
							VSELL_CONT_REV_NO.add(sell_cont_rev);
							VSELL_CONTRACT_TYPE.add(sell_cont_type);
							VSELL_CONT_MAP.add(sell_cont_map);

							String sell_cont_ref_no="";
							
							String queryString5="SELECT CONT_REF_NO "
									+ "FROM FMS_SUPPLY_CONT_MST "
									+ "WHERE COMPANY_CD=? AND COUNTERPARTY_CD=? "
									+ "AND AGMT_NO=? AND AGMT_REV=? "
									+ "AND CONT_NO=? AND CONT_REV=? "
							...
							stmt5.setString(3, sell_agmt_no);
							stmt5.setString(4, sell_agmt_rev);
							stmt5.setString(5, sell_cont_no);
							stmt5.setString(6, sell_cont_rev);
							stmt5.setString(7, sell_cont_type);
							rset5 = stmt5.executeQuery();
							while(rset5.next()) 
							{
								sell_cont_ref_no = rset5.getString(1)==null?"":rset5.getString(1);
							}
							rset5.close();
							stmt5.close();
							
							VSELL_DISP_CONT.add(utilBean.NewDealMappingId(comp_cd, countpty_cd, sell_agmt_no, sell_agmt_rev, sell_cont_no, sell_cont_rev, sell_cont_type, cargo_no));
							VSELL_CONT_REF.add(sell_cont_ref_no);
							
							double totalTruckInvQty = getTotalTruckInvQty(own_cd, plant_seq, bu_plant_seq,countpty_cd, sell_agmt_no, sell_agmt_rev, sell_cont_no, sell_cont_rev, sell_cont_type);
							
							VTOTAL_TRUCK_INV_QTY.add(totalTruckInvQty);
							
							String inv_flag="F";
							VINV_FLAG.add(inv_flag);
							getSvcInvDetailForPreparationList(own_cd,countpty_cd,contno, agmtno,cont_type, plant_seq, 
							bu_plant_seq, temp_st_dt, temp_end_dt,state_code,inv_flag,
							sell_agmt_no,sell_agmt_rev,sell_cont_no,sell_cont_rev,sell_cont_type,sell_cont_map,cargo_no);
						}
						rset0.close();
						stmt0.close();
					}
			...
			else
			{
				String queryString="SELECT A.COMPANY_CD,A.COUNTERPARTY_CD,A.AGMT_NO,A.AGMT_REV,A.CONT_NO,A.CONT_REV,"
						+ "TO_CHAR(A.START_DT,'DD/MM/YYYY'),TO_CHAR(A.END_DT,'DD/MM/YYYY'),A.CONT_NAME,A.CONT_REF_NO,A.CONTRACT_TYPE,"
						+ "TO_CHAR(C.EFF_DT,'DD/MM/YYYY'),A.FCC_FLAG,A.FILL_STATION_CD,A.PLANT_SEQ_NO,D.PLANT_SEQ_NO, E.SELL_CONT_MAP "
						+ "FROM FMS_SVC_CONT_MST A, "
							+ "FMS_SVC_CONT_BILLING_DTL C, "
							+ "FMS_SVC_CONT_BU D, "
							+ "FMS_SVC_CONT_MAP E "
						...
						+ "AND A.COMPANY_CD=C.COMPANY_CD AND A.COUNTERPARTY_CD=C.COUNTERPARTY_CD AND A.CONT_NO=C.CONT_NO "
						+ "AND A.AGMT_NO=C.AGMT_NO AND A.CONTRACT_TYPE=C.CONTRACT_TYPE AND A.PLANT_SEQ_NO=C.PLANT_SEQ_NO AND C.BILLING_FREQ=? AND C.BILLING_FLAG=? "
						+ "AND C.EFF_DT=(SELECT MAX(E.EFF_DT) FROM FMS_SVC_CONT_BILLING_DTL E WHERE C.COMPANY_CD=E.COMPANY_CD "
							+ "AND C.COUNTERPARTY_CD=E.COUNTERPARTY_CD AND C.AGMT_NO=E.AGMT_NO AND C.CONT_NO=E.CONT_NO "
							+ "AND C.CONTRACT_TYPE=E.CONTRACT_TYPE AND E.PLANT_SEQ_NO=C.PLANT_SEQ_NO AND E.EFF_DT<=TO_DATE(?,'DD/MM/YYYY')) "
						+ ""
						+ "AND A.COMPANY_CD=D.COMPANY_CD AND A.COUNTERPARTY_CD=D.COUNTERPARTY_CD AND A.CONT_NO=D.CONT_NO AND A.CONT_REV=D.CONT_REV "
				...
				while(rset.next())
				{
					...
					String bu_plant_seq = rset.getString(16)==null?"":rset.getString(16);
					String bu_plant_abbr=utilBean.getCounterpartyPlantABBR(conn,own_cd, own_cd, bu_plant_seq, "B");
					
					String state_code=utilBean.getState_TIN(conn, own_cd, own_cd, "B", bu_plant_seq);
					
					String sell_cont_map = rset.getString(17)==null?"":rset.getString(17);
					
					String sell_cont_type = sell_cont_map.split("-")[0];
					String sell_agmt_no = sell_cont_map.split("-")[1];
					String sell_agmt_rev = sell_cont_map.split("-")[2];
					String sell_cont_no = sell_cont_map.split("-")[3];
					String sell_cont_rev = sell_cont_map.split("-")[4];
					...
					stmt0.setString(6, sell_cont_no);
					//stmt0.setString(7, sell_cont_rev);
					stmt0.setString(7, period_end_dt);
					stmt0.setString(8, period_start_dt);
					stmt0.setString(9, plant_seq);
					stmt0.setString(10, bu_plant_seq);
					rset0 = stmt0.executeQuery();
					if(rset0.next()) 
					{
						...
						VINV_FLAG.add(inv_flag);
						getSvcInvDetailForPreparationList(own_cd,countpty_cd,contno, agmtno,cont_type, plant_seq, 
						bu_plant_seq, period_start_dt, period_end_dt,state_code,inv_flag, 
						sell_agmt_no,sell_agmt_rev,sell_cont_no,sell_cont_rev,sell_cont_type,sell_cont_map,cargo_no);
					}
					rset0.close();
					stmt0.close();
	...
	private double getTotalTruckInvQty(String own_cd, String plant_seq , String bu_plant_seq,String countpty_cd, String sell_agmt_no, String sell_agmt_rev,
			String sell_cont_no, String sell_cont_rev, String sell_cont_type) 
	{
		String function_nm="getTotalTruckInvQty()";
		double totalTruckInvQty=0.00;
		
		try
		{
			String qString = "SELECT SUM(ALLOC_QTY) "
					+ "FROM FMS_DLNG_INVOICE_MST "
					+ "WHERE COMPANY_CD = ? AND COUNTERPARTY_CD=? "
					+ "AND CONTRACT_TYPE = ? AND AGMT_NO = ? AND AGMT_REV = ? AND CONT_NO = ? "
					//+ "AND CONT_REV = ? "
					+ "AND INVOICE_DT <= TO_DATE(?,'DD/MM/YYYY') AND INVOICE_DT >= TO_DATE(?,'DD/MM/YYYY') AND PLANT_SEQ = ? AND BU_UNIT = ? ";
			stmt_temp = conn.prepareStatement(qString);
			stmt_temp.setString(1, own_cd);
			stmt_temp.setString(2, countpty_cd);
			stmt_temp.setString(3, sell_cont_type);
			stmt_temp.setString(4, sell_agmt_no);
			stmt_temp.setString(5, sell_agmt_rev);
			stmt_temp.setString(6, sell_cont_no);
			//stmt_temp.setString(7, sell_cont_rev);
			stmt_temp.setString(7, period_end_dt);
			stmt_temp.setString(8, period_start_dt);
			stmt_temp.setString(9, plant_seq);
			stmt_temp.setString(10, bu_plant_seq);
			rset_temp = stmt_temp.executeQuery();
			if(rset_temp.next()) 
			{
				totalTruckInvQty = rset_temp.getDouble(1);
			}
			rset_temp.close();
			stmt_temp.close();
		}
		...
		return totalTruckInvQty;
	}

	public void getSvcInvoiceCalculation()
	{
		String function_nm="getSvcInvoiceCalculation()";
		try
		{
			if((operation.equals("INSERT") && !submission_chk))// || operation.equals("MODIFY"))
			{
			...
			if(!inv_flag.equals("LP"))
			{
				if(transport_mgmt_unit.equals("4"))//MT
				{
					if(qty_mmbtu.isEmpty()) 
					{
						qty_mmbtu = qtyMmbtu;
					}
					qty_mmbtu =nf.format(Double.parseDouble(qty_mmbtu)/mmbtuToMtConv);
					qtyMmbtu =nf.format(Double.parseDouble(qtyMmbtu)/mmbtuToMtConv);
				}
				
				if(!qtyMmbtu.equals(qty_mmbtu))
				{
					qty_mmbtu = qtyMmbtu;
				}
				...
				if((operation.equals("INSERT") && !submission_chk) || operation.equals("MODIFY"))
				{
					gross_amt = nf.format(Double.parseDouble(qtyMmbtu) * Double.parseDouble(price));
				}
				...
				if((operation.equals("INSERT") && !submission_chk) || operation.equals("MODIFY"))
				{
					Vector temp = new Vector();
					gross_include_transport_tariff=gross_amt1;	
					temp=TaxCalc.ServiceTaxAmountCalculationWithInfo(conn, comp_cd, counterparty_cd, "C", plant_seq, bu_unit, period_start_dt, "DTM", gross_include_transport_tariff);
					tax_amt = nf.format(Double.parseDouble(""+temp.elementAt(0)));
					tax_struct_cd = ""+temp.elementAt(1);
					tax_struct_dt = ""+temp.elementAt(2);
					tax_info = ""+temp.elementAt(3);
...
//				}
				...
				if((operation.equals("INSERT") && !submission_chk) || operation.equals("MODIFY"))
				{
					Vector temp = new Vector();
					gross_include_transport_tariff=gross_amt1;	
					temp=TaxCalc.ServiceTaxAmountCalculationWithInfo(conn, comp_cd, counterparty_cd, "C", plant_seq, bu_unit, cont_start_dt, "DTM", gross_include_transport_tariff);
					tax_amt = nf.format(Double.parseDouble(""+temp.elementAt(0)));
					tax_struct_cd = ""+temp.elementAt(1);
					tax_struct_dt = ""+temp.elementAt(2);
					tax_info = ""+temp.elementAt(3);
	...
	public void getSvcInvDetailForPreparationList(String own_cd,String countpty_cd, String contno, String agmtno,String cont_type, String plant_seq, 
			String bu_plant_seq, String period_start_dt, String period_end_dt,String state_code,String inv_flag,
			String sell_agmt_no,String sell_agmt_rev,String sell_cont_no,String sell_cont_rev,String sell_cont_type,String sell_cont_map,String cargo_no)
	{
		String function_nm="getSvcInvDetailForPreparationList()";
		try
		{
			String inv_no="";
			String inv_seq="";
			...
			queryString5="SELECT INVOICE_NO,CHECKED_FLAG,APPROVED_FLAG,INVOICE_SEQ,PDF_INV_DTL,"
					+ "PRINT_BY_ORI,PRINT_BY_TRI,PRINT_BY_DUP,SAP_APPROVAL,FINANCIAL_YEAR,FIN_SYS,QTY,TO_CHAR(INVOICE_DT,'DD-MON-YY') "
					+ "FROM FMS_DLNG_SVC_INVOICE_MST "
					+ "WHERE COMPANY_CD=? AND COUNTERPARTY_CD=? AND CONT_NO=? AND AGMT_NO=? AND CONTRACT_TYPE=? "
					+ "AND PLANT_SEQ=? AND BU_UNIT=? "
					+ "AND PERIOD_START_DT=TO_DATE(?,'DD/MM/YYYY') AND PERIOD_END_DT=TO_DATE(?,'DD/MM/YYYY') "
					+ "AND BU_STATE_TIN=? AND INV_FLAG=? ";
			if(inv_flag.equals("TLU")) 
			{
				queryString5+= "AND CARGO_NO=?";
			}
			int stcount=0;
			stmt5=conn.prepareStatement(queryString5);
			stmt5.setString(++stcount, own_cd);
			...
			if(inv_flag.equals("TLU")) 
			{
				stmt5.setString(++stcount, cargo_no);
			}
			rset5=stmt5.executeQuery();
			if(rset5.next())
			{
				...
				fiscal_yr=rset5.getString(10)==null?"":rset5.getString(10);
				fin_sys=rset5.getString(11)==null?"":rset5.getString(11);
				String qty=rset5.getString(12)==null?"":rset5.getString(12);
				inv_dt=rset5.getString(13)==null?"":rset5.getString(13);
				
				VINVOICE_EXIST.add("Y");
				
				VTOTAL_TRUCK_INV_QTY.add(qty);
			}
			else
			{
			...
			if(print_pdf_type.equals("O") && !pdf_ori.isEmpty())
			{
				VPDF_TYPE.add(print_pdf_type);
			}
			else if(print_pdf_type.equals("D") && !pdf_dup.isEmpty())
			{
				VPDF_TYPE.add(print_pdf_type);
			}
			else if(print_pdf_type.equals("T") && !pdf_tri.isEmpty())
			{
				VPDF_TYPE.add(print_pdf_type);
			}
			else if(print_pdf_type.equals("All"))
	...
	public void getSvcInvoiceDetail()
	{
		String function_nm="getSvcInvoiceDetail()";
		try
		{
			String queryString="SELECT BU_CONTACT_PERSON_CD,CONTACT_PERSON_CD,INVOICE_NO,TO_CHAR(INVOICE_DT,'DD-MON-YY'),"
					+ "TO_CHAR(DUE_DT,'DD-MON-YY'),TO_CHAR(PERIOD_START_DT,'DD-MON-YY'),TO_CHAR(PERIOD_END_DT,'DD-MON-YY'),REMARK_1,REMARK_2,"
					...
					+ "INVOICE_AMT,NET_PAYABLE_AMT,TCS_TDS,TCS_AMT,TCS_FACTOR,CHECKED_FLAG,APPROVED_FLAG, "
					+ "INVOICE_ID_SEQ,NULL,NULL,EXCHG_RATE_CD,TO_CHAR(EXCHG_RATE_DT,'DD/MM/YYYY'),EXCHG_RATE_TYPE,"
					+ "INVOICE_RAISED_IN, SAC_CD, QTY_UNIT, DIST_UNIT "
					+ "FROM FMS_DLNG_SVC_INVOICE_MST "
					+ "WHERE COMPANY_CD=? AND COUNTERPARTY_CD=? AND CONT_NO=? AND AGMT_NO=? AND PLANT_SEQ=? AND BU_UNIT=? AND CONTRACT_TYPE=? "
					+ "AND BU_STATE_TIN=? AND PERIOD_START_DT=TO_DATE(?,'DD/MM/YYYY') AND PERIOD_END_DT=TO_DATE(?,'DD/MM/YYYY') "
					+ "AND FINANCIAL_YEAR=? AND INVOICE_SEQ=? AND INV_FLAG=?  AND CARGO_NO=? ";
			if(contract_type.equals("B") || contract_type.equals("M")) {
				queryString+= "AND SELL_CONT_MAP = ?";
			}
			stmt=conn.prepareStatement(queryString);
			stmt.setString(1, comp_cd);
			stmt.setString(2, counterparty_cd);
			...
			if(contract_type.equals("B") || contract_type.equals("M")) {
				stmt.setString(15, sell_cont_map);
			}
			rset=stmt.executeQuery();
			if(rset.next())
			{
				...
				qty_mmbtu=rset.getString(10)==null?"":rset.getString(10);
				price_cd=rset.getString(12)==null?"":rset.getString(12);
				price=utilBean.RateNumberFormat(rset.getDouble(11), price_cd);
				gross_amt=nf.format(rset.getDouble(13));
				exchang_rate=nf2.format(rset.getDouble(14));
				...
				sac_cd=rset.getString(33)==null?"":rset.getString(33);
				
				String queryString1 = "SELECT SAC_CODE,SAC_DESC,REMARKS,SAC_FLAG "
						+ "FROM FMS_SAC_MST "
						+ "WHERE SAC_FLAG=? AND SAC_CD=? ";
				stmt1 = conn.prepareStatement(queryString1);
				stmt1.setString(1, "Y");
				stmt1.setString(2, sac_cd);
				rset1 = stmt1.executeQuery();
				while(rset1.next())
				{
				...
				String qty_unit  = rset.getString(34)==null?"":rset.getString(34);
				String dist_unit  = rset.getString(35)==null?"":rset.getString(35);
			...
			if(contract_type.equals("B") || contract_type.equals("M")) 
			{
				String queryString1="SELECT CONT_REF_NO,TO_CHAR(DDA_DT,'DD-MON-YY'),TO_CHAR(SIGNING_DT,'DD-MON-YY'),DCQ,MMCQ_PERCENTAGE,TRANSPORT_MGMT_UNIT,TRANSPORT_MGMT_CHARGE  "
						+ "FROM FMS_SVC_CONT_MST A "
						+ "WHERE COMPANY_CD=? AND CONTRACT_TYPE=? "
						+ "AND CONT_NO=? AND AGMT_NO=? AND COUNTERPARTY_CD=? "
						+ "AND CONT_REV = (SELECT MAX(CONT_REV) FROM FMS_SVC_CONT_MST B WHERE A.COMPANY_CD=B.COMPANY_CD "
				...
				if(rset1.next())
				{
					contRef=rset1.getString(1)==null?"":rset1.getString(1);
					dda_dt=rset1.getString(2)==null?"":rset1.getString(2);
					signingDt=rset1.getString(3)==null?"":rset1.getString(3);
					dcq=rset1.getString(4)==null?"":rset1.getString(4);
					unit=rset1.getString(6)==null?"":rset1.getString(6);
					
					
					mdcq_percentage=rset1.getDouble(5);
					if(Double.doubleToRawLongBits(mdcq_percentage)==Double.doubleToRawLongBits(0))
					{
						mdcq_percentage=100;
					}
			...
			if(contract_type.equals("O") || contract_type.equals("Q"))
			{
				...
				bu_tax_info+="<br>Place Of Supply : "+bu_plantState;
			}
			else
			{
				tax_info=utilBean.getCounterpartyPlantTaxInfo(conn,comp_cd, "C", counterparty_cd, plant_seq);
			...
			String parameter="counterparty_cd="+counterparty_cd+"&contract_type="+contract_type+"&cont_no="+cont_no+""
					+ "&cont_rev="+cont_rev_no+"&agmt_no="+agmt_no+"&agmt_rev="+agmt_rev_no+"&plant_seq="+plant_seq+"&billing_cycle="+billing_cycle+""
					+ "&period_start_dt="+period_start_dt+"&period_end_dt="+period_end_dt+"&bu_unit="+bu_unit+"&financial_year="+financial_year+""
					+ "&bu_state_tin="+bu_state_tin+"&invoice_seq="+invoice_seq+"&activityFlag="+activityFlag+"&cargo_no="+cargo_no+"&inv_flag="+inv_flag+"&sell_cont_map="+sell_cont_map;
			...
			if(contract_type.equals("B") || contract_type.equals("M"))
			{
				VPDF_COL2.add("Transport Management Services Charge");
			}
			else
			{
				VPDF_COL2.add("TLU Charges");
			}
			VPDF_COL3.add("<a onclick=openAtt1('rpt_svc_view_attachment1.jsp?"+parameter+"')>Att1</a>");
			VPDF_COL4.add(""+utilBean.getRateUnitNm(conn,price_cd));
			VPDF_COL5.add(qty_mmbtu);
			VPDF_COL6.add(price);
			VPDF_COL7.add(gross_amt);
			...
			if(!price_cd.equals(invoice_raised_in))
			{
				...
				*/
				...
				VPDF_COL1.add(srno);
				VPDF_COL2.add("Exchange Rate");
				VPDF_COL3.add("<a onclick=openAtt2('rpt_svc_view_attachment2.jsp?"+parameter+"')>Att2</a>");
				VPDF_COL4.add("INR/USD");
				VPDF_COL5.add("");
				VPDF_COL6.add(exchang_rate);
				VPDF_COL7.add("");
	...
	public void getSvcInvoiceNumber()
	{
		String function_nm="getSvcInvoiceNumber()";
		try
		{
			String invoice_prefix=utilBean.getInvoicePrefix(conn,comp_cd);
			...
			contType=contract_type.equals("B") || contract_type.equals("M")?"'B','M'":"'O','Q'";
			invSeries=contract_type.equals("B") || contract_type.equals("M")?"TMS":"L";
			...
			if(!invoice_prefix.equals(""))
			{
				...
				for(int i=1;i<=no_inv_no;i++)
				{
					...
					if(invSeries.equals("L"))
					{
						...
						String temp_queryString1=queryString;
						...
						String queryString1="SELECT COUNT(*) "
								...
								+ "AND INVOICE_ID_SEQ=? ";
						String temp_queryString2=queryString1;
						stmt1=conn.prepareStatement(temp_queryString2);
						stmt1.setString(1, comp_cd);
						stmt1.setString(2, bu_state_tin);
						stmt1.setString(3, financial_year);
						stmt1.setString(4, invoice_id_seq);
						...
						String queryString1_1="SELECT COUNT(*) "
								+ "FROM FMS_DLNG_FFLOW_INV_MST "
								+ "WHERE COMPANY_CD=? AND BU_STATE_TIN=? "
								+ "AND FINANCIAL_YEAR=? "
								+ "AND INVOICE_TYPE IN ('TLU') "
								+ "AND INVOICE_ID_SEQ=? ";
						stmt1=conn.prepareStatement(queryString1_1);
						stmt1.setString(1, comp_cd);
						stmt1.setString(2, bu_state_tin);
						stmt1.setString(3, financial_year);
						stmt1.setString(4, invoice_id_seq);
	...
	public void getSvc_attachment1()
	{
		String function_nm="getSvc_attachment1()";
		try
		{   
			int sr = 0;
			double totalTruckInvQty = 0;
			double totalTruckInvMt = 0;
			queryString2="SELECT INVOICE_SEQ, INVOICE_NO, QTY_MMBTU, DISTANCE, MAPPING_ID, TRUCK_CD, TRUCK_TRANS_CD "
					...
					+ "AND BU_STATE_TIN=? AND FINANCIAL_YEAR=? AND SVC_INVOICE_SEQ=? AND CARGO_NO=? ";
			stmt2=conn.prepareStatement(queryString2);
			stmt2.setString(1, comp_cd);
			stmt2.setString(2, counterparty_cd);
			...
			stmt2.setString(9, invoice_seq);
			stmt2.setString(10, cargo_no);
			rset2=stmt2.executeQuery();
			while(rset2.next())
			{
				String inv_seq = rset2.getString(1) == null ? "" : rset2.getString(1);
				String inv_no = rset2.getString(2) == null ? "" : rset2.getString(2);
				double qty = rset2.getDouble(3);
				double mt = qty / mmbtuToMtConv;
				double dist = rset2.getDouble(4);
				String mapping_id = rset2.getString(5) == null ? "" : rset2.getString(5);
				String truck_cd = rset2.getString(6) == null ? "" : rset2.getString(6);
				String trans_cd = rset2.getString(7) == null ? "" : rset2.getString(7);
				...
				if(contract_type.equals("B") || contract_type.equals("M"))
				{
					...
					stmt3.setString(2, counterparty_cd);
					stmt3.setString(3, inv_seq);
					stmt3.setString(4, inv_no);
					stmt3.setString(5, mapping_id);
					stmt3.setString(6, truck_cd);
					stmt3.setString(7, trans_cd);
					rset3 = stmt3.executeQuery();
					
					if(rset3.next()) 
					{
						VATT1_COL1.add(sr);
						VATT1_COL2.add(rset3.getString(1) == null ? "": rset3.getString(1));
						VATT1_COL3.add(dlng_util.getTruckRegNo(conn, rset3.getString(2)));
						VATT1_COL4.add(nf.format(qty));
						VATT1_COL5.add(price);
						if(unit_desc.equals("TRUCK")) 
						{
							VATT1_COL6.add(price);
						}
						else if(unit_desc.equals("MMBTU")) 
						{
							VATT1_COL6.add(nf.format(qty*Double.parseDouble(price)));
						}
						else if(unit_desc.equals("KM"))
						{
							VATT1_COL6.add(nf.format(dist*Double.parseDouble(price)));
						}
						else if(unit_desc.equals("MT"))
						{
							VATT1_COL6.add(nf.format(mt*Double.parseDouble(price)));
						}
					}
					rset3.close();
					stmt3.close();
					totalTruckInvQty += qty;
					totalTruckInvMt += mt;
				}
				else
				{
					/*VATT1_COL1.add(sr);
					VATT1_COL2.add(rset3.getString(1) == null ? "": rset3.getString(1));
					VATT1_COL3.add(dlng_util.getTruckRegNo(conn, truck_cd));
					VATT1_COL4.add(nf.format(qty));
					VATT1_COL5.add(price);
					VATT1_COL6.add(nf.format(qty*Double.parseDouble(price)));*/
			...
			VTOTAL_TRUCK_INV_QTY.add(nf.format(totalTruckInvQty));
			VTOTAL_TRUCK_INV_QTY.add(nf.format(totalTruckInvMt));
		}
		catch(Exception e)
		{
			new SystemErrorLogger().InsertErrorLogger(db_src_file_name, function_nm, e);
	...
	public void getSvc_attachment2()
	{
		String function_nm="getSvc_attachment2()";
		try
		{
			...
			else
			{
            	...
            	queryString2="SELECT COMPONENT_FLAG,COMPONENT1,COMPONENT2,EXC_RATE_NM,BANK_ABBR "
						+ "FROM FMS_EXCHG_RATE_MST "
						+ "WHERE EXC_RATE_CD=?";
				...
				{
					queryString2="SELECT EXCHG_RATE_VALUE,TO_CHAR(ALLOCATION_DT,'DD-MON-YY') "
							+ "FROM FMS_INVOICE_DTL "
							+ "WHERE COMPANY_CD=? AND FINANCIAL_YEAR=? "
		...
		try
		{
			if(operation.equals("PREPARE"))
			{
				String queryString="SELECT INVOICE_NO "
						...
						+ "AND NVL((SELECT COUNT(*) FROM FMS_DLNG_INVOICE_MST B WHERE A.COMPANY_CD=B.COMPANY_CD AND B.REF_NO=A.INVOICE_NO AND B.INV_FLAG IN (?,?)),0) = 0 "
						+ "ORDER BY COUNTERPARTY_CD ";
				stmt=conn.prepareStatement(queryString);
				stmt.setString(1, comp_cd);
				stmt.setString(2, counterparty_cd);
	...
	public void getCriteriaList()
	{
		...
		try
		{
			...
			String queryString="SELECT INVOICE_RAISED_IN,SALE_PRICE_UNIT "
					+ "FROM FMS_DLNG_INVOICE_MST "
					+ "WHERE COMPANY_CD=? AND COUNTERPARTY_CD=? "
					+ "AND INVOICE_NO=? ";
			stmt=conn.prepareStatement(queryString);
	...
	public void getCommonInvoiceDetailforCrDr()
	{
		...
		try
		{
			...
			String queryString="SELECT AGMT_NO,AGMT_REV,CONT_NO,CONT_REV,CONTRACT_TYPE,NULL,BU_UNIT,PLANT_SEQ,"
					...
					+ "TRUCK_TRANS_CD,TRUCK_CD,MAPPING_ID "
					+ "FROM FMS_DLNG_INVOICE_MST "
			...
			if(rset.next())
			{
				...
				truck_trans_cd=rset.getString(17)==null?"":rset.getString(17);
				truck_cd=rset.getString(18)==null?"":rset.getString(18);
				mapping_id=rset.getString(19)==null?"":rset.getString(19);
			}
			rset.close();
			stmt.close();
		}
	...
	public void getInvoiceDetailforCrDr()
	{
		...
		try
		{
			...
			String queryString="SELECT BU_CONTACT_PERSON_CD,CONTACT_PERSON_CD,INVOICE_SEQ,INVOICE_NO,TO_CHAR(INVOICE_DT,'DD/MM/YYYY'),TO_CHAR(DUE_DT,'DD/MM/YYYY'),"
	...
	public void getCrDrInvoiceDetail()
	{
		...
		try
		{
			...
			String queryString="SELECT BU_CONTACT_PERSON_CD,CONTACT_PERSON_CD,INVOICE_SEQ,INVOICE_NO,TO_CHAR(INVOICE_DT,'DD/MM/YYYY'),TO_CHAR(DUE_DT,'DD/MM/YYYY'),"
					...
					+ "TO_CHAR(ENT_DT,'DD/MM/YYYY HH24:MI:SS'),TO_CHAR(APPROVED_DT,'DD/MM/YYYY HH24:MI:SS'),FINANCIAL_YEAR,NULL,NULL,"
					+ "AGMT_NO,AGMT_REV,CONT_NO,CONT_REV,CONTRACT_TYPE,BU_UNIT,PLANT_SEQ,"
					+ "TO_CHAR(PERIOD_START_DT,'DD/MM/YYYY'),TO_CHAR(PERIOD_END_DT,'DD/MM/YYYY'),BU_STATE_TIN,CRITERIA "
					+ "FROM FMS_DLNG_INVOICE_MST "
			...
			{	
				...
				bu_state_tin =rset.getString(52)==null?"":rset.getString(52);
				criteri_formula =rset.getString(53)==null?"":rset.getString(53);
	...
	public void getNewCrDrInvoiceDetail()
	{
		...
		try
		{
			if(criteri_formula.contains("QTY"))
			{
				...
				if(rset1.next())
				{
					changed_qty_mmbtu=nf.format(rset1.getDouble(1));
				}
				rset1.close();
				stmt1.close();
			}
			
			String queryString="SELECT ALLOC_QTY,SALE_PRICE,SALE_PRICE_UNIT,SALE_AMT,EXCHG_RATE_CD,EXCHG_RATE_DT,EXCHG_RATE_VALUE,"
	...
	public void getCreditDebitNoteList(String contType)
	{
		...
		{			
			...
			String queryString="SELECT COMPANY_CD,COUNTERPARTY_CD,AGMT_NO,AGMT_REV,CONT_NO,CONT_REV,CONTRACT_TYPE,NULL,BU_UNIT,PLANT_SEQ,"
					...
					+ "TO_CHAR(INVOICE_DT,'DD/MM/YYYY'),CRITERIA,TRUCK_TRANS_CD,TRUCK_CD "
					+ "FROM FMS_DLNG_INVOICE_MST "
			...
			{	
				...
				String truckTransCd=rset.getString(28)==null?"":rset.getString(28);
				String truckCd=rset.getString(29)==null?"":rset.getString(29);
				...
				int re_print_count=isRePrintPDFRequested(comp_cd,buStateTin,inv_seq,financial_year,"DLNG","REPRINT_PDF","A");
				VRE_PRINT_PDF.add(re_print_count>0?"Y":"N");
			}
			rset.close();
			stmt.close();
	...
	public void getInvoiceDetailCrDr()
	{
		...
		try
		{
			String queryString="SELECT BU_CONTACT_PERSON_CD,CONTACT_PERSON_CD,INVOICE_NO,TO_CHAR(INVOICE_DT,'DD-MON-YY'),"
					...
					+ "INVOICE_AMT,NET_PAYABLE_AMT,TCS_TDS,TCS_AMT,TCS_FACTOR,CHECKED_FLAG,APPROVED_FLAG, "
					+ "INVOICE_ID_SEQ,NULL,NULL,EXCHG_RATE_CD,TO_CHAR(EXCHG_RATE_DT,'DD/MM/YYYY'),EXCHG_RATE_TYPE,"
					+ "INVOICE_RAISED_IN,NULL,NULL,NULL,NULL,NULL,NULL,CRITERIA,TRUCK_TRANS_CD,TRUCK_CD,MAPPING_ID "
					+ "FROM FMS_DLNG_INVOICE_MST "
			...
			if(rset.next())
			{
				...
				truck_trans_cd=rset.getString(40)==null?"":rset.getString(40);
				truck_cd=rset.getString(41)==null?"":rset.getString(41);
				mapping_id=rset.getString(42)==null?"":rset.getString(42);
			...
			//SELECTED INVOICE
			String main_inv_dt="";
			String queryString1="SELECT ALLOC_QTY,SALE_PRICE,SALE_PRICE_UNIT,SALE_AMT,EXCHG_RATE_VALUE,GROSS_AMT,TO_CHAR(INVOICE_DT,'DD-MON-YY') "
					+ "FROM FMS_DLNG_INVOICE_MST "
					+ "WHERE COMPANY_CD=? AND COUNTERPARTY_CD=? AND INVOICE_NO=? ";
			stmt1=conn.prepareStatement(queryString1);
			stmt1.setString(1, comp_cd);
			...
			srno+=1;
			VPDF_COL1.add(srno);
			//VPDF_COL2.add("LNG Delivered as per Invoice No : "+sel_inv_no+" dated "+main_inv_dt);
			VPDF_COL2.add("Reference Att-1 "+NormalFont+" against Invoice No : "+sel_inv_no+" dated "+main_inv_dt);
			VPDF_COL3.add("");
			...
			VPDF_COL5.add("");
			VPDF_COL6.add("");
			VPDF_COL7.add(gross_amt1.equals("")?"":Double.doubleToRawLongBits(Double.parseDouble(gross_amt1))==Double.doubleToRawLongBits(0)?"":nf.format(Math.abs(Double.parseDouble(gross_amt1))));
			...
			String invAmtLbl=NormalFont+" Amount";
			srno+=1;
			VPDF_COL1.add(srno);
			VPDF_COL2.add(invAmtLbl);
			VPDF_COL3.add("");
			...
			if(!invoice_prefix.equals(""))
			{
				...
				for(int i=1;i<=no_inv_no;i++)
				{
					...
					String queryString="SELECT COUNT(*) "
							+ "FROM FMS_DLNG_INVOICE_MST "
							+ "WHERE COMPANY_CD=? AND FINANCIAL_YEAR=? "
							+ "AND BU_STATE_TIN=? AND INVOICE_ID_SEQ=? "
							+ "AND CONTRACT_TYPE IN ("+contType+") AND INV_FLAG=? ";
					String temp_queryString=queryString;
					stmt=conn.prepareStatement(temp_queryString);
					stmt.setString(1, comp_cd);
					stmt.setString(2, financial_year);
					...
					{	
						...
						stmt2.close();
						String invoice_no="";
						invoice_no=invoice_prefix+""+invSeries+""+inv_flag+""+state_abbr+""+utilBean.PrePaddingZero(invoice_id_seq, 4)+"/"+fin_yr;
	...
	public void getLpDlngInvoicePreparationList()
	{
		String function_nm="getLpDlngInvoicePreparationList()";
		...
		try
		{
			...
			String queryString="SELECT COMPANY_CD,COUNTERPARTY_CD,AGMT_NO,AGMT_REV,CONT_NO,CONT_REV,"
					...
					+ "TRUCK_CD,TRUCK_TRANS_CD,MAPPING_ID "
					+ "FROM FMS_DLNG_INVOICE_MST "
					+ "WHERE COMPANY_CD=? "
					...
					+ "AND PAY_RECV_DT > DUE_DT AND CONTRACT_TYPE IN ('F','E','W') "
			...
			while(rset.next())
			{
				...
				String contno=rset.getString(5)==null?"0":rset.getString(5);
				String contrev=rset.getString(6)==null?"0":rset.getString(6);
				String contract_type=rset.getString(7)==null?"":rset.getString(7);
				String bu_plant_seq=rset.getString(9)==null?"":rset.getString(9);
				String bu_plant_abbr=utilBean.getCounterpartyPlantABBR(conn,own_cd, own_cd, bu_plant_seq, "B");
				String state_code=rset.getString(10)==null?"":rset.getString(10);
				String plant_seq=rset.getString(11)==null?"":rset.getString(11);
				String plant_abbr=utilBean.getCounterpartyPlantABBR(conn,countpty_cd, own_cd, plant_seq, "C");
				String invoice_seq=rset.getString(12)==null?"":rset.getString(12);
				String invoice_no=rset.getString(13)==null?"":rset.getString(13);
				String invoice_dt=rset.getString(14)==null?"":rset.getString(14);
				String due_dt = rset.getString(15)==null?"":rset.getString(15);
				String pay_recv_amt = rset.getString(16)==null?"":rset.getString(16);
				String pay_recv_dt = rset.getString(17)==null?"":rset.getString(17);
				double qtyMMBTU = rset.getDouble(18);
				String period_start_dt = rset.getString(19)==null?"":rset.getString(19);
				String period_end_dt = rset.getString(20)==null?"":rset.getString(20);
				String financial_year = rset.getString(21)==null?"":rset.getString(21);
				String inv_flag = rset.getString(22)==null?"":rset.getString(22);
				String truck_cd = rset.getString(23)==null?"":rset.getString(23);
				String truck_trans_cd = rset.getString(24)==null?"":rset.getString(24);
				String mapping_id= rset.getString(25)==null?"":rset.getString(25);
				...
				String cont_ref="";
				String queryString1="SELECT CONT_REF_NO,TRADE_REF_NO, TO_CHAR(START_DT,'DD/MM/YYYY'), TO_CHAR(END_DT,'DD/MM/YYYY')  "
						+ "FROM FMS_SUPPLY_CONT_MST A "
						+ "WHERE COMPANY_CD=? AND CONTRACT_TYPE=? "
						+ "AND CONT_NO=? AND AGMT_NO=? AND COUNTERPARTY_CD=? "
						+ "AND CONT_REV = (SELECT MAX(CONT_REV) FROM FMS_SUPPLY_CONT_MST B WHERE A.COMPANY_CD=B.COMPANY_CD "
				...
				VCONT_NO.add(contno);
				VCONT_REV_NO.add(contrev);
				VCONT_NAME.add(rset.getString(8)==null?"":rset.getString(8));
				...
				VTRUCK_NO.add(dlng_util.getTruckRegNo(conn, truck_cd));
				...
				getLpInvDetailForPreparationList(own_cd,countpty_cd, contno, agmtno,contract_type, plant_seq, 
						bu_plant_seq, truck_cd, truck_trans_cd, cont_start_dt, cont_end_dt, state_code,lp_inv_flag,mapping_id,invoice_no,"");
			}
			rset.close();
			stmt.close();
		}
	...
	public void getLpDlngSvcInvoicePreparationList()
	{
		String function_nm="getLpDlngSvcInvoicePreparationList()";
		...
		try
		{
			...
			String queryString="SELECT COMPANY_CD,COUNTERPARTY_CD,AGMT_NO,AGMT_REV,CONT_NO,CONT_REV,"
					+ "CONTRACT_TYPE,NULL,BU_UNIT,BU_STATE_TIN,PLANT_SEQ,INVOICE_SEQ,INVOICE_NO,"
					+ "TO_CHAR(INVOICE_DT,'DD/MM/YYYY'),TO_CHAR(DUE_DT,'DD/MM/YYYY'),PAY_RECV_AMT,TO_CHAR(PAY_RECV_DT,'DD/MM/YYYY'),"
					+ "SELL_CONT_MAP,TO_CHAR(PERIOD_START_DT,'DD/MM/YYYY'),TO_CHAR(PERIOD_END_DT,'DD/MM/YYYY'),FINANCIAL_YEAR,INV_FLAG "
					+ "FROM FMS_DLNG_SVC_INVOICE_MST "
					+ "WHERE COMPANY_CD=? "
					+ "AND INVOICE_DT >= TO_DATE(?,'DD/MM/YYYY') AND INVOICE_DT <= TO_DATE(?,'DD/MM/YYYY') "
					+ "AND APPROVED_FLAG=? AND INVOICE_NO IS NOT NULL AND PAY_RECV_DT IS NOT NULL "
					+ "AND PAY_RECV_DT > DUE_DT AND CONTRACT_TYPE IN ('B','M')";
			stmt=conn.prepareStatement(queryString);
			stmt.setString(1, comp_cd);
			stmt.setString(2, temp_period_start_dt);
			stmt.setString(3, temp_period_end_dt);
			...
			while(rset.next())
			{
				...
				String pay_recv_amt = rset.getString(16)==null?"":rset.getString(16);
				String pay_recv_dt = rset.getString(17)==null?"":rset.getString(17);
				String sell_cont_map = rset.getString(18)==null?"":rset.getString(18);
				String period_start_dt = rset.getString(19)==null?"":rset.getString(19);
				String period_end_dt = rset.getString(20)==null?"":rset.getString(20);
				String financial_year = rset.getString(21)==null?"":rset.getString(21);
				String inv_flag = rset.getString(22)==null?"":rset.getString(22);
...
//				VALLOC_QTY.add(nf.format(qtyMMBTU));
				...
				VORI_INV_FLAG.add(inv_flag);
				getLpInvDetailForPreparationList(own_cd,countpty_cd, contno, agmtno,contract_type, plant_seq, 
						bu_plant_seq, "", "", cont_start_dt, cont_end_dt, state_code,lp_inv_flag,"",invoice_no,sell_cont_map);
			}
			rset.close();
			stmt.close();
		}
	...
	public void getLpInvDetailForPreparationList(String own_cd,String countpty_cd, String contno, String agmtno,String cont_type, String plant_seq, String bu_plant_seq, String truckCd, 
			String track_trans_cd, String period_start_dt, String period_end_dt,String state_code, String inv_flag, String mapping_id, String invoice_no, String sell_cont_map)
	{
		...
		try
		{
			...
			{
				queryString5="SELECT INVOICE_NO,CHECKED_FLAG,APPROVED_FLAG,INVOICE_SEQ,PDF_INV_DTL,"
						+ "PRINT_BY_ORI,PRINT_BY_TRI,PRINT_BY_DUP,SAP_APPROVAL,FINANCIAL_YEAR,FIN_SYS,TO_CHAR(INVOICE_DT,'DD-MON-YY') "
						+ "FROM FMS_DLNG_SVC_INVOICE_MST "
						+ "WHERE COMPANY_CD=? AND COUNTERPARTY_CD=? AND CONT_NO=? AND AGMT_NO=? AND CONTRACT_TYPE=? "
						+ "AND PLANT_SEQ=? AND BU_UNIT=? AND SELL_CONT_MAP=? "
						+ "AND PERIOD_START_DT=TO_DATE(?,'DD/MM/YYYY') AND PERIOD_END_DT=TO_DATE(?,'DD/MM/YYYY') "
						+ "AND BU_STATE_TIN=? AND INV_FLAG=? "
						//+ "AND FINANCIAL_YEAR=?";
						+ "AND REF_NO=?";
				...
				stmt5.setString(++stcount, plant_seq);
				stmt5.setString(++stcount, bu_plant_seq);
				stmt5.setString(++stcount, sell_cont_map);
				stmt5.setString(++stcount, period_start_dt);
				stmt5.setString(++stcount, period_end_dt);
				stmt5.setString(++stcount, state_code);
				stmt5.setString(++stcount, inv_flag);
				//stmt5.setString(12, financial_year);
				stmt5.setString(++stcount, invoice_no);
				rset5=stmt5.executeQuery();
				if(rset5.next())
				{
					inv_no=rset5.getString(1)==null?"":rset5.getString(1);
		...
		try
		{
			if(contract_type.equals("B") || contract_type.equals("M")) 
			{
				String queryString="SELECT TO_CHAR(DUE_DT,'DD/MM/YYYY'),NET_PAYABLE_AMT,PAY_RECV_AMT,TO_CHAR(PAY_RECV_DT,'DD/MM/YYYY'),TO_CHAR(INVOICE_DT,'DD-MON-YY'),"
						...
						+ "AND INVOICE_SEQ = ? AND SELL_CONT_MAP = ? ";
				stmt=conn.prepareStatement(queryString);
				stmt.setString(1, comp_cd);
				stmt.setString(2, counterparty_cd); 
				stmt.setString(3, cont_no);
				...
				stmt.setString(13, ori_invoice_seq);
				stmt.setString(14, sell_cont_map);
				rset=stmt.executeQuery();
				if(rset.next())
				{
			...
			else
			{
				String queryString="SELECT TO_CHAR(DUE_DT,'DD/MM/YYYY'),NET_PAYABLE_AMT,PAY_RECV_AMT,TO_CHAR(PAY_RECV_DT,'DD/MM/YYYY'),TO_CHAR(INVOICE_DT,'DD-MON-YY'), "
						...
						+ "AND BU_STATE_TIN=? AND PERIOD_START_DT=TO_DATE(?,'DD/MM/YYYY') "
						+ "AND PERIOD_END_DT=TO_DATE(?,'DD/MM/YYYY') AND FINANCIAL_YEAR=? AND INV_FLAG=? "
						+ "AND INVOICE_SEQ = ? AND TRUCK_CD = ? AND TRUCK_TRANS_CD = ? ";
				stmt=conn.prepareStatement(queryString);
				stmt.setString(1, comp_cd);
				stmt.setString(2, counterparty_cd);
				stmt.setString(3, cont_no);
	...
	public void getExistingLpInvoiceDtl()
	{
		...
		try
		{
			if(contract_type.equals("B") || contract_type.equals("M"))
			{
				String queryString="SELECT BU_CONTACT_PERSON_CD,CONTACT_PERSON_CD,INVOICE_SEQ,INVOICE_NO,TO_CHAR(INVOICE_DT,'DD/MM/YYYY'),"
						+ "TO_CHAR(DUE_DT,'DD/MM/YYYY'),QTY,SALE_PRICE,SALE_PRICE_UNIT,SALE_AMT,EXCHG_RATE_CD,TO_CHAR(EXCHG_RATE_DT,'DD/MM/YYYY'),"
						+ "EXCHG_RATE_VALUE,INVOICE_RAISED_IN,GROSS_AMT,TAX_AMT,TAX_STRUCT_CD,INVOICE_AMT,NET_PAYABLE_AMT,"
						+ "REMARK_1,REMARK_2,TCS_TDS,TCS_AMT,TCS_FACTOR,NULL,NULL,TCS_STRUCT_CD,"
						+ "TDS_GROSS_AMT,TDS_GROSS_PERCENT,TDS_STRUCT_CD,NULL,NULL,NULL,NULL,"
						+ "TO_CHAR(ENT_DT,'DD/MM/YYYY HH24:MI:SS'),TO_CHAR(APPROVED_DT,'DD/MM/YYYY HH24:MI:SS'),FINANCIAL_YEAR,DISCOUNT_DAYS "
						+ "FROM FMS_DLNG_SVC_INVOICE_MST "
						+ "WHERE COMPANY_CD=? AND COUNTERPARTY_CD=? AND CONT_NO=? "
						+ "AND AGMT_NO=? AND PLANT_SEQ=? AND BU_UNIT=? AND CONTRACT_TYPE=? AND BU_STATE_TIN=? "
						+ "AND PERIOD_START_DT=TO_DATE(?,'DD/MM/YYYY') AND PERIOD_END_DT=TO_DATE(?,'DD/MM/YYYY') AND FINANCIAL_YEAR=? "
						+ "AND INV_FLAG=? AND SELL_CONT_MAP=? ";
				stmt=conn.prepareStatement(queryString);
				stmt.setString(1, comp_cd);
				stmt.setString(2, counterparty_cd);
				stmt.setString(3, cont_no);
				...
				stmt.setString(12, inv_flag);
				stmt.setString(13, sell_cont_map);
				rset=stmt.executeQuery();
				if(rset.next())
				{
					submission_chk=true;
...
//						tax_struct_dt=rset.getString(18)==null?"":rset.getString(18);
						...
						net_payable=rset.getString(19)==null?"":nf.format(rset.getDouble(19));
						
						applicable_abbr=rset.getString(22)==null?"":rset.getString(22);
						applicable_amt=rset.getString(23)==null?"":rset.getString(23);
						TCS_factor=rset.getString(24)==null?"":rset.getString(24);
						...
						transportation_charges=rset.getString(25)==null?"":nf2.format(rset.getDouble(25));
						transportation_amount=rset.getString(26)==null?"":nf.format(rset.getDouble(26));
						
						tcs_struct_cd=rset.getString(27)==null?"":rset.getString(27);
//						tcs_struct_dt=rset.getString(29)==null?"":rset.getString(29);
						
						tds_amt=rset.getString(28)==null?"":nf.format(rset.getDouble(28));
						tds_factor=rset.getString(29)==null?"":nf.format(rset.getDouble(29));
						tds_struct_cd=rset.getString(30)==null?"":rset.getString(30);
//						tds_struct_dt=rset.getString(33)==null?"":rset.getString(33);
						
						marketing_margin=rset.getString(31)==null?"":nf.format(rset.getDouble(31));
						marketing_margin_amount=rset.getString(32)==null?"":nf.format(rset.getDouble(32));
						other_charges=rset.getString(33)==null?"":nf.format(rset.getDouble(33));
						other_charges_amount=rset.getString(34)==null?"":nf.format(rset.getDouble(34));
						inv_entered_on = rset.getString(35)==null?"":rset.getString(35);
						inv_approved_on = rset.getString(36)==null?"":rset.getString(36);
						
						String fiscal_yr=rset.getString(37)==null?"":rset.getString(37);
					...
					remark_1=rset.getString(21)==null?"":rset.getString(21);
//					remark_2=rset.getString(22)==null?"":rset.getString(22);
					discount_days = rset.getString(38)==null?"0":rset.getString(38);
				}
				rset.close();
				stmt.close();
			}
			else
			{
				String queryString="SELECT BU_CONTACT_PERSON_CD,CONTACT_PERSON_CD,INVOICE_SEQ,INVOICE_NO,TO_CHAR(INVOICE_DT,'DD/MM/YYYY'),"
						...
						+ "REMARK_1,REMARK_2,TCS_TDS,TCS_AMT,TCS_FACTOR,NULL,NULL,TCS_STRUCT_CD,TO_CHAR(TCS_EFF_DT,'DD/MM/YYYY'),"
						+ "TDS_GROSS_AMT,TDS_GROSS_PERCENT,TDS_STRUCT_CD,TO_CHAR(TDS_EFF_DT,'DD/MM/YYYY'),NULL,NULL,NULL,NULL,"
						+ "TO_CHAR(ENT_DT,'DD/MM/YYYY HH24:MI:SS'),TO_CHAR(APPROVED_DT,'DD/MM/YYYY HH24:MI:SS'),FINANCIAL_YEAR,CHKPOST_CD,DRIVER_CD,DISCOUNT_DAYS "
						+ "FROM FMS_DLNG_INVOICE_MST "
						+ "WHERE COMPANY_CD=? AND COUNTERPARTY_CD=? AND CONT_NO=? "
						+ "AND AGMT_NO=? AND PLANT_SEQ=? AND BU_UNIT=? AND CONTRACT_TYPE=? AND BU_STATE_TIN=? "
						+ "AND PERIOD_START_DT=TO_DATE(?,'DD/MM/YYYY') AND PERIOD_END_DT=TO_DATE(?,'DD/MM/YYYY') AND FINANCIAL_YEAR=? "
						+ "AND MAPPING_ID=? AND INV_FLAG=? AND TRUCK_TRANS_CD=? AND TRUCK_CD=? 	AND REF_NO = ? ";
				stmt=conn.prepareStatement(queryString);
				stmt.setString(1, comp_cd);
				stmt.setString(2, counterparty_cd);
				stmt.setString(3, cont_no);
				...
				stmt.setString(14, truck_trans_cd);
				stmt.setString(15, truck_cd);
				stmt.setString(16, ori_invoice_no);
				rset=stmt.executeQuery();
				if(rset.next())
				{
					submission_chk=true;
...
//					remark_2=rset.getString(22)==null?"":rset.getString(22);
	...
	public void getLpInvoiceDetail()
	{
		...
		try
		{
			...
			String queryString="SELECT BU_CONTACT_PERSON_CD,CONTACT_PERSON_CD,INVOICE_NO,TO_CHAR(INVOICE_DT,'DD-MON-YY'),"
					...
					+ "ALLOC_QTY,SALE_PRICE,SALE_PRICE_UNIT,SALE_AMT,EXCHG_RATE_VALUE,GROSS_AMT,TAX_AMT,TAX_STRUCT_CD,TO_CHAR(TAX_EFF_DT,'DD/MM/YYYY'),"
					+ "INVOICE_AMT,NET_PAYABLE_AMT,TCS_TDS,TCS_AMT,TCS_FACTOR,CHECKED_FLAG,APPROVED_FLAG, "
					+ "INVOICE_ID_SEQ,EXCHG_RATE_CD,TO_CHAR(EXCHG_RATE_DT,'DD/MM/YYYY'),EXCHG_RATE_TYPE,"
					+ "INVOICE_RAISED_IN,DISCOUNT_DAYS,REF_NO,INT_RATE "
					+ "FROM FMS_DLNG_INVOICE_MST "
					+ "WHERE COMPANY_CD=? AND COUNTERPARTY_CD=? AND CONT_NO=? "
					+ "AND AGMT_NO=? AND PLANT_SEQ=? AND BU_UNIT=? AND CONTRACT_TYPE=? "
					+ "AND BU_STATE_TIN=? "
					+ "AND PERIOD_START_DT=TO_DATE(?,'DD/MM/YYYY') AND PERIOD_END_DT=TO_DATE(?,'DD/MM/YYYY') AND FINANCIAL_YEAR=? "
					+ "AND INVOICE_SEQ=? AND INV_FLAG=? AND TRUCK_TRANS_CD = ? AND TRUCK_CD = ? AND REF_NO = ? ";
			stmt=conn.prepareStatement(queryString);
			stmt.setString(1, comp_cd);
			stmt.setString(2, counterparty_cd);
			stmt.setString(3, cont_no);
			...
			if(rset.next())
			{
				...
				exchng_rate_cd=rset.getString(27)==null?"":rset.getString(27);
				exchang_rate_dt=rset.getString(28)==null?"":rset.getString(28);
				exchang_rate_type=rset.getString(29)==null?"":rset.getString(29);
				invoice_raised_in=rset.getString(30)==null?"":rset.getString(30);
				
				
				discount_days=rset.getString(31)==null?"0":rset.getString(31);
				ori_inv_no=rset.getString(32)==null?"":rset.getString(32);
				int_rate=rset.getString(33)==null?"":nf.format(rset.getDouble(33));
	...
	{
		String function_nm="getLpInvoiceDateCalc()";
		try
		{
			String sysdate = utilDate.getSysdate();
	...
	public int isRePrintPDFRequestedFflow(String comp_cd,String buStateTin,String inv_seq,String financial_year,String segment,String chnage_type,String flag,String inv_type)
	{
		...
		try
		{
			...
			stmt_temp.setString(6, chnage_type);
			stmt_temp.setString(7, flag);
			stmt_temp.setString(8, inv_type);
			rset_temp=stmt_temp.executeQuery();
			if(rset_temp.next())
			{
				re_print_count=rset_temp.getInt(1);
	...
	public void getDLNGInvoiceChangeList()
	{
		String function_nm="getDLNGInvoiceChangeList()";
		try
		{		
			for(int i=0; i<VINVOICE_LIST_ABBR.size(); i++)
			{
				...
				String queryString="SELECT COMPANY_CD,COUNTERPARTY_CD,AGMT_NO,AGMT_REV,CONT_NO,CONT_REV,CONTRACT_TYPE,BU_UNIT,PLANT_SEQ,"
						+ "BU_STATE_TIN,FINANCIAL_YEAR,INVOICE_SEQ,TO_CHAR(PERIOD_START_DT,'DD/MM/YYYY'),TO_CHAR(PERIOD_END_DT,'DD/MM/YYYY'),"
				...
				{	
					...
					String bu_plant_abbr=utilBean.getCounterpartyPlantABBR(conn,own_cd, own_cd, bu_plant_seq, "B");
					String plant_seq=rset.getString(9)==null?"":rset.getString(9);
					String plant_abbr=utilBean.getCounterpartyPlantABBR(conn,countpty_cd, own_cd, plant_seq, "C");
					...
					VINVOICE_TYPE_NM.add(invType.equals("CR")?"Credit Note"
									...
									:invType.equals("F")?"DLNG Sales Invoice"
									:"");
					VINVOICE_DT.add(inv_dt);
					VOTH_INVOICE_TYPE.add(inv_type);
	...
	public void getDLNGfflowInvoiceChangeList()
	{
		String function_nm="getDLNGfflowInvoiceChangeList()";
		try
		{		
				inv_index=0;
				String temp_period_start_dt=""+utilDate.getFirstDateOfMonth(month, year);
				...
				{	
					...
					VINV_FLAG.add(invType);
					VINVOICE_TYPE.add(invType);
					VINVOICE_TYPE_NM.add(invType.equals("P")?"DLNG FFLOW Invoice"
									:"");
					VINVOICE_DT.add(inv_dt);
					VOTH_INVOICE_TYPE.add(inv_type);
	...
	public void forAllLtcoraBillingOption()
	{
		String function_nm="forAllLtcoraBillingOption()";
		try
		{
			String temp_billing_cycle=billing_cycle;
	...
	public void getLtcoraDlngInvoicePreparationList()
	{
		String function_nm="getLtcoraDlngInvoicePreparationList()";
		...
		try
		{
			...
			if(billing_cycle.equals("8"))
			{
				...
				String queryString="SELECT A.COMPANY_CD,A.COUNTERPARTY_CD,A.AGMT_NO,A.AGMT_REV,A.CONT_NO,A.CONT_REV,"
						+ "TO_CHAR(A.START_DT,'DD/MM/YYYY'),TO_CHAR(A.END_DT,'DD/MM/YYYY'),A.CONT_NAME,A.CONT_REF_NO,A.CONTRACT_TYPE,"
						+ "NULL,NULL,A.AGMT_BASE,A.FCC_FLAG,E.PLANT_SEQ_NO AS PLANT_SEQ,D.PLANT_SEQ_NO AS BU_SEQ,"
						//+ "F.TRUCK_TRANS_CD,F.TRUCK_CD,F.FILL_STATION_CD,F.BAY_CD,F.SLOT_START_TIME,F.SLOT_END_TIME,B.CARGO_NO,"
						+ "NULL,NULL,NULL,NULL,NULL,NULL,"
						+ "B.CARGO_NO,B.CARGO_REF "
						+ "FROM FMS_LTCORA_CONT_MST A,"
						+ "FMS_LTCORA_CONT_CARGO_DTL B,"
						+ "FMS_LTCORA_CONT_BILLING_DTL C,"
						+ "FMS_LTCORA_CONT_BU D,"
						+ "FMS_SUPPLY_CONT_PLANT E "
						//+ "FMS_DLNG_ALLOC_MST F "
					+ "WHERE A.COMPANY_CD=? AND A.BUY_SALE=? AND A.FCC_FLAG=? AND B.CARGO_STATUS=? AND A.AGMT_TYPE=? AND A.CONTRACT_TYPE IN ('O','Q') "
					+ "AND TO_DATE(TO_CHAR(B.ACTUAL_RECPT_DT,'DD/MM/YYYY'),'DD/MM/YYYY')<=TO_DATE(?,'DD/MM/YYYY') "
					+ "AND TO_DATE(TO_CHAR(B.ACTUAL_RECPT_DT,'DD/MM/YYYY'),'DD/MM/YYYY')+NVL(STORAGE_DAYS-1,0)+NVL(STORAGE_EXT_DAYS,0) >=TO_DATE(?,'DD/MM/YYYY') "
					+ "AND A.CONT_REV = (SELECT MAX(B.CONT_REV) FROM FMS_LTCORA_CONT_MST B WHERE A.COMPANY_CD=B.COMPANY_CD "
					...
					+ "AND A.CONT_REV=D.CONT_REV "
					+ "AND A.AGMT_NO=D.AGMT_NO "
					+ "AND A.AGMT_REV=D.AGMT_REV "
					+ "AND A.CONTRACT_TYPE=D.CONTRACT_TYPE AND A.BUY_SALE=D.BUY_SALE AND A.AGMT_TYPE=D.AGMT_TYPE "
					+ ""
					...
					+ "AND A.AGMT_NO=C.AGMT_NO "
					+ "AND A.AGMT_REV=C.AGMT_REV "
					+ "AND A.CONTRACT_TYPE=C.CONTRACT_TYPE AND A.BUY_SALE=C.BUY_SALE AND A.AGMT_TYPE=C.AGMT_TYPE "
					+ "AND E.PLANT_SEQ_NO=C.PLANT_SEQ_NO AND C.BILLING_FREQ=? AND C.BILLING_FLAG=?  "
					+ ""
					+ "AND (SELECT NVL(SUM(QTY_MMBTU),0) FROM FMS_DLNG_ALLOC_MST F WHERE A.COMPANY_CD=F.COMPANY_CD AND A.COUNTERPARTY_CD=F.COUNTERPARTY_CD "
					+ "AND A.CONT_NO=F.CONT_NO AND A.AGMT_NO=F.AGMT_NO AND A.CONTRACT_TYPE=F.CONTRACT_TYPE AND E.PLANT_SEQ_NO=F.PLANT_SEQ AND D.PLANT_SEQ_NO=F.BU_SEQ "
					+ "AND B.CARGO_NO=F.CARGO_NO "
					+ "AND F.GAS_DT >=TO_DATE(?,'DD/MM/YYYY') AND F.GAS_DT <=TO_DATE(?,'DD/MM/YYYY') "
					+ "AND F.NOM_REV_NO=(SELECT MAX(NOM_REV_NO) FROM FMS_DLNG_ALLOC_MST C "
						+ "WHERE F.CONT_NO=C.CONT_NO AND F.AGMT_NO=C.AGMT_NO AND F.COMPANY_CD=C.COMPANY_CD AND F.COUNTERPARTY_CD=C.COUNTERPARTY_CD "
						+ "AND F.PLANT_SEQ=C.PLANT_SEQ AND F.CONTRACT_TYPE=C.CONTRACT_TYPE AND F.BU_SEQ=C.BU_SEQ "
						+ "AND F.GAS_DT=C.GAS_DT "
						+ "AND F.CARGO_NO=C.CARGO_NO "
						+ "AND F.TRUCK_TRANS_CD=C.TRUCK_TRANS_CD AND F.TRUCK_CD=C.TRUCK_CD AND F.FILL_STATION_CD=C.FILL_STATION_CD "
						+ "AND F.BAY_CD=C.BAY_CD AND F.SLOT_START_TIME=C.SLOT_START_TIME AND F.SLOT_END_TIME=C.SLOT_END_TIME"
						+ ") "
					+ ") > 0";
				stmt=conn.prepareStatement(queryString);
				stmt.setString(count++, comp_cd);
				...
				while(rset.next())
				{
					...
					String cargo_ref=rset.getString(25)==null?"":rset.getString(25);
					...
					VSELL_AGMT_NO.add("");
					VSELL_AGMT_REV_NO.add("");
					VSELL_CONT_NO.add("");
					VSELL_CONT_REV_NO.add("");
					VSELL_CONTRACT_TYPE.add("");
					VSELL_CONT_MAP.add("");
					...
					VINV_FLAG.add(inv_flag);
					getSvcInvDetailForPreparationList(own_cd,countpty_cd,contno, agmtno,cont_type, plant_seq, 
					bu_plant_seq, temp_period_start_dt, temp_period_end_dt,state_code,inv_flag, 
					"","","","","","",cargo_no);
	...
	public void getDlngSalesAllocatedTruckList()
	{
		String function_nm="getDlngSalesAllocatedTruckList()";
		try
		{
			int selCnt=0;
			
			int count_truck_invoice =0;
			String queryString="SELECT TRUCK_CD,QTY_MMBTU,TRUCK_TRANS_CD,FILL_STATION_CD,BAY_CD,SLOT_START_TIME,SLOT_END_TIME,"
					+ "TO_CHAR(LOAD_START_DT,'DD/MM/YYYY'),LOAD_START_TIME,TO_CHAR(LOAD_END_DT,'DD/MM/YYYY'),LOAD_END_TIME,TO_CHAR(GAS_DT,'DD/MM/YYYY'),QTY_MT "
					+ "FROM FMS_DLNG_ALLOC_MST A "
					+ "WHERE A.COMPANY_CD=? AND A.COUNTERPARTY_CD=? AND A.AGMT_NO=? AND A.CONT_NO=? AND A.CONTRACT_TYPE=? "
					+ "AND A.PLANT_SEQ=? AND A.BU_SEQ=? AND A.CARGO_NO=? AND A.QTY_MMBTU > 0 "
					...
					+ "AND A.NOM_REV_NO=(SELECT MAX(NOM_REV_NO) FROM FMS_DLNG_ALLOC_MST B "
						+ "WHERE A.CONT_NO=B.CONT_NO AND A.AGMT_NO=B.AGMT_NO AND A.COMPANY_CD=B.COMPANY_CD AND A.COUNTERPARTY_CD=B.COUNTERPARTY_CD "
						+ "AND A.PLANT_SEQ=B.PLANT_SEQ AND A.CONTRACT_TYPE=B.CONTRACT_TYPE AND A.BU_SEQ=B.BU_SEQ AND A.GAS_DT=B.GAS_DT "
						+ "AND A.TRUCK_TRANS_CD=B.TRUCK_TRANS_CD AND A.TRUCK_CD=B.TRUCK_CD AND A.FILL_STATION_CD=B.FILL_STATION_CD "
						+ "AND A.BAY_CD=B.BAY_CD AND A.SLOT_START_TIME=B.SLOT_START_TIME AND A.SLOT_END_TIME=B.SLOT_END_TIME AND A.CARGO_NO=B.CARGO_NO) ";
			stmt=conn.prepareStatement(queryString);
			stmt.setString(++selCnt, comp_cd);
			stmt.setString(++selCnt, counterparty_cd);
			stmt.setString(++selCnt, agmt_no);
			stmt.setString(++selCnt, cont_no);
			stmt.setString(++selCnt, contract_type);
			stmt.setString(++selCnt, plant_seq);
			stmt.setString(++selCnt, bu_unit);
			stmt.setString(++selCnt, cargo_no);
			stmt.setString(++selCnt, period_start_dt);
			stmt.setString(++selCnt, period_end_dt);
			rset=stmt.executeQuery();
			while(rset.next())
			{
				count_truck_invoice++;
				...
				double alloc_qty=rset.getDouble(2);
				String truck_trans_cd =rset.getString(3)==null?"":rset.getString(3);
				
				String buyer_fill_station_cd=rset.getString(4)==null?"":rset.getString(4);
				String buyer_bay_cd=rset.getString(5)==null?"":rset.getString(5);
				String buyer_slot_start_time=rset.getString(6)==null?"":rset.getString(6);
				String buyer_slot_end_time=rset.getString(7)==null?"":rset.getString(7);

				String load_start_dt=rset.getString(8)==null?"":rset.getString(8);
				String load_start_time=rset.getString(9)==null?"":rset.getString(9);
				String load_end_dt=rset.getString(10)==null?"":rset.getString(10);
				String load_end_time=rset.getString(11)==null?"":rset.getString(11);
				String gas_dt=rset.getString(12)==null?"":rset.getString(12);
				double alloc_qty_mt=rset.getDouble(13);
				
				VSALES_TRUCK_CD.add(truck_cd);
				VSALES_TRUCK_REG_NO.add(dlng_util.getTruckRegNo(conn, truck_cd));
				VSALES_INV_QTY.add(nf.format(alloc_qty));
				VSALES_INV_QTY_MT.add(nf.format(alloc_qty_mt));
				VSALES_TRUCK_TRANS_CD.add(truck_trans_cd);
				
				String map_id=buyer_fill_station_cd+"-"+buyer_bay_cd+"-"+buyer_slot_start_time+"-"+buyer_slot_end_time;
				VSALES_MAPPING_ID.add(map_id);
				
				VFILL_STATION_CD.add(buyer_fill_station_cd);
	  			VBAY_CD.add(buyer_bay_cd);
				...
				stmt2 = conn.prepareStatement(queryString2);
				stmt2.setString(1, "Y");
				stmt2.setString(2, buyer_fill_station_cd);
				rset2=stmt2.executeQuery();
				if(rset2.next())
				{
					...
					VFILL_STATION_ABBR.add(stAbbr);
				}
				rset2.close();
				stmt2.close();
				
	  			String queryString3="SELECT BAY_CD,BAY_NAME,SLOT_START_TIME,SLOT_INTERVAL "
						+ "FROM FMS_BAY_SLOT_MST A "
						+ "WHERE EFF_DT=(SELECT MAX(B.EFF_DT) FROM FMS_BAY_SLOT_MST B "
						+ "WHERE A.FILL_STATION_CD=B.FILL_STATION_CD "
						+ "AND A.BAY_CD=B.BAY_CD "
						+ "AND B.EFF_DT<=TO_DATE(TO_CHAR(SYSDATE,'DD/MM/YYYY'),'DD/MM/YYYY')) "
						+ "AND ACTIVE_FLAG=? AND FILL_STATION_CD=? AND BAY_CD=?";
				stmt2 = conn.prepareStatement(queryString3);
				stmt2.setString(1, "Y");
				stmt2.setString(2, buyer_fill_station_cd);
				stmt2.setString(3, buyer_bay_cd);
				rset2=stmt2.executeQuery();
				if(rset2.next())
				{
				...
				VSELL_PERIOD_START_DT.add(load_start_dt+" ("+load_start_time+")");
				VSELL_PERIOD_END_DT.add(load_end_dt+" ("+load_end_time+")");
				VGAS_DT.add(gas_dt);
				
	  			VDISP_SLOT_DTL.add(buyer_slot_start_time+" - "+buyer_slot_end_time);
			}
			total_truck_invoice = ""+count_truck_invoice;
	...
	public void getLtcoraDlngInvoiceCalculation()
	{
		String function_nm="getLtcoraDlngInvoiceCalculation()";
		try
		{
			if((operation.equals("INSERT") && !submission_chk))// || operation.equals("MODIFY"))
			{
			...
			if((operation.equals("INSERT") && !submission_chk) || operation.equals("MODIFY"))
			{
				...
				stmt1.setString(++selCnt, contract_type);
				stmt1.setString(++selCnt, bu_unit);
				stmt1.setString(++selCnt, period_start_dt);
				stmt1.setString(++selCnt, period_end_dt);
			...
			if((operation.equals("INSERT") && !submission_chk) || operation.equals("MODIFY"))
			{
				...
				{
					gross_include_transport_tariff=gross_amt1;
					temp=TaxCalc.ServiceTaxAmountCalculationWithInfo(conn,comp_cd, counterparty_cd, "C", plant_seq, bu_unit, storage_start_dt, "TLU", gross_amt1);
				}
				else
				{
					gross_include_transport_tariff=gross_amt1;	
	...
	public void getLtcora_attachment1()
	{
		String function_nm="getLtcora_attachment1()";
		try
		{
			String temp_dcq=dcq;
            double mdcq_qty=0;
            ...
            int selCnt=0;
            String queryString1="SELECT SUM(TOTAL_QTY_SUM) FROM "
            		+ "(SELECT SUM(QTY_MMBTU) AS TOTAL_QTY_SUM "
            		+ "FROM FMS_DLNG_ALLOC_MST A "
					+ "WHERE CONT_NO=? AND AGMT_NO=? "
					+ "AND COMPANY_CD=? AND COUNTERPARTY_CD=? "
					+ "AND PLANT_SEQ=? AND CONTRACT_TYPE=? AND BU_SEQ=? "
			...
			String queryString="SELECT TO_CHAR(TO_DATE(TD.END_DATE + 1 - ROWNUM),'DD/MM/YYYY') MONTH_DATE "
					...
					+ "ORDER BY TO_DATE(MONTH_DATE,'DD/MM/YYYY')";
			stmt=conn.prepareStatement(queryString);
			stmt.setString(1, period_start_dt);
			...
			while(rset.next())
			{
				...
				selCnt=0;
				String queryString7 ="SELECT TRUCK_TRANS_CD,TRUCK_CD,FILL_STATION_CD,BAY_CD,"
						...
						+ "AND B.GAS_DT=A.GAS_DT AND B.TRUCK_TRANS_CD=A.TRUCK_TRANS_CD AND A.TRUCK_CD=B.TRUCK_CD "
						+ "AND B.FILL_STATION_CD=A.FILL_STATION_CD AND B.BAY_CD=A.BAY_CD "
						+ "AND B.SLOT_START_TIME=A.SLOT_START_TIME AND B.SLOT_END_TIME=A.SLOT_END_TIME AND A.CARGO_NO=B.CARGO_NO) "
						+ "";
				stmt7=conn.prepareStatement(queryString7);
				stmt7.setString(++selCnt, cont_no);
				stmt7.setString(++selCnt, agmt_no);
				stmt7.setString(++selCnt, comp_cd);
				stmt7.setString(++selCnt, counterparty_cd);
				stmt7.setString(++selCnt, plant_seq);
				stmt7.setString(++selCnt, contract_type);
				stmt7.setString(++selCnt, bu_unit);
				stmt7.setString(++selCnt, date);
				stmt7.setString(++selCnt, cargo_no);
				rset7=stmt7.executeQuery();
				while(rset7.next()) 
				{
					String truck_trans_cd = rset7.getString(1)==null?"":rset7.getString(1);
					truck_cd = rset7.getString(2)==null?"":rset7.getString(2);
					String fill_station_cd = rset7.getString(3)==null?"":rset7.getString(3);
					String bay_cd = rset7.getString(4)==null?"":rset7.getString(4);
					String slot_start_time = rset7.getString(5)==null?"":rset7.getString(5);
					String slot_end_time = rset7.getString(6)==null?"":rset7.getString(6);
					
					String variable_dcq=utilBean.getCargoVariableCSOC(conn, comp_cd, counterparty_cd,"C", agmt_no, cont_no, contract_type, cargo_no, date);
					...
					queryString2="SELECT SUM(QTY_MMBTU) "
							...
							+ "AND CARGO_NO=? "
							+ "AND FILL_STATION_CD=? AND BAY_CD=? AND SLOT_START_TIME=? AND SLOT_END_TIME=? "
					...
					stmt2.setString(12, fill_station_cd);
					stmt2.setString(13, bay_cd);
					stmt2.setString(14, slot_start_time);
					stmt2.setString(15, slot_end_time);
					rset2=stmt2.executeQuery();
					if(rset2.next())
					{
					...
					stmt3.setString(9, truck_trans_cd);
					stmt3.setString(10, truck_cd);
					stmt3.setString(11, cargo_no);
					stmt3.setString(12, fill_station_cd);
					stmt3.setString(13, bay_cd);
					stmt3.setString(14, slot_start_time);
					stmt3.setString(15, slot_end_time);
					rset3=stmt3.executeQuery();
					if(rset3.next())
					{
					...
					if(Double.parseDouble(""+sell_nom)<mdcq_qty)
					{
						Seller_PNQ_Qty = Double.parseDouble(""+sell_nom);
					}
					else
					{
						Seller_PNQ_Qty = mdcq_qty;
					}
					
					Seller_PNQ_Qty = Double.parseDouble(nf.format(Seller_PNQ_Qty));
				
					Seller_RE_Qty = (Double.parseDouble(""+sell_nom) - Seller_PNQ_Qty);
					...
					total_sell_pnq+=Seller_PNQ_Qty;
					total_sell_regas+=Seller_RE_Qty;
					
					//HM20260211: Using Bigdecimal to prevent floating point vulnerability.
					BigDecimal buyNomValue = new BigDecimal(buy_nom);
					
					//HM20260607: If no buyer nomination is available for the truck, displaying the seller nomination data as per Vijay’s feedback. 
					if(buy_nom.equals("") || (buyNomValue.compareTo(BigDecimal.ZERO)==0)) 
					{
						buy_nom = sell_nom;
						total_buy_nom = total_sell_pnq;
					}
					
					VATT1_DATE.add(date);
					VATT1_DCQ.add(dcq);
					VATT1_TRCUK_REG_NO.add(dlng_util.getTruckRegNo(conn, truck_cd));
					VATT1_BUYNOM.add(buy_nom);
					VATT1_SELLNOM_PNG.add(nf.format(Seller_PNQ_Qty));
					...
					queryString4="SELECT SUM(QTY_MMBTU) "
							...
							+ ") ";
					stmt4=conn.prepareStatement(queryString4);
					stmt4.setString(++selCnt1, cont_no);
					stmt4.setString(++selCnt1, agmt_no);
					stmt4.setString(++selCnt1, comp_cd);
					stmt4.setString(++selCnt1, counterparty_cd);
					stmt4.setString(++selCnt1, plant_seq);
					stmt4.setString(++selCnt1, contract_type);
					stmt4.setString(++selCnt1, bu_unit);
					stmt4.setString(++selCnt1, date);
					//stmt4.setString(++selCnt1, period_start_dt);
					//stmt4.setString(++selCnt1, period_end_dt);
					stmt4.setString(++selCnt1, truck_trans_cd);
					stmt4.setString(++selCnt1, truck_cd);
					stmt4.setString(++selCnt1, cargo_no);
					stmt4.setString(++selCnt1, fill_station_cd);
					stmt4.setString(++selCnt1, bay_cd);
					stmt4.setString(++selCnt1, slot_start_time);
					stmt4.setString(++selCnt1, slot_end_time);
					rset4=stmt4.executeQuery();
					if(rset4.next())
					{
					...
					selCnt=0;
					queryString5="SELECT SUM(TOTAL_QTY_MMBTU) "
							+ "FROM ("
								+ "SELECT SUM(QTY_MMBTU) AS TOTAL_QTY_MMBTU "
								+ "FROM FMS_DLNG_ALLOC_MST A "
								+ "WHERE CONT_NO=? AND AGMT_NO=? "
								...
								+ ") ";
					queryString5+= "UNION ";
					if(VATT1_DATE.contains(date))
					{
						queryString5+= "SELECT 0 AS TOTAL_QTY_MMBTU FROM DUAL ";
					}
					else
					{
						queryString5+= "SELECT SUM(QTY_MMBTU) AS TOTAL_QTY_MMBTU "
				  				+ "FROM FMS_DAILY_ALLOCATION_DTL A "
				  				+ "WHERE CONT_NO=? AND AGMT_NO=? "
								+ "AND COMPANY_CD=? AND COUNTERPARTY_CD=? "
								+ "AND CONTRACT_TYPE=? "
					...
					queryString5+= ")";
					stmt5=conn.prepareStatement(queryString5);
					stmt5.setString(++selCnt, cont_no);
					stmt5.setString(++selCnt, agmt_no);
					stmt5.setString(++selCnt, comp_cd);
					stmt5.setString(++selCnt, counterparty_cd);
					stmt5.setString(++selCnt, contract_type);
					stmt5.setString(++selCnt, date);
					//stmt5.setString(++selCnt, period_start_dt);
					//stmt5.setString(++selCnt, period_end_dt);
					stmt5.setString(++selCnt, truck_trans_cd);
					stmt5.setString(++selCnt, truck_cd);
					stmt5.setString(++selCnt, cargo_no);
					stmt5.setString(++selCnt, fill_station_cd);
					stmt5.setString(++selCnt, bay_cd);
					stmt5.setString(++selCnt, slot_start_time);
					stmt5.setString(++selCnt, slot_end_time);
					if(!VATT1_DATE.contains(date)) 
					{
						stmt5.setString(++selCnt, cont_no);
						stmt5.setString(++selCnt, agmt_no);
	...
	public void updateSAPapprovalFlag() 
	{
		...
		try
		{
			...
			if(type_flag.equals("SG"))
			{
				String sapAprvFlag="";
				String invNo="";
				String query="SELECT SAP_APPROVAL,INVOICE_NO "
						+ "FROM FMS_DLNG_INVOICE_MST "
						+ "WHERE COMPANY_CD=? AND FINANCIAL_YEAR=? "
						+ "AND INVOICE_SEQ=? AND BU_STATE_TIN=? ";
				stmt0 = conn.prepareStatement(query);
				stmt0.setString(1, comp_cd);
				stmt0.setString(2, financial_year);
				stmt0.setString(3, invoice_seq);
				stmt0.setString(4, bu_state_tin);
				rset0=stmt0.executeQuery();
				if(rset0.next())
				{
					sapAprvFlag=rset0.getString(1)==null?"":rset0.getString(1);
					invNo=rset0.getString(2)==null?"":rset0.getString(2);
				}
				rset0.close();
				stmt0.close();
				
				String abbr=utilBean.getCounterpartyABBR(conn,counterparty_cd);
				if(sapAprvFlag.equals("Y"))
				{
					msg = "Automated SAP Approval Blocked! - "+abbr+" Invoice("+invNo+") SAP Approval Already Done!";
				}
				else
				{
					String queryString0 ="UPDATE FMS_DLNG_INVOICE_MST SET SAP_APPROVAL=?,SAP_APPROVED_BY=?,SAP_APPROVED_DT=SYSDATE "
							+ "WHERE COMPANY_CD=? AND FINANCIAL_YEAR=? "
							+ "AND INVOICE_SEQ=? AND BU_STATE_TIN=? ";
					sap_approval_flag="Y";
					...
					msg = "Successful! - "+abbr+" Invoice("+invNo+") Auto Approved Successfully Done By System!";
					
					setSap_approval_flag("Y");
					generateSalesInvoiceXML();
				}
			}
		}
		...
		{
			new SystemErrorLogger().InsertErrorLogger(db_src_file_name, function_nm, e);
			msg="Error In Exception! - Insert/Update SAP Auto Approval Failed By System!";
		}
	...
	// SagarB20251006 Added for DLNG Sales Actual Report
	String chk = "";
	...
	String sell_cont_map = "";
	String sell_cont_no = "";
	String sell_cont_rev_no = "";
	String sell_agmt_no = "";
	String sell_agmt_rev_no = "";
	String sell_contract_type = "";

	String total_truck_invoice = "";
	...
	public void setQtyMmbtu(String qtyMmbtu) {this.qtyMmbtu = qtyMmbtu;}

	public void setSell_cont_map(String sell_cont_map) {this.sell_cont_map = sell_cont_map;}
	public void setSell_cont_no(String sell_cont_no) {this.sell_cont_no = sell_cont_no;}
	public void setSell_cont_rev_no(String sell_cont_rev_no) {this.sell_cont_rev_no = sell_cont_rev_no;}
	public void setSell_agmt_no(String sell_agmt_no) {this.sell_agmt_no = sell_agmt_no;}
	public void setSell_agmt_rev_no(String sell_agmt_rev_no) {this.sell_agmt_rev_no = sell_agmt_rev_no;}
	public void setSell_contract_type(String sell_contract_type) {this.sell_contract_type = sell_contract_type;}
	...
	public String getTruck_cd() {return truck_cd;}
	public String getTruck_trans_cd() {return truck_trans_cd;}
	public String getMapping_id() {return mapping_id;}

	public String getTotal_truck_invoice() {return total_truck_invoice;}
	...
	Vector VALLOC_MT = new Vector();	// SagarB20251006 Added for DLNG Sales Actual Report
	...
	Vector VMST_SAC_DESC = new Vector();
	...
	Vector VRECV_DT = new Vector();
	Vector VRECV_REMARK = new Vector();
	Vector VRECV_AMT = new Vector();
	
	Vector VSALES_INVOICE_NO = new Vector();
	Vector VSALES_INV_DT =new Vector();
	Vector VSALES_TRUCK_REG_NO = new Vector();
	Vector VSALES_TRUCK_CD = new Vector();
	Vector VSALES_INV_QTY = new Vector();
	Vector VSALES_INV_QTY_MT = new Vector();
	Vector VSALES_INVOICE_SEQ = new Vector();
	Vector VSALES_MAPPING_ID = new Vector();
	Vector VSALES_TRUCK_TRANS_CD = new Vector();
	Vector VSELL_PERIOD_START_DT = new Vector();
	Vector VSELL_PERIOD_END_DT = new Vector();
	Vector VSALES_INV_LINKED = new Vector();
	Vector VSALES_CONT_MAPP =  new Vector();
	Vector VSELL_CONT_PERIOD_START_DT =  new Vector();
	Vector VSELL_CONT_PERIOD_END_DT =  new Vector();
	
	Vector VGAS_DT = new Vector();
	Vector VDISP_SLOT_DTL = new Vector();
	...
	Vector VSELL_AGMT_NO = new Vector();
	Vector VSELL_AGMT_REV_NO = new Vector();
	Vector VSELL_CONTRACT_TYPE = new Vector();
	Vector VSELL_CONT_MAP = new Vector();
	Vector VSELL_CONT_REF = new Vector();
	Vector VSELL_DISP_CONT = new Vector();

	Vector VTOTAL_TRUCK_INV_QTY = new Vector();
	
	Vector VATT1_COL1 = new Vector();
	Vector VATT1_COL2 = new Vector();
	Vector VATT1_COL3 = new Vector();
	Vector VATT1_COL4 = new Vector();
	Vector VATT1_COL5 = new Vector();
	Vector VATT1_COL6 = new Vector();
	...
	Vector VATT1_TRCUK_REG_NO = new Vector();
	...
	public Vector getVALLOC_MT() {return VALLOC_MT;}	// SagarB20251006 Added for DLNG Sales Actual Report
	...
	public Vector getVMST_SAC_DESC() {return VMST_SAC_DESC;}
	
	public Vector getVRECV_DT() {return VRECV_DT;}
	public Vector getVRECV_REMARK() {return VRECV_REMARK;}
	public Vector getVRECV_AMT() {return VRECV_AMT;}
	
	public Vector getVSALES_INVOICE_NO() {return VSALES_INVOICE_NO;}
	public Vector getVSALES_INV_DT() {return VSALES_INV_DT;}
	public Vector getVSALES_TRUCK_CD() {return VSALES_TRUCK_CD;}
	public Vector getVSALES_TRUCK_REG_NO() {return VSALES_TRUCK_REG_NO;}
	public Vector getVSALES_INV_QTY() {return VSALES_INV_QTY;}
	public Vector getVSALES_INV_QTY_MT() {return VSALES_INV_QTY_MT;}
	public Vector getVSALES_TRUCK_TRANS_CD() {return VSALES_TRUCK_TRANS_CD;}
	public Vector getVSALES_MAPPING_ID() {return VSALES_MAPPING_ID;}
	public Vector getVSALES_INVOICE_SEQ() {return VSALES_INVOICE_SEQ;}
	public Vector getVSALES_INV_LINKED() {return VSALES_INV_LINKED;}
	public Vector getVSELL_PERIOD_START_DT() {return VSELL_PERIOD_START_DT;}
	public Vector getVSELL_PERIOD_END_DT() {return VSELL_PERIOD_END_DT;}
	public Vector getVSALES_CONT_MAPP() {return VSALES_CONT_MAPP;}
	public Vector getVSELL_CONT_PERIOD_END_DT() {return VSELL_CONT_PERIOD_END_DT;}
	public Vector getVSELL_CONT_PERIOD_START_DT() {return VSELL_CONT_PERIOD_START_DT;}
	
	public Vector getVGAS_DT() {return VGAS_DT;}
	public Vector getVDISP_SLOT_DTL() {return VDISP_SLOT_DTL;}
	...
	public Vector getVSELL_AGMT_NO() {return VSELL_AGMT_NO;}
	public Vector getVSELL_AGMT_REV_NO() {return VSELL_AGMT_REV_NO;}
	public Vector getVSELL_CONTRACT_TYPE() {return VSELL_CONTRACT_TYPE;}

	public Vector getVTOTAL_TRUCK_INV_QTY() {return VTOTAL_TRUCK_INV_QTY;}
	public Vector getVSELL_CONT_MAP() {return VSELL_CONT_MAP;}
	public Vector getVSELL_DISP_CONT() {return VSELL_DISP_CONT;}
	public Vector getVSELL_CONT_REF() {return VSELL_CONT_REF;}
	
	public Vector getVATT1_COL1() {return VATT1_COL1;}
	public Vector getVATT1_COL2() {return VATT1_COL2;}
	public Vector getVATT1_COL3() {return VATT1_COL3;}
	public Vector getVATT1_COL4() {return VATT1_COL4;}
	public Vector getVATT1_COL5() {return VATT1_COL5;}
	public Vector getVATT1_COL6() {return VATT1_COL6;}
	...
	public Vector getVATT1_TRCUK_REG_NO() {return VATT1_TRCUK_REG_NO;}
	...
	public String getTransport_mgmt_unit_nm() {return transport_mgmt_unit_nm;}
	...
	public String getQty_unit() {return qty_unit;}
	public String getQty_unit_nm() {return qty_unit_nm;}
	public String getDist_unit() {return dist_unit;}
	public String getSac_no() {return sac_no;}
	public String getSac_desc() {return sac_desc;}
	...
	public String getUnit_desc() {return unit_desc;}
