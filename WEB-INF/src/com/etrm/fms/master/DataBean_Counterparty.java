...
public class DataBean_Counterparty 
{
		...
		try
		{
	    	...
	    	if(ds != null) 
	    	{
	    		...
	    		{	    			
	    			...
	    			else if(callFlag.equalsIgnoreCase("ENTITY_BANK_MASTER"))
	    			{
	    				getCountryMst();
	    				getStateMst();
	    				getEntityMaster();
	    				...
	    				getCounterpartyVertualBankCodeDetail();
	    			}
	    		}
	...
	public void getCounterpartyDetail()
	{
		...
		try
		{
			String queryString="SELECT TO_CHAR(EFF_DT,'DD/MM/YYYY'),COUNTERPARTY_NM,COUNTERPARTY_ABBR,PAN_NO,TO_CHAR(PAN_ISSUE_DT,'DD/MM/YYYY'),NOTES,STATUS,KYC,IGX,"
					+ "CATEGORY,WEB_ADDR,SAP_CODE,NCF_CATEGORY,MAX(TO_CHAR(EFF_DT,'DD/MM/YYYY')) AS MAX_EFF_DT "
					...
					+ " GROUP BY EFF_DT,COUNTERPARTY_NM, COUNTERPARTY_ABBR, PAN_NO, PAN_ISSUE_DT, NOTES, STATUS, KYC, IGX, CATEGORY, WEB_ADDR, SAP_CODE, NCF_CATEGORY";
			...
			if(rset.next())
			{
				...
				max_eff_dt = rset.getString(14)==null?"0":rset.getString(14);
				
				if("N".equals(status)) 
				{
					String queryString1 = "SELECT TO_CHAR(EFF_DT, 'DD/MM/YYYY') "
				            + "FROM FMS_COUNTERPARTY_MST A "
				            + "WHERE COUNTERPARTY_CD = ? "
				            + "AND EFF_DT = ("
				            + " SELECT MAX(B.EFF_DT) "
				            + " FROM FMS_COUNTERPARTY_MST B "
				            + " WHERE A.COUNTERPARTY_CD = B.COUNTERPARTY_CD "
				            + " AND B.EFF_DT < (SELECT MAX(C.EFF_DT) "
				            + " FROM FMS_COUNTERPARTY_MST C "
				            + " WHERE A.COUNTERPARTY_CD = C.COUNTERPARTY_CD)"
				            + ")";
				...
				String queryString2 = "SELECT TO_CHAR(EFF_DT, 'DD/MM/YYYY'),STATUS,COUNTERPARTY_ABBR "
			            + "FROM FMS_COUNTERPARTY_MST A "
			            + "WHERE COUNTERPARTY_CD = ? "
				...
				while(rset2.next())
				{
					String cp_eff_dt = rset2.getString(1)==null?"":rset2.getString(1);
					String cp_status = rset2.getString(2)==null?"":rset2.getString(2);
					String cp_abbr = rset2.getString(3)==null?"":rset2.getString(3);

					String cp_status_nm = cp_status.equals("Y") ? "Active" : cp_status.equals("N") ? "Deactive" : "";
					
					if(cp_hist_desc.equals("")) 
					{
						//cp_hist_desc = cp_abbr+" :: "+cp_eff_dt+" ("+cp_status_nm+")";
						cp_hist_desc = cp_status_nm+" Effective "+cp_eff_dt;
					}
					else
					{
						//cp_hist_desc += "\n"+cp_abbr+" :: "+cp_eff_dt+" ("+cp_status_nm+")";
						cp_hist_desc += "\n"+cp_status_nm+" Effective "+cp_eff_dt;
					}
				}
				rset2.close();
				stmt2.close();
	...
	public void getCompanyOwnerDetail()
	{
		...
		try
		{
			String queryString="SELECT TO_CHAR(EFF_DT,'DD/MM/YYYY'),COMPANY_NM,COMPANY_ABBR,PAN_NO,TO_CHAR(PAN_ISSUE_DT,'DD/MM/YYYY'),NOTES,STATUS,"
	...
	public void getCompanyExchangeDetail()
	{
		...
		try
		{
			String queryString="SELECT TO_CHAR(EFF_DT,'DD/MM/YYYY'),COUNTERPARTY_NM,COUNTERPARTY_ABBR,PAN_NO,TO_CHAR(PAN_ISSUE_DT,'DD/MM/YYYY'),NOTES,STATUS,"
					+ "CATEGORY,WEB_ADDR,SAP_CODE,NCF_CATEGORY "
	...
	public void getCounterpartyAddressDetail()
	{
		...
		try
		{
			String queryString="SELECT ADDRESS_TYPE,TO_CHAR(EFF_DT,'DD/MM/YYYY'),ADDR,CITY,PIN,STATE,ZONE,COUNTRY,PHONE,MOBILE,ALT_PHONE,FAX_1,FAX_2,EMAIL "
	...
	public void getEntityRoleRequest()
	{
		...
		try 
		{
			name=utilBean.getCounterpartyName(conn,counterparty_cd);
			
			String queryString="SELECT ENTITY,COUNT(*) "
			...
			String queryString3="SELECT COUNTERPARTY_CD,ENTITY,REMARK,REQ_BY,TO_CHAR(REQ_DT,'DD/MM/YYYY'),SEQ_NO "
					+ "FROM FMS_ENTITY_REQ_DTL "
			...
			while(rset3.next())
			{
				...
				VCOUNTERPTY_NM.add(""+utilBean.getCounterpartyName(conn,counterpty_cd));
	...
	public void getEntityMaster()
	{
		...
		try
		{
			if(!entity_role.equals("0"))
			{
				...
				{
					String queryString1 = "SELECT COUNTERPARTY_CD,COUNTERPARTY_NM,COUNTERPARTY_ABBR "
							+ "FROM FMS_COMPANY_EXCHG_MST A "
				...
				{
					int cont=0;
					String queryString2 = "SELECT A.COUNTERPARTY_CD,A.COUNTERPARTY_NM,A.COUNTERPARTY_ABBR "
	...
	public void getEntityTurnoverEntryMaster()
	{
		String function_nm="getEntityTurnoverEntryMaster()";
		
		try
		{
			if(!financial_year.equals("0") && VFINANCIAL_YEAR.size()>0)
			{
				String finyear_start_date = "01/04/"+financial_year.split("-")[0];
				...
				if(!entity_role.equals("0"))
				{
					if(entity_role.equals("B"))
					{
						...
						String queryString = "SELECT COMPANY_CD,COMPANY_NM,COMPANY_ABBR "
								...
								+ "WHERE COMPANY_CD=? "
								+ "AND EFF_DT=(SELECT MAX(EFF_DT) FROM FMS_COMPANY_OWNER_MST B WHERE A.COMPANY_CD=B.COMPANY_CD) "
								+ "AND (A.STATUS = 'Y' OR (A.STATUS = 'N' AND (A.EFF_DT >= TO_DATE(?, 'DD/MM/YYYY')))) ";
						queryString+= "ORDER BY COMPANY_NM";
						stmt=conn.prepareStatement(queryString);
						stmt.setString(++cont, comp_cd);
						stmt.setString(++cont, finyear_start_date);
						rset=stmt.executeQuery();
						while (rset.next()) 
						{
							VCOUNTERPARTY_CD.add(rset.getString(1)==null?"":rset.getString(1));
					...
					else if(entity_role.equals("G"))
					{
						...
						queryString1+= "ORDER BY COUNTERPARTY_NM";
						stmt1=conn.prepareStatement(queryString1);
						stmt1.setString(++cont, finyear_start_date);
						rset1=stmt1.executeQuery();
						while (rset1.next()) 
						{
							VCOUNTERPARTY_CD.add(rset1.getString(1)==null?"":rset1.getString(1));
					...
					else
					{
						...
						stmt2.setString(++cont, comp_cd);
						stmt2.setString(++cont, entity_role);
						stmt2.setString(++cont, finyear_start_date);
						rset2=stmt2.executeQuery();
						while (rset2.next()) 
						{
							VCOUNTERPARTY_CD.add(rset2.getString(1)==null?"":rset2.getString(1));
	...
	public void getSectorMaster()
	{
		...
		try
		{
			String queryString="SELECT SECTOR_CD,SECTOR_NAME "
					+ "FROM FMS_SECTOR_MST "
					+ "WHERE STATUS_FLAG=? "
	...
	public void getCounterpartyRequesterApproverNote()
	{
		...
		try
		{
			String queryString="SELECT ENTITY,REMARK,APRV_NOTE,REQ_BY,TO_CHAR(REQ_DT,'DD/MM/YYYY HH:MM:SS'),APRV_BY,TO_CHAR(APRV_DT,'DD/MM/YYYY HH:MM:SS') "
					+ "FROM FMS_ENTITY_REQ_DTL "
	...
	public void getCounterpartyPlantDetail()
	{
		...
		try
		{
			String queryString="SELECT COUNTERPARTY_CD,ENTITY,SEQ_NO,TO_CHAR(EFF_DT,'DD/MM/YYYY'),PLANT_NAME,PLANT_ABBR,PLANT_ADDR,PLANT_STATE,"
			...
			while(rset.next())
			{
				...
				String tax_id="";
				String queryString1 = "SELECT A.STAT_NO, B.STAT_NM "
	...
	public void getCounterpartyBankDetail()
	{
		...
		try
		{
			...
			while(rset.next())
			{
				...
				//bank_formula+=bank_category+" : "+bank_name+", "+bank_branch+", "+bank_state+", A/C No : "+bank_account_no+", IFSC Code : "+ifsc_code+"\n";
				bank_formula+=bank_category+" : "+utilBean.getEntityBankFormula(conn, comp_cd, counterparty_cd,entity_role, bank_category, sysdt)+"\n";
			}
			rset.close();
			stmt.close();
			
			selCnt=0;
			String queryString1="SELECT COUNTERPARTY_CD,ENTITY,TO_CHAR(EFF_DT,'DD/MM/YYYY'),BANK_NAME,ACCOUNT_ID,IFSC_CODE,BRANCH_NAME,"
					+ "STATE_NM,CATEGORY  "
					+ "FROM FMS_ENTITY_BANK_MST A "
					+ "WHERE ENTITY=? AND COMPANY_CD=? AND COUNTERPARTY_CD=? "
					+ "ORDER BY EFF_DT DESC";
			stmt1=conn.prepareStatement(queryString1);
			stmt1.setString(++selCnt, entity_role);
			...
			while(rset1.next())
			{
				...
				String bank_category=rset1.getString(9)==null?"":rset1.getString(9);
				
				
				//String  bank_formula=bank_name+", "+bank_branch+", "+bank_state+", A/C No : "+bank_account_no+", IFSC Code : "+ifsc_code;
				String  bank_formula=utilBean.getEntityBankFormula(conn, comp_cd, counterparty_cd,entity_role, bank_category, sysdt);
				
				VBANK_FORMULA.add(bank_formula);
				VBANK_EFF_DT.add(bank_eff_dt);
				VBANK_CATEGORY.add(bank_category);
	...
	public void fetchGovtStatNoList()
	{
		...
		try 
		{
			...
			while(rset.next())
			{
				...
				{
					stat_type = "Commodity Invoice Tax";
				}
				else if(stat_type.trim().equals("R"))
				{
					stat_type = "Commodity | Service Invoice Tax";
				}
	...
	public void getCounterpartyMatrix()
	{
		...
		try
		{
			String queryString="SELECT A.COUNTERPARTY_CD,TO_CHAR(A.EFF_DT,'DD/MM/YYYY'),A.COUNTERPARTY_NM,A.COUNTERPARTY_ABBR,"
			...
			while(rset.next())
			{
				...
				String effDate =rset.getString(2)==null?"":rset.getString(2);
				...
				String req_by= ""+utilBean.getEmpName(conn,rset.getString(12)==null?"0":rset.getString(12));
				String req_dt = rset.getString(13)==null?"":rset.getString(13);
				String aprv_by= ""+utilBean.getEmpName(conn,rset.getString(14)==null?"0":rset.getString(14));
				String aprv_dt = rset.getString(15)==null?"":rset.getString(15);
				String category = rset.getString(16)==null?"":rset.getString(16);
				String ncf_category = rset.getString(17)==null?"":rset.getString(17);
	...
	public void getBuPlantDetail()
	{
		...
		try
		{
			String queryString="SELECT SEQ_NO,PLANT_ABBR "
	...
	public void getEntityContactDetail()
	{
		...
		try
		{
			String queryString="SELECT SEQ_NO,TO_CHAR(EFF_DT,'DD/MM/YYYY'),CONTACT_PERSON,DESIGNATION,PHONE,MOBILE,FAX_1,FAX_2,EMAIL,ADDR_FLAG,ADDL_ADDR_LINE,"
			...
			while(rset.next())
			{
				...
				int index=0;
				String queryString1="SELECT ADDR_FLAG "
				...
				while(rset1.next())
				{
					...
					if(add_flag.startsWith("P"))
					{
						...
						add_type_nm = utilBean.getCounterpartyPlantABBR(conn,counterparty_cd, comp_cd, no, entity_role);
					}
	...
	public void getEntityContactDetailV2()
	{
		...
		try
		{
			String queryString="SELECT SEQ_NO,TO_CHAR(EFF_DT,'DD/MM/YYYY'),CONTACT_PERSON,DESIGNATION,MOBILE,EMAIL,"
					+ "ACTIVE_FLAG "
					+ "FROM FMS_ENTITY_CONTACT_MST A "
					+ "WHERE COMPANY_CD=? AND COUNTERPARTY_CD=? AND ENTITY=? "
					+ "AND EFF_DT=(SELECT MAX(B.EFF_DT) FROM FMS_ENTITY_CONTACT_MST B WHERE A.COMPANY_CD=B.COMPANY_CD "
			...
			while(rset.next())
			{
				...
				VSTATUS.add(rset.getString(7)==null?"N":rset.getString(7));
				...
				for(int i=0;i<2;i++)
				{
					...
					String temp_rm="N";
					String temp_f402="N";
					String type=i==1?"DLNG":"RLNG";
					
					String query = "SELECT NOM_FLAG,INV_FLAG,FM_FLAG,PM_FLAG,JT_FLAG,OTHER_FLAG,RM_FLAG,F402_FLAG "
							+ "FROM FMS_ENTITY_CONTACT_MST A "
							+ "WHERE COMPANY_CD=? AND COUNTERPARTY_CD=? AND ENTITY=? AND SEQ_NO=? "
							+ "AND EFF_DT=TO_DATE(?,'DD/MM/YYYY') AND TYPE=? "
							+ "GROUP BY NOM_FLAG,INV_FLAG,FM_FLAG,PM_FLAG,JT_FLAG,OTHER_FLAG,RM_FLAG,F402_FLAG ";
					...
					if(rset1.next())
					{
						temp_nom=rset1.getString(1)==null?"N":rset1.getString(1);
						temp_inv=rset1.getString(2)==null?"N":rset1.getString(2);
						temp_fm=rset1.getString(3)==null?"N":rset1.getString(3);
						temp_pm=rset1.getString(4)==null?"N":rset1.getString(4);
						temp_jt=rset1.getString(5)==null?"N":rset1.getString(5);
						temp_other=rset1.getString(6)==null?"N":rset1.getString(6);
						temp_rm=rset1.getString(7)==null?"N":rset1.getString(7);
						temp_f402=rset1.getString(8)==null?"N":rset1.getString(8);
					}
					rset1.close();
					stmt1.close();
					if("RLNG".equals(type)) 
					{						
						...
						VPM.add(temp_pm);
						VJT.add(temp_jt);
						VOTHER.add(temp_other);
						VRM.add(temp_rm);
					}
					else if("DLNG".equals(type))
					{
						VDLNG_NOM.add(temp_nom);
						VDLNG_INV.add(temp_inv);
						VDLNG_FM.add(temp_fm);
						VDLNG_PM.add(temp_pm);
						VDLNG_JT.add(temp_jt);
						VDLNG_OTHER.add(temp_other);
						VDLNG_RM.add(temp_rm);
						VDLNG_F402.add(temp_f402);
					}
				}
				...
				String temp_dlng_to_f402="";
				
				int index=0;
				String queryString1="SELECT ADDR_FLAG,ADDL_ADDR_LINE,PHONE,FAX_1,FAX_2,ADDR_IS_ACTIVE "
						//+ "TO_NOM,TO_INV,TO_FM,TO_PM,TO_JT,TO_OTHER,TO_RM "
						+ "FROM FMS_ENTITY_CONTACT_MST "
						+ "WHERE COMPANY_CD=? AND COUNTERPARTY_CD=? AND ENTITY=? "
						+ "AND SEQ_NO=? AND EFF_DT=TO_DATE(?,'DD/MM/YYYY')"
						+ "GROUP BY ADDR_FLAG,ADDL_ADDR_LINE,PHONE,FAX_1,FAX_2,ADDR_IS_ACTIVE ";
				stmt1=conn.prepareStatement(queryString1);
				stmt1.setString(1, comp_cd);
				stmt1.setString(2, counterparty_cd);
				stmt1.setString(3, entity_role);
				...
				while(rset1.next())
				{
					...
					{
						String no = add_flag.substring(1,add_flag.length());
						add_type_nm = utilBean.getCounterpartyBuPlantABBR(conn,counterparty_cd, comp_cd, no, entity_role);
					}
					else if(add_flag.equals("R"))
					{
						add_type_nm="Registered";
					...
					for(int i=0;i<2;i++)
					{
						...
						String query = "SELECT TO_NOM,TO_INV,TO_FM,TO_PM,TO_JT,TO_OTHER,TO_RM  "
								+ "FROM FMS_ENTITY_CONTACT_MST A "
								+ "WHERE COMPANY_CD=? AND COUNTERPARTY_CD=? AND ENTITY=? AND SEQ_NO=? "
								+ "AND EFF_DT=TO_DATE(?,'DD/MM/YYYY') AND TYPE=? AND ADDR_FLAG=? ";
								//+ "GROUP BY NOM_FLAG,INV_FLAG,FM_FLAG,PM_FLAG,JT_FLAG,OTHER_FLAG,RM_FLAG ";
						stmt2=conn.prepareStatement(query);
						stmt2.setString(1, comp_cd);
						stmt2.setString(2, counterparty_cd);
						stmt2.setString(3, entity_role);
						stmt2.setString(4, seq_no);
						stmt2.setString(5, eff_dt);
						stmt2.setString(6, type);
						stmt2.setString(7, add_flag);
						rset2=stmt2.executeQuery();
						if(rset2.next())
						{
							to_nom=rset2.getString(1)==null?"N":rset2.getString(1);
							to_inv=rset2.getString(2)==null?"N":rset2.getString(2);
							to_fm=rset2.getString(3)==null?"N":rset2.getString(3);
							to_pm=rset2.getString(4)==null?"N":rset2.getString(4);
							to_jt=rset2.getString(5)==null?"N":rset2.getString(5);
							to_other=rset2.getString(6)==null?"N":rset2.getString(6);
							to_rm=rset2.getString(7)==null?"N":rset2.getString(7);
						}
						rset2.close();
						stmt2.close();
						if("RLNG".equals(type)) 
						...
						{
							temp_dlng_to_nom=temp_dlng_to_nom+"@@"+to_nom;
							temp_dlng_to_inv=temp_dlng_to_inv+"@@"+to_inv;
							temp_dlng_to_fm=temp_dlng_to_fm+"@@"+to_fm;
							temp_dlng_to_pm=temp_dlng_to_pm+"@@"+to_pm;
							temp_dlng_to_jt=temp_dlng_to_jt+"@@"+to_jt;
							temp_dlng_to_other=temp_dlng_to_other+"@@"+to_other;
							temp_dlng_to_rm=temp_dlng_to_rm+"@@"+to_rm;
							temp_dlng_to_f402=temp_dlng_to_f402+"@@"+to_f402;
						}
					}
				...
				VTEMP_DLNG_TO_F402.add(temp_dlng_to_f402);
			}
			rset.close();
			stmt.close();
		}
	...
	public void getEntityPlantDetail()
	{
		...
		try
		{
			...
			String queryString = "SELECT PLANT_STATE "
					+ "FROM FMS_COUNTERPARTY_PLANT_DTL A "
			...
			{
				String queryString0="SELECT COUNTERPARTY_CD,SEQ_NO,PLANT_NAME,PLANT_ABBR,PLANT_STATE "
						+ "FROM FMS_COUNTERPARTY_BU_DTL A "
						+ "WHERE ENTITY=? AND COMPANY_CD=? AND COUNTERPARTY_CD=? "
						+ "AND EFF_DT=(SELECT MAX(b.EFF_DT) FROM FMS_COUNTERPARTY_BU_DTL B WHERE A.COUNTERPARTY_CD=B.COUNTERPARTY_CD "
						+ "AND A.SEQ_NO=B.SEQ_NO AND A.COMPANY_CD=B.COMPANY_CD AND B.EFF_DT<=TO_DATE(TO_CHAR(SYSDATE,'DD/MM/YYYY'),'DD/MM/YYYY') AND A.ENTITY=B.ENTITY) "
				...
				while(rset0.next())
				{
					...
					tax_category ="S";
					int index=0;
					String queryString1="SELECT A.TAX_STRUCT_CD,TO_CHAR(B.APP_DATE,'DD/MM/YYYY'),A.TAX_STRUCT_DTL,A.TAX_STRUCT_REMARK,A.INVOICE_TYPE,"
							+ "TO_CHAR(A.ENT_DT,'DD/MM/YYYY HH24:MI:SS'),A.ENT_BY,TO_CHAR(A.EFF_DT,'DD/MM/YYYY'),B.SAP_TAX_CODE "
					...
					while(rset1.next())
					{
						...
						struct_nm+=" Commencement on "+app_dt;
						...
						VBU_SER_ENTERED_BY.add(""+utilBean.getEmpName(conn,ent_by)+"<br>"+ent_dt);
						...
						if(rset3.next())
						{
							if(rset3.getInt(1) > 1)
							{
								...
								/*String queryString2="SELECT TAX_STRUCT_CD,TO_CHAR(TAX_STRUCT_DT,'DD/MM/YYYY'),TAX_STRUCT_DTL,TAX_STRUCT_REMARK,TO_CHAR(ENT_DT,'DD/MM/YYYY'),ENT_BY,"
										+ "TO_CHAR(EFF_DT,'DD/MM/YYYY'),SAP_TAX_CODE "
										+ "FROM FMS_ENTITY_BU_SVC_TAX_DTL A "
										+ "WHERE ENTITY=? AND COMPANY_CD=? AND COUNTERPARTY_CD=? "
										+ "AND PLANT_SEQ_NO=? AND INVOICE_TYPE=? AND BU_UNIT=? "
										+ "ORDER BY EFF_DT DESC";*/
								String queryString2="SELECT A.TAX_STRUCT_CD,TO_CHAR(B.APP_DATE,'DD/MM/YYYY'),A.TAX_STRUCT_DTL,A.TAX_STRUCT_REMARK,"
										+ "TO_CHAR(A.ENT_DT,'DD/MM/YYYY HH24:MI:SS'),A.ENT_BY,TO_CHAR(A.EFF_DT,'DD/MM/YYYY'),B.SAP_TAX_CODE "
										+ "FROM FMS_ENTITY_BU_SVC_TAX_DTL A, FMS_TAX_STRUCTURE B "
										+ "WHERE A.ENTITY=? AND A.COMPANY_CD=? AND A.COUNTERPARTY_CD=? "
										+ "AND A.PLANT_SEQ_NO=? AND A.INVOICE_TYPE=? AND A.BU_UNIT=? "
										+ "AND A.TAX_STRUCT_CD=B.TAX_STR_CD "
								...
								while(rset2.next())
								{
									...
									VTEMP_BU_SER_ENTERED_BY.add(""+utilBean.getEmpName(conn,ent_by)+"<br>"+ent_dt);
			...
			else
			{
				...
				String queryString0="SELECT COUNTERPARTY_CD,ENTITY,SEQ_NO,TO_CHAR(EFF_DT,'DD/MM/YYYY'),PLANT_NAME,PLANT_ABBR,PLANT_ADDR,PLANT_STATE,"
						+ "PLANT_ZONE,PLANT_CITY,PLANT_PIN,PLANT_SECTOR "
						+ "FROM FMS_COUNTERPARTY_PLANT_DTL A "
						+ "WHERE ENTITY=? AND COMPANY_CD=? AND COUNTERPARTY_CD=? "
						+ "AND EFF_DT=(SELECT MAX(b.EFF_DT) FROM FMS_COUNTERPARTY_PLANT_DTL B WHERE A.COUNTERPARTY_CD=B.COUNTERPARTY_CD "
				...
				while(rset0.next())
				{
					...
					String queryString1="SELECT A.TAX_STRUCT_CD,TO_CHAR(B.APP_DATE,'DD/MM/YYYY'),A.TAX_STRUCT_DTL,A.TAX_STRUCT_REMARK,"
							+ "TO_CHAR(A.EFF_DT,'DD/MM/YYYY'),B.SAP_TAX_CODE,A.INVOICE_TYPE,A.ENT_BY,TO_CHAR(A.ENT_DT,'DD/MM/YYYY HH24:MI:SS') "
					...
					while(rset1.next())
					{
						...
						VCOLOR.add("#99ffcc");
						inv_type=rset1.getString(7)==null?"":rset1.getString(7);
						String ent_by = rset1.getString(8)==null?"":rset1.getString(8);
						String ent_dt = rset1.getString(9)==null?"":rset1.getString(9);
						VENTERED_BY.add(""+utilBean.getEmpName(conn,ent_by)+"<br>"+ent_dt);
						VINVOICE_TYPE.add(inv_type);
						VINVOICE_TYPE_NM.add(""+utilBean.getInvoiceNameByType(entity_role, tax_category, inv_type));
						...
						stmt4.setString(6, inv_type);
						rset4=stmt4.executeQuery();
						if(rset4.next())
						{
							if(rset4.getInt(1) > 0)
							{
								VTEMP_INDEX.add(rset4.getInt(1));
//								System.out.println("entity_role....."+entity_role);
//								System.out.println("comp_cd....."+comp_cd);
//								System.out.println("counterparty_cd....."+counterparty_cd);
//								System.out.println("plant_seq....."+plant_seq);
//								System.out.println("bu_unti....."+bu_unti);
								
								String queryString2="SELECT A.TAX_STRUCT_CD,TO_CHAR(B.APP_DATE,'DD/MM/YYYY'),A.TAX_STRUCT_DTL,A.TAX_STRUCT_REMARK,TO_CHAR(A.ENT_DT,'DD/MM/YYYY HH24:MI:SS'),A.ENT_BY,"
										+ "TO_CHAR(A.EFF_DT,'DD/MM/YYYY'),B.SAP_TAX_CODE,A.INVOICE_TYPE "
										+ "FROM FMS_ENTITY_TAX_STRUCT_DTL A, FMS_TAX_STRUCTURE B "
										+ "WHERE A.ENTITY=? AND A.COMPANY_CD=? AND A.COUNTERPARTY_CD=? "
										+ "AND A.PLANT_SEQ_NO=? AND A.BU_UNIT=? AND A.INVOICE_TYPE=? "
										+ "AND A.TAX_STRUCT_CD=B.TAX_STR_CD "
										+ "ORDER BY A.EFF_DT DESC";
								stmt2=conn.prepareStatement(queryString2);
								stmt2.setString(1, entity_role);
					...
					tax_category ="S";
					index=0;
					String queryString5="SELECT A.TAX_STRUCT_CD,TO_CHAR(B.APP_DATE,'DD/MM/YYYY'),A.TAX_STRUCT_DTL,A.TAX_STRUCT_REMARK,A.INVOICE_TYPE,"
							+ "TO_CHAR(A.ENT_DT,'DD/MM/YYYY HH24:MI:SS'),A.ENT_BY,TO_CHAR(A.EFF_DT,'DD/MM/YYYY'),B.SAP_TAX_CODE "
					...
					while(rset4.next())
					{
						...
						inv_type=rset4.getString(5)==null?"0":rset4.getString(5);
						...
						VSER_ENTERED_BY.add(""+utilBean.getEmpName(conn,ent_by)+"<br>"+ent_dt);
							...
							if(rset3.getInt(1) > 1)
							{
								...
								*/
								...
								while(rset2.next())
								{
									...
									VTEMP_SER_ENTERED_BY.add(""+utilBean.getEmpName(conn,ent_by)+"<br>"+ent_dt);
	...
	public void getEntityContactPersonListRlng()
	{
		...
		try
		{
			String queryStringTemp="SELECT SEQ_NO,CONTACT_PERSON,TO_CHAR(EFF_DT,'DD/MM/YYYY'),TO_"+notification_type+",EMAIL "
					+ "FROM FMS_ENTITY_CONTACT_MST A "
					+ "WHERE COMPANY_CD=? AND COUNTERPARTY_CD=? AND ENTITY=?"
					+ "AND ACTIVE_FLAG=? AND ADDR_IS_ACTIVE=? AND ADDR_FLAG=? AND "+notification_type+"_FLAG=? "
					+ "AND EFF_DT=(SELECT MAX(B.EFF_DT) FROM FMS_ENTITY_CONTACT_MST B WHERE A.COMPANY_CD=B.COMPANY_CD "
					+ "AND A.COUNTERPARTY_CD=B.COUNTERPARTY_CD AND A.ENTITY=B.ENTITY AND A.SEQ_NO=B.SEQ_NO AND A.TYPE=B.TYPE) AND TYPE=? ";
			String queryString=queryStringTemp;
			stmt=conn.prepareStatement(queryString);
			...
			while(rset.next())
			{
				...
				VTO_LIST_FLAG.add(rset.getString(4)==null?"":rset.getString(4));
	...
	public void getEntityContactPersonListDlng()
	{
		...
		try
		{
			...
			while(rset.next())
			{
				...
				VDLNG_TO_LIST_FLAG.add(rset.getString(4)==null?"":rset.getString(4));
	...
	public void getCounterpartyAuditReport()
	{
		...
		{
			int cont=0;
			String queryString="SELECT NEW_VALUE,OLD_VALUE,LOG_TIME,TO_CHAR(LOG_DT,'DD/MM/YYYY'),LOG_UID "
					+ "FROM FMS_ALL_LOG "
					+ "WHERE FORM_NAME IN (?,?) "
	...
	public void getCounterpartyList()
	{
		...
		try
		{
			String queryString = "SELECT COUNTERPARTY_CD,COUNTERPARTY_NM,COUNTERPARTY_ABBR,NVL(STATUS,'N'),KYC,IGX "
					+ "FROM FMS_COUNTERPARTY_MST A "
	...
	public void getEntityTcsTdsTaxDetail()
	{
		...
		try
		{
			...
			String queryString1="SELECT A.TAX_STRUCT_CD,TO_CHAR(B.APP_DATE,'DD/MM/YYYY'),A.TAX_STRUCT_DTL,A.TAX_STRUCT_REMARK,A.INVOICE_TYPE,"
					+ "TO_CHAR(A.ENT_DT,'DD/MM/YYYY HH24:MI:SS'),A.ENT_BY,TO_CHAR(A.EFF_DT,'DD/MM/YYYY'),B.SAP_TAX_CODE "
					+ "FROM FMS_ENTITY_TCS_TDS_MST A, FMS_TAX_STRUCTURE B "
					+ "WHERE A.ENTITY=? AND A.COMPANY_CD=? AND A.COUNTERPARTY_CD=? "
					+ "AND A.TAX_CATEGORY=? AND A.TAX_APP=? "
					+ "AND A.EFF_DT=(SELECT MAX(B.EFF_DT) FROM FMS_ENTITY_TCS_TDS_MST B "
					+ "WHERE A.ENTITY=B.ENTITY AND A.COMPANY_CD=B.COMPANY_CD AND A.COUNTERPARTY_CD=B.COUNTERPARTY_CD "
					+ "AND A.INVOICE_TYPE=B.INVOICE_TYPE AND A.TAX_CATEGORY=B.TAX_CATEGORY AND A.TAX_APP=B.TAX_APP) " // JD : Added TAX_APP Clause
					+ "AND A.TAX_STRUCT_CD=B.TAX_STR_CD "; 
			stmt1=conn.prepareStatement(queryString1);
			stmt1.setString(1, entity_role);
			stmt1.setString(2, comp_cd);
				...
				if(rset3.next())
				{
					if(rset3.getInt(1) > 1)
					{
						...
						String queryString2="SELECT A.TAX_STRUCT_CD,TO_CHAR(B.APP_DATE,'DD/MM/YYYY'),A.TAX_STRUCT_DTL,A.TAX_STRUCT_REMARK,"
								...
								+ "FROM FMS_ENTITY_TCS_TDS_MST A, FMS_TAX_STRUCTURE B "
								+ "WHERE A.ENTITY=? AND A.COMPANY_CD=? AND A.COUNTERPARTY_CD=? "
								+ "AND A.INVOICE_TYPE=? AND A.TAX_CATEGORY=? AND A.TAX_APP=? "
								+ "AND A.TAX_STRUCT_CD=B.TAX_STR_CD "
								+ "ORDER BY A.EFF_DT DESC";
						stmt2=conn.prepareStatement(queryString2);
						stmt2.setString(1, entity_role);
			...
			index=0;
			tax_category="S";
			String queryString4="SELECT A.TAX_STRUCT_CD,TO_CHAR(B.APP_DATE,'DD/MM/YYYY'),A.TAX_STRUCT_DTL,A.TAX_STRUCT_REMARK,A.INVOICE_TYPE,"
					+ "TO_CHAR(A.ENT_DT,'DD/MM/YYYY HH24:MI:SS'),A.ENT_BY,TO_CHAR(A.EFF_DT,'DD/MM/YYYY'),B.SAP_TAX_CODE "
					+ "FROM FMS_ENTITY_TCS_TDS_MST A, FMS_TAX_STRUCTURE B  "
					+ "WHERE A.ENTITY=? AND A.COMPANY_CD=? AND A.COUNTERPARTY_CD=? "
					+ "AND A.TAX_CATEGORY=? AND A.TAX_APP=? "
					+ "AND A.EFF_DT=(SELECT MAX(B.EFF_DT) FROM FMS_ENTITY_TCS_TDS_MST B "
					+ "WHERE A.ENTITY=B.ENTITY AND A.COMPANY_CD=B.COMPANY_CD AND A.COUNTERPARTY_CD=B.COUNTERPARTY_CD "
					+ "AND A.INVOICE_TYPE=B.INVOICE_TYPE AND A.TAX_CATEGORY=B.TAX_CATEGORY AND A.TAX_APP=B.TAX_APP) "// JD: Added TAX_APP
					+ "AND A.TAX_STRUCT_CD=B.TAX_STR_CD "; 
			stmt4=conn.prepareStatement(queryString4);
			stmt4.setString(1, entity_role);
			stmt4.setString(2, comp_cd);
	...
	public void getCounterpartyVertualBankCodeDetail()
	{
		String function_nm="getCounterpartyVertualBankCodeDetail()";
		...
		try
		{
			...
			if(!counterparty_cd.equals("0") && !counterparty_cd.equals("")) 
			{
				queryString+= "AND COUNTERPARTY_CD=? ";//AND CATEGORY=? "
			}
			queryString+= "AND EFF_DT=(SELECT MAX(B.EFF_DT) FROM FMS_ENTITY_BANK_MST B WHERE A.COUNTERPARTY_CD=B.COUNTERPARTY_CD "
					+ "AND A.ENTITY=B.ENTITY AND A.COMPANY_CD=B.COMPANY_CD AND A.CATEGORY=B.CATEGORY)";
			stmt=conn.prepareStatement(queryString);
			stmt.setString(++selCnt, entity_role);
			stmt.setString(++selCnt, comp_cd);
			...
			{
				queryString1+= "AND EFF_DT=(SELECT MAX(B.EFF_DT) FROM FMS_ENTITY_BANK_MST B WHERE A.COUNTERPARTY_CD=B.COUNTERPARTY_CD "
						+ "AND A.ENTITY=B.ENTITY AND A.COMPANY_CD=B.COMPANY_CD AND A.CATEGORY=B.CATEGORY)";
			}
			queryString1+= "ORDER BY COUNTERPARTY_CD DESC";
			stmt1=conn.prepareStatement(queryString1);
			stmt1.setString(++selCnt, entity_role);
			stmt1.setString(++selCnt, comp_cd);
			if(!counterparty_cd.equals("0") && !counterparty_cd.equals("")) 
			...
			while(rset1.next())
			{
				...
				VBANK_EFF_DT.add(bank_eff_dt);
				VBANK_CATEGORY.add(bank_category);
				VBANK_NAME.add(bank_name);
				VBANK_ACCOUNT_NO.add(bank_account_no);
				VIFSC_CODE.add(ifsc_code);
				VBANK_BRANCH.add(bank_branch);
	...
	public String getCp_hist_desc() {return cp_hist_desc;}
	...
	public String getBank_category() {return bank_category;}
	...
	Vector VBANK_CATEGORY = new Vector();
	Vector VBANK_NAME = new Vector();
	Vector VBANK_ACCOUNT_NO = new Vector();
	Vector VIFSC_CODE = new Vector();
	...
	Vector VTEMP_DLNG_TO_F402 = new Vector();
	...
	public Vector getVDLNG_F402() {return VDLNG_F402;}
	...
	public Vector getVBANK_ACCOUNT_NO() {return VBANK_ACCOUNT_NO;}
	public Vector getVIFSC_CODE() {return VIFSC_CODE;}
	...
	public Vector getVBANK_CATEGORY() {return VBANK_CATEGORY;}
	...
	public Vector getVTEMP_DLNG_TO_F402() {return VTEMP_DLNG_TO_F402;}
