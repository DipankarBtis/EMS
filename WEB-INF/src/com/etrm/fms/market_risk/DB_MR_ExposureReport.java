...
//CR Date			: 23/09/2023 
//Status	  		: Developing
public class DB_MR_ExposureReport 
{
	...
	*/
	...
	public void getQPDetails()
	{
		...
		try
		{
			String queryString = "SELECT TO_CHAR(COB_DT,'DD/MM/YYYY'), LEGAL_ENTITY,DEAL_NUM,COUNTERPARTY,BUY_SELL,PRICE_TYPE,UNIT,CONTRACT_MONTH,"
					+ "CURVE_NAME,EXPOSURE,FINANCIAL_PHYSICAL,REALISED_UNREALISED,FORWARD_PRICE,CONT_TYPE_NM,REMARKS "
					+ "FROM QP_EXPOSURE ";
			...
			while(rset.next()) 
			{
				...
				VCONT_TYPE_NM.add(rset.getString(14)==null?"":rset.getString(14));
	...
	public void getContractListForExposure()
	{
		...
		try
		{
			...
			String lastYearStartDt=utilDate.getFirstDateOfYear(""+(utilDate.getCurrentYear(report_dt)-1));
			System.out.println("Last Year Date :: "+lastYearStartDt);
			...
			String queryString="SELECT COMPANY_CD,COUNTERPARTY_CD,AGMT_NO,AGMT_REV,CONT_NO,CONT_REV,CONT_REF_NO,TRADE_REF_NO,CONTRACT_TYPE,"
					...
					+ "WHERE CONT_STATUS NOT IN ('X') " //'F' removed as per INC#2410044 //'C' Removed closed status
			...
			queryString+="SELECT A.COMPANY_CD,A.COUNTERPARTY_CD,A.AGMT_NO,A.AGMT_REV,A.CONT_NO,A.CONT_REV,B.CARGO_REF,NULL,A.CONTRACT_TYPE,"
					...
					+ "WHERE A.CONT_STATUS NOT IN ('X') " //'F' removed as per INC#2410044 //'C' Removed closed status
			...
			queryString+="SELECT A.COMPANY_CD,A.COUNTERPARTY_CD,A.AGMT_NO,A.AGMT_REV,A.CONT_NO,A.CONT_REV,A.CONT_REF_NO,NULL,A.CONTRACT_TYPE,"
					+ "TO_CHAR(A.SIGNING_DT,'DD/MM/YYYY'),TO_CHAR(LAST_DAY(B.CONT_DD_MM_YR), 'DD/MM/YYYY'),TO_CHAR(LAST_DAY(B.CONT_DD_MM_YR), 'DD/MM/YYYY'),NULL,"
					+ "B.RATE,TO_CHAR(B.RATE_UNIT),A.CONT_STATUS,B.BUY_SELL,(B.CONT_DD_MM_YR) START_DT,B.INSTRUMENT_NO,B.INSTRUMENT_TYPE,B.CURVE_NM "
					+ "FROM FMS_DERV_CONT_MST A, FMS_DERV_INSTRUMENT_MST B "
					+ "WHERE A.CONT_STATUS NOT IN ('X') AND A.AGMT_TYPE=? " //'F' removed as per INC#2410044 //'C' Removed closed status
					//+ "AND B.PRICE_END_DT >= TO_DATE(?,'DD/MM/YYYY') " //AS PER INC#2600015
					+ "AND B.CONT_DD_MM_YR >= TO_DATE(?,'DD/MM/YYYY') "
			...
			stmt.setString(++count, lastYearStartDt);
			if(!expo_type.equals(""))
			{
			...
			while(rset.next())
			{
				...
				double exchngRate=getExchangeRate(companyCd, countpty_cd, agmt, cont, cont_type, dt,buy_sell);
				...
				else if(cont_type.equals("E") || cont_type.equals("F") || cont_type.equals("W")) //FOR DLNG DEALS ONLY AS CONFIRMED BY VIJAY ON 20250925
				{
					phys_curve_nm="LNG_PHYS_INDIA";
				}
				String price_type="";
				...
				else
				{
					...
					if(rset1.next())
					{
						...
						else if(price_type.equals("M"))
						{
							...
							{
								price_type_desc="FLOAT";
								fin_curve_nm="Min of ["+formula_desc.substring(8,formula_desc.length()).replaceAll("@", ",")+"]";
							}
	...
	public void getContractDetail()
	{
		...
		try
		{
			contract_type_nm=utilBean.getMR_ContractTypeName(contract_type);
			...
			else
			{
				...
				if(contract_type.equals("V"))
				{
					queryString="SELECT A.CONT_REF_NO,NULL,"
							...
							+ "B.RATE,TO_CHAR(B.RATE_UNIT),B.CONV_FACTOR,B.CURVE_NM,A.REMARKS,B.QTY_UNIT "
							+ "FROM FMS_DERV_CONT_MST A, FMS_DERV_INSTRUMENT_MST B "
				...
				else if(account.equals("Buy"))
				{
					if(contract_type.equals("N"))
					{
						queryString="SELECT B.CARGO_REF,NULL,"
								...
								+ "B.RATE,B.RATE_UNIT,1,NULL,NULL,1 "
								+ "FROM FMS_TRADER_CN_MST A, FMS_TRADER_CARGO_MST B "
								+ "WHERE A.COMPANY_CD=? AND A.COUNTERPARTY_CD=? "
					...
					else
					{
						queryString="SELECT CONT_REF_NO,TRADE_REF_NO,"
								...
								+ "RATE,RATE_UNIT,1,NULL,NULL,QUANTITY_UNIT "
								+ "FROM FMS_TRADER_CONT_MST A "
				...
				if(rset.next())
				{
					...
					/*if(contract_type.equals("V")) //NO NEED TO CONVERT IN MMBTU AS DISCUSSED WITH VIJAY AND RS ON 03/09/2025
					{
					...
					qty_unit=rset.getString(17)==null?"1":rset.getString(17);
					qty_unit_nm=utilBean.getEnergyUnitNm(conn, qty_unit);
				}
				rset.close();
				stmt.close();
	...
	public void getStorageExposure(String temp_own_cd)
	{
		...
		try
		{
			...
			for(int i=0;i<VCOMPANY_CD.size(); i++)
			{
				...
				String inventory_dt="";
				queryString="SELECT SUM(A.TANK_MMBTU),SUM(NVL(B.TANK_D1_VOLUME,0)+NVL(B.TANK_D2_VOLUME,0)),A.TANK_CONV_FACTOR_1,A.TANK_CD,TO_CHAR(INV_LEVEL_DT,'DD/MM/YYYY') "
				...
				while(rset.next())
				{
					...
					if(Double.doubleToRawLongBits(temp_factor_m3_to_mmscm)==Double.doubleToRawLongBits(0))
					{
		...
		try
		{
			...
			while(rset.next())
			{
				...
				else if(utilDate.getDays(gas_dt, report_dt) <= 1) //AS PER INC#2410083
				{
					if(account.equals("Sell"))
					{
						...
						{	
							...
							while(rset1.next())
							{
								...
								/*COMMENTED AS REQUESTED BY RADHESHYAM AND VENISHA CHEE ON 20250918
					...
					else
					{
						if(contract_type.equals("N"))
						{
							...
							//IF CARGO ARRIVE OUTSIDE WINDOW
							if(end_dt.equals(gas_dt))
							{
								queryString1 = "SELECT A.QQ_QTY_MMBTU "
										+ "FROM FMS_BUY_CARGO_ALLOC A "
						  				+ "WHERE A.COMPANY_CD=? AND A.COUNTERPARTY_CD=? AND A.AGMT_TYPE=? "
						  				+ "AND A.CONT_NO=? AND A.AGMT_NO=? AND A.CONTRACT_TYPE=? AND A.CARGO_NO=? AND A.QQ_DT > TO_DATE(?,'DD/MM/YYYY')"
						  				+ "AND A.ALLOC_REV=(SELECT MAX(B.ALLOC_REV) FROM FMS_BUY_CARGO_ALLOC B "
				...
				*/
				...
				if(rate_unit.equals("1"))
				{	
					exchngRate=getExchangeRate(comp_cd, counterparty_cd, agmt_no, cont_no, contract_type, gas_dt,account);
				}
				...
				String queryString1="SELECT RATE,RATE_UNIT,PRICE_TYPE,CURVE_NM,SLOPE,CONST,PHYS_CURVE_NM,"
						...
						+ "CURVE_LOGIC,SEQ_NO,FORMULA,PRICE_DECI,FINAL_PRICE_DECI,PREM_DISC_RATE "
						+ "FROM FMS_CONT_PRICE_DTL "
						+ "WHERE COMPANY_CD=? AND MAPPING_ID=? AND CONTRACT_TYPE=? "
						+ "AND FROM_DT<=TO_DATE(?,'DD/MM/YYYY') AND TO_DT>=TO_DATE(?,'DD/MM/YYYY')";
				stmt1=conn.prepareStatement(queryString1);
				...
				if(rset1.next())
				{
					...
					decimal=rset1.getString(14)==null?"4":rset1.getString(14);
					final_decimal=rset1.getString(15)==null?"4":rset1.getString(15);
					primdiscount=rset1.getString(16)==null?"":rset1.getString(16);
					
					if(!primdiscount.equals(""))
					{
						constant= nf2.format(Double.parseDouble(constant) + (Double.parseDouble(primdiscount) * Double.parseDouble(slope)));
					}
				...
				if(phys_curve_nm.equals("") && !contract_type.equals("V"))
				{
					...
					else if(contract_type.equals("E") || contract_type.equals("F") || contract_type.equals("W")) //FOR DLNG DEALS ONLY AS CONFIRMED BY VIJAY ON 20250925
					{
						phys_curve_nm="LNG_PHYS_INDIA";
					}
					else
				...
				if(cont_price.equals(""))
				{
					...
					{	
						if(exchngRate>0)
						{
							cont_price=nf2.format(Double.parseDouble(rate)/exchngRate);
						}
					}
				}
				...
				Vector VTEMP_EFF_PRICE_FOR_MIN_ADV= new Vector();
				Vector VTEMP_EFF_PRICE_INFO_FOR_MIN_ADV= new Vector();
				Vector VTEMP_DECIMAL = new Vector();
				Vector VTEMP_FINAL_DECIMAL= new Vector();
				...
				if(contract_type.equals("V"))
				{
					...
					VTEMP_DECIMAL.add(decimal);
					VTEMP_FINAL_DECIMAL.add(final_decimal);
				...
				else if(price_type.equals("M"))
				{
					...
					else if(curve_logic.equals("MIN") || curve_logic.equals("MAX") || curve_logic.equals("AVG") || curve_logic.equals("MIN_ADV"))
					{
						...
						while(rset1.next())
						{
							...
							String min_formula=rset1.getString(7)==null?"":rset1.getString(7);
							String min_decimal=rset1.getString(8)==null?"4":rset1.getString(8);
							String min_primdiscount=rset1.getString(9)==null?"":rset1.getString(9);
							
							if(!min_primdiscount.equals(""))
							{
								min_constant= nf2.format(Double.parseDouble(min_constant) + (Double.parseDouble(min_primdiscount) * Double.parseDouble(min_slope)));
							}
							...
							if(!min_formula.equals(""))
							{
								...
								String min_temp_dcq=nf.format(Double.parseDouble(var_dcq)/((Integer.parseInt(m2)-Integer.parseInt(m1))+1));
								String min_temp_alloc_qty=temp_alloc_qty=nf.format(actual_qty/((Integer.parseInt(m2)-Integer.parseInt(m1))+1));
								...
								{	
									...
									VTEMP_DCQ.add(min_temp_dcq);
									//VTEMP_ALLOC_QTY.add(temp_alloc_qty);
									VTEMP_ALLOC_QTY.add(min_temp_alloc_qty);
									...
									stmt2.setInt(2, temp_i);
									rset2=stmt2.executeQuery();
									if(rset2.next())
									{
										multi_cont_month="01/"+(rset2.getString(1)==null?"":rset2.getString(1));
									}
									rset2.close();
									stmt2.close();
									
									Vector temp=SettlementDateCalculation(min_curve_nm, multi_cont_month, min_price_range, min_price_start_dt, min_price_end_dt);
									VTEMP_FIN_MMYYYY.add(temp.elementAt(0));
									VTEMP_SETTLE_START_DT.add(temp.elementAt(1));
									VTEMP_SETTLE_END_DT.add(temp.elementAt(2));
									VTEMP_FIN_CONT_MONTH.add(temp.elementAt(4));
									...
									String settle_start_dt=""+temp.elementAt(1);
									String settle_end_dt=""+temp.elementAt(2);
									String fin_cont_month=""+temp.elementAt(4);
									int count_u=0;
									int count_r=0;
									double eff_rate=0;
									double row_wise_eff_rate=0;
									String row_wise_eff_rate_info="";
									...
									{	
										...
										if(isSettlmentPeriodActive==1)
										{
											...
											count_r=getPriceCurveWorkingDays(min_curve_nm, settle_start_dt, report_dt); 	
										}
										else
										{
									...
									{
										settle_price=getForwardPrice(min_curve_nm, report_dt, fin_cont_month,"Forward","Financial");
									}
									else
									{
										settle_price=getSettledPrice(min_curve_nm, settle_start_dt, settle_end_dt);
									}
									fin_fwd_price=getForwardPrice(min_curve_nm, report_dt, fin_cont_month,"Forward","Financial");
									
									String formatted_settle_price= utilBean.getNumberFormat(settle_price,min_decimal);
									String formatted_fin_fwd_price= utilBean.getNumberFormat(fin_fwd_price,min_decimal);
									
									double withDecimal= (((count_u * Double.parseDouble(formatted_fin_fwd_price)) + (count_r * Double.parseDouble(formatted_settle_price)))/(count_r+count_u));
									
									row_wise_eff_rate=(withDecimal * Double.parseDouble(min_slope)) + Double.parseDouble(min_constant);
									String formatted_row_wise_eff_rate= utilBean.getNumberFormat(row_wise_eff_rate,final_decimal);
									row_wise_eff_rate=Double.parseDouble(formatted_row_wise_eff_rate);
									
									eff_rate= withDecimal;
									if(Double.isNaN(row_wise_eff_rate))
									{
										row_wise_eff_rate=0;
									}
									...
									row_wise_eff_rate_info="= {((#Fwd. days * Fwd. Price) + (#Settled days * Settle Price)) / (#Fwd. days + #Settled days)} * slope + const.\n"
											+ " = {(( "+count_u+" * "+utilBean.getNumberFormat(fin_fwd_price,min_decimal)+" ) + ("+count_r+" * "+utilBean.getNumberFormat(settle_price,min_decimal)+" )) / ( "+count_u+" + "+count_r+")} * "+min_slope+" + "+min_constant+"\n"
											+ " = "+formatted_row_wise_eff_rate;
									row_wise_eff_rate_info+="\n\n= "+formatted_row_wise_eff_rate+" + "+nf2.format(weighted_average)+"(Charges)";
									row_wise_eff_rate+=weighted_average;
									formatted_row_wise_eff_rate= utilBean.getNumberFormat(row_wise_eff_rate,final_decimal);
									row_wise_eff_rate_info+="\n= "+formatted_row_wise_eff_rate;
									
									/////////////////////////////////////////////////////////
									eff_rate_info+="= {((#Fwd. days * Fwd. Price) + (#Settled days * Settle Price)) / (#Fwd. days + #Settled days)}\n"
											+ " = {(( "+count_u+" * "+utilBean.getNumberFormat(fin_fwd_price,min_decimal)+" ) + ("+count_r+" * "+utilBean.getNumberFormat(settle_price,min_decimal)+" )) / ( "+count_u+" + "+count_r+")} \n"
											+ " = "+nf2.format(eff_rate);
									eff_rate_info+="\n\n";
									//=================
									...
									VTEMP_AVG_SETTLE_PRICE.add("");
									VTEMP_EFF_PRICE.add(nf2.format(row_wise_eff_rate));
									VTEMP_EFF_PRICE_INFO.add(row_wise_eff_rate_info);
									
									VTEMP_EFF_PRICE_FOR_MIN_ADV.add("");
									VTEMP_EFF_PRICE_INFO_FOR_MIN_ADV.add("");
									VTEMP_DECIMAL.add(min_decimal);
									VTEMP_FINAL_DECIMAL.add(final_decimal);
								}
								...
								avgPriceInfo=eff_rate_info;
								avgPrice = avgSettlePrice * Double.parseDouble(min_slope) + Double.parseDouble(min_constant);
								String formatted_avgPrice=utilBean.getNumberFormat(avgPrice,final_decimal);
								
								avgPrice=Double.parseDouble(formatted_avgPrice);
								avgPriceInfo+="\n= [Sum(Settle Price)/#Month] * Slope + Const \n= ["+nf2.format(totalSettlePrice)+"/"+settle_count_withoutzero+"] * "+min_slope+" + "+min_constant+"\n= ["+nf2.format(avgSettlePrice)+"] * "+min_slope+" + "+min_constant+"\n= "+nf2.format(avgPrice);
								avgPriceInfo+="\n\n= "+formatted_avgPrice+" + "+nf2.format(weighted_average)+"(Charges)";
								avgPrice+=weighted_average;
								
								formatted_avgPrice=utilBean.getNumberFormat(avgPrice,final_decimal);
								avgPriceInfo+="\n= "+formatted_avgPrice;
								
								if(first_index_of_minAdv > 0)
								{
									VTEMP_EFF_PRICE_FOR_MIN_ADV.remove(first_index_of_minAdv-1);
									VTEMP_EFF_PRICE_INFO_FOR_MIN_ADV.remove(first_index_of_minAdv-1);
									
									VTEMP_EFF_PRICE_FOR_MIN_ADV.add(first_index_of_minAdv-1,formatted_avgPrice);
									VTEMP_EFF_PRICE_INFO_FOR_MIN_ADV.add(first_index_of_minAdv-1, avgPriceInfo);
								}
							}
							else
							{
								...
								double eff_rate=((((count_u * Double.parseDouble(formatted_fin_fwd_price)) + (count_r * Double.parseDouble(formatted_settle_price)))/(count_r+count_u)) * Double.parseDouble(min_slope)) + Double.parseDouble(min_constant);
								if(Double.isNaN(eff_rate)) {
									eff_rate=0;
								}
								
								String formatted_eff_rate=utilBean.getNumberFormat(eff_rate, final_decimal);
								
								temp_avgPriceInfo+="= {((#Fwd. days * Fwd. Price) + (#Settled days * Settle Price)) / (#Fwd. days + #Settled days)} * slope + const.\n"
										+ "= {(( "+count_u+" * "+formatted_fin_fwd_price+" ) + ("+count_r+" * "+formatted_settle_price+" )) / ( "+count_u+" + "+count_r+")} * "+min_slope+" + "+min_constant+"\n"
										+ "= "+formatted_eff_rate;
								avgPriceInfo=avgPriceInfo.equals("")?temp_avgPriceInfo:avgPriceInfo+"\n"+temp_avgPriceInfo;
								temp_avgPriceInfo+="\n\n= "+formatted_eff_rate+" + "+nf2.format(weighted_average)+" (WA Charges)";
								eff_rate=Double.parseDouble(formatted_eff_rate);
								eff_rate+=weighted_average;
								formatted_eff_rate=utilBean.getNumberFormat(eff_rate, final_decimal);
								temp_avgPriceInfo+="\n= "+formatted_eff_rate;
								
								VTEMP_SETTLE_PRICE.add(formatted_settle_price);
								VTEMP_AVG_SETTLE_PRICE.add("");
								VTEMP_EFF_PRICE.add(formatted_eff_rate);
								VTEMP_EFF_PRICE_INFO.add(temp_avgPriceInfo);
								
								VTEMP_EFF_PRICE_FOR_MIN_ADV.add(formatted_eff_rate); //FOR MIN ADV CASE ONLY
								VTEMP_EFF_PRICE_INFO_FOR_MIN_ADV.add(temp_avgPriceInfo); //FOR MIN ADV CASE ONLY
					...
					else
					{
						...
						{	
							...
							for(int i=Integer.parseInt(m1);i<=Integer.parseInt(m2);i++)
							{
								...
								String formatted_settle_price= utilBean.getNumberFormat(settle_price,decimal);
								settle_price=Double.parseDouble(formatted_settle_price);
								if(settle_price>0)
								{
				...
				int finalMinMax_index=0;
				String min_var_dcq=nf.format(Double.parseDouble(var_dcq));
				String min_actual_qty=nf.format(actual_qty);
				...
				if(curve_logic.equals("MIN") || curve_logic.equals("MAX"))// && !VTEMP_FIN_RU.contains("U"))
				{
					...
					finalMinMax_index=min_index;
				}
				else if(curve_logic.equals("MIN_ADV"))
				{
					int min_index=0;
					min_index_curve=""+VTEMP_CURVE_NM.elementAt(0);
					double price=Double.parseDouble(""+VTEMP_EFF_PRICE_FOR_MIN_ADV.elementAt(min_index));
					for(int i=1;i<VTEMP_EFF_PRICE_FOR_MIN_ADV.size();i++)
					{	
						if(!VTEMP_EFF_PRICE_FOR_MIN_ADV.elementAt(i).equals(""))
						{
							if(Double.parseDouble(""+VTEMP_EFF_PRICE_FOR_MIN_ADV.elementAt(i)) < price)
							{
								price=Double.parseDouble(""+VTEMP_EFF_PRICE_FOR_MIN_ADV.elementAt(i));
						...
						else
						{
							...
							{
								VTEMP_EFF_PRICE_FOR_MIN_ADV.remove(i);
								VTEMP_EFF_PRICE_FOR_MIN_ADV.add(i, nf2.format(0));
								
								VTEMP_EFF_PRICE_INFO_FOR_MIN_ADV.remove(i);
								VTEMP_EFF_PRICE_INFO_FOR_MIN_ADV.add(i, "");
							}
						}
					}
					...
					for(int i=0;i<VTEMP_EFF_PRICE.size();i++)
					{
						if(!min_index_curve.equals("PPAC"))
						{
							if(!VTEMP_FIN_RU.contains("U") && !VTEMP_PHY_RU.contains("U"))
							{
								if(min_index==i && VTEMP_CURVE_NM.elementAt(i).equals(min_index_curve))
								{
									VTEMP_DCQ.remove(i);
									VTEMP_DCQ.add(i, nf.format(Double.parseDouble(var_dcq)));
									...
									VTEMP_AVG_SETTLE_PRICE.add(i, utilBean.getNumberFormat(avgSettlePrice, ""+VTEMP_DECIMAL.elementAt(i) ));
									
									VTEMP_EFF_PRICE.remove(i);
									VTEMP_EFF_PRICE.add(i, ""+VTEMP_EFF_PRICE_FOR_MIN_ADV.elementAt(i));
									
									VTEMP_EFF_PRICE_INFO.remove(i);
									VTEMP_EFF_PRICE_INFO.add(i, ""+VTEMP_EFF_PRICE_INFO_FOR_MIN_ADV.elementAt(i));
								}
								else
								{
									VTEMP_ALLOC_QTY.remove(i);
									...
									VTEMP_AVG_SETTLE_PRICE.add(i, utilBean.getNumberFormat(0, ""+VTEMP_DECIMAL.elementAt(i)));
									
									if(!VTEMP_CURVE_NM.elementAt(i).equals("PPAC"))
									{
										VTEMP_EFF_PRICE.remove(i);
						...
						else
						{
							...
							if(!VTEMP_CURVE_NM.elementAt(i).equals("PPAC") && !VTEMP_FIN_RU.contains("U") && !VTEMP_PHY_RU.contains("U"))
							{
								temp_k++;
								if(temp_k==1)
								{
				...
				if(curve_logic.equals("MULTI_LEG") && !VTEMP_FIN_RU.contains("U") && !VTEMP_PHY_RU.contains("U"))
				{
					for(int i=0;i<VTEMP_SETTLE_PRICE.size();i++)
					{
						if(i==0)
						{
							...
							VTEMP_EFF_PRICE.add(i, utilBean.getNumberFormat(avgPrice, final_decimal));
						...
						else
						{
							...
							VTEMP_EFF_PRICE.remove(i);
							VTEMP_EFF_PRICE.add(i, utilBean.getNumberFormat(0,final_decimal));
				...
				for(int i=0;i<VTEMP_CURVE.size();i++)
				{
					...
					else if(curve_logic.equals("MIN") || curve_logic.equals("MAX") || curve_logic.equals("MIN_ADV"))
					{
						if(i==0)
						{
							sr=sr + 1;
						...
						if(finalMinMax_index==i)
						{
							
							//VDCQ.add(Double.parseDouble(var_dcq));
							VDCQ.add(min_var_dcq);
							VDCQ_COLOR.add(var_color);
							//VACTUAL_QTY.add(nf.format(actual_qty));
							VACTUAL_QTY.add(min_actual_qty);
							VCONTRACT_PRICE.add(nf2.format(Double.parseDouble(cont_price)));
							VTOTAL_CHARGE.add(nf2.format(weighted_average));
					...
					String temp_decimal=""+VTEMP_DECIMAL.elementAt(i);
					String temp_final_decimal=""+VTEMP_FINAL_DECIMAL.elementAt(i);
					...
					String formatted_settle_price="";
					String formatted_fin_fwd_price="";
					String formatted_eff_rate="";
					...
					if(price_type.equals("M"))
					{
						...
						formatted_settle_price= utilBean.getNumberFormat(settle_price,temp_decimal);
						formatted_fin_fwd_price= utilBean.getNumberFormat(fin_fwd_price,temp_decimal);
						
						settle_price=Double.parseDouble(formatted_settle_price);
						fin_fwd_price=Double.parseDouble(formatted_fin_fwd_price);
						...
						if(!VTEMP_EFF_PRICE.elementAt(i).equals("")) //THIS IS SPECIAL FOR MULTI LEG
						{
							...
							formatted_eff_rate=utilBean.getNumberFormat(eff_rate, temp_final_decimal);
						}
						else
						{
							...
							eff_rate_info="= {((#Fwd. days * Fwd. Price) + (#Settled days * Settle Price)) / (#Fwd. days + #Settled days)} * slope + const.\n"
									+ " = {(( "+count_u+" * "+formatted_fin_fwd_price+" ) + ("+count_r+" * "+formatted_settle_price+" )) / ( "+count_u+" + "+count_r+")} * "+slope+" + "+constant+"\n"
									+ " = "+formatted_eff_rate;
							eff_rate_info+="\n\n= "+formatted_eff_rate+" + "+nf2.format(weighted_average)+"(Charges)";
					...
					{
						formatted_settle_price= utilBean.getNumberFormat(0,temp_decimal);
						formatted_fin_fwd_price= utilBean.getNumberFormat(0,temp_decimal);
						...
						formatted_eff_rate=utilBean.getNumberFormat(eff_rate, temp_final_decimal);
						eff_rate_info=" = "+formatted_eff_rate;
						eff_rate=Double.parseDouble(formatted_eff_rate);
					}
					
					if(settle_price>0)
					{
						VSETTLE_PRICE.add(formatted_settle_price);
					}
					else
					{
						VSETTLE_PRICE.add("");
					...
					if(contract_type.equals("V")) //AS PER EMS UAT INC#2500043, NO NEED SHOW ANY PHY VALUE FOR DERIVATIVE DEAL 
					{
						VPHY_REALIZED_UNREALIZED.add("");
						VPHY_EXPO_ORIGINAL.add("");
						VPHY_EXPO_ORIGINAL_INFO.add("");
						VPHY_EXPO_REALIZED.add("");
						VPHY_EXPO_UNREALIZED.add("");
						VPHY_EXPO_REALIZED_INFO.add("");
						VPHY_EXPO_UNREALIZED_INFO.add("");
					}
					else
					{
					...
					if(Double.doubleToRawLongBits(eff_rate)!=Double.doubleToRawLongBits(0))
					{
						VEFF_RATE.add(formatted_eff_rate);
					}
					else
					{
						VEFF_RATE.add("");
					...
					if(contract_type.equals("V")) //AS PER EMS UAT INC#2500043, NO NEED SHOW ANY PHY VALUE FOR DERIVATIVE DEAL 
					{
						VPHY_FORWARD_PRICE.add("");
					}
					else
					{
						VPHY_FORWARD_PRICE.add(nf2.format(phy_fwd_price));
					}
					VFIN_FORWARD_PRICE.add(formatted_fin_fwd_price);
					...
					if(fin_ru.equals("U"))
					{
						fin_realized_leg=Double.parseDouble(fin_expo_r) * ((Double.parseDouble(formatted_settle_price) +(Double.parseDouble(constant)/Double.parseDouble(slope)))+(weighted_average/Double.parseDouble(slope)));
						fin_realized_leg_info = "= Fin. Realized Expo. * ((Settle Price +(Const. / Slope)) + (WA Charges / Slope)) \n = "+fin_expo_r+" MMBTU * (("+formatted_settle_price+" + ("+constant+" / "+slope+")) + ("+nf2.format(weighted_average)+" / "+slope+"))\n = "+nf.format(fin_realized_leg);
					}
					else
					{
						/*if(phy_count_ru<=1)
						...
						else if(curve_logic.equals("MIN_ADV") && VTEMP_CURVE_NM.elementAt(i).equals(min_index_curve) && !VTEMP_CURVE_NM.elementAt(i).equals("PPAC") && !VTEMP_FIN_RU.contains("U") && !VTEMP_PHY_RU.contains("U"))
						{
							fin_realized_leg=Double.parseDouble(fin_expo_r) * ((Double.parseDouble(""+VTEMP_AVG_SETTLE_PRICE.elementAt(i)) +(Double.parseDouble(constant)/Double.parseDouble(slope)))+(weighted_average/Double.parseDouble(slope)));
							fin_realized_leg_info = "= Fin. Realized Expo. * ((Eff. Settle Price +(Const. / Slope)) + (WA Charges / Slope)) \n = "+fin_expo_r+" MMBTU * (("+VTEMP_AVG_SETTLE_PRICE.elementAt(i)+" + ("+constant+" / "+slope+")) + ("+nf2.format(weighted_average)+" / "+slope+"))\n = "+nf.format(fin_realized_leg);
						}
					...
					{
						fin_unrealized_leg=Double.parseDouble(fin_expo_u) * ((Double.parseDouble(formatted_fin_fwd_price) + (Double.parseDouble(constant)/Double.parseDouble(slope)))+(weighted_average/Double.parseDouble(slope)));
						fin_unrealized_leg_info = "= Fin. Unrealized Expo. * ((Fin. Forward Price +(Const. / Slope)) + (WA Charges / Slope) \n = "+fin_expo_u+" MMBTU * (("+formatted_fin_fwd_price+" * ("+constant+" / "+slope+")) + ("+nf2.format(weighted_average)+" / "+slope+"))\n = "+nf.format(fin_unrealized_leg);
					}
					...
					if(contract_type.equals("V")) //AS PER EMS UAT INC#2500043, NO NEED SHOW ANY PHY VALUE FOR DERIVATIVE DEAL 
					{
						VPHY_UNREALIZED_LEG.add("");
						VPHY_UNREALIZED_LEG_INFO.add("");
					}
					else
					{
	...
	public void freezMrExpoEodData(int i)
	{
		...
		try
		{
			...
			String queryString="INSERT INTO FMS_MR_EXPO_EOD_MST(COMPANY_CD,REPORT_DT,BUY_SELL,COUNTERPARTY_CD,"
					...
					+ "DEAL_NUM,ENT_BY,ENT_DT,REMARKS,CONT_TYPE_NM) "
			...
			stmt_temp.setString(38, remarks);
			stmt_temp.setString(39, contract_type_nm);
			...
			for(int j=0;j<VGAS_DT_MULTI.size();j++)
			{
				...
				queryString="INSERT INTO FMS_MR_EXPO_EOD_DTL(COMPANY_CD,REPORT_DT,BUY_SELL,COUNTERPARTY_CD,"
						...
						+ "R_FIN_LEG, MTM_TOTAL,ENT_BY,ENT_DT,PHY_CURVE_NM,WA_RATE,QTY_UNIT) "
						+ "VALUES(?,TO_DATE(?,'DD/MM/YYYY'),?,?,"
						+ "?,?,TO_DATE(?,'DD/MM/YYYY'),"
						+ "TO_DATE(?,'DD/MM/YYYY'),?,?,"
						+ "?,?,?,"
				...
				stmt_temp1.setString(3, temp_account);
				...
				stmt_temp1.setString(40, qty_unit);
		...
		try
		{
			...
			while(rset.next())
			{
				...
				String queryString1="SELECT A.COUNTERPARTY_NM,A.BUY_SELL,TO_CHAR(A.CONT_SIGN_DT,'DD/MM/YYYY'),A.MAPPING_ID,A.CONTRACT_TYPE,"
						...
						+ "B.FWD_PRICE_PHY,B.FWD_PRICE_FIN,B.SETTLE_PRICE,B.UNR_PHY_LEG,B.UNR_FIN_LEG,B.R_FIN_LEG,B.MTM_TOTAL,B.SEQ_NO AS SEQ_NO,"
						+ "B.PHY_CURVE_NM, A.DEAL_NUM,B.WA_RATE,A.REMARKS,B.GAS_DT AS GAS_DT,B.QTY_UNIT "
						...
						+ "AND A.MAPPING_ID=? AND A.CONTRACT_TYPE NOT IN ('V') "
						...
						+ " UNION ALL "
						+ "SELECT A.COUNTERPARTY_NM,B.BUY_SELL,TO_CHAR(A.CONT_SIGN_DT,'DD/MM/YYYY'),A.MAPPING_ID,A.CONTRACT_TYPE,"
						+ "B.FIN_CURVE_NM,A.PHYS_CURVE,TO_CHAR(B.GAS_DT,'DD/MM/YYYY'),B.DCQ,B.PRICE_TYPE,B.ALLOC_QTY,B.CONT_PRICE,"
						+ "TO_CHAR(B.SPOT_MTH,'DD/MM/YYYY'),TO_CHAR(B.SPOT_START_DT,'DD/MM/YYYY'),TO_CHAR(B.SPOT_END_DT,'DD/MM/YYYY'),"
						+ "B.SLOPE,B.CONST,B.EFF_RATE_USD,TO_CHAR(B.CONT_MTH,'DD/MM/YYYY'),B.ORI_EXPO_PHY,B.ORI_EXPO_FIN,"
						+ "B.RU_PHY_FLAG,B.RU_FIN_FLAG,B.UNR_EXPO_PHY,B.UNR_EXPO_FIN,B.R_EXPO_PHY,B.R_EXPO_FIN,"
						...
						+ "WHERE A.COMPANY_CD=? AND A.REPORT_DT=TO_DATE(?,'DD/MM/YYYY') "
						+ "AND A.COUNTERPARTY_CD=? AND A.BUY_SELL=? AND A.CONTRACT_TYPE=? "
						+ "AND A.MAPPING_ID=? AND A.CONTRACT_TYPE IN ('V') "
						+ "AND A.COMPANY_CD=B.COMPANY_CD AND A.REPORT_DT=B.REPORT_DT "
				...
				stmt1.setString(9, cd);
				stmt1.setString(10, buySell);
				stmt1.setString(11, cont_type);
				stmt1.setString(12, mapp_id);
				rset1=stmt1.executeQuery();
				while(rset1.next())
				{
					...
					*/
					...
					VREMARKS.add(rset1.getString(39)==null?"":rset1.getString(39));
					
					String qty_unit=rset1.getString(41)==null?"":rset1.getString(41);
					String qty_unit_nm=utilBean.getEnergyUnitNm(conn, qty_unit);
					VQTY_UNIT_NM.add(qty_unit_nm);
				}
				rset1.close();
				stmt1.close();
			}
	...
	public void getExposureDetailFreezedData()
	{
		...
		try
		{
			...
			queryString="SELECT TO_CHAR(GAS_DT,'DD/MM/YYYY'),TO_CHAR(CONT_MTH,'MM/YYYY'),DCQ,ALLOC_QTY,"
					...
					+ "UNR_PHY_LEG,UNR_FIN_LEG,R_FIN_LEG,MTM_TOTAL,SEQ_NO,PHY_CURVE_NM,FIN_CURVE_NM,WA_RATE,QTY_UNIT "
					+ "FROM FMS_MR_EXPO_EOD_DTL "
					+ "WHERE COMPANY_CD=? AND REPORT_DT=TO_DATE(?,'DD/MM/YYYY') "
			...
			if(!contract_type.equals("V"))
			{
				queryString+=" AND BUY_SELL=? ";
			}
			queryString+= " ORDER BY GAS_DT,SEQ_NO ";
			...
			stmt.setString(++stcount, mapp_id);
			if(!contract_type.equals("V"))
			{
				stmt.setString(++stcount, account);
			}
			rset=stmt.executeQuery();
			while(rset.next())
			{
		...
		try
		{
			...
			if(!QP_file_path.equals(""))
			{
				...
				{	
			    	...
			    	pw.println("COB DT,LEGAL ENTITY,CONTRACT TYPE,DEAL NUM,COUNTERPARTY,BUY/SELL,PRICE TYPE,UNIT,CONTRACT MONTH,CURVE NAME,EXPOSURE,FINANCIAL/PHYSICAL,REALISED/UNREALISED,FORWARD PRICE,REMARKS");
					...
					while(rset.next()) 
					{
						...
						String cont_type_nm=rset.getString(14)==null?"":rset.getString(14);
						String remarks=rset.getString(15)==null?"":rset.getString(15);
						
						pw.println(""+cob_dt+","+legal_entity+","+cont_type_nm+","+dealNum+","+countpty_nm+","+buy_sell+","+priceType+","+unit+",  "+contMth+","+curveNm+","
								+ ""+exposure+","+phys_fin+","+r_ur+","+fwd_price+","+remarks);
					}
					rset.close();
					stmt.close();
	...
	Vector VCONT_TYPE_NM = new Vector();
	...
	public Vector getVCONT_TYPE_NM() {return VCONT_TYPE_NM;}
