...
@WebServlet("/servlet/Frm_sf_interface")
@MultipartConfig(fileSizeThreshold=1024*1024*10, 	// 10 MB 
maxFileSize=1024*1024*50,      	// 50 MB
maxRequestSize=1024*1024*100)   	// 100 MB
public class Frm_sf_interface extends HttpServlet
{
	static String frm_src_file_name="Frm_sf_interface.java";
		...
		else
		{
			try
			{
				...
				if(dbcon != null)
				{
					...
					if(option.equalsIgnoreCase("GENERATE_CONTACT_DTL_XML"))
					{
						sf_xml_inbound_dir=utilBean.getAutomationKeyDetail(dbcon,"SF_XML_INBOUND");
			            sf_xml_outbound_dir=utilBean.getAutomationKeyDetail(dbcon,"SF_XML_OUTBOUND_"+comp_cd);
			            
						GenerateContactDetailsXml(request);
					}
				}
	...
	private void GenerateContactDetailsXml(HttpServletRequest request) throws SQLException 
	{
		...
		try
		{
			...
			String[] dwn_rlng_cont_dtl = request.getParameterValues("dwn_checkbox_flag0");
			String[] dwn_dlng_cont_dtl = request.getParameterValues("dwn_checkbox_flag1");
			
			if(dwn_rlng_cont_dtl!=null || dwn_dlng_cont_dtl!=null) 
			{
				/*String appPath = request.getServletContext().getRealPath("");
				
				String main_folder=CommonVariable.work_dir+comp_cd+File.separator+"SF_XML";//This path for SF is only for OUTBOUND.
				...
				String savePath = appPath+File.separator+main_folder+File.separator+"OUTBOUND";
				...
				try (FileOutputStream fileOut = new FileOutputStream(new File(file_path)))
				{
		            ...
		            {
		            	for(int i=0;i<dwn_rlng_cont_dtl.length;i++)
			            {
			            	String rlng_deal_mapping = dwn_rlng_cont_dtl[i];
							...
							stmt_temp=dbcon.prepareStatement(queryString_temp);
							...
							if(cont_type.equals("O") || cont_type.equals("Q")) 
							{
								...
								cont_name = utilBean.getCounterpartyABBR(dbcon,countpty_cd)
										+"-"+utilBean.getCompanyAbbr(dbcon, comp_cd)
				            ...
				            addElement(contDtl, "CUSTOMER_NAME", utilBean.getCounterpartyName(dbcon,countpty_cd));
							...
							while(rset3.next())
			            	{
								...
								bu_plant_abbr=utilBean.getCounterpartyPlantABBR(dbcon,buCd, comp_cd, bu_plant_seq, "B");
								...
								stmt1 = dbcon.prepareStatement(tempQstring);
								...
								while(rset1.next())
								{
									...
									plant_abbr=utilBean.getCounterpartyPlantABBR(dbcon,countpty_cd, own_cd, cust_plant_seq, "C");
									String plant_nm=utilBean.getCounterpartyPlantName(dbcon,countpty_cd, own_cd, cust_plant_seq, "C");
									
									HashMap<String, String> plant_detail = utilBean.getCounterpartyPlantDetail(dbcon,own_cd, "C", countpty_cd, cust_plant_seq);
									...
									stmt2.close();
							  		...
							  		if(cont_type.equals("O") || cont_type.equals("Q"))
							  		{
							  			taxStructure = utilBean.getEntityServiceStructureDtl(dbcon,own_cd, countpty_cd, "C", cust_plant_seq, bu_plant_seq, periodStDt,"SI");
							  		}
							  		else
							  		{
							  			taxStructure = utilBean.getEntityTaxStructureDtl(dbcon,own_cd, countpty_cd, "C", cust_plant_seq, bu_plant_seq, periodStDt, "S");
							  		}
									...
									addPlantDtl(contPlantDtl, agmtContNo, bu_plant_seq+"{-}"+cust_plant_seq, bu_plant_abbr+"{-}"+plant_abbr, plantAddr, taxStructure);
									 ...
									 String queryString4="SELECT TRANSPORTER_CD, PLANT_SEQ_NO ";
										...
										while(rset4.next())
										{
											...
											String transAbbr = utilBean.getCounterpartyABBR(dbcon,transCd);
											String trans_plant_seq1 = rset4.getString(2)==null?"":rset4.getString(2);
											//String plant_abbr=utilBean.getCounterpartyPlantABBR(dbcon,transCd, own_cd, plant_seq, "R");
											
											String plant_name=utilBean.getCounterpartyPlantName(dbcon,transCd, own_cd, trans_plant_seq1, "R");
											...
											stmt5=dbcon.prepareStatement(queryString5);
											...
											while(rset5.next())
											{
												...
												String trans_plant_seq_nm=""+utilBean.getCounterpartyPlantName(dbcon,transCd, comp_cd, trans_plant_seq, "R");
												...
												String exit_countpty_nm=""+utilBean.getCounterpartyName(dbcon,exit_countpty_cd);
												String exit_plant_seq=rset5.getString(4)==null?"":rset5.getString(4);
												String plant_seq_nm=""+utilBean.getCounterpartyPlantName(dbcon,exit_countpty_cd, comp_cd, exit_plant_seq, exit_type);
												...
												stmt6=dbcon.prepareStatement(queryString6);
											...
											stmt5=dbcon.prepareStatement(queryStringTemp);
											...
											while(rset5.next())
											{
												...
												String countpty_abbr=""+utilBean.getCounterpartyABBR(dbcon,countpty_cd1);
												String plant_seq2=rset5.getString(2)==null?"":rset5.getString(2);
												String plant_seq_nm=""+utilBean.getCounterpartyPlantName(dbcon,countpty_cd1, own_cd, plant_seq2, "C");
		            ...
		            {
		            	for(int i=0;i<dwn_dlng_cont_dtl.length;i++)
			            {
			            	String dlng_deal_mapping = dwn_dlng_cont_dtl[i];
							...
							while(rset3.next())
			            	{
								...
								while(rset1.next())
								{
									...
									stmt2.close();
							  		...
							  		String taxStructure = utilBean.getEntityTaxStructureDtl(dbcon,own_cd, countpty_cd, "C", cust_plant_seq, bu_plant_seq, periodStDt, "DS");
									...
									while(rset4.next())
									{
										...
										String queryString7="SELECT ENTRY_TRANS_PLANT, CONTRACT_TYPE, EXIT_COUNTERPARTY, EXIT_PLANT_SEQ, "
												...
												+ "AND TO_DATE(TO_CHAR(A.END_DT,'DD/MM/YYYY'),'DD/MM/YYYY') >=TO_DATE(TO_CHAR(SYSDATE,'DD/MM/YYYY'),'DD/MM/YYYY') "
												+ "ORDER BY START_DT DESC";
										stmt7=dbcon.prepareStatement(queryString7);
										...
										while(rset5.next())
										{
											...
											String plant_seq_nm=""+utilBean.getCounterpartyPlantName(dbcon,countpty_cd1, own_cd, plant_seq1, "C");
											String eff_dt=rset5.getString(4)==null?"":rset5.getString(4);
											String custCode = rset5.getString(3)==null?"":rset5.getString(3);
	            ...
	            msg = "Successful! SF XML file for Contract Generated Successfully!";
				msg_type = "S";
			}
			else
			{
				msg = "Failed! - No Contract Selected!!";
				msg_type="E";
			}
			
			url = "../extn_interface/frm_all_active_deal.jsp?msg="+msg+"&msg_type="+msg_type+"&report_dt="+report_dt+commonUrl_pra;
		...
		catch(Exception e)
		{
			...
			new SystemErrorLogger().InsertErrorLogger(frm_src_file_name, function_nm, e);
			url=CommonVariable.errorpage_url+"?e="+e;
			msg = "Error in Exception! SF XML Generation of Contracts Details Failed!!";
		}
