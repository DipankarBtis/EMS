...
public class DataBean_Admin 
{
		...
		try
		{
	    	...
	    	if(ds != null) 
	    	{
	    		...
	    		if(conn != null)  
	    		{
	    			...
	    			{
	    				GroupDetail();
	    				getActiveEmpList();		//Added for Group Owner
	    			}
	...
	public void getPasswordPolicyList()
	{
		...
		try
		{
			queryString="SELECT USR_MIN,USR_MAX,PWD_LEN_MIN,PWD_LEN_MAX,PREV_PWD_NOREP,PWD_EXP_DAYS,PWD_EXP_REMDR,DORMNT_DAYS,MAX_ADMINISTRATOR,MAX_ADMIN, "
					+ "OTP_TIMER, MAX_OTP, OTP_LEN_MAX, OTP_LEN_MIN " //Pratham Bhatt 20241007: for otp details
					+ "FROM FMS_ADMIN_POLICY "
					+ "WHERE EFF_DT<=TO_DATE(?,'DD/MM/YYYY') "
					+ "ORDER BY EFF_DT DESC";
			stmt=conn.prepareStatement(queryString);
			...
			if(rset.next())
			{
				...
				otp_timer = rset.getString(11)==null?"2":rset.getString(11);
				max_otp = rset.getString(12)==null?"2":rset.getString(12);
				otp_max_length = rset.getString(13)==null?"4":rset.getString(13);
				otp_min_length = rset.getString(14)==null?"4":rset.getString(14);
			}
			rset.close();
			stmt.close();
	...
	public void GroupDetail()
	{
		...
		try
		{
			...
			{
				queryString="SELECT GROUP_CD,GROUP_NAME,ACTIVE,GROUP_OWNER "
						+ "FROM FMS_GROUP_MST "
						+ "ORDER BY GROUP_CD";
				stmt=conn.prepareStatement(queryString);
				rset=stmt.executeQuery();
				while(rset.next())
				{
					...
					VGROUP_OWNER.add(rset.getString(4)==null?"0":rset.getString(4));
				}
				rset.close();
				stmt.close();
			}
	...
	public void getMenuDetail()
	{
		...
		try
		{
			queryString="SELECT FORM_NAME,CLASSPATH,SUB_MENU_CD,FORM_TYPE,SUB_MENU_SEQ,FLAG,SEI_CENTRAL "
					+ "FROM FMS_FORM_MST "
			...
			if(rset.next())
			{
				...
				sei_central = rset.getString(7)==null?"":rset.getString(7);
	...
	public void getAccessGroupAllocationDetail()
	{
		...
		try
		{
			queryString="SELECT A.GROUP_CD,B.GROUP_NAME,TO_CHAR(A.FROM_DT,'DD/MM/YYYY'),TO_CHAR(A.TO_DT,'DD/MM/YYYY'),A.SEQ_NO "
	...
	public void getAuditTrail()
	{
		...
		try 
		{
			...
			while(rset.next())
			{
				...
				VCOMPANY_ABBR.add(utilBean.getCompanyAbbr(conn,rset.getString(10)==null?"":rset.getString(10)));
			}
			rset.close();
			stmt.close();
		}
	...
	//PRATHAM BHATT 20241007: OTP CONFIGURATION VARIABLES
	String otp_timer = "";
	String max_otp = "";
	String otp_max_length = "";
	String otp_min_length = "";
	...
	public String getSei_central() {return sei_central;}
	
	//Pratham Bhatt 20241007: Otp Configuration variables getter
	public String getOtp_timer() {return otp_timer;}
	public String getMax_otp() {return max_otp;}
	public String getOtp_max_length() {return otp_max_length;}
	public String getOtp_min_length() {return otp_min_length;}
	...
	Vector VGROUP_OWNER = new Vector();
	...
	public Vector getVGROUP_OWNER() {return VGROUP_OWNER;}
