...
public class Frm_reset_password extends HttpServlet
{
	...
	private void ResetPassword(HttpServletRequest request) throws Exception 
	{
		...
		try
		{
			...
			String user_id = request.getParameter("emp_uid")==null?"":request.getParameter("emp_uid");
			user_id = user_id.equals("")?(request.getParameter("inputUserID_nm")==null?"":request.getParameter("inputUserID_nm")):user_id;
...
//							System.out.println("The new password is:"+gen_password);
...
//									new com.etrm.fms.util.InfoLogger().InsertInfoLogger(emp_cd, comp_cd,emp_nm,ip, "0", "Login Page","0","Login Page", old_value, new_value, audit_log_msg);  	
							...
							else if (option.equalsIgnoreCase("RESET_PASSWORD")) 
							{
								try
								{
									new com.etrm.fms.util.InfoLogger().InsertInfoLogger((String)session.getAttribute("emp_cd"),"",(String)session.getAttribute("emp_nm"),ip, form_id, form_nm,mod_cd,mod_nm, old_value, new_value, audit_log_msg);  	
//									new com.etrm.fms.util.InfoLogger().InsertInfoLogger((String)session.getAttribute("emp_cd"), comp_cd,(String)session.getAttribute("emp_nm"),ip, form_id, form_nm,mod_cd,mod_nm, old_value, new_value, audit_log_msg);  	
			...
			{
				session.setAttribute("emp_uid", user_id);
...
//			}
			...
			if (option.equalsIgnoreCase("RESET_PASSWORD")) 
			{
				try
				{
					new com.etrm.fms.util.InfoLogger().InsertInfoLogger((String)session.getAttribute("emp_cd"),"",(String)session.getAttribute("emp_nm"), (String)session.getAttribute("ip"), form_id, form_nm,mod_cd,mod_nm, old_value, new_value, audit_log_msg);  	
	...
	private void ReleaseLock(HttpServletRequest request) throws SQLException 
	{
		...
		try
		{
			...
			user_id = request.getParameter("emp_uid")==null?"":request.getParameter("emp_uid");
			String email_id = request.getParameter("inputEmail_Id")==null?"":request.getParameter("inputEmail_Id");
			...
			if(!emp_cd.equals("")) 
			{
				...
				if(lock_count > 0)
				{
					query = "UPDATE FMS_EMP_MST SET LOCK_STATUS='', MODIFY_DT=SYSDATE, MODIFY_BY=?, INVALID_LOGIN=?,MOD_PROFILE=? "
							+ "WHERE EMP_CD=? ";
					stmt=dbcon.prepareStatement(query);
					stmt.setString(1, emp_cd);
					stmt.setString(2, "0");
	...
	private void LockedUserMailBody(String comp_cd,String user_nm,String user_cd,String lock_status) throws Exception
	{
		...
		try
		{
			...
			to_mail_lock_user+=","+utilBean.getToMailReceipentList(dbcon,"0","User Activity Notification","Admin","NA","On-Event");
			String cc_mail_lock_user ="";
			cc_mail_lock_user+= utilBean.getCcMailReceipentList(dbcon,"0","User Activity Notification","Admin","NA","On-Event");
	...
	//Password change during reset and forgot password
	private void ChangePassword(HttpServletRequest request) throws SQLException 
	{
		...
		try
		{
			...
			current_pwd = request.getParameter("current_pwd")==null?"":request.getParameter("current_pwd");
			new_pwd = request.getParameter("new_pwd")==null?"":request.getParameter("new_pwd");
			confirm_pwd = request.getParameter("confirm_pwd")==null?"":request.getParameter("confirm_pwd");
			ip = request.getParameter("ip")==null?"":request.getParameter("ip");
			user_nm = utilBean.getEmpName(dbcon,user_cd);
			
			session.setAttribute("emp_uid", user_id);
			session.setAttribute("emp_cd", user_cd);
			session.setAttribute("emp_nm", user_nm);
			session.setAttribute("ip",ip);
			...
			{
				url = "../home/frm_requried_password_change.jsp?msg="+msg+"&msg_type="+msg_type+commonUrl_pra;
			}
			else
			{
				url = "../home/fms_change_password.jsp?msg="+msg+"&msg_type="+msg_type+commonUrl_pra;
			}
		}
		catch(Exception e)
		{
		...
		if(form_nm.equals("") || mod_nm.equals("") || mod_cd.equals("") || form_id.equals("")) // THis Is for Forced Password change page
		{
			try
			{
				new com.etrm.fms.util.InfoLogger().InsertInfoLogger(user_cd, "",user_nm, ip, "0", "Change Password","0","Login Page", old_value, new_value, msg);  	
			}
			catch(Exception infoLogger)
			{
				new SystemErrorLogger().InsertErrorLogger(db_src_file_name, function_nm, infoLogger);
