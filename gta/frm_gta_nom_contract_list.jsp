...
function refresh(active_status)
{
	...
	var url = "../gta/frm_gta_nom_contract_list.jsp?active_status="+active_status+"&counterparty_cd="+counterparty_cd+"&customer_cd="+customer_cd+"&contract_type="+contract_type;
...
function refresh_date()
{
	...
	if(flag && flg)
	{
		if(trim(from_dt)!="" && trim(to_dt)!="")
		{
			...
			{
			var url = "../gta/frm_gta_nom_contract_list.jsp?active_status="+active_status+"&counterparty_cd="+counterparty_cd+"&contract_type="+contract_type+
					"&customer_cd="+customer_cd+"&from_dt="+from_dt+"&to_dt="+to_dt;
			location.replace(url);
			}
		}
	}
}
function setValue(counterparty_cd,agmtno,agmt_revno,contno,cont_revno)
{
	window.opener.setContractDetail(counterparty_cd,agmtno,agmt_revno,contno,cont_revno);
	window.close();	
}
</script>
</head>
...
gta.setCallFlag("GTA_CONTRACT_LIST_FOR_NOM");
				...
				<div class="card-body cdbody">
					...
					<div class="col-sm-12 col-xs-12 col-md-12">
						<div class="table-responsive">
							<table class="table table-bordered table-hover ems_sorttbl" id="ems_table0">
								<thead id="ems_tbsort0">
									<tr>
										<th></th>
										<th class="ems_thsort0" >Transporter</th>
										<th class="ems_thsort0" >Contract#</th>
										<th class="ems_thsort0" >Contract Ref#</th>
										<th class="ems_thsort0" >CT</th>				    		
								...
								<%if(VCONT_NO.size() > 0){ %>
									...
									<tr>
										<td align="center">
											<input type="radio" onclick="setValue('<%=VCOUNTERPARTY_CD.elementAt(i)%>','<%=VAGMT_NO.elementAt(i)%>','<%=VAGMT_REV_NO.elementAt(i)%>','<%=VCONT_NO.elementAt(i)%>','<%=VCONT_REV_NO.elementAt(i)%>');">
										</td>
								...
								<%}else{ %>
									<tr>
										<td align="center" <%if(contract_type.equals("C")){ %>colspan="10"<%}else{ %>colspan="9"<%} %>><%=utilmsg.infoMessage("<b>No GT Contract is Created!</b>") %></td>
									</tr>
								<%} %>
								</tbody>
							</table>
