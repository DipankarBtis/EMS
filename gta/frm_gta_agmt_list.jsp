...
function refresh(active_status)
{
	...
	var url = "../gta/frm_gta_agmt_list.jsp?active_status="+active_status+"&counterparty_cd="+counterparty_cd+"&agreement_type="+agreement_type;
...
function refresh_date()
{
	...
	if(flag && flg)
	{
		if(trim(from_dt)!="" && trim(to_dt)!="")
		{
			...
			{
			var url = "../gta/frm_gta_agmt_list.jsp?active_status="+active_status+"&counterparty_cd="+counterparty_cd+
							...
							<table class="table table-bordered ems_sorttbl" id="ems_table0">
								<thead id="ems_tbsort0">
									<tr>
								    	...
								    	<th class="ems_thsort0">Total MMBTU</th>
								    	<th class="ems_thsort0">Transported MMBTU</th>
								    	<th class="ems_thsort0">Calorific Base</th>
								    </tr>
								</thead>
								<tbody>
								<%if(VAGMT_NO.size()>0){ %>
									...
									<tr>
										<td align="center">
											<input type="radio" onclick="setValue('<%=VAGMT_NO.elementAt(i)%>','<%=VAGMT_REV_NO.elementAt(i)%>','<%=VAGMT_NAME.elementAt(i)%>','<%=VAGMT_TYPE.elementAt(i)%>')">
										</td>
