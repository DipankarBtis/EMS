...
function refresh(opration)
{
	...
	if(flag)
	{
		var url = "frm_gta_f_flow_invoice.jsp?u="+u+"&counterparty_cd="+counterparty_cd+
		...
		url+= "&mapping_id="+mapping_id+
				...
				"&address_type="+address_type+"&bu_unit="+bu_unit+"&invoice_type="+invoice_type+"&opration="+opration+
				"&tcs_tds="+tcs_tds+
				"&bu_contact_person="+bu_contact_person;
...
function doSubmit()
{
	...
	var year = document.forms[0].year.value;
	var billing_cycle = document.forms[0].billing_cycle.value;
	var gta_invoice_type = document.forms[0].gta_invoice_type.value;
...
function printPDF()
{
	...
	var url = "../gta/pdf_gta_f_flow_invoice.jsp?counterparty_cd="+counterparty_cd+"&is_print="+is_print+"&contract_type="+contract_type+
...
function openTaxStructMst()
{
	...
	else
	{
		...
		if(!newWindow || newWindow.closed)
		{
			newWindow = window.open("frm_gta_tax_structure_list.jsp?type="+type,"Tax Structure List","top=10,left=10,width=1100,height=600,scrollbars=1");
		}
		else
		{
			newWindow.close();
...
function openTcsTdsStructMst(tax_app)
{
	...
	else
	{
		...
		if(!newWindow || newWindow.closed)
		{
			newWindow = window.open("frm_gta_tcs_tds_tax_structure_list.jsp?type="+type+"&tax_app="+tax_app,"Tax Structure List","top=10,left=10,width=1100,height=600,scrollbars=1");
		}
		else
		{
			newWindow.close();
...
function calcgrossamtInr()
{
	var gross_usd = document.getElementById("gross_amt_usd");
	var exchange_rate = document.getElementById("exchange_rate");
	var gross_amt_inr = document.getElementById("gross_amt_inr");
	var inv_raised_in = document.getElementById("invoice_raised_in");
	var gross_inr ;
	
	if(trim(inv_raised_in.value)=="1")
	{
		if(trim(gross_usd.value)!="" || trim(exchange_rate.value)!="")
		{
			gross_inr = round(parseFloat(gross_usd.value)*parseFloat(exchange_rate.value),2);
		}
		
		if(!isNaN(gross_inr))
		{
			gross_amt_inr.value=round(parseFloat(gross_inr),2)
		}
		else
		{
			gross_amt_inr.value="";
	...
	else if(trim(inv_raised_in.value)=="2")
	{
		if(trim(gross_usd.value)!="" || trim(exchange_rate.value)!="")
		{
			gross_inr = round(parseFloat(gross_usd.value)/parseFloat(exchange_rate.value),2);
...
function calcInvAmt()
{
	...
	if(trim(inv_raised_in.value)=="1")
	{
		gross_amt = document.getElementById("gross_amt_inr");
	}
	else if(trim(inv_raised_in.value)=="2")
	{
		gross_amt = document.getElementById("gross_amt_usd");
	}
	var tax_amt = document.getElementById("tax_amt");
	var invoice_amt = document.getElementById("invoice_amt");
	var adjust_amt = document.getElementById("adjust_amt");
	var net_amt = document.getElementById("net_amt");
	
	var inv_amt;
	var net_amount;
	
	if(trim(gross_amt.value)!="" || trim(tax_amt.value)!="")
	{
		inv_amt = round(parseFloat(gross_amt.value)+parseFloat(tax_amt.value),2);
	}
	...
	if(trim(adjust_amt.value)!="" || trim(invoice_amt.value)!="")
	{
		net_amount = round(parseFloat(invoice_amt.value)+parseFloat(adjust_amt.value),2);
	}
	else
	{
		net_amount = round(parseFloat(invoice_amt),2);
	}
	
	if(!isNaN(net_amount))
	{
		net_amt.value = round(parseFloat(net_amount),2);
	}
	else
	{
		net_amt.value ="";
...
String sub_invoice_type = remittance.getSub_inv_type();
			...
			<div class="card cardmain">
				<div class="card-header cdheader">
					<div class="d-flex justify-content-between">
					    <div class="topheader">
					    	CT | Parking Free Flow Invoice Generation
					    </div>
					</div>
				</div>
					...
					<div class="row m-b-5" <%if(opration.equals("MODIFY")){%>style="pointer-events: none;"<%}%>>
						<div class="col-sm-6 col-xs-2 col-md-4">
							<div class="form-group row">
								<div class="col-auto">  	
					    			<label class="form-label"><b>Select Transporter</b></label>
					  			</div>
								<div class="col">  
				...
				<div class="card-body cdbody">
							...
							<div class="form-group row">
								<div class="col"> 
									<div class="input-group input-group-sm">
				      					<input type="text" class="form-control form-control-sm date <%if(billing_cycle.equals("8")){ %>fmsdtpick<%}%>" name="period_start_dt" value="<%=period_start_dt %>" maxLength="10" 
				      					onchange="validateDate(this);checkStartEndDate('F');refresh('<%=opration%>');"><%-- <%if(!billing_cycle.equals("8")){ %>readonly<%} %> --%>
								...
								<div class="col"> 	
				      				<div class="input-group input-group-sm">
				      					...
				      					onchange="validateDate(this);checkStartEndDate('T');refresh('<%=opration%>');" ><%-- <%if(!billing_cycle.equals("8")){ %>readonly<%} %> --%>
				      					<span class="input-group-text"><i class="fa fa-calendar fa-lg"></i></span>
    								</div>
								</div>
							</div>
					...
					<div class="row m-b-5">	
						<div class="col-sm-2 col-xs-2 col-md-2">	
							<div class="form-group row">
					    		<label class="form-label"><b>Transporter Business Unit<span class="s-red">*</span></b></label>
					  		</div>
					  	</div>
						<div class="col-sm-4 col-xs-4 col-md-4"> 
							<div class="form-group row">	
				...
				<div class="card-body cdbody">
					...
					<div class="row m-b-5">
						...
						<div class="col-sm-4 col-xs-4 col-md-4">  
							<div class="form-group row">
				    			<div class="col-sm-12 col-xs-12 col-md-12">
				    				<input type="radio" style="pointer-events: none;" name="invoice_category" value="P" <%if(invoice_category.equals("P")){ %>checked<%}%>>&nbsp;Product&nbsp;&nbsp; 
				    ...
				    <div class="row m-b-5">
						...
						<div class="col-sm-4 col-xs-4 col-md-4">  
							...
							<%}else { %>
				    			<div class="col-sm-12 col-xs-12 col-md-12">
				    		<%} %>
			    				<select class="form-select form-select-sm" name="invoice_type" onchange="refresh('<%=opration%>');" <%if(opration.equals("MODIFY")){ %>style="pointer-events: none;"<%} %>>				    				
									<option value="0">--Select--</option>
									<% if(contract_type.equals("C") || contract_type.equals("R")) { %>
									<option value="IC">Imbalance Charge (IC)</option>
									<option value="TC">Transmission Charge (TC)</option>
									<%} else if(contract_type.equals("K")) {%>
									<option value="PC">Parking Charge (PC)</option>	
									<%} %>
				    	...
				    	<%if (invoice_type.equals("CR") || invoice_type.equals("DR") || invoice_type.equals("LP") || invoice_type.equals("OR")) {%>
				    	<div class="col-sm-2 col-xs-2 col-md-2">  
							<div class="form-group row">
				    				...
				    				<select class="form-select form-select-sm" name="sub_invoice_type">
										<option value="">--Select--</option>											
											<%if(invoice_type.equals("CR") || invoice_type.equals("DR")){ %>
												<% if(contract_type.equals("C") || contract_type.equals("R")) { %>
												<option value="TRN">Transmission Charge</option>
												<option value="IMB">Imbalance Charge</option>
												<%} else if(contract_type.equals("K")) {%>
												<option value="PAR">Parking Charge</option>
												<%} %>	
												<option value="LP">Late payment</option>
											<%} else if(invoice_type.equals("LP") || invoice_type.equals("OR")){ %>
												<% if(contract_type.equals("C") || contract_type.equals("R")) { %>
												<option value="TRN">Transmission Charge</option>
												<option value="IMB">Imbalance Charge</option>
												<%} else if(contract_type.equals("K")) {%>
				    ...
				    <div class="row m-b-5">
				   	...
				   	<div class="row m-b-5">
						...
						<div class="col-sm-4 col-xs-4 col-md-4">  
							<div class="form-group row">
				    			<div class="col-sm-12 col-xs-12 col-md-12">
				    				<select class="form-select form-select-sm" name="invoice_raised_in" id="invoice_raised_in" onchange="setUnitValue(this);">
										<!-- <option value="0">--Select--</option>  -->
				    	...
				    	<div class="col-sm-2 col-xs-2 col-md-2">  
							<div class="form-group row">
				    			<label class="form-label"><b>Invoiced Qty <%if(!invoice_type.equals("LP")){%><span class="s-red">*</span><%} %></b></label>
				  			</div>
						</div>
						<div class="col-sm-4 col-xs-4 col-md-4">  
							<div class="form-group row">
								<div class="col-8">
				    				<input type="text" class="form-control form-control-sm" name="alloc_qty" placeholder="Specify difference in Qty | Zero as applicable" value="<%=qty_mmbtu%>">
				    			</div>
				    			<div class="col-4">								
									...
									<table class="table table-bordered table-hover">
										...
										<tbody>
											<tr>
												...
												<td align="center"></td>
												<td align="center">
													<input type="text" class="form-control form-control-sm" name="gross_amt_usd" id="gross_amt_usd" value="<%=gross_amt%>" onkeyup="calcgrossamtInr();" maxlength='20' style="width:100px;text-align:right;">
												</td>
												<td align="center"></td>
											</tr>
											<tr>
												...
												<td align="center"></td>
												<td align="center">
													<input type="text" class="form-control form-control-sm" name="exchange_rate" id="exchange_rate" value="<%=exchang_rate%>" onkeyup="calcgrossamtInr();" maxlength='20' style="width:100px;text-align:right;">
												</td>
												<td align="center"></td>
												<td align="center"></td>
											</tr>
											<tr>
												...
												<td align="center"></td>
												<td align="center">
													<input type="text" class="form-control form-control-sm" name="gross_amt_inr" id="gross_amt_inr" value="<%=gross_amt1%>" maxlength='20' style="width:100px;text-align:right;">
												</td>
												<td align="center"></td>
											</tr>
											<tr>
												...
												<td align="center"></td>
												<td align="center">
													<input type="text" class="form-control form-control-sm" name="tax_amt" id="tax_amt" value="<%=tax_amt%>" onkeyup="calcInvAmt();" maxlength='20' style="width:100px;text-align:right;">
												</td>
												<td align="center"></td>
											</tr>
											...
											<tr>
												...
												<td align="center"></td>
												<td align="center">
													<input type="text" class="form-control form-control-sm" name="invoice_amt" id="invoice_amt" value="<%=invoice_amt%>" maxlength='20' style="width:100px;text-align:right;">
												</td>
												<td align="center"></td>
											</tr>
											<tr>
												...
												<td align="center"></td>
												<td align="center">
													<input type="text" class="form-control form-control-sm" name="adjust_amt" id="adjust_amt" onkeyup="calcInvAmt();" value="<%=invoice_adj_amt%>" maxlength='20' style="width:100px;text-align:right;">
												</td>
												<td align="center"></td>
											</tr>
											...
											<tr>
												...
												<td align="center"></td>
												<td align="center">
													<input type="text" class="form-control form-control-sm" name="net_amt" id="net_amt" value="<%=net_payable%>" maxlength='20' style="width:100px;text-align:right;">
												</td>
												<td align="center"></td>
											</tr>
										</tbody>	
...
<input type="hidden" name="option" value="GTA_FFLOW_INVOICE">
...
<input type="hidden" name="gta_invoice_type" value="F">
