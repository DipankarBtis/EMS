...
function setContDetail(countpty_cd,agmt_no,agmt_rev_no,cont_no,cont_rev_no,st_dt,end_dt,cont_type)
{
	...
	var url = "frm_seller_periodic_nom.jsp?counterparty_cd="+countpty_cd+"&cont_no="+cont_no+"&cont_rev_no="+cont_rev_no+
			...
			"&nomination_freq="+nomination_freq+"&u="+u+"&contract_type="+cont_type;

	document.getElementById("loading").style.visibility = "visible";
	location.replace(url);
}

function checkAll(obj,range)
{
...
function doSubmit()
{
	...
	else
	{
		...
		if(trim(qty_mmbtu.value) != "")
		{
			var tot_mmbtu = tot_mmbtu + parseFloat(qty_mmbtu.value);
		}
	}
														...
														<div class="table-responsive">
															<table class="table table-bordered table-hover">
																<thead>
																	<tr>
																		<th>Gas Date</th>
																		<th><input class="form-check-input" type="checkbox" name="chkAll" onclick="checkAll(this,'<%=VENTRY_RANGE.elementAt(j)%>');"></th>
																		<th>DCQ</th>
																		<th>Business Unit</th>
																			...
																			%>
																				<tr>
																					...
																					<td align="center" valign="middle" <%if(VNOM_BLOCK.elementAt(l).equals("Y")) {%>style="background: #df9fbf;" title="Invoice Generated!"<%} %>>
																						...
																						onclick="setEnableDisabled(this,'<%=l%>');calculateSCM('<%=l%>');"
																					...
																					<td align="center">
																						<div style="width:100px;">
																							...
																							<%-- onpaste="doPaste('<%=j%>','<%=l%>');"  --%>
																							onblur="negNumber(this);checkNumber1(this,9,2);checkQty('<%=l%>');calculateSCM('<%=l%>');" disabled>
																							<%-- checkTCQ('<%=l%>'); --%>
																					...
																					%>
																						<tr>
																							<td>
																								...
																								onclick="setSubEnableDisabled(this,'<%=l%>','<%=q%>');calculateSCM('<%=l%>');totalSubQty('<%=l%>');checkSubQty('<%=l%>','<%=q%>');" disabled>
																							...
																							<td align="center">
																								<div style="width:100px;">
																									...
																									onblur="negNumber(this);checkNumber1(this,9,2);calculateSCM('<%=l%>');totalSubQty('<%=l%>');checkSubQty('<%=l%>','<%=q%>');" disabled>
																									<%-- checkTCQ('<%=l%>'); --%>
																								</div>
																							</td>
																							<td align="center">
