...
function refresh(active_status)
{
	...
	var url = "../gta/frm_gtpa_agmt_list.jsp?active_status="+active_status+"&counterparty_cd="+counterparty_cd+"&agreement_type="+agreement_type;
...
function refresh_date()
{
	...
	if(flag && flg)
	{
		if(trim(from_dt)!="" && trim(to_dt)!="")
		{
			...
			{
			var url = "../gta/frm_gtpa_agmt_list.jsp?active_status="+active_status+"&counterparty_cd="+counterparty_cd+
...
gta.setCallFlag("GTPA_AGMT_LIST");
								...
								<%if(VCOUNTERPTY_CD.size() > 0){ %>
									...
									<tr>
										<td align="center"><input type="radio"
										 onclick="setValue('<%=VCOUNTERPTY_CD.elementAt(i) %>','<%=VAGMT_NO.elementAt(i)%>','<%=VAGMT_REV_NO.elementAt(i)%>','<%=VAGMT_TYPE.elementAt(i)%>','<%=VAGMT_NAME.elementAt(i) %>')"></td>
