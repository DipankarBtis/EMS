...
function refresh(opration)
{
	...
	var url = "frm_gta_parking_cont_mst.jsp?counterparty_cd="+counterparty_cd+"&opration="+opration+"&u="+u+"&contract_type="+contract_type;
...
function setContractDetail(agmt_no,agmt_rev_no,cont_no,cont_rev_no,agmt_type,countpty_cd,cont_type)
{
	...
	var url = "frm_gta_parking_cont_mst.jsp?counterparty_cd="+countpty_cd+"&opration="+opration+
...
function setAgreementDetail(agmt_no,agmt_rev_no,agmt_nm,agmt_type)
{
	...
	var url = "frm_gta_parking_cont_mst.jsp?counterparty_cd="+counterparty_cd+"&opration="+opration+
...
function openSalesContList()
{
	...
	if(chk_trans_bu!=null && chk_trans_bu!=undefined)
	{
		if(chk_trans_bu.length!=undefined)
		{
			for(var i=0;i<chk_trans_bu.length;i++)
			{
				...
				{
					trans_bu_unit=chk_trans_bu[i].value;	
					transchkFlg=true;
				}
			}
		}
		else
		{
			...
			{
				trans_bu_unit=chk_trans_bu.value;	
				transchkFlg=true;
			}
		}
	}
	...
	if(transchkFlg==false)
	{
		msg += "Please Select Atleast One Transporter Business Unit!\n";
		flag=false;
	}	
	
	var url ="frm_gta_mapping_cont_list.jsp?transporter_cd="+transporter_cd+"&start_dt="+start_dt+"&end_dt="+end_dt+
			"&entry_point="+entry_point+"&exit_point="+exit_point+"&bu_unit="+bu_unit+"&trans_bu_unit="+trans_bu_unit;
...
function setSalesContDetail(customer_cd,agmt_no,agmt_rev_no,cont_no,cont_rev_no,cont_type,dealNo,contRef)
{
	...
	document.forms[0].transporter_cd.value=customer_cd;
	document.forms[0].gta_cont_nm.value=sales_cont_nm;
	
	document.forms[0].gta_cont_map.value=sales_cont_map;
}
...
function doSubmit()
{
	...
	var parking_rate = document.forms[0].parking_rate.value;
	var park_rate_cd = document.forms[0].park_rate_cd.value;
	var max_park_qty = document.forms[0].max_park_qty.value;
	var max_park_unit_cd = document.forms[0].max_park_unit_cd.value;
	
	var transporter_cd = document.forms[0].transporter_cd.value;
	var gta_cont_map = document.forms[0].gta_cont_map.value;
	...
	if(trim(start_dt)!="" && trim(end_dt)!="")
   	...
   	{
   		msg += "Please Insert Start and End Date of ..\n";
     	flag = false; 
   	}
	...
	if(trim(max_park_qty) == "")
	{
		msg+="Enter Max Parking Qty!\n";
		flag=false;
	}
	if(trim(max_park_unit_cd) == "")
	{
		msg+="Select Max Parking Qty Unit!\n";
		flag=false;
	}
	...
	if(contract_type=="K")
	{
		...
		if(trim(gta_cont_map) == "")
		{
			msg+="Capacity Tranche Mapping is Missing!\n";
			flag=false;
		}
	}
	...
	if(trim(parking_rate) == "")
	{
		msg+="Enter Parking Rate!\n";
		flag=false;
	}
	if(trim(park_rate_cd) == "")
	{
		msg+="Select Parking rate Unit!\n";
		flag=false;
	}
	...
	if(flag)
	{
		...
		if(opration=="MODIFY")
		{
			a = confirm("Do you want to Modify Parking Contract?");
		}
		else
		{
			a = confirm("Do you want to Create New Parking Contract?");
		}
		if(a)
		{
			var temp_msg="";
...
String agreement_type = request.getParameter("agreement_type")==null?"P":request.getParameter("agreement_type");

gta.setCallFlag("PARKING_CONTRACT");
...
String park_rate_cd =gta.getRate_unit();
String parking_rate =gta.getParking_rate();
String no_of_billing_dtl=gta.getNo_of_billing_dtl();
String max_park_qty=gta.getMax_park_qty();
String max_park_qty_unit=gta.getMax_park_qty_unit();

String transporter_cd=gta.getCustomer_cd();
String gta_cont_nm=gta.getSales_cont_nm();
String gta_cont_map=gta.getSales_cont_map();
...
if(parking_rate.equals(""))
{
	parking_rate="5.75";
}
if(park_rate_cd.equals(""))
{
	park_rate_cd="1";
}
%>
		...
		<div class="col-md-12 col-sm-12 col-xs-12">
			...
			<div class="card cardmain">
				...
				<div class="card-body cdbody">
					...
					<div class="row m-b-5">
						<div class="col-sm-2 col-xs-2 col-md-2">  
							<div class="form-group row">
				    			...
				    			<input type="button" class="btn btn-info btn-sm select_btn" value="Select Parking Contract" onclick="openContList();" style="font-weight: bold;">
				    			</div>
				  			</div>
						</div>
						<div class="col-sm-4 col-xs-4 col-md-4">  
					...
					<div class="row m-b-5">
						<div class="col-sm-2 col-xs-2 col-md-2">
							<label class="form-label"><b>Maximum Parked Qty<span class="s-red">*</span></b></label>
						</div>
						<div class="col-sm-4 col-xs-4 col-md-4">
							<div class="form-group row">
								<div class="col-auto">
									<input type="text" class="form-control form-control-sm" name="max_park_qty" value="<%=mdq%>" onblur="checkNumber1(this,11,3);">
				    			</div>
				    			<div class="col-auto">
									<select class="form-select form-select-sm" name="max_park_unit_cd" style="pointer-events: none;">
				    					<option value="1">MMBTU</option>
				    				</select>
				    			</div>	
				  			</div>
				    			...
				    			<div class="col-sm-12 col-xs-12 col-md-12">
				    				<%if(VTRANS_BU_SEQ_NO.size() > 0) {%>
					    				<%for(int i=0; i<VTRANS_BU_SEQ_NO.size(); i++){ %>
					    					<input type="radio" name="chk_trans_bu" value="<%=VTRANS_BU_SEQ_NO.elementAt(i)%>">&nbsp;<%=VTRANS_BU_ABBR.elementAt(i)%>&nbsp;&nbsp;
					...
					<div>
						...
						<div class="row m-b-5">
							<label class="form-label subheader"><i class="fa fa-snowflake-o"></i> Capacity Tranche Detail</label>
						</div>
						<%-- <div class="row m-b-5">
							<div class="col-sm-2 col-xs-2 col-md-2">
						...
						<div class="row m-b-5">
							<div class="col-sm-2 col-xs-2 col-md-2">
								<input type="button" class="btn btn-sm config_btn" value="Link Capacity Tranche" onclick="openSalesContList();" style="font-weight: bold;">
							</div>
							<div class="col-sm-4 col-xs-4 col-md-4"> 
								<div class="form-group row">		
									<div class="col">
										<input type="text" class="form-control form-control-sm" name="gta_cont_nm" value="<%=gta_cont_nm%>" style="background:#99ffcc;" readOnly>
				    					<input type="hidden" name="gta_cont_map" value="<%=gta_cont_map%>">
					...
					<div class="row m-b-5">
						<label class="form-label subheader"><i class="fa fa-snowflake-o"></i> Parking Charges</label>
					</div>
					<div class="row m-b-5">
						<div class="col-sm-2 col-xs-2 col-md-2">  
							<div class="form-group row">
				    			<label class="form-label"><b>Parking Rate<span class="s-red">*</span></b></label>
				  			</div>
						</div>
						<div class="col-sm-4 col-xs-4 col-md-4">  
							<div class="form-group row">
				    			<div class="col">
				      				<input type="text" class="form-control form-control-sm" name="parking_rate" 
				      				value="<%=parking_rate%>" onblur="checkNumber1(this,7,4);setAmount(this);">
				    			</div>
				    			<div class="col">
		      						<select class="form-select form-select-sm" name="park_rate_cd">
		      							<option value="1">MMBTU/Day</option>
		      						</select>
		      						<script>document.forms[0].park_rate_cd.value="<%=park_rate_cd%>"</script>
		      					</div>
				  			</div>
						</div>
					</div>
...
<input type="hidden" name="option" value="PARKING_CONTRACT_MST">
