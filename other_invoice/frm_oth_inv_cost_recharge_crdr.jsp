...
function refresh(operation)
{
	...
	if(flag)
	{
		var url = "frm_oth_inv_cost_recharge_crdr.jsp?u="+u+"&invoice_type="+invoice_type+"&cr_dr_type="+cr_dr_type+
...
async function calc()
{
	...
	else//!price_cd = 1
	{
		...
		var new_tax_struct_info = document.forms[0].new_tax_struct_info.value;
		var main_gross_amt = document.forms[0].main_gross_amt.value;
		var main_exchang_rate = document.forms[0].main_exchng_rate.value;
		var new_gross_amt = document.forms[0].new_gross_amt.value;
		...
		if(trim(main_gross_amt) != "")
		{
			...
			if(gross!="")
			{
				...
				var new_gross = document.forms[0].new_gross_amt.value;
				if(trim(exchng_rate)!="" && trim(gross) != "")
				{
					gross1=parseFloat(parseFloat(exchng_rate)*parseFloat(new_gross)).toFixed(2);
				...
				if(gross1!="")
				{
					...
					{
						new_tax_struct_factor=new_tax_struct_info.split(",")[0].split(" ")[1].split("%")[0];
						sub_tax_amt = Math.round(parseFloat((new_tax_struct_factor*gross1)/100).toFixed(2)).toFixed(2);
					}
					else
					{
						new_tax_struct_info = new_tax_struct_info.split(", ");
						//for(i=0;i<new_tax_struct_info.length;i++) 
				 		{
				 			taxcomp=parseFloat(((new_tax_struct_info[0].split(",")[0].split(" ")[1].split("%")[0]) * gross1) / 100).toFixed(2);
							taxcomp=Math.round(taxcomp);
							sub_tax_amt = taxcomp * 2;
				 		}
					}
...
other_inv.setCallFlag("COSTR_CRDR_LIST");
								...
								<div class="table-responsive">
									<table class="table table-bordered table-hover">
										...
										<tbody>
											...
											<%if(!price_cd.equals("1")&& !price_cd.equals("")){  %>
											<tr>
...
<input type="hidden" name="option" value="GENERATE_COSTR_CR_DR">
