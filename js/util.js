...
var date_range_days=1850;
function checkDateRangeOnApply(from_dt,to_dt)
{
		...
		if(trim(from_dt)!="" && trim(to_dt)!="")
	   	{
         	...
         	if(parseInt(days_difference)+1 <= date_range_days)
         	{
         	...
         	{
         		alert("Date range can not exceed more than "+date_range_days+" days!!")
         		flag1=false;
         	}
			return flag1;
		}
...
function checkMonthYearRange(month_from,year_from,month_to,year_to)
{
	const start_dt = new Date(year_from.value,month_from.value-1,1);
	const end_dt = new Date(year_to.value,month_to.value-1,1);
	const oneYearLater = new Date(start_dt);
	oneYearLater.setFullYear(start_dt.getFullYear() + 1);
	var flag=true;
	
	if(month_from.value !='' && year_from.value !='' && month_to.value !='' && year_to.value !='')
	{
		if(trim(month_from.value)!="" && trim(year_from.value)!="" && trim(month_to.value)!="" && trim(year_to.value)!="")
	   	{
         	if(end_dt>oneYearLater)
			{
				flag=false;
				alert("Selected Month/Year Should be in Range of One Year!");
				return flag;
			}
			else
			{
				 if(start_dt>end_dt)
				{
					alert('From Month/Year should be less than or equal to To Month/Year ');
					flag=false;
					return flag;
				}
			}
