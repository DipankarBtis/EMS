...
function setTaxStructDetail(index,tax_struct_cd,tax_struct_nm,tax_struct_eff_dt,type,sap_tax_code)
{
	...
	if(type=="P")
	{
		...
		document.getElementById("dis_tax_struct_nm"+index).value=tax_struct_nm+" since "+tax_struct_eff_dt;
	...
	else if(type=="S")
	{
		if(entity_role == "R" || entity_role == "G" || entity_role == "H" || entity_role == "S" || entity_role == "V")
		{
			...
			document.getElementById("dis_bu_ser_tax_struct_nm"+index).value=tax_struct_nm+" since "+tax_struct_eff_dt;
		...
		else
		{
			...
			document.getElementById("dis_ser_tax_struct_nm"+index).value=tax_struct_nm+" since "+tax_struct_eff_dt;
									...
									if(VPLANT_SEQ_NO.size() > 0){ %>
										...
										%>
											<tr>
												...
												<td <%if(size>0){ %>onclick="hide_show('tbody<%=i %>','hidCont<%=i%>');"<%} %>>
											...
											<tbody id="tbody<%=i%>" style="display:none;">
												...
												<%for(j=j; j<VTEMP_TAX_STRUCT_CD.size(); j++){ 
												k+=1;
												%>
													<tr>
														<td align="center"><%=VTEMP_SAP_TAX_CODE.elementAt(j)%></td>
														<%-- <td align="center"><%=VTEMP_TAX_STRUCT_CD.elementAt(j)%></td> --%>
														<td><%=VTEMP_TAX_STRUCT_NM.elementAt(j)%></td>
														<td align="center"><%=VTEMP_EFF_DT.elementAt(j)%></td>
														<td><%=VTEMP_TAX_STRUCT_RMK.elementAt(j)%></td>
														<td align="center"><%=VTEMP_ENTERED_BY.elementAt(j) %></td>
													</tr>
												<%
													if(k==size)
													{
												...
												<td align="center">
													<div style="width:150px;">
														<select class="form-select form-select-sm" name="ser_invoice_type">
															...
															<option value="UG">SUG Invoice</option>
															<option value="ST">Storage Invoice</option>
															<option value="DI">Deficiency Invoice</option>
														</select>
												...
												<td align="center">
													<div style="width:150px;">
														<select class="form-select form-select-sm" name="bu_ser_invoice_type">
															...
															<option value="PC">Parking Charges</option>
