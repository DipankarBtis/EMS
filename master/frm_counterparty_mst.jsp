...
function refresh(opration)
{
	...
	var url = "frm_counterparty_mst.jsp?opration="+opration+"&u="+u+"&counterparty_cd="+counterparty_cd;
...
function doSubmit()
{
	...
	if (!optionExists) {
	    ...
	    statusSelect.insertBefore(newOption, statusSelect.firstChild); // add at top
	}
	...
	if(flag)
	{
		...
		if(opration == "MODIFY")
		{
			...
			if(b)
			{
				a = confirm("Do you want to "+opration+" Counterparty Detail?")
			}
		}
		else
		{
...
%>
<body onload="changeStatus(document.forms[0].kyc_chk);">
		...
		<div class="col-md-12 col-sm-12 col-xs-12">
			...
			<div class="card cardmain">
				...
				<div class="card-body cdbody">
								...
								<div class="form-group row">
				    				<div class="col-sm-12 col-xs-12 col-md-12">
				    					<%if(!counterparty_cd.equals("0")){ %>
				    						<%if(status.equals("Y")){ %>
											<%=utilmsg.infoMessage("<b>Counterparty Status : Active (Effective from : "+max_eff_dt+")</b>")%>
											<%} %>
											<%-- <%if(status.equals("N")){ %>
											<div class='container'><div class='alert alert-danger'><i title="<%=cp_hist_desc%>" class="fa fa-header fa-lg" aria-hidden="true"></i>&nbsp;<b>Counterparty Deactivated since <%=max_eff_dt %></b></div></div>
											<%=utilmsg.errorMessage("<b>Counterparty Status : Deactive (Effective from : "+max_eff_dt+")</b>")%> 
											<%}else if(status.equals("E")){ %>
											<%=utilmsg.warningMessage("<b>Counterparty Status : E-Rate (Effective from : "+max_eff_dt+")</b>")%>
								...
								<div class="d-flex justify-content-end">
									<div class="form-group row">
				    					...
				    					<%if(write_access.equals("Y")){ %>
				    						<input type="button" class="btn btn config_btn" title="<%if(status.equals("Y")){%>Active<%}else if(status.equals("N")){%>Deactive<%}else if(status.equals("E")){%>E-Rate<%} %>"
				    						data-bs-toggle="modal" data-bs-target="#StatusToggelModal"
				    						value="Change Status" 
				    						<%-- <%if(status.equals("Y")){%>Active<%}else if(status.equals("N")){%>Deactive<%}else if(status.equals("E")){%>E-Rate<%} %> --%>
				    						>
				    					<%}else{ %>
				    						<input type="button" class="btn btn-sm request_btn" value="<%if(status.equals("N")){%>Activate<%}else{%>Deactivate<%}%>" disabled>
				    					<%} %>
				    					</div>
				    				</div>
				    			</div>
					...
					<%} %>
					<div <%if(status.equals("N")){ %> style="pointer-events: none" <%} %>>
					<div class="row m-b-5">
					...
					<div class="row m-b-5">
						...
						<div class="col-sm-4 col-xs-4 col-md-4">  
							<div class="form-group row">
				    			...
				    			<div class="col-sm-2 col-xs-2 col-md-2">
				    				<div class="input-group input-group-sm" >
										<i title="<%=cp_hist_desc%>" class="fa fa-info-circle fa-2x" aria-hidden="true" style="pointer-events: auto; color:#800040"></i>
										<%-- <input style="pointer-events: auto" title="<%=cp_hist_desc%>" type="button" class="btn btn-sm config_btn" value="H" onclick=""> --%>
									</div>
				    			</div>
				  			</div>
						</div>
					...
					<div class="row m-b-5">
						...
						<div class="col-sm-4 col-xs-4 col-md-4">  
							<div class="form-group row">
				    			<div class="col-sm-12 col-xs-12 col-md-12">
				      				<input type="checkbox" name="kyc_chk" <%if(kyc_flg.equals("Y") || opration.equals("INSERT")){%>checked<%} %> onclick="changeStatus(this)">&nbsp;<label class="form-label">KYC</label>&nbsp;&nbsp;&nbsp;
      				...
      				<div class="row m-b-5">
				    			...
				    			<div class="col-sm-12 col-xs-12 col-md-12">
				      				<select class="form-select form-select-sm" name="status">
										<%for(int i=0; i<VSTATUS_TYPE.size();i++) {%>
											<option value="<%=VSTATUS_TYPE.elementAt(i)%>"><%=VSTATUS_NM.elementAt(i)%></option>
										<%} %>
									</select>
				    			</div>
				  			</div>
						...
						<div class="col-sm-4 col-xs-4 col-md-4">  
							<div class="form-group row">
				    			<div class="col-sm-12 col-xs-12 col-md-12">
				      				<div class="input-group input-group-sm" >
			      						<input type="text" class="form-control form-control-sm date fmsdtpick" name="status_eff_dt" value="<%=sysdate%>" maxLength="10" 
...
<input type="hidden" name="max_eff_dt" value="<%=max_eff_dt%>">
<input type="hidden" name="curr_status" value="<%=status%>">

<input type="hidden" name="form_clearance" value="KYC">
<input type="hidden" name="temp_pan_no" value="<%=pan_no%>">
<input type="hidden" name="category" value="<%=category%>">
<input type="hidden" name="ncf_category" value="<%=ncf_category%>">
...
<script>
<%if(kyc_flg.equals("I") || status.equals("N")){%>
