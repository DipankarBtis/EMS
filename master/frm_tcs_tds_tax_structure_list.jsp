...
function setValue(VTAX_STRUCT_CD,VTAX_STRUCT_NM,VTAX_STRUCT_APP_DT,SAP_TAX_CODE)
{
	...
	window.opener.setTaxStructDetail(index,VTAX_STRUCT_CD,VTAX_STRUCT_NM,VTAX_STRUCT_APP_DT,type,SAP_TAX_CODE);
	window.close();	
}
</script>
</head>
...
String tax_pay_recv=request.getParameter("tax_pay_recv")==null?"":request.getParameter("tax_pay_recv");
...
dbmaster.setPay_recv(tax_pay_recv);
dbmaster.init();
									...
									<tr>
										<td width="5%" align="center">
											<div align="center">
												...
												onclick="setValue('<%=VTAX_STRUCT_CD.elementAt(i)%>','<%=VTAX_STRUCT_NM.elementAt(i)%>',
												'<%=VTAX_STRUCT_APP_DT.elementAt(i)%>','<%=VSAP_TAX_CODE.elementAt(i) %>');" <%if(VTAX_STRUCT_STATUS.elementAt(i).equals("0")){%>disabled<%} %>>
											</div>
										</td>
										...
										<td><%=VPAY_RECV_NM.elementAt(i)%></td>
