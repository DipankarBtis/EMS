...
function refresh(active_status)
{
	...
	var url = "../ltcora/frm_ltcora_agmt_list.jsp?active_status="+active_status+"&counterparty_cd="+counterparty_cd+"&buy_sale="+buy_sale+"&agreement_type="+agreement_type;
...
function refresh_date()
{
	...
	if(flag && flg)
	{
		if(trim(from_dt)!="" && trim(to_dt)!="")
		{
			...
			{
			var url = "../ltcora/frm_ltcora_agmt_list.jsp?active_status="+active_status+"&counterparty_cd="+counterparty_cd+"&buy_sale="+buy_sale+
...
function setValue(countpty_cd,buy_sale,agreement_type,agmt_no,agmt_rev_no)
{
	window.opener.setAgmtDetail(countpty_cd,buy_sale,agreement_type,agmt_no,agmt_rev_no);
	window.close();	
}
</script>
</head>
						...
						<div class="table-responsive">
							<table class="table table-bordered ems_sorttbl" id="ems_table0">
								...
								<%if(VCOUNTERPARTY_CD.size() > 0){ %>
									...
									<tr>
										<td align="center"><input type="radio" onclick="setValue('<%=VCOUNTERPARTY_CD.elementAt(i) %>','<%=buy_sale%>','<%=agreement_type%>','<%=VAGMT_NO.elementAt(i)%>','<%=VAGMT_REV_NO.elementAt(i)%>')"></td>
