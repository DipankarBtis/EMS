        ...
        if (chk.length != undefined) {
            for (var i = 0; i < chk.length; i++) {
                if (chk[i].checked) {
                    ...
                    if (trim(adq_qty[i].value) == "" && iscargoadded.value == "N") 
                    {
...
function arrivedCargoDtl(id_no,cargo_ref,ship_cd,ship_name,buyer_cd,buyer_nm,start_dt,end_dt,act_arrv_dt,cargo_qty,qq_qty_mmbtu,csoc_mmbtu,qq_qty_cer_no,qq_qty_cer_dt,no_win_days,cargo_mapping)
{
	...
	document.getElementById('qq_no'+id_no).value = qq_qty_cer_no;
						...
						<div class="table-responsive">
							<table class="table table-bordered table-hover" id="payable">
								...
								<%for(int i=0;i<VCARGO_NO.size();i++){ %>
									<tr>
										<td align="center" <%if(VCARGO_IS_NOMINATED.elementAt(i).equals("Y")) {%>style="background: #df9fbf;" title="Cargo Nominated!"<%} %>>
										...
										<td align="center">
											<div class="input-group input-group-sm" >
												...
												<span 
												<%if(!VCARGO_IS_NOMINATED.elementAt(i).equals("Y")) {%>onclick="selectArrivedCargo('<%=i%>','<%=VDISP_ATTACH_LNG_CARGO.elementAt(i)%>');"<%} %>
												<%if(VATTACH_LNG_CARGO.elementAt(i).equals("")){%>title="Attach Arrived Cargo"  style="background: #31d2f2"<%}else{ %>title="Cargo (<%=VDISP_ATTACH_LNG_CARGO.elementAt(i)%>) Attached"  style="background: #a6ff4d"<%} %> 
												...
												<input type="hidden" name="iscargoadded" id="iscargoadded<%=i%>" value="<%=VISCARGOADDED.elementAt(i)%>" disabled>
												<input type="hidden" name="attach_cargo_mapping" id="attach_cargo_mapping<%=i%>" value="<%=VATTACH_LNG_CARGO.elementAt(i)%>" disabled>
												<input type="hidden" name="disp_attach_cargo_mapping" id="disp_attach_cargo_mapping<%=i%>" value="<%=VDISP_ATTACH_LNG_CARGO.elementAt(i)%>" disabled>
											</div>
										</td>
										<td align="center">
										...
										<td align="center">
											<div style="width:100px;">
												<input style="text-align: right;" type="text" class="form-control form-control-sm" name="edq_qty" id="edq_qty<%=i%>" value="<%=VEDQ_QTY.elementAt(i)%>" disabled 
										...
										<td align="center">
										    <div class="form-group d-flex justify-content-between">
										        <div style="width:100px;">
										            <input style="text-align: right;" type="text" class="form-control form-control-sm" name="adq_qty" id="adq_qty<%=i%>" value="<%=VTOTAL_ADQ_QTY.elementAt(i)%>" readonly disabled>
										        </div>
										...
										<td align="center">
											<div style="width:100px;">
												<div class="input-group input-group-sm" >
													<input style="text-align: right;" type="text" class="form-control form-control-sm" name="csoc_qty" id="csoc_qty<%=i%>" value="<%=VCSOC_QTY.elementAt(i)%>" 
										...
										<td align="center"  valign="middle">
											<div >
												<i name="req_mod_btn" id="req_mod_btn" class="fa fa-cogs fa-lg" aria-hidden="true" onclick="openRequestModification('<%=i %>');" <%if(VSHIP_CD.elementAt(i).equals("")){ %>style="pointer-events : none;"  <%} else if(VMOD_REQUEST_FLAG.elementAt(i).equals("N")){%>style="color:red" title="Modification Request generated"<%} %>></i>
										...
										<td align="center">
											<div style="width:100px;">
												<div class="input-group input-group-sm" >
													<input style="text-align: right;" type="text" class="form-control form-control-sm" name="closure_qty" id="closure_qty<%=i %>" value="" disabled>
												</div>
											</div>
										</td>
										<td align="center"  valign="middle">
											<div >
												<i  name="req_clsr_btn" id="req_clsr_btn" class="fa fa-window-close fa-lg" aria-hidden="true" onclick="openClosureRequest('<%=i%>')" style="color:#bfbfbf"></i>
					...
					<div class="accordion">
						<div class="accordion-item accor_item">
							<h2 class="accordion-header" id="heading1">
								<button name="sub_module_cd" class="accordion-button collapsed accor-btn" type="button" data-bs-toggle="collapse" data-bs-target="#collapse" aria-expanded="false" aria-controls="collapse">LTCORA (Buy) CN/Period Cargo Requests&nbsp;&nbsp;<font color="blue">(<%=count%> Items)</button>	
					    	</h2>
