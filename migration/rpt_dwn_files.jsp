...
function refresh(index)
{
	...
	if(path!="")
	{
		var url = "rpt_dwn_files.jsp?&u="+u+"&path="+path+"&dwn_folder="+dwn_folder;
...
function refresh1()
{
	...
	var url = "rpt_dwn_files.jsp?&u="+u+"&dwn_folder="+dwn_folder;
...
function goBack()
{
	...
	var url = "rpt_dwn_files.jsp?path="+path_sub_sub+"&u="+u+"&dwn_folder="+dwn_folder;
...
String dwn_folder = request.getParameter("dwn_folder")==null?"DLOG":request.getParameter("dwn_folder");
String path_sub_sub = request.getParameter("path_sub_sub")==null?"":request.getParameter("path_sub_sub");
String path_sub = "";

if(dwn_folder.equals("DLOG"))
{
path_sub = ""+"migration"+"/"+"DataLogs"+"/"+path;
	
	dwn_path = "migration"+"/"+"DataLogs"+"/"+path;
}
else if(dwn_folder.equals("EXP"))
{
path_sub = ""+"migration"+"/"+"EXPORT"+"/"+path;
	
	dwn_path = "migration"+"/"+"EXPORT"+"/"+path;
}
/* else if(dwn_folder.equals("MIG"))
{
	path_sub = ""+"migration"+"/"+path;
	
	dwn_path = "migration"+"/"+path;
} */

String file_path = request.getRealPath(path_sub);

dev_tool.setCallFlag("DTLESS_FILE_DOWNLOAD");
								...
								<div class="col-sm-12 col-xs-12 col-md-12" align="center">
									<div class="btn-group">
										<select class="btn btn-outline-secondary subbtngrp" name="dwn_folder" onchange="refresh1();">
											...
											<option value="DLOG">DataLogs</option>
