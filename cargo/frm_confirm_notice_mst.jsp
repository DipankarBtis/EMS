...
function doModifyCargo(VDISP_CARGO_NO,VCARGO_NO,VCARGO_REF,VCARGO_STATUS_FLG,VCARGO_START_DT,VCARGO_END_DT,VCARGO_QTY,VCARGO_PRICE,VCARGO_PRICE_UNIT_FLG,VIS_CARGO_NOMINATED,VIS_CARGO_ALLOCATED,VCARGO_TOLERANCE_PER)
{
	...
	document.forms[0].cargo_tolenrance_per.value = VCARGO_TOLERANCE_PER;
...
%>
<body onload="demurrageShows();messurmentShows();terminatorShows();off_spec_gasShows();billingShows();liabilityShows();setValues('<%=strPlant%>','<%=strBuPlant%>');revisionEffDateShows();"> 
				    ...
				    <div class="row">
						<div class="table-responsive">
							<table class="table table-bordered table-hover" id="cargo_tb">
									...
									{%>
										<tr id="row">
											<td align="center">
												...
												onclick="doModifyCargo('<%= VDISP_CARGO_NO.elementAt(i)%>',
												'<%= VCARGO_NO.elementAt(i)%>',
												'<%= VCARGO_REF.elementAt(i) %>',
												'<%= VCARGO_STATUS_FLG.elementAt(i) %>',
												'<%= VCARGO_START_DT.elementAt(i) %>',
												'<%= VCARGO_END_DT.elementAt(i) %>',
												'<%= VCARGO_QTY.elementAt(i) %>',
												'<%= VCARGO_PRICE.elementAt(i) %>',
												'<%= VCARGO_PRICE_UNIT_FLG.elementAt(i)%>',
												'<%= VIS_CARGO_NOMINATED.elementAt(i) %>',
												'<%= VIS_CARGO_ALLOCATED.elementAt(i) %>',
												'<%= VCARGO_TOLERANCE_PER.elementAt(i)%>')">
												</i> 
												</font>
											</td>
													...
													<div class="form-group row">
														...
														<div style="width:100px;" >
															...
															onclick="variablePriceConfig('<%=contract_type%>','<%=agmt_no%>','<%=agmt_rev_no%>','<%=cont_no%>',
																						'<%=cont_rev_no%>','<%=cont_ref_no%>','<%=VCARGO_STATUS.elementAt(i)%>',
																						'<%=VCARGO_START_DT.elementAt(i) %>','<%=VCARGO_END_DT.elementAt(i) %>','<%=VCARGO_NO.elementAt(i)%>','<%=cont_status%>');">
														</div>
														<div style="width:100px;">
								      						...
								      						onclick="openVariablePriceInfo('<%=contract_type%>','<%=agmt_no%>','<%=agmt_rev_no%>','<%=cont_no%>','<%=cont_rev_no%>',
								      														'<%=cont_ref_no%>','<%=cont_status%>','<%=cont_status_flg%>','<%=VCARGO_NO.elementAt(i)%>',
								      														'<%=VCARGO_START_DT.elementAt(i) %>','<%=VCARGO_END_DT.elementAt(i) %>')"></i>
										    			</div>
													</div>
												</div>
											</td>
								...
								<%}else{ %>
								<tr>
									<td colspan="9" align="center"><%=utilmsg.infoMessage("<b>No Cargo Available!</b>")%></td>
								</tr>
								<%} %>
								</tbody>
							</table>
				...
				<div class="card-body cdbody">
					...
					<div class="row m-b-5">
						<div class="col-sm-2 col-xs-2 col-md-2">  
							<div class="form-group row">
								<div class="col-auto">
				    				<label class="form-label"><b><input type="checkbox" class="form-check-input" name="demmurageCheckbox" id="demmurageCheckbox" value="Y" onchange="demurrageShows()" <%if(demurrage.equals("Y")){ %>checked<%} %>> Demurrage Rate<span class="s-red">*</span></b></label>
				  				</div>
				...
				<div class="card-body cdbody">
					...
					<%if(!cont_no.equals("0") && !cont_no.equals("")){ %>
						...
						<%if((no_of_cargo_dtl.equals("0") || no_of_cargo_dtl.equals(""))){ %>
							<div class="row m-b-5">
								<div class="col-sm-12 col-xs-12 col-md-12">
									<%=utilmsg.errorMessage("<b>Cargo Detail is not Configured. The Contract will not appear for FCC!</b>")%>
								</div>
							</div>
						<%} %>
	...
	<div class="modal-dialog modal-dialog-scrollable modal-xl">
    	<div class="modal-content">
    		<div class="modal-header cdheader">
        		...
        		<input type="button" class="btn-close" data-bs-dismiss="modal" onclick="clearCargoDtls('<%=show_cargo_number %>','<%=cargo_number %>')">
        	</div>
						...
						<div class="col-sm-4 col-xs-4 col-md-4">  
							<div class="form-group row">
				    			...
				    			<%if(agmt_base.equals("X")){%>
				    				<input type="radio" name="cargo_agmt_base" value="X" <%if(agmt_base.equals("X")){%>checked<%}%> >&nbsp;Ex-Ship&nbsp;&nbsp;
				    			<%}else if(agmt_base.equals("D")){ %>
				    				<input type="radio" name="cargo_agmt_base" value="D" <%if(agmt_base.equals("D")){%>checked<%}%> >&nbsp;DES&nbsp;&nbsp;
					...
					<div class="row m-b-5">
						...
						<div class="col-sm-4 col-xs-4 col-md-4">
							<div class="form-group row">
				    			...
				    			<div class="col-sm-6 col-xs-6 col-md-6">
				      				<select class="form-select form-select-sm" name="cargo_rate_unit" onchange="checkRateFormate(this);checkInrUsd();"   <%if(opration.equals("MODIFY")) {%>style="pointer-events: none;"<%} %>>							   	 	 
