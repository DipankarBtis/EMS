...
<script>
function setValue(cont_full_no,entity_type,entity_name,ref_no)
{
...
function doProceed()
{
	...
	if(isSelected == "Y")
	{
		...
		if(a)
		{
			...
			window.opener.setContractDetail(cont_full_no,entity_type,entity_name,ref_no);
			window.close();
		}
	}
	else
								...
								<table class="table table-bordered" id="example">
									...
									<%if(VCOUNTERPARTY_CD.size() > 0){ %>
										...
										<tr>
											...
											onclick="setValue('<%=VCONT_FULL_NO.elementAt(i)%>','<%=entity_type%>','<%=VCOUNTERPARTY_NM.elementAt(i)%>','<%=VCONT_REF_NO.elementAt(i) %>')"></td>
