...
<%
	...
	writer.println("COB DT,LEGAL ENTITY,DEAL NUM,COUNTERPARTY,BUY/SELL,PRICE TYPE,UNIT,CONTRACT MONTH,CURVE NAME,EXPOSURE,FINANCIAL/PHYSICAL,REALISED/UNREALISED,FORWARD PRICE");
	
	if(VCOB_DT.size()>0)
	{
		for(int i=0;i<VCOB_DT.size();i++)
		{
			writer.println(""+VCOB_DT.elementAt(i)+","+VLEGAL_ENTITY.elementAt(i)+","+VDEAL_NUM.elementAt(i)+","+VCOUNTERPARTY.elementAt(i)+","+VBUY_SELL.elementAt(i)+","+VPRICE_TYPE.elementAt(i)+","+VUNIT.elementAt(i)+", "+VCONTRACT_MONTH.elementAt(i)+","+VCURVE_NAME.elementAt(i)+","+VEXPOSURE.elementAt(i)+","+VFINANCIAL_PHYSICAL.elementAt(i)+","+VREALISED_UNREALISED.elementAt(i)+","+VFORWARD_PRICE.elementAt(i));
		}
	}
	else
	{
