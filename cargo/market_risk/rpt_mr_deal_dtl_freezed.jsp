...
function exportToXls()
{
	...
	var url = "xls_mr_deal_dtl_freezed.jsp?report_dt="+report_dt+"&counterparty_cd="+counterparty_cd+
...
market_risk.setCallFlag("FETCH_EOD_FREEZED_DATA");
							...
							<table class="table table-bordered table-hover" id="filterbysearch">
								<thead id="tbsearch">
									<tr style="background: skyblue;color:black;">										
										...
										<th rowspan="2"  style="background: #000066; color: white;" title="(Settle Price * Slope) + Constant"> Effective<br>Contract Price<br>($/MMBTU)</th>
										<th colspan="2"> Original Exposure (MMBTU)</th>																		
										<th colspan="2"> Unrealized Exposure (MMBTU)</th>
										<th colspan="2"> Realized Exposure (MMBTU)</th>
										<th rowspan="2" style="background: #f9ecf2;" title="Unrealized (Phy.) Exposure * Forward Price (Phy.)"> Unrealized Physical Leg($)</th>
										<th rowspan="2" style="background: #ffe6ff;" title="Unrealized (Fin.) Exposure * (Forward Price (Fin.) + Constant/Slope)"> Unrealized Financial Leg($)</th>
										<th rowspan="2" style="background: #000066; color: white;" title="Allocated MMBTU * Effective Contract Price"> Realized Financial Leg($)</th>
									...
									<tr style="background: skyblue;color:black;">	
										...
										<th style="background: #e6e6ff;" title="Avg of SPOT prices for Settle Period (NB. Slope decides conversion to $/MMBTU)">Settled Price <br> ($/UNIT)</th>
										<th>Physical Leg</th>		
										<th>Financial Leg</th>
								...
								<%if(VGAS_DT.size()>0){ %>
									...
									<%} else {%>style="background:<%=VROW_COLOR.elementAt(i)%>" <%} %>> 					
										<td align="center" style="background: #99ffcc;"><%=VSR.elementAt(i)%></td>
										...
										<td align="right" style="background: #b3f0ff;"><%=VEFF_RATE.elementAt(i)%></td>	
										<td align="right"><%=VPHY_EXPO_ORIGINAL.elementAt(i)%></td>
										<td align="right"><%=VFIN_EXPO_ORIGINAL.elementAt(i)%></td>										
										<td align="right" style="color: #ff0080;"><%=VPHY_EXPO_UNREALIZED.elementAt(i)%></td>
										<td align="right" style="color: #ff0080;"><%=VFIN_EXPO_UNREALIZED.elementAt(i)%></td>
										<td align="right" style="color: blue;"><%=VPHY_EXPO_REALIZED.elementAt(i)%></td>
										<td align="right" style="color: blue;"><%=VFIN_EXPO_REALIZED.elementAt(i)%></td>
