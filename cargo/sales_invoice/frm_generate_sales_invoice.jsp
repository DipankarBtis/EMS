...
function refresh()
{
	...
	var url = "frm_generate_sales_invoice.jsp?counterparty_cd="+counterparty_cd+"&contract_type="+contract_type+"&cont_no="+cont_no+
		...
		"&period_end_dt="+period_end_dt+"&bu_unit="+bu_unit+"&billing_cycle="+billing_cycle+"&inv_dt="+invoice_dt+
		"&user_defined_dt="+user_defined_dt+"&operation="+operation+"&sel_exchng_cd="+exchng_cd+
		"&financial_year="+financial_year+"&bu_state_tin="+bu_state_tin+"&temp_period_start_dt="+temp_period_start_dt+"&temp_period_end_dt="+temp_period_end_dt+
		"&exchng_rate_mapp="+exchng_rate_mapp+"&u="+u;
...
function viewBeforeSubmit()
{
	...
	else
	{
		var url = "../sales_invoice/rpt_view_before_submit.jsp?counterparty_cd="+counterparty_cd+
			...
			"&bu_state_tin="+bu_state_tin+"&invoice_dt="+invoice_dt+"&agmt_base="+agmt_base+"&tcs_tds="+tcs_tds;
...
%>
<body onload="<%if(!msg.equals("")){ %>Do_close('<%=msg%>','<%=msg_type%>');<%} %> <%if(price_cd.equals("2")){ %>checkExchngRate();<%}%>"
