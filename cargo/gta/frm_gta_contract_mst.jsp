...
function openContList()
{
	...
	else
	{
		...
		{
			newWindow = window.open("frm_gta_contract_list.jsp?counterparty_cd="+counterparty_cd+"&contract_type="+contract_type,"GTA Contract List","top=10,left=10,width=1100,height=600,scrollbars=1");
		}
		else
		{
			newWindow.close();
...
function setContractDetail(agmt_no,agmt_rev_no,cont_no,cont_rev_no)
{
	...
	var url = "frm_gta_contract_mst.jsp?counterparty_cd="+counterparty_cd+"&opration="+opration+
			...
			"&cont_no="+cont_no+"&cont_rev_no="+cont_rev_no+"&u="+u;
...
function openAgreementList()
{
	...
	else
	{
		if(!newWindow || newWindow.closed)
		{
			newWindow = window.open("frm_gta_agmt_list.jsp?counterparty_cd="+counterparty_cd,"Agreement List","top=10,left=10,width=1100,height=600,scrollbars=1");
		}
		else
		{
			newWindow.close();
...
function setAgreementDetail(agmt_no,agmt_rev_no,agmt_nm)
{
	var opration = document.forms[0].opration.value;
	var contract_type = document.forms[0].contract_type.value;
	var counterparty_cd = document.forms[0].counterparty_cd.value;
...
function openCTList()
{
	...
	var url="frm_ct_ref_list.jsp?counterparty_cd="+counterparty_cd+"&contract_type="+contract_type+
			...
			"&customer_cd="+customer_cd+"&sales_cont_map="+sales_cont_map;
			...
			<div class="card cardmain">
				...
				<div class="card-body cdbody">
					...
					<div class="row m-b-5">
						...
						<div class="col-sm-2 col-xs-2 col-md-2">  
							<div class="form-group row">
				    			<label class="form-label"><b>GTC Type<span class="s-red">*</span></b></label>
				  			</div>
						</div>
						<div class="col-sm-4 col-xs-4 col-md-4"> 
							<div class="form-group row">
								<div class="col">
				      			...
				      			<div class="col-auto">
				    				<input type="button" class="btn btn-info btn-sm select_btn" value="Select GTA" style="font-weight: bold;" onclick="openAgreementList();">
				    			</div>
				    			<div class="col">
				    			...
				    			<div class="col-sm-12 col-xs-12 col-md-12">
				    				<%if(VBU_CD.size() > 0) {%>
					    				<%for(int i=0; i<VBU_CD.size(); i++){ %>
					    					...
					    					<input type="radio" name="chk_bu_plant" value="<%=VBU_PLANT_SEQ_NO.elementAt(i)%>">&nbsp;<%=VBU_PLANT_ABBR.elementAt(i)%>&nbsp;&nbsp;
					    				<%}%>
				    				<%}else{ %>
