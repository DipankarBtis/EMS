							...
							<table class="table table-bordered table-hover">
								<thead>
										<tr>
											...
											<%if(VDISPLAY_SEGMENT_TYP.elementAt(m).equals("L")){ %>
											<th title="Allocated Quantity">Regassified qty </th>
											<th title="Booked Qty = Booked MMBTU - Regassified Qty"> Balance qty </th>
											<%}else{ %>
								...
								if(index > 0){ %>
									...
									%>
										<tr>
											...
											<%if(VDISPLAY_SEGMENT_TYP.elementAt(m).equals("L")){%> 
											<td align="right" title="Balance Qty = <%=VBAL_INFO.elementAt(k-1)%>"><%=VBALANCE_QTY.elementAt(k-1)%></td>
											<%}%> 
										</tr>
