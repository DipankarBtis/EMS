...
function openContList()
{
	...
	{
		newWindow = window.open("frm_trader_contract_list_fcc.jsp?counterparty_cd="+counterparty_cd+"&contract_type="+contract_type,"Trader Contract List","top=10,left=10,width=1100,height=600,scrollbars=1");
	}
	else
	{
		newWindow.close();
...
function setValues(strTransCd,strTrans,strPlant,strBuPlant,strGxBuPlant,strSplitVal,splitFlag,strOthTrdCd,strOthPlant,strOthSplitVal)
{
	var chk_plant = document.forms[0].chk_plant;
...
%>
<body style="pointer-events: none;" onload="setValues('<%=strTransCd%>','<%=strTrans%>','<%=strPlant%>','<%=strBuPlant%>',
'<%=strGxBuPlant%>','<%=strSplitVal%>','<%=split_flag%>','<%=strOthTrdCd%>','<%=strOthPlant%>','<%=strOthSplitVal%>');messurmentShows();terminatorShows();off_spec_gasShows();billingShows();liabilityShows();">
				...
				<div class="card-body cdbody">
					...
					<div class="row m-b-5">
						...
						<div class="col-sm-4 col-xs-4 col-md-4">  
							<div class="form-group row">
				    			<div class="col-sm-12 col-xs-12 col-md-12">
				    				<select class="form-select form-select-sm" name="counterparty_cd" onchange="refresh('<%=opration%>','<%=clearance%>');" style="pointer-events: auto;">
					...
					<div class="row m-b-5">
						...
						<div class="col-sm-4 col-xs-4 col-md-4">  
							<div class="form-group row">
				    			<div class="col-sm-12 col-xs-12 col-md-12">
				    				<select class="form-select form-select-sm" name="contract_type" onchange="refresh('<%=opration%>','<%=clearance%>');" style="pointer-events: auto;">
				    					<%if(clearance.equals("IGX")){ %>
	...
	<div class="modal-dialog modal-dialog-scrollable modal-xl">
    	<div class="modal-content">
    		<div class="modal-header cdheader">
			    				...
			    				<%if(VPLANT_SEQ_NO.size() > 0) {%>
				    				<%for(int i=0; i<VPLANT_SEQ_NO.size(); i++){ %>
				    					<div class="col-auto">
				    						<div class="form-group row">
				    							<div class="col-auto">
					    							<input type="checkbox" class="form-check-input" name="chk_plant" value="<%=VPLANT_SEQ_NO.elementAt(i)%>" onclick="enableSplit(this,'split_value_<%=VPLANT_SEQ_NO.elementAt(i)%>');">&nbsp;<%=VPLANT_ABBR.elementAt(i)%>
					    						</div>
					    						<div class="col-auto" id="hid_col_split_<%=VPLANT_SEQ_NO.elementAt(i)%>" style="display:none;">
						    						<div class="input-group input-group-sm" >
      		...
      		<div id="othTrdPlantListDis" style="display:none;">
	      		<div class="modal-header cdheader">
	        		<div class="topheader">
						Split Trader Configuration
					</div>
	        	</div>
	        	<div class="modal-body mdbody">
	      			<div class="cdbody">
								...
								<div class="col-sm-12 col-xs-12 col-md-12">  
									<div class="form-group row">
					    				...
					    				if(index > 0) {%>
						    					...
						    					<div class="col-auto">
						    						<div class="form-group row">
						    							<div class="col-auto">
								    						...
								    						onclick="enableSplit(this,'oth_split_value_<%=oth_countpty_cd%>_<%=VOTH_PLANT_SEQ_NO.elementAt(j)%>');
								    						enabled_TransPlantDtl(this,'oth_trd_cd_<%=oth_countpty_cd%>_<%=VOTH_PLANT_SEQ_NO.elementAt(j)%>','oth_plant_seq_no_<%=oth_countpty_cd%>_<%=VOTH_PLANT_SEQ_NO.elementAt(j)%>')">&nbsp;<%=VOTH_PLANT_ABBR.elementAt(j)%>
								    					...
								    					<div class="col-auto" id="hid_col_oth_split_<%=oth_countpty_cd%>_<%=VOTH_PLANT_SEQ_NO.elementAt(j)%>" style="display:none;">
								    						<div class="input-group input-group-sm" >
						    				...
						    				<%}%>
					    				<%}else{ %>
					    					<%= utilmsg.warningMessage("Please configure Plants for Trader!")%>
					    				<%} %>
									</div>
								</div>
							</div>
