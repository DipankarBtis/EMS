...
<script>
function setValue(countpty_cd,agmt_no,agmt_rev_no,cont_no,cont_rev_no,contract_type)
{
	window.opener.setContDetail(countpty_cd,agmt_no,agmt_rev_no,cont_no,cont_rev_no,contract_type);
	window.close();	
}
</script>
</head>
							...
							<div class="table-responsive">
								<table class="table table-bordered" id="example">
									<thead>
										<tr valign="top">
											<th>Select</th>
											<th>Counterparty<br><input class="form-control form-control-sm" type="text" id="table_cp" onkeyup="Search(this,'1');" placeholder="Search.." style="width:100px"/></th>
											...
											<th>Contract Type<br><input class="form-control form-control-sm" type="text" id="table_Contact" onkeyup="Search(this,'3');" placeholder="Search.." style="width:100px"/></th>
											<th>Contract Rev#</th>
											<th><%if(contract_type.equals("I")){ %>Trade Ref#<%}else{%>Contract Ref#<%} %><br><input class="form-control form-control-sm" type="text" id="table_Contract_Ref" onkeyup="Search(this,'5');" placeholder="Search.." style="width:100px"/></th>
											<th>Contract Name</th>
									...
									<%if(VBUYER_CD.size() > 0){ %>
										...
										<tr>
											<td align="center"><input type="radio" onclick="setValue('<%=VBUYER_CD.elementAt(i) %>','<%=VAGMT_NO.elementAt(i)%>',
											'<%=VAGMT_REV_NO.elementAt(i)%>','<%=VCONT_NO.elementAt(i)%>','<%=VCONT_REV_NO.elementAt(i)%>'
											,'<%=VCONTRACT_TYPE.elementAt(i)%>','<%=VCARGO_NO.elementAt(i)%>','<%=VCONTRACT_TYPE.elementAt(i)%>')"></td>
