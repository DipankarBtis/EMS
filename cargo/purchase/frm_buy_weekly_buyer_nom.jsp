...
function refresh()
{
	...
	*/	
	...
	if(flag)
	{
		var url = "frm_buy_weekly_buyer_nom.jsp?counterparty_cd="+counterparty_cd+"&u="+u+
...
function openContList()
{
	...
	{
		newWindow = window.open("frm_nomination_contract_list.jsp?gas_dt="+gas_dt+"&nomination_freq="+nomination_freq+"&nomination_type=B","Trader Contract List","top=10,left=10,width=1100,height=600,scrollbars=1");
	}
	else
	{
		newWindow.close();
...
function setContDetail(countpty_cd,agmt_no,agmt_rev_no,cont_no,cont_rev_no,st_dt,end_dt,cont_type,cargo_no)
{
	...
	var url = "frm_buy_weekly_buyer_nom.jsp?counterparty_cd="+countpty_cd+"&cont_no="+cont_no+"&cont_rev_no="+cont_rev_no+
...
function checkQty(index)
{
	...
	var gas_dt = document.getElementById("week_gas_dt"+index);
...
purchase.setCallFlag("WEEKLY_BUYER_NOM");
...
<input type="hidden" name="option" value="WEEKLY_BUYER_NOM">
