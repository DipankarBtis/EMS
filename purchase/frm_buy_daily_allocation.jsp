		...
		{ 	chk_count++
			...
			for(var p=0;p<parseInt(mole_chk);p++)
			{
				...
				if(mole_chk_obj.checked)
				{
					...
					if(trim(mole_qty_mmbtu.value) == "")
					{
						msg+="Enter Molecule MMBTU for "+mole_abbr.value+" for ROW - "+parseInt(0+1)+"!\n";
						flag=false;
					}
...
function totalQty()
{
	...
	if(index!=null && index!=undefined)
	{
		if(index.length!=undefined)
		{
			...
			{
				var subIndex = parseInt(index[j].value);
		...
		{
			var subIndex = parseInt(index.value);
			tot_mmbtu=parseFloat("0");
			tot_scm=parseFloat("0");
			m=0;
			if(qty_mmbtu.length!=undefined)
					...
					<div class="row">
						<div class="table-responsive">
							<table class="table table-bordered table-hover" id="example">
								...
								<%if(VCOUNTERPARTY_PLANT_SEQ.size()>0){ %>
									...
									<tr>
										...
										<td align="center">
											...
											<span
											<%if(VCOUNTERPATY_STATUS.elementAt(i).equals("N")){ %>class='alert alert-danger'
										...
										<td align="center">
											<div style="width:100px;">
												<input type="text" class="form-control form-control-sm" name="qty_mmbtu" id="qty_mmbtu<%=i%>" value="<%=VQTY_MMBTU.elementAt(i)%>" 
												style="text-align:right;background:<%=VNOM_COLOR.elementAt(i)%>" 
												onblur="negNumber(this);checkNumber1(this,9,2);checkQty('<%=i%>');calculateSCM('<%=i%>');totalQty();" disabled readonly>
												<input type="hidden" class="form-control form-control-sm" name="tmp_qty_mmbtu" id="tmp_qty_mmbtu<%=i%>" value="<%=VQTY_MMBTU.elementAt(i)%>" style="text-align:right" disabled>
											</div>
										</td>
										<td align="center">
										...
										<td>
											<div class="row m-b-5">
												<div class="col-auto">
													<input type="button" class="btn btn-sm config_btn" id="moleConfigBtn<%=i%>" disabled value="MM" onclick="variableMolecule('<%=i%>');">
													<%-- <div class="col-auto" id="hed_mole_qty<%=i%>"></div> --%>
												</div>
											</div>
										</td>
										<td align="center">
								...
								<%}else{ %>
									<tr>
										<td colspan="16" align="center"><%=utilmsg.infoMessage("<b>Seller Nomination is not Done for the Selected Gas Day!</b>")%></td>
									</tr>
								<%} %>
								</tbody>
							</table>
...
<%for(int x=0; x<VCOUNTERPARTY_PLANT_SEQ.size(); x++){
									...
									<tr>
										...
										<td align="center">
											<div style="width:100px;">
												...
												onchange="negNumber(this);checkNumber1(this,9,2);calculateSCM('<%=x%>');calcRemainMoleQty('<%=x%>');" 
