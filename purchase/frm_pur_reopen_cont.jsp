...
function refresh()
{
	...
	var url = "frm_pur_reopen_cont.jsp?u="+u+
...
function approve_reject_reopen(counterpty_cd,agmt,agmt_rev,cont,cont_rev,cont_type,deal_map,action,buySale,agmtType)
{
	var cnf_msg="";
	if(action == 'APPROVE')
	{
		cnf_msg="New Contract Revision will be Submitted Along With Re-Open approval!\n\nDo You Want To Revise The Contract Details, With Contract Re-Open approval?";
	}
	else if(action == 'REJECT')
	{
		cnf_msg="Do you want to Reject Re-Open request?";
	}
	
	if(agmtType=='L')
	{
		var a = confirm(cnf_msg);
		if(a)
		{
			document.forms[0].trader_cd.value=counterpty_cd;
			document.forms[0].agmt_no.value=agmt;
			document.forms[0].agmt_rev_no.value=agmt_rev;
			document.forms[0].cont_no.value=cont;
			document.forms[0].cont_rev_no.value=cont_rev;
			document.forms[0].contract_type.value=cont_type;
			document.forms[0].agmt_type.value=agmtType;
			document.forms[0].buy_sale.value=buySale;
			
			document.forms[0].action = "../servlet/Frm_LtcoraMaster";
			document.forms[0].method="post";
			document.forms[0].option.value = "REOPEN";
			document.forms[0].opration.value=action;
			document.getElementById("loading").style.visibility = "visible";
...
purchase.setCallFlag("REOPEN_REQUEST");
					...
					<div class="row">
						<div class="table-responsive">
							<table class="table table-bordered table-hover">
										...
										<tr>
											...
											<td align="center">
												<div style="width:100px;">
													<div class="input-group input-group-sm" >
							      						<input type="text" class="form-control form-control-sm date fmsdtpick" name="closure_eff_dt_<%=j%>_<%=i%>" id="closure_eff_dt_<%=j%>_<%=i%>" value="<%=VCLOSEURE_EFF_DT.elementAt(i)%>" maxLength="10" onblur="validateDate(this);" onchange="validateDate(this);" autocomplete="off" disabled>
											...
											<td>
												<div style="width:180px;">
													<div class="input-group input-group-sm" >
														<input type="text" class="form-control form-control-sm" id="closure_remark_<%=j%>_<%=i%>" value="<%=VCLOSURE_NOTE.elementAt(i) %>"  maxLength="60" readOnly>
													</div>
												</div>
											</td>
											...
											<td align="center">
												...
												onclick="approve_reject_reopen('<%=VCOUNTERPARTY_CD.elementAt(i)%>','<%=VAGMT_NO.elementAt(i)%>','<%=VAGMT_REV_NO.elementAt(i)%>',
												'<%=VCONT_NO.elementAt(i)%>','<%=VCONT_REV_NO.elementAt(i)%>','<%=VCONTRACT_TYPE.elementAt(i)%>',
												'<%=VDEAL_MAP.elementAt(i)%>','APPROVE','<%=VBUY_SALE.elementAt(i)%>','<%=VAGMT_TYPE.elementAt(i)%>');" />
											</td>
											<td>
												<input type="button" class="btn btn-warning com-btn" value="Reject Re-Open" style="border-radius: 50px;"
												onclick="approve_reject_reopen('<%=VCOUNTERPARTY_CD.elementAt(i)%>','<%=VAGMT_NO.elementAt(i)%>','<%=VAGMT_REV_NO.elementAt(i)%>',
												'<%=VCONT_NO.elementAt(i)%>','<%=VCONT_REV_NO.elementAt(i)%>','<%=VCONTRACT_TYPE.elementAt(i)%>',
												'<%=VDEAL_MAP.elementAt(i)%>','REJECT','<%=VBUY_SALE.elementAt(i)%>','<%=VAGMT_TYPE.elementAt(i)%>');">
											</td>
										</tr>
								...
								<%}else{ %>
									<tr>
										<td colspan="17" align="center"><%=utilmsg.infoMessage("<b>No Reopen Requested!</b>") %></td>
									</tr>
								<%} %>
								</tbody>
							</table>
...
<input type="hidden" name="option" value="REOPEN_CLOSURE">
