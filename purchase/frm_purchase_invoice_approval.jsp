...
function doGenXML(couterpty_cd,invoice_no,financial_year,invoice_seq,contract_type,purchase_type_flag,invoice_type,sap_approval_flag, bu_seq, cont_no, inv_flag,sgpg_type,invoice_dt,inv_raised_in)
{
	var u = document.forms[0].u.value;
	...
	var url = "";
	if(inv_flag=="CR" || inv_flag=="DR")
	{
		url = "rpt_view_crdr_purchase_sap_approval.jsp?financial_year="+financial_year+"&invoice_seq="+invoice_seq+
			"&contract_type="+contract_type+"&purchase_type_flag="+purchase_type_flag+"&invoice_type="+invoice_type+"&invoice_dt="+invoice_dt+
			"&counterparty_cd="+couterpty_cd+"&crdr_no="+invoice_no+"&sap_approval_flag="+sap_approval_flag+"&sgpg_type="+sgpg_type+
			"&bu_unit="+bu_seq+"&cont_no="+cont_no+"&inv_flag="+inv_flag+"&u="+u;
	}
	else
	{
		url = "rpt_view_purchase_sap_approval.jsp?financial_year="+financial_year+"&invoice_seq="+invoice_seq+
...
Vector VFIN_SYS = purchase.getVFIN_SYS();
							...
							<table class="table table-bordered table-hover serchtbl" id="example<%=j%>">
								...
								if(index > 0){ %>
									<%for(i=i; i<VINVOICE_SEQ.size(); i++){ 
										...
										if(VSEGMENT_TYPE.elementAt(j).equals("N") 
												&& VINVOICE_RAISED_IN.elementAt(i).equals("USD") 
												&& VSAP_APPROVAL_FLAG.elementAt(i).equals("Y") 
												&& VCONTRACT_TYPE.elementAt(i).toString().equals("N")) 
										{
											isEditable=true; 
										}
										else if((VINVOICE_RAISED_IN.elementAt(i).equals("USD") && !VSAP_APPROVAL_FLAG.elementAt(i).equals("Y")) 
												&& !VCONTRACT_TYPE.elementAt(i).toString().equals("N"))
										{
											isEditable=true; 
										}
									...
									<tr>
											...
											<div style="width:100px;">
												<div class="row">
													<div class="col">
														...
														onblur="negNumber(this);checkNumber1(this,7,4);" 
														<%if(!isEditable){ %>readOnly<%} %>>
													</div>
													<div class="col-auto">
														<%if(VSEGMENT_TYPE.elementAt(j).equals("N") 
										...
										<%}else{%>	
											<%if(approve_access.equals("Y")){ %>																						
												<i class="fa <%if (VSAP_APPROVAL_FLAG.elementAt(i).equals("Y")) {%>fa-eye<%} else {%>fa-file-code-o<%}%> fa-2x "
													 ...
													 '<%=VINV_FLAG.elementAt(i)%>','<%=VPAYMENT_TYPE_NM.elementAt(i)%>','<%=VINVOICE_DT.elementAt(i) %>','<%=VINVOICE_RAISED_IN.elementAt(i) %>');"
