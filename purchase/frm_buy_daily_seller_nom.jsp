							...
							<table class="table table-bordered table-hover" id="example">
								...
								<%if(VCOUNTERPARTY_PLANT_SEQ.size()>0){ %>
									...
									<tr>
										...
										<td align="center"><%=VCOUNTERPARTY_ABBR.elementAt(i) %>
											<span 
											<%if(VCOUNTERPATY_STATUS.elementAt(i).equals("N")){ %>class='alert alert-danger' title="Counterparty Deactive "
											<%}else if(VCOUNTERPATY_STATUS.elementAt(i).equals("E")){ %>class='alert alert-warning'
											<%} %>
											><b> <%if(VCOUNTERPATY_STATUS.elementAt(i).equals("N")){ %> De-active 
											<%}else if(VCOUNTERPATY_STATUS.elementAt(i).equals("E")){ %> E-Rate
											<%} %> </b>
											</span>
											...
											<input type="hidden" name="agmt_no" id="agmt_no<%=i%>" value="<%=VAGMT_NO.elementAt(i)%>" disabled>
						      				...
						      				<input type="hidden" name="dis_cont_mapping" id="dis_cont_mapping<%=i%>" value="<%=VDIS_CONT_MAPPING.elementAt(i)%>" disabled>
											<input type="hidden" name="cp_status" id="cp_status<%=i%>" value="<%=VCOUNTERPATY_STATUS.elementAt(i)%>">
										</td>
										<td align="center">
								...
								<%}else{ %>
									<tr>
										<td colspan="15" align="center"><%=utilmsg.infoMessage("<b>Buyer Nomination is not Done for the Selected Gas Day!</b>")%></td>
									</tr>
								<%} %>
								</tbody>
							</table>
