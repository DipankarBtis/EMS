...
function splitEnabled(obj,enable_split)
{
	...
	else
	{
		...
		else
		{
			...
			{
				obj.checked=true;
				alert("Split Trader can't be un-selected nomination is done!");
			}
		}
	}
}
...
function doSubmit()
{
	...
	var checkbox = document.getElementById("liability_checkbox");
	...
	if(flag)
	{
		...
		if(a)
		{
			...
			if(trim(no_of_liability_dtl) != "" && checkbox.checked)
			{
...
function openContList()
{
	...
	if(flag)
	{
		...
		{
			newWindow = window.open("frm_trader_contract_list.jsp?counterparty_cd="+counterparty_cd+"&contract_type="+contract_type+"&fcc_flag=N","Trader Contract List","top=10,left=10,width=1100,height=600,scrollbars=1");
		}
		else
		{
			newWindow.close();
...
function openBillingDtl(cont_status_flg)
{
	var counterparty_cd = document.forms[0].counterparty_cd.value;
	var contract_type = document.forms[0].contract_type.value;
	...
	var url=""
	if(cont_status_flg=="X")
	{
		url = "frm_trader_billing_dtl_fcc.jsp?counterparty_cd="+counterparty_cd+"&contract_type="+contract_type+"&cont_no="+cont_no+
	...
	{
		url = "frm_trader_billing_dtl.jsp?counterparty_cd="+counterparty_cd+"&contract_type="+contract_type+"&cont_no="+cont_no+
			"&cont_rev_no="+cont_rev_no+"&agmt_no="+agmt_no+"&agmt_rev_no="+agmt_rev_no+"&clearance="+clearance+"&start_dt="+start_dt+
			"&u="+u+"&rate_unit="+rate_unit;
	}
...
function setValues(strTransCd,strTrans,strPlant,strPlantChrg,strBuPlant,strGxBuPlant,strSplitVal,splitFlag,
		strOthTrdCd,strOthPlant,strOthPlantChrg,strOthSplitVal,strChargehistdesc,strOthChargehistdesc)
{
	var chk_plant = document.forms[0].chk_plant;
	...
	strOthChargehistdesc = strOthChargehistdesc.replace(/~/g, "\n");
	...
	var sepOthChargehistdesc = strOthChargehistdesc.split("@");
	...
	if(chk_oth_plant!=null && chk_oth_plant.length!=undefined)
 	{
  		for(var i=0;i<chk_oth_plant.length;i++)
  		{
   			for(var j=0;j<sepOthTrdCd.length;j++)
   			{
     			...
     			{
     				document.getElementById("oth_history_btn_"+sepOthPlant[j]).setAttribute("title", sepOthChargehistdesc[j]);
	     				...
	     				for(var k=0;k<charge_abbr.length;k++)
	     		  		{
	     					var tempOthPlantChrgVal = sepOthPlantChrgVal[k].split("#");
	     		  			...
	     		  			document.getElementById("max_oth_eff_dt_"+temp+"_"+i+"_"+k).value=tempOthPlantChrgVal[1];
	     		  		}
     			 	}
     				else if(charge_abbr!=null)
     			 	{
     					...
     					var temp = document.getElementById("charge_abbr_0").value;
     		  			...
     		  			document.getElementById("max_oth_eff_dt_"+temp+"_"+i+"_0").value=tempOthPlantChrgVal[1];
     			 	}
     			}
   			} 
  		} 
     				...
     				for(var k=0;k<charge_abbr.length;k++)
     		  		{
     					var tempOthPlantChrgVal = sepOthPlantChrgVal[k].split("#");
     		  			...
     		  			document.getElementById("max_oth_eff_dt_"+temp+"_0_"+k).value=tempOthPlantChrgVal[1];
     		  		}
 			 	}
 				else if(charge_abbr!=null)
 			 	{
 					...
 					var temp = document.getElementById("charge_abbr_0").value;
 		  			...
 		  			document.getElementById("max_oth_eff_dt_"+temp+"_0_0").value=tempOthPlantChrgVal[1];
 			 	}
     		}
   		} 
 	}
...
function enableSplit(obj,id,index,flag,nom_stat)
{
	var charge_abbr = document.forms[0].charge_abbr;
	if(obj.checked)
	{
	...
	else
	{
		...
		{
			obj.checked=true;
			alert("Can't un-checked, nomination is done for the plant!");
		}
	}
}
...
function openLiabilityDtl(cont_status_flg)
{
	var opration = document.forms[0].opration.value;
	var counterparty_cd = document.forms[0].counterparty_cd.value;
	var cont_no = document.forms[0].cont_no.value;
	...
	if(cont_status_flg=='X'||cont_status_flg=='C' ||cont_status_flg=='T')
	{
		url = "frm_trader_cont_liability_clause_fcc.jsp?counterparty_cd="+counterparty_cd+
	...
	{
		url = "frm_trader_cont_liability_clause.jsp?counterparty_cd="+counterparty_cd+
			"&agmt_no="+agmt_no+"&agmt_rev_no="+agmt_rev_no+"&start_dt="+start_dt+"&contract_type="+contract_type+"&tcq="+tcq+"&dcq="+dcq+
			"&end_dt="+end_dt+"&cont_no="+cont_no+"&cont_rev_no="+cont_rev_no+"&rate="+rate+"&rate_unit="+rate_unit+"&u="+u;
	}
...
function variableDCQ(cont_type,agmt,agmt_rev,cont,cont_rev,cont_status_flg)
{
	...
	if(trim(cont_type)!="" && (trim(cont)!="" && cont!="0") && (trim(counterparty_cd)!="" && counterparty_cd!="0"))
	{
		...
		if(cont_status_flg=='X'||cont_status_flg=='C' ||cont_status_flg=='T')
		{
			url = "../purchase/frm_pur_contract_dcq_dtl_fcc.jsp?counterparty_cd="+counterparty_cd+"&cont_no="+cont+"&cont_rev_no="+cont_rev+
			"&agmt_no="+agmt+"&agmt_rev_no="+agmt_rev+"&contract_type="+cont_type+"&start_dt="+temp_start_dt+"&end_dt="+temp_end_dt+
			"&u="+u;
		}
		else
		{
			url = "../purchase/frm_pur_contract_dcq_dtl.jsp?counterparty_cd="+counterparty_cd+"&cont_no="+cont+"&cont_rev_no="+cont_rev+
				"&agmt_no="+agmt+"&agmt_rev_no="+agmt_rev+"&contract_type="+cont_type+"&start_dt="+temp_start_dt+"&end_dt="+temp_end_dt+
				"&u="+u;
		}
...
function doClosure(typ)
{
	var cfm_msg= typ=='T'?"Terminate":"Closure"
	var clsr_req_flg=typ=='T'?'R':'Y'
	var a = confirm("New Contract Revision will be Submitted Along With Contract "+cfm_msg+" Request!\n\nDo You Want To Revise The Contract Details, With Contract "+cfm_msg+" Request?");
	if(a)
	{
		document.forms[0].closure_req_flag.value=clsr_req_flg;
		document.forms[0].change_request.value="CLOSURE_OR_TERMINATE";
		document.getElementById("loading").style.visibility = "visible";
		document.forms[0].submit();
	}
}
...
function doReopen()
{
	var a = confirm("Do you Re-Open Contract? ");
	if(a)
	{
		document.forms[0].closure_req_flag.value="O";
		document.forms[0].change_request.value="CONTRACT_REOPEN";
		document.getElementById("loading").style.visibility = "visible";
		document.forms[0].submit();
	}
}
...
Vector VSEL_CHARGE_DESC = purchase.getVSEL_CHARGE_DESC();
...
Vector VOTH_SEL_CHARGE_DESC = purchase.getVOTH_SEL_CHARGE_DESC();
...
Vector VNOM_SEL_BU_CHK = purchase.getVNOM_SEL_BU_CHK();
Vector VNOM_SEL_TRADER_CHK = purchase.getVNOM_SEL_TRADER_CHK();
Vector VNOM_SEL_OTH_TRADER_CHK = purchase.getVNOM_SEL_OTH_TRADER_CHK();

String enable_split=VNOM_SEL_OTH_TRADER_CHK.contains("Y")?"Y":"N";
...
String max_date = purchase.getMax_date();
String min_nom_date = purchase.getMin_nom_date();
...
for(int i=0;i<VOTH_SEL_PLANT_SEQ_NO.size();i++)
{
	...
	strOthChargehistdesc = strOthChargehistdesc + VOTH_SEL_CHARGE_DESC.elementAt(i)+"@";
}
...
String closure_eff_dt = purchase.getClosure_eff_dt();
String closure_note = purchase.getClosure_note();
String is_expired = purchase.getIs_expired();
String closure_mmbtu = purchase.getClosure_mmbtu();
String closure_req_flag = purchase.getClosure_req_flag();
String is_nominated = purchase.getIs_nominated();
String delta_tcq_sign = purchase.getDelta_tcq_sign();
String contdt_change_request_flag = purchase.getContdt_change_request_flag();

//<!--Harsh Maheta 20230903 : Added for old values to show in Deal audit history-->//
String cp_name = purchase.getContpty_name();
String cp_abbr = purchase.getContpty_abbr();

String mapped_cont_no = purchase.getMapped_cont_no();

String old_value="CP="+counterparty_cd+"#NAME="+cp_name+"#ABBR="+cp_abbr+"#CONTNAME="+contract_type+cont_name+"#CONTNO="+mapped_cont_no+"#CONTREFNO="+cont_ref_no+"#CONTTYPE="+contract_type+"#TRADE_REFNO="+trade_ref_no+"#DDADT="+dda_dt+"#DDATIME="+dda_time+"#SIGNDT="+signing_dt+"#SIGNTIME="+signing_time+
...
'<%=strGxBuPlant%>','<%=strSplitVal%>','<%=split_flag%>','<%=strOthTrdCd%>','<%=strOthPlant%>','<%=strOthPlantChrg%>',
'<%=strOthSplitVal%>','<%=strChargehistdesc%>','<%=strOthChargehistdesc%>');messurmentShows();terminatorShows();off_spec_gasShows();billingShows();liabilityShows();">
			...
			<div class="card cardmain">
				...
				<div class="card-body cdbody">
							...
							<div class="form-group row">
				    			<div class="col-sm-12 col-xs-12 col-md-12">
				    				<select class="form-select form-select-sm" name="contract_type" onchange="refresh('<%=opration%>','<%=clearance%>');" <%if(opration.equals("MODIFY") && (!cont_no.equals("0") && !cont_no.equals(""))){ %>style="pointer-events: none;"<%} %>>
				    					...
				    					<%}else{ %>
				    					<option value=""><%if(opration.equals("MODIFY")){%>All<%}else{ %>--Select--<%} %></option>
						...
						<div class="col-sm-4 col-xs-4 col-md-4"> 
							<div class="form-group row">
				    			<div class="col">
				      				...
				      				<%if(opration.equals("MODIFY") && cont_status.equals("Y") && fcc_flg.equals("Y")) {%>readOnly<%} %>>
		      					</div>
				    			<div class="col">
					...
					<div class="row m-b-5">
						...
						<div class="col-sm-2 col-xs-2 col-md-2">  
							<div class="form-group row">
				    			<label class="form-label"><b>Closure MMBTU</b></label>
				  			</div>
						</div>
						<div class="col-sm-4 col-xs-4 col-md-4"> 
							<div class="form-group row">
								<div class="col-auto">
		      					...
		      					<div class="col-auto">
					      			<input type="text" class="form-control form-control-sm" name="closure_mmbtu" value="<%=closure_mmbtu%>" readOnly>
					    		</div>
					    		<div class="col-auto">
						...
						<div class="col-sm-4 col-xs-4 col-md-4">  
							<div class="form-group row">
				    			<div id="liability_div2" class="col-sm-12 col-xs-12 col-md-12">
				    				<input type="button" class="btn btn-sm config_btn" value="Configure Liability" <%if(opration.equals("MODIFY")){ %>onclick="openLiabilityDtl('<%=cont_status_flg%>');"<%} %>>&nbsp;
				    				<%if(opration.equals("MODIFY")){ %>
						...
						<div class="col-sm-4 col-xs-4 col-md-4">  
							<div class="form-group row">
				    			<div id="billing_flag_div1" class="col-sm-12 col-xs-12 col-md-12">
						      		<input type="button" class="btn btn-sm config_btn" value="Configure Billing" <%if(opration.equals("MODIFY")){ %>onclick="openBillingDtl('<%=cont_status_flg%>');"<%} %>>
				    			</div>
				  			</div>
						</div>
					</div>
				...
				<div class="card-footer cdfooter text-center">
					<div class="d-flex justify-content-between">
						...
						<%if(write_access.equals("Y") && !cont_status_flg.equals("X") && !closure_req_flag.equals("Y") && !closure_req_flag.equals("R") && !cont_status_flg.equals("C") && !cont_status_flg.equals("C")&& !cont_status_flg.equals("T")){ %>
				...
				<div class="card-body cdbody">
					<div class="row m-b-5">
						<div class="col-sm-12 col-xs-12 col-md-12">  
							<div class="form-group row">
				    			...
				    			<div class="col-auto" title="<%if(cont_status_flg.equals("C")||cont_status_flg.equals("T")){%><%}else if(!is_nominated.equals("Y")){%>Buyer nomination is not done!<%}else if(!is_expired.equals("Y")){%>Contract is active,Terminate the contract!<%}%>">
				    				<input type="button" class="btn btn-sm request_btn" value="Closure Request" <%if(opration.equals("MODIFY")&&!counterparty_cd.equals("0")&&!cont_no.equals("")&&!cont_status_flg.equals("X") && is_expired.equals("Y") && !closure_req_flag.equals("Y") && !closure_req_flag.equals("R") && !cont_status_flg.equals("C") && !cont_status_flg.equals("T") && is_nominated.equals("Y") && !cont_status_flg.equals("R")){%>onclick="doClosure('C');"<%}else{%>style="background:#e6e6e6;color:black;" disabled<%} %>>
				    			</div>
				    			<!-- Cancel Contract Button -->
				    			<div class="col-auto" title="<%if(cont_status_flg.equals("C")||cont_status_flg.equals("T")){%><%}else if(is_nominated.equals("Y")){%>Buyer nomination is done for this contract,close/terminate it!<%}%>">
				    				<%-- <input type="button" class="btn btn-sm request_btn" value="Cancel Contract" <%if(opration.equals("MODIFY")&&!counterparty_cd.equals("0")&&!cont_no.equals("")){%>onclick="cancelContract();"<%}else{%>style="background:#e6e6e6;color:black;"<%}%>> --%>
				    				<input type="button" class="btn btn-sm request_btn" value="Cancel Contract" <%if(opration.equals("MODIFY")&&!counterparty_cd.equals("0")&&!cont_no.equals("")&&!cont_status_flg.equals("X") && !closure_req_flag.equals("Y") && !closure_req_flag.equals("R")&& !cont_status_flg.equals("C") && !cont_status_flg.equals("T") && !is_nominated.equals("Y")){%>data-bs-toggle="modal" data-bs-target="#CancelModal"<%}else{%>style="background:#e6e6e6;color:black;" disabled<%}%>>
				    			</div>
				    			<!-- /Cancel Contract Button -->
				    			<!-- Terminate Button -->
				    			<div class="col-auto" title="<%if(cont_status_flg.equals("C")||cont_status_flg.equals("T")){%><%}else if(!is_nominated.equals("Y")){%>Buyer Nomination is not done!<%}else if(is_expired.equals("Y")){%>Contract is expired,Close the contract!<%}else if(!termination_forced.equals("Y")){%>For Termination, configure Termination/Suspention (Force Majeure) clause!<%}%>">
				    				<input type="button" class="btn btn-sm request_btn" value="Terminate Request" 
				    				<%if(opration.equals("MODIFY")&&!counterparty_cd.equals("0")&&!cont_no.equals("")&&!cont_status_flg.equals("X") && !is_expired.equals("Y")&&termination_forced.equals("Y") && is_nominated.equals("Y")&& !closure_req_flag.equals("Y") && !closure_req_flag.equals("R") && !cont_status_flg.equals("C") && !cont_status_flg.equals("T") && !cont_status_flg.equals("R")){%>onclick="doClosure('T');"<%}else{%>style="background:#e6e6e6;color:black;" disabled<%}%>>
				    			</div>
				    			<!-- /Terminate Button -->
				    			<!-- RE-OPEN REQUEST BUTTON -->
				    			<div class="col-auto">
				    				<input type="button" class="btn btn-sm request_btn" value="Re-Open Request" 
				    				<%if(opration.equals("MODIFY")&&!counterparty_cd.equals("0")&&!cont_no.equals("")&&(cont_status_flg.equals("C")||cont_status_flg.equals("T"))&&!closure_req_flag.equals("O")){%>onclick="doReopen();"<%}else{%>style="background:#e6e6e6;color:black;" disabled<%}%>>
				    			</div>
				    			<!-- /RE-OPEN REQUEST BUTTON -->
				  			</div>
						</div>
					...
					<%if(!cont_no.equals("0") && !cont_no.equals("")){ %>
						...
						<%}else if(cont_status_flg.equals("R")){%>
							<div class="row m-b-5">
								<div align="center" class="col-sm-12 col-xs-12 col-md-12">
									<%=utilmsg.infoMessage("<b>This contract has been Re-Opened!</b>")%>
								</div>
							</div>
						<%} %>
						...
						<%}else if(closure_req_flag.equals("R")){%>
							<div class="row m-b-5">
								<div align="center" class="col-sm-12 col-xs-12 col-md-12">
									<%=utilmsg.infoMessage("<b>Terminate request has been generated!</b>")%>
								</div>
							</div>
						<%}else if(closure_req_flag.equals("O")){%>
							<div class="row m-b-5">
								<div align="center" class="col-sm-12 col-xs-12 col-md-12">
									<%=utilmsg.infoMessage("<b>Re-Open request has been generated!</b>")%>
								</div>
							</div>
						<%}%>
							...
							<div class="form-group row">
								<div class="d-flex justify-content-end">
					    			<label class="form-label">
					    				<b>
					    					<input type="checkbox" class="form-check-input" name="chk_split" onclick="splitEnabled(this, '<%=enable_split%>');">&nbsp;Split Trader(% wise)
							...
							<table class="table table-bordered table-hover">
								<thead>
									<tr>
										...
										<th colspan="2"><%=VCHARGE_NAME.elementAt(j)%>&nbsp;
										</th>
										<%}%>
									</tr>
									<tr>
							    	...
							    	<%for(int i=0; i<VPLANT_SEQ_NO.size(); i++){ %>	
							    		<tr> 
											<td align="center">	
				      							...
				      							onclick="enableSplit(this,'split_value_<%=VPLANT_SEQ_NO.elementAt(i)%>','<%=i%>','','<%=VNOM_SEL_TRADER_CHK.elementAt(i)%>');" >
					    					...
					    					<td align="center">
					    						<div style="width:100px;">
						    						<div class="input-group input-group-sm" >
							      						...
							      						checkChargesEffDate(this,'<%=VCHARGE_NAME.elementAt(j)%>','max_eff_dt_<%=VCHARGE_ABBR.elementAt(j)%>_<%=i%>_<%=j%>');"
							    				...
							    				<tr>
													<td align="center">	
						      							<input type="checkbox" class="form-check-input" name="chk_oth_plant" 
									    					onclick="enableSplit(this,'oth_split_value_<%=oth_countpty_cd%>_<%=VOTH_PLANT_SEQ_NO.elementAt(j)%>','<%=j%>','oth','<%=VNOM_SEL_OTH_TRADER_CHK.elementAt(j) %>');
						      						...
						      						<td>
					      								<%=VOTH_PLANT_ABBR.elementAt(j)%>&nbsp;
					      								<i name="oth_history_btn_<%=VOTH_PLANT_SEQ_NO.elementAt(j)%>" id="oth_history_btn_<%=VOTH_PLANT_SEQ_NO.elementAt(j)%>" style="pointer-events: auto; color:#800040" class="fa fa-info-circle fa-lg" aria-hidden="true"></i>
						      						</td>
						      						<td>
							    					...
							    					<td align="center">
							    						<div style="width:100px;">
								    						<div class="input-group input-group-sm" >
									      						...
									      						onblur="validateDate(this);
									      						checkChargesEffDate(this,'<%=VCHARGE_NAME.elementAt(m)%>','max_oth_eff_dt_<%=VCHARGE_ABBR.elementAt(m)%>_<%=j%>_<%=m%>');"
									      						 onchange="validateDate(this);
									      						 checkChargesEffDate(this,'<%=VCHARGE_NAME.elementAt(m)%>','max_oth_eff_dt_<%=VCHARGE_ABBR.elementAt(m)%>_<%=j%>_<%=m%>');"
									      						 autocomplete="off" disabled>
									      						<input type="hidden"name="max_oth_eff_dt_<%=VCHARGE_ABBR.elementAt(m)%>" id="max_oth_eff_dt_<%=VCHARGE_ABBR.elementAt(m)%>_<%=j%>_<%=m%>" >
									      						<span class="input-group-text"><i class="fa fa-calendar fa-lg"></i></span>
								      						</div>
								      					</div>
													</td>
...
<INPUT TYPE="hidden" name="closure_req_flag" value="<%=closure_req_flag %>">
